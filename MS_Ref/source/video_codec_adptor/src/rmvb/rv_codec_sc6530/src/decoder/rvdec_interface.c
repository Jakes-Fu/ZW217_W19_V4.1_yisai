...
PUBLIC MMDecRet RvDecInit(MMCodecBuffer *buffer_ptr, MMDecVideoFormat *pVideoFormat)
{
	...
	if(NULL == (g_rv_decoder_ptr = (RV_DECODER_T *)RvDec_InterMemAlloc(sizeof(RV_DECODER_T))))
        {
	...
	if(NULL == (g_ClipTab_rv89 = (uint8 *)RvDec_InterMemAlloc(CLIP_TAB_SIZE)))
        {
            return MMDEC_MEMORY_ERROR;
        }
#endif
        ...
        if(NULL == (g_intra_decode_tables = (RV_VLC_SET_T *)RvDec_InterMemAlloc(sizeof(RV_VLC_SET_T)*MAX_INTRA_QP_REGIONS)))
        {
            return MMDEC_MEMORY_ERROR;
        }

        if(NULL == (g_inter_decode_tables = (RV_VLC_SET_T *)RvDec_InterMemAlloc(sizeof(RV_VLC_SET_T)*MAX_INTER_QP_REGIONS)))
        {
            return MMDEC_MEMORY_ERROR;
        }

        if(NULL == (g_aic_mode1_vlc = (BYTE_VLC_SYMBOL_T *)RvDec_InterMemAlloc(sizeof(BYTE_VLC_SYMBOL_T)*AIC_MODE1_NUM)))
        {
            return MMDEC_MEMORY_ERROR;
        }
...
PUBLIC MMDecRet RvDecDecode(MMDecInput *dec_input_ptr, MMDecOutput *dec_output_ptr)
{
...
#else
        if(dec_input_ptr->expected_IVOP)
	{
	    if(rv_decoder_ptr->uPicCodeType == INTRAPIC)
	    {
	        g_rv_skip_bfrm_after_seek = TRUE;
	    }
	    else
	    {
                if (g_nFrame_rv)
	...
	{
	    if(TRUE == g_rv_skip_bfrm_after_seek)
	    {
                ...
                {
                    g_rv_skip_bfrm_after_seek = FALSE;
                }
	    }
	}
#endif
