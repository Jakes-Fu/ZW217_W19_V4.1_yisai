...
#if 0 //ndef MIN_STACK
#define MIN_STACK
#define SM3BlockSize    (SM3_DIGEST_LENGTH*2)
#endif
static void sm3_process( sm3_context *ctx, uint8_t data[64] )
{
    ...
    unsigned long *W1 = NULL;
#else
    unsigned long W[68], W1[64], T[64];
#endif
...
#ifdef MIN_STACK
    W=AntiRegAlloc(SM3BlockSize*sizeof(unsigned long)+4);
    W1=AntiRegAlloc(SM3BlockSize*sizeof(unsigned long));
    T=AntiRegAlloc(SM3BlockSize*sizeof(unsigned long));
    if(!W || !W1 || !T){
        EMsg("System error, alloc failed");
        return;
    }
#endif
	for(j = 0; j < 16; j++)
...
#define ROTL(x,n) (SHL((x),n) | (((x) & 0xFFFFFFFF) >> (32-(n>32?n%32:n))))
 ...
 * SM3 final digest
 */
//void sm3_finish( sm3_context *ctx, uint8_t *output)
