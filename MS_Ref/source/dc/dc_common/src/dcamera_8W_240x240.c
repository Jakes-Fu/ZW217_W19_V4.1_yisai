...
PUBLIC DCAMERA_IOCTL_CFG_T  g_dcam_cfg_8W_240x240 =
{
    get_GoudaOffset,
    Gouda_config,
...
static int Gouda_config(int mode, BLOCKCFG_T* cfg, DC_CAMERA_CFG ** param)
{
    ...
    if(!cfg || !param)
    {
        SCI_PASSERT(SCI_FALSE, ("DC:Gouda_config param error!"));
    }
    ...
    {
        SCI_PASSERT(SCI_FALSE, ("DC:Gouda_config mode Error!"));
    }
...
static uint32 Preview_init_config(DC_CAMERA_CFG* param)
{
	...
	pCfg->hal_cfg.spi_pixels_per_column = 280;//320;                 //IspOutHeight; // wuxx modify for 6133
	pCfg->hal_cfg.spi_yuv_out           = SPI_OUT_U0_Y0_V0_Y1;//SPI_OUT_Y0_U0_Y1_V0;// wuxx modify for 6133
	...
	pCfg->hal_cfg.dstWinRowEnd   = 280;//320; //wuxx modify for 6133

	pCfg->preview_rot = 0;   //普通预览旋转90度

	pCfg->width  = 240;
	pCfg->height = 280;//320; //wuxx modify for 6133

	pCfg->display_width = 240;
	pCfg->display_height =280;//240;//wuxx modify for 6133
