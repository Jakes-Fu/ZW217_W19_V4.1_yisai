// FirAutoDlgEx.cpp : implementation file
//
...
/*static*/
CString CFirAutoDlgEx::m_strTarEqFile;

int CFirAutoDlgEx::m_vPowerFrequence[ROW_COUNT_EX] = 
{
...
CFirAutoDlgEx::CFirAutoDlgEx(CWnd* pParent /*=NULL*/)
	: CDialog(CFirAutoDlgEx::IDD, pParent)
{
	//{{AFX_DATA_INIT(CFirAutoDlgEx)
...
void CFirAutoDlgEx::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CFirAutoDlgEx)
...
BEGIN_MESSAGE_MAP(CFirAutoDlgEx, CDialog)
	//{{AFX_MSG_MAP(CFirAutoDlgEx)
...
/////////////////////////////////////////////////////////////////////////////
// CFirAutoDlgEx message handlers
BOOL CFirAutoDlgEx::BrowseFile( CString& strFile,CString& strFilter )
{
...
// Load *.trc file
BOOL CFirAutoDlgEx::LoadTrcFile( CString strFile,int* pFrequency,double* pEq,int* pEqNum )
{
...
// Load *.xls file
BOOL CFirAutoDlgEx::LoadXlsFile( CString strFile,int* pFrequency,double* pEq,int* pEqNum )
{
...
BOOL CFirAutoDlgEx::LoadSpec( CString& strFile,int* pHighFreq,double* pHighEq,int& high_num,
...
// Load eq file
BOOL CFirAutoDlgEx::LoadEqFile( CString strFile,int* pFrequency,double* pEq,int* pEqNum )
{
...
void CFirAutoDlgEx::OnButtonFirSpec() 
{
...
void CFirAutoDlgEx::OnButtonFirOriginal() 
{
...
void CFirAutoDlgEx::OnButtonFirTarget() 
{
    CString strFile,strFilter = "Eq files(*.trc,*.xls)|*.trc;*.xls||";
...
// We check it here
BOOL CFirAutoDlgEx::CheckEqData( int nEqNum,int* pFrequency,BOOL bTarget )
{
...
double CFirAutoDlgEx::CalculateEq( int nEqNum,
                    double* pEqData,
                    int nGain,
                    double dFreq
                  )
...
void CFirAutoDlgEx::CalculateGraphyData( std::vector<CFrequencyGraphyStaticEx::FreqItem>* vAllFreqItems,
                          int nEqNum,
                          double* pEqData,
                          int nGain
                        )
...
void CFirAutoDlgEx::GetOriEq( std::vector<CFrequencyGraphyStaticEx::FreqItem>* vAllFreqItems )
{
    if( !m_bOriReady )
    {
        // No valid data
	...
	{
		std::vector<SpecialRectOfCoefficientEx::FreqItem> arrFreq = m_pTempArea->GetFreqItem();
...
void CFirAutoDlgEx::GetOriEqXValue(double dX[65])
{
	for( int i=0;i<m_nEqNum;i++ )
	{
...
void CFirAutoDlgEx::GetTarEq( std::vector<CFrequencyGraphyStaticEx::FreqItem>* vAllFreqItems )
{
    if( !m_bTarReady )
    {
        // No valid data
...
void CFirAutoDlgEx::GetRealEq( std::vector<CFrequencyGraphyStaticEx::FreqItem>* vAllFreqItems )
{
    if( !m_bReady )
    {
        // No valid data;
...
void CFirAutoDlgEx::OnCheckOriginal() 
{
	m_bShowOri = !m_bShowOri;
    m_Graphy.Invalidate();
}

void CFirAutoDlgEx::OnCheckResult() 
{
	m_bShowReal = !m_bShowReal;
    m_Graphy.Invalidate();
}

void CFirAutoDlgEx::OnCheckTarget() 
{
	m_bShowTar = !m_bShowTar;
    m_Graphy.Invalidate();
}

HBRUSH CFirAutoDlgEx::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
...
void CFirAutoDlgEx::OnCheckModified() 
{
    m_bShowModified = !m_bShowModified;
    m_Graphy.Invalidate();
}

void CFirAutoDlgEx::GetModifiedEq( std::vector<CFrequencyGraphyStaticEx::FreqItem>* vAllFreqItems )
{
    ...
    for( i=0;i<m_nEqNum;i++ )
    {
        ...
        fi.second = m_arrOriEq[i] + CalculateEq( 65,eq,m_nGain,m_arrFrequency[i] );
        vAllFreqItems->push_back( fi );
    }    
}

void CFirAutoDlgEx::OnCheckSpec() 
{
    m_bShowSpec = !m_bShowSpec;
    m_Graphy.Invalidate();
}

void CFirAutoDlgEx::GetHighEq( std::vector<CFrequencyGraphyStaticEx::FreqItem>* vAllFreqItems )
{
...
void CFirAutoDlgEx::GetLowEq( std::vector<CFrequencyGraphyStaticEx::FreqItem>* vAllFreqItems )
{
...
void CFirAutoDlgEx::GetFinalEq( std::vector<CFrequencyGraphyStaticEx::FreqItem>* vAllFreqItems )
{
    if( !m_bReady )
    {
        // No valid data;
...
void CFirAutoDlgEx::OnCheckFinal() 
{
    m_bShowFinal = !m_bShowFinal;
    m_Graphy.Invalidate();
}

void CFirAutoDlgEx::OnButtonOffsetApply() 
{
    UpdateData();
    Invalidate();    
}
...
void CFirAutoDlgEx::LoadDefault()
{
...
BOOL CFirAutoDlgEx::OnInitDialog() 
{
...
//		GetDlgItem( IDC_BUTTON_FIR_ORIGINAL )->EnableWindow( FALSE );
		...
		//
		m_pTempArea = new SpecialRectOfCoefficientEx;
		*m_pTempArea = *m_pArea;

		for( int i=0;i<X_SCALE_COUNT_FIR_EX;i++ )
		{
			m_Drag.m_arrPowerData[i] = m_pArea->m_vPoValue[i];		
		}
		
		m_bOriReady = TRUE;
		m_Drag.m_pRelateWnd = &m_Graphy;

		if(m_pTempArea->m_strName == _T("DL_FIR_EQ_COEFF_EX"))
		{
			SetWindowText(_T("DL Fir adjust"));
		}
		else if(m_pTempArea->m_strName == _T("UL_FIR_EQ_COEFF_EX"))
		{
...
void CFirAutoDlgEx::ResetTarEq(int nIndex, double dTarEq)
{
	m_arrTarEq[nIndex] = dTarEq;
}

int	CFirAutoDlgEx::GetHighEQYValue(int x)
{
	for( int i=0;i<m_nHighEqNum;i++ )
	{
...
void CFirAutoDlgEx::ModifySpecFreqValue(int nDiff)
{
	for( int i=0;i<m_nHighEqNum;i++ )
	{
		m_arrHighEq[i] += nDiff;
...
void CFirAutoDlgEx::OnButtonDefaultFir() 
{
	...
	for( int i=0;i<X_SCALE_COUNT_FIR_EX;i++ )
	{
		m_pTempArea->m_vPoValue[i] = 0;
		m_Drag.m_arrPowerData[i] = m_pTempArea->m_vPoValue[i];	
	}
	...
	if(m_pTempArea->m_strName == _T("DL_FIR_EQ_COEFF_EX"))
	{
		...
		for( int i=0;i<32;i++ )
		{
			value.st_fir_eq_coeff[i] = 0;
		}
		value.st_fir_eq_coeff[32] = 4096;
...
void CFirAutoDlgEx::OnButtonRefreshFir()
{
	if(m_bDefaultFir)
	{
		return;
...
void CFirAutoDlgEx::GetConfig()
{
	...
	if(m_pTempArea->m_strName == _T("UL_FIR_EQ_COEFF_EX"))
	{
		m_Graphy.SetYMinMaxValue(nMin, nMax);
	}
}

void CFirAutoDlgEx::SetListCtrl()
{
...
void CFirAutoDlgEx::SetListData()
{
    for( int i=0; i<ROW_COUNT_EX; ++i )
    {
		CString strText;
		strText.Format(_T("%.1f"), m_pTempArea->m_vPoValue[i]);
		m_listFir.SetItemText(i, 1, strText);
    }    
	UpdateData(FALSE);
}

void CFirAutoDlgEx::SetFirListHighLight(int nItem)
{
	if(nItem == -1)
	{
		return;
