...
typedef struct _MIDST_RAW_DATA{
    __int64 llOffset;
    int nLength;
    _MIDST_RAW_DATA()
    {
        memset(this, 0, sizeof(_MIDST_RAW_DATA));
    }
}MIDST_RAW_DATA;
...
public:
    ...
    BOOL SetTimeStamp(void* pFileInfo);
    BOOL ReadPartitionInfo(void* pFileInfo);
	BOOL DumpUbootLog(void* pFileInfo);
	BOOL ChangeSelinux(void * pFileInfo);
	//]] Special notice
	...
	void CloseLog( DWORD dwError , LPCTSTR lpStep = NULL);
...
private:
    ...
    BOOL SetCompatibleItem(DWORD& dwError);
