...
void CMainPage::OnOK() 
{
	...
	if(m_pCurProductInfo != NULL)
	{
		for(int i=0;i<nCount;i++)
		{
            ...
            for(int j=0; j<m_pCurProductInfo->dwFileCount; j++)
            {
...
void CMainPage::OnChkAll() 
{
	...
	for(int i =0; i<nCount; i++)
	{
        ...
        int index = 0;
        for(int j=0; j<m_pTmpProductInfo->dwFileCount; j++)
        {
            if(m_pTmpProductInfo->pFileInfoArr[j].szIDAlias == strIDAlias)
            {
                index = j;
                break;
            }
		...
		strID = m_pTmpProductInfo->pFileInfoArr[index].szID;
...
BOOL CMainPage::DoPacket() 
{
	...
	for(i = 0;i<nItemCount;i++)
	{
		...
		CString strFileID = m_pTmpProductInfo->pFileInfoArr[index].szID;
	...
	{		
		...
		pFT->dwAddrNum = m_pTmpProductInfo->pFileInfoArr[index].dwBlockCount;
		for(UINT k = 0; k< pFT->dwAddrNum; k++)
		{
			pFT->dwAddr[k] = (DWORD)(m_pTmpProductInfo->pFileInfoArr[index].arrBlock[k].llBase);
		}
    ...
    {		
        ...
        pFT->nCheckFlag = m_lstMergeNV.GetCheck(i);
...
void CMainPage::OnRclickLstProductInfo(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(dlg.DoModal() == IDOK)
    {
		...
		if(m_pTmpProductInfo != NULL && m_pTmpProductInfo->dwOmaDMFlag != 0)
		{
            ...
            for(int j=0; j<m_pTmpProductInfo->dwFileCount; j++)
            {
                if(m_pTmpProductInfo->pFileInfoArr[j].szIDAlias == strID)
                {
                    index = j;
                    break;
                }
			...
			CString fileID = m_pTmpProductInfo->pFileInfoArr[index].szID;
			if(fileID.CompareNoCase(_T("FDL2"))== 0)
			{
...
void CMainPage::OnClickLstProductInfo(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(m_pTmpProductInfo!= NULL)
	{
		...
		if(_tcslen(m_pTmpProductInfo->pFileInfoArr[index].szFileDescript)>0)
		{
			strFileDescript.Format(_T("Note: %s"),m_pTmpProductInfo->pFileInfoArr[index].szFileDescript);
		}
	}
