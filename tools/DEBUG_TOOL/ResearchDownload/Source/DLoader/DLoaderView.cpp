...
LRESULT CDLoaderView::OnAcquireBarcode(WPARAM wParam,LPARAM lParam)
{
    ...
    for(;;)
    {
        ...
        m_dlgCurBarcode.SetPortNum(dwPort);
		m_dlgCurBarcode.SetSNInputMethod(pFrame->m_sheetSettings.IsAutoGenSN());
		UINT nRet;
		LPCSTR pSN =NULL;	   
		nRet = m_dlgCurBarcode.DoModal();
		USES_CONVERSION;
		pSN= T2CA(m_dlgCurBarcode.m_strBarcode);
...
#ifndef _SPUPGRADE
	//Close the SN Dialog
	if( m_dlgCurBarcode.GetPortNum() == nPort )
	{
		m_dlgCurBarcode.Close();
	}
