...
BOOL XFindNVOffset(WORD wId,LPBYTE lpCode,DWORD dwCodeSize,
				   DWORD& dwOffset,DWORD& dwLength,BOOL bBigEndian,BOOL bCoreNV)
{
...
BOOL XFindNVOffsetExMerge(WORD wId,LPBYTE lpCode,DWORD dwCodeSize,
                     DWORD& dwOffset,DWORD& dwLength,BOOL &bBigEndian,BOOL bModule,BOOL bCoreNV)
{
    BOOL bOK = FALSE;
    BOOL _bBigEndian = FALSE;
    if( XCheckNVStructExMerge(lpCode,dwCodeSize,_bBigEndian,bModule,bCoreNV) )
    {
        if(XFindNVOffset(wId,lpCode,dwCodeSize,dwOffset,dwLength, _bBigEndian,bCoreNV))
        {
            bOK = TRUE;
        }
    }
...
//BOOL IsValidFixnv(const LPBYTE lpFixData, const DWORD& dwFixLength)
//{
//
//    if (NULL == lpFixData || 4>dwFixLength)
//    {
//        return FALSE;
//    }
//    BOOL bOK = FALSE;
...
//    WORD wCurID  = 0;
//    WORD wLen = 0;
//    const DWORD ALIGNMENT = 4;
//    const BYTE* pTemp = lpFixData + dwOffset;
//    while (dwOffset < dwFixLength)
//    {
//
//        wCurID = *(WORD *)pTemp;
//        pTemp += 2;
...
BOOL XCheckNVStructMerge(const LPBYTE lpPhoBuf, DWORD dwPhoSize,BOOL bBigEndian, BOOL bModule, BOOL bCoreNV)
{
    UNUSED_ALWAYS(bModule);
...
BOOL  XCheckNVStructExMerge(LPBYTE lpPhoBuf, DWORD dwPhoSize,BOOL &bBigEndian,BOOL bModule, BOOL bCoreNV)
{
    BOOL bOK = TRUE;
    BOOL _bBigEndian = FALSE;
    if(!XCheckNVStructMerge(lpPhoBuf,dwPhoSize,_bBigEndian,bModule,bCoreNV))
    {
        _bBigEndian = TRUE;
        if(!XCheckNVStructMerge(lpPhoBuf,dwPhoSize,_bBigEndian,bModule,bCoreNV))
        {
    ...
    if(!bOK && bModule)
    {
        ...
        if(!XCheckNVStructMerge(pBuf,dwPhoSize,_bBigEndian,bModule,bCoreNV))
        {
            _bBigEndian = TRUE;
            if(!XCheckNVStructMerge(pBuf,dwPhoSize,_bBigEndian,bModule,bCoreNV))
            {
...
BOOL IsMultiCore(const LPBYTE lpBuf, const DWORD& dwSize,PMULTI_CORE_NV_HEADER_PTR& pCoreHeader,PNV_CORE_INFO_PTR& ppCoreItem)
{
    ...
    {
        WORD wID = *(WORD*)(lpBuf+4);
        WORD wLen = *(WORD*)(lpBuf+6);
        wLen = ((wLen+3)/4) * 4;
        if (g_INDEX_NV_ID == wID && wLen > sizeof(MULTI_CORE_NV_HEADER_T) )
        {
            pCoreHeader = (MULTI_CORE_NV_HEADER_T*)(lpBuf + 8);
            ppCoreItem	= (NV_CORE_INFO_T*)(lpBuf + 8 + sizeof(MULTI_CORE_NV_HEADER_T));
...
BOOL MergeNormalNV(LPBYTE lpCodeSrc, DWORD dwCodeSizeSrc, LPBYTE lpCodeDest, DWORD & dwCodeSizeDest, LPBYTE lpCodeFinal, DWORD & dwCodeSizeFinal, BOOL bCoreNV)
{
    if(dwCodeSizeFinal < dwCodeSizeDest)
    {
        return FALSE;
    }
    memcpy(lpCodeFinal, lpCodeDest, dwCodeSizeDest);
    dwCodeSizeFinal = dwCodeSizeDest;

    DWORD dwOffsetSrc = 0;
    DWORD dwLengthSrc = 0;
    DWORD dwOffsetDest = 0;
    DWORD dwLengthDest = 0;
    BOOL  bBigEndianSrc = FALSE;
    BOOL  bBigEndianDest = FALSE;
    WORD  wCurID;

    if( !XCheckNVStructExMerge(lpCodeSrc,dwCodeSizeSrc,bBigEndianSrc,TRUE, bCoreNV) )
    {
        return ERR_NOT_FIND_PARAM_IN_FILE;
    }
    else
    {
        //查找所有的nv项id
        if(lpCodeSrc == NULL || (int)dwCodeSizeSrc < 0)
        {
            return ERR_NONE;
        }

        dwOffsetSrc = bCoreNV ? 0:4;     // Skip first four bytes,that is time stamp
        dwLengthSrc = 0;
        BYTE* pTemp = lpCodeSrc + dwOffsetSrc;

        /// 添加逻辑：需要先判断是否为多核NV的情况

        while(dwOffsetSrc < dwCodeSizeSrc)
        {
            wCurID = *(uint16*)pTemp;
            if(bBigEndianSrc)
            {
            ...
            dwLengthSrc = *(uint16*)pTemp;
            if(bBigEndianSrc)
            {
                dwLengthSrc = CONV_SHORT(dwLengthSrc);
            }

            pTemp += 2;
            dwOffsetSrc += 4;
            // Must be four byte aligned
            BYTE bRet = (BYTE)(dwLengthSrc % 4);
            if(bRet != 0)
            {
                dwLengthSrc += 4 - bRet;
            }
            ...
            else if(wCurID == g_PADDING_NV_ID)
            {
                break;;
            }
            else
            {
                if(!XFindNVOffsetExMerge( wCurID, lpCodeFinal, dwCodeSizeDest,dwOffsetDest,dwLengthDest,bBigEndianDest,FALSE, bCoreNV))
                {	
                    /// 未完成  没找到NV项，则添加到结尾
                    memcpy( lpCodeFinal + dwCodeSizeFinal,
                        lpCodeSrc + dwOffsetSrc,
                        dwLengthSrc);
                    dwCodeSizeFinal += dwLengthSrc;
                    continue;
                }

                if(dwLengthDest != dwLengthSrc)
                {        
                    return FALSE;
                }

                memcpy( lpCodeFinal + dwOffsetDest,
                    lpCodeSrc + dwOffsetSrc,
                    dwLengthSrc);

            }
            pTemp += dwLengthSrc;
            dwOffsetSrc += dwLengthSrc;
        }
...
// 要保证lpCodeDest这个buffer够大
DWORD MergeNV(LPBYTE lpCodeSrc, DWORD dwCodeSizeSrc, LPBYTE lpCodeDest, DWORD &dwCodeSizeDest)
{  
    BOOL bMultiCoreSrc = FALSE;
    BOOL bMultiCoreDest = FALSE;
    MULTI_CORE_NV_HEADER_T* pCoreHeaderSrc  = NULL;
    NV_CORE_INFO_T* ppCoreItemSrc			= NULL;
    MULTI_CORE_NV_HEADER_T* pCoreHeaderDest = NULL;
    NV_CORE_INFO_T* ppCoreItemDest			= NULL;
    CString strError;

    // 判断Src和Dest是否为多核NV。两者必须一致，否则报错

    if (NULL == lpCodeSrc || 4 > dwCodeSizeSrc || NULL == lpCodeDest || 4 >dwCodeSizeDest)
    {
        strError = _T("Invalid parameter.");
        return 0;
    }
    
    bMultiCoreSrc = IsMultiCore(lpCodeSrc,dwCodeSizeSrc,pCoreHeaderSrc,ppCoreItemSrc);
    bMultiCoreDest = IsMultiCore(lpCodeDest,dwCodeSizeDest,pCoreHeaderDest,ppCoreItemDest);

    if(bMultiCoreSrc != bMultiCoreDest)
    {
        strError = _T("Mismatched NV files");
        return 0;
    }

    LPBYTE lpCodeFinal = new BYTE[MAX_NVITEM_IMG_SIZE];
    memset(lpCodeFinal, 0, MAX_NVITEM_IMG_SIZE);
    memcpy(lpCodeFinal, lpCodeDest, dwCodeSizeDest);
    DWORD dwOffsetFinal = 0;
    DWORD dwCodeSizeFinal = dwCodeSizeDest;

    LPBYTE lpCodeCore = new BYTE[MAX_NVITEM_IMG_SIZE];
    //memcpy(lpCodeCore, lpCodeDest, dwCodeSizeDest);
    //DWORD dwOffsetCore = 8 + sizeof(MULTI_CORE_NV_HEADER_T) + pCoreHeaderDest->wCountCore * sizeof(NV_CORE_INFO_T);
    

    if (bMultiCoreSrc && bMultiCoreDest && (pCoreHeaderSrc->wCountCore == pCoreHeaderDest->wCountCore))
    {
        dwOffsetFinal = 8 + sizeof(MULTI_CORE_NV_HEADER_T) + pCoreHeaderDest->wCountCore * sizeof(NV_CORE_INFO_T);
        for(int i=0; i<pCoreHeaderSrc->wCountCore; ++i)
        {
            NV_CORE_INFO_T*  pItemSrc = ppCoreItemSrc +i;
            NV_CORE_INFO_T*  pItemDest = ppCoreItemDest +i;
            
            memset(lpCodeCore, 0, MAX_NVITEM_IMG_SIZE);
            DWORD dwCodeSizeCore = 0;

            if((pItemSrc->dwDataOffset + pItemSrc->dwDataSize <= dwCodeSizeSrc) && (pItemDest->dwDataOffset + pItemDest->dwDataSize <= dwCodeSizeDest))
            {
                if(MergeNormalNV(lpCodeSrc+pItemSrc->dwDataOffset, pItemSrc->dwDataSize, lpCodeDest+pItemDest->dwDataOffset, pItemDest->dwDataSize, lpCodeCore, dwCodeSizeCore, TRUE))
                {
                    WORD dwPaddingLen = MAX_PADDING_SIZE - dwCodeSizeCore%MAX_PADDING_SIZE;
                    DWORD dwOffset = dwCodeSizeCore;
                    if (MAX_PADDING_SIZE != dwPaddingLen)
                    {
                        if (dwPaddingLen <= 4)
                        {
                            dwPaddingLen += MAX_PADDING_SIZE; 
                        }
                        *(WORD*)(lpCodeCore+dwOffset) = g_PADDING_NV_ID;
                        *(WORD*)(lpCodeCore+dwOffset+2) = dwPaddingLen - 4;
                        dwOffset += dwPaddingLen; 
                    }
                    pItemDest->dwDataSize = dwOffset;
                    pItemDest->dwDataOffset = dwOffsetFinal;
                    memcpy(lpCodeFinal+dwOffsetFinal, lpCodeCore, dwOffset);
                    dwOffsetFinal += dwOffset;
                }    
            }
            else
            {
                strError = _T("Invalid core nv image file.");
                delete[] lpCodeFinal;
                lpCodeFinal = NULL;
                delete[] lpCodeCore;
                lpCodeCore = NULL;
        ...
        memcpy(lpCodeFinal, lpCodeDest, 8 + sizeof(MULTI_CORE_NV_HEADER_T) + pCoreHeaderDest->wCountCore * sizeof(NV_CORE_INFO_T));
        dwCodeSizeFinal = dwOffsetFinal;
    }
    else
    {
        if(!MergeNormalNV(lpCodeSrc, dwCodeSizeSrc, lpCodeDest, dwCodeSizeDest, lpCodeFinal, dwCodeSizeFinal))
        {
            delete[] lpCodeFinal;
            lpCodeFinal = NULL;
            delete[] lpCodeCore;
    ...
    memcpy(lpCodeDest, lpCodeFinal, dwCodeSizeFinal);
    dwCodeSizeDest = dwCodeSizeFinal;
    delete[] lpCodeFinal;
    lpCodeFinal = NULL;
    delete[] lpCodeCore;
    lpCodeCore = NULL;
