...
BOOL CPageOptions::OnInitDialog() 
{
    ...
    int nLevel = _tstoi(m_strDebugLevel.Right(1));
...
BOOL CPageOptions::LoadSettings(LPCTSTR pFileName,CString &strErrorMsg)
{
    ...
    m_bReadPartition    = GetPrivateProfileInt( _T("Options"),_T("ReadPartition"),0,pFileName );
    ...
    memset(szBuf,0,sizeof(szBuf));
    GetPrivateProfileString(_T("Debug"),_T("Data"),_T("enable:level=0"),szBuf,_MAX_PATH,pFileName);
    m_strDebugLevel = szBuf;

	m_bEnableDumpUbootLog = GetPrivateProfileInt( _T("Options"),_T("DumpUbootLog"),0,pFileName );
#endif
	...
	if (m_bEnableDumpUbootLog)
	{
		_TCHAR szBMTimeoutIniFile[MAX_PATH];    
		GetModuleFilePath( g_theApp.m_hInstance, szBMTimeoutIniFile );
		_tcscat( szBMTimeoutIniFile,  _T("\\BMTimeout.ini") );

		WritePrivateProfileString( _T("Log"),_T("PassRemoveLog"),_T("0"),szBMTimeoutIniFile ); 
	}
...
BOOL CPageOptions::SaveSettings(LPCTSTR pFileName)
{
    ...
    WritePrivateProfileString( _T("Debug"),_T("Data"),m_strDebugLevel,pFileName ); 

	strTemp.Format( _T("%d"),m_bReadPartition );
	WritePrivateProfileString( _T("Options"),_T("ReadPartition"),strTemp,pFileName ); 

	strTemp.Format( _T("%d"),m_bEnableDumpUbootLog );
	WritePrivateProfileString( _T("Options"),_T("DumpUbootLog"),strTemp,pFileName ); 
