...
BOOL CSettingsSheet::LoadSettings(LPCTSTR pFileName,CString &strErrorMsg)
{
    ...
    return 
        (
         ...
         m_pageCustomization.LoadSettings(pFileName, strErrorMsg) &&
		 m_pageMiscOptions.LoadSettings(pFileName, strErrorMsg)
        );
#endif
}
...
BOOL CSettingsSheet::IsEnableDumpUbootLog()
{
	return m_pageOptions.m_bEnableDumpUbootLog;
}
...
BOOL CSettingsSheet::IsReadPartition()
{
    return m_pageOptions.m_bReadPartition;
}
...
{
	return (!m_pageOptions.m_strDebugLevel.IsEmpty() &&
        m_pageOptions.m_strDebugLevel.Right(1).Compare(_T("0")) != 0);
}

BOOL CSettingsSheet::IsChangeSelinux()
{
	return (!m_pageMiscOptions.m_strSelinux.IsEmpty() &&
		m_pageMiscOptions.m_strSelinux.Right(1).Compare(_T("-")) != 0);
}
