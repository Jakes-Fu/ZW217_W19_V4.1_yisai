...
class CGraphBaseClass
{
	...
	virtual void AppendMenuItems(CMenu* menu){UNREFERENCED_PARAMETER(menu);};
	virtual void OnRBMenuCommand(UINT command_id){UNREFERENCED_PARAMETER(command_id);};
	virtual void AppendPropertyPage(CPropertySheet* prop_sheet){UNREFERENCED_PARAMETER(prop_sheet);};
	virtual void ReleasePropertyPage(UINT dialog_status){UNREFERENCED_PARAMETER(dialog_status);};
