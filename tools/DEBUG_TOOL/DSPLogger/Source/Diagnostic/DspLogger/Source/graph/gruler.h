...
public:
	...
	virtual int Width(CDC* dest_dc){UNREFERENCED_PARAMETER(dest_dc);return 0;};
	void SetMinMax(double minv, double maxv, BOOL bRedraw);
	void SetNewTitles(char* newUOM, char* newTitle);
	virtual int GetNMax(CDC* dc_to_draw, CRect rect_to_draw){UNREFERENCED_PARAMETER(dc_to_draw);UNREFERENCED_PARAMETER(rect_to_draw);return 0;};
	virtual void DrawRuler(CDC* dc_to_draw, CRect rect_to_draw){UNREFERENCED_PARAMETER(dc_to_draw);UNREFERENCED_PARAMETER(rect_to_draw);};
