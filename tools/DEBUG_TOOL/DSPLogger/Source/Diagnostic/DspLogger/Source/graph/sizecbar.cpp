...
CSCBArr CSizingBar::m_arrBars; // static member

IMPLEMENT_DYNAMIC(CSizingBar, baseCSizingBar);

CSizingBar::CSizingBar()
{
    m_szMin = CSize(33, 32);
...
BEGIN_MESSAGE_MAP(CSizingBar, baseCSizingBar)
    //{{AFX_MSG_MAP(CSizingBar)
    ON_WM_CREATE()
    ON_WM_PAINT()
    ON_WM_NCPAINT()
    ON_WM_NCCALCSIZE()
...
BOOL CSizingBar::Create(LPCTSTR lpszWindowName, CWnd* pParentWnd,
...
/////////////////////////////////////////////////////////////////////////
// CSizingBar message handlers

int CSizingBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
    if (baseCSizingBar::OnCreate(lpCreateStruct) == -1)
        return -1;
...
BOOL CSizingBar::DestroyWindow() 
{
    ...
    return baseCSizingBar::DestroyWindow();
}

const BOOL CSizingBar::IsFloating() const
{
    return !IsHorzDocked() && !IsVertDocked();
}

const BOOL CSizingBar::IsHorzDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

const BOOL CSizingBar::IsVertDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_LEFT ||
        m_nDockBarID == AFX_IDW_DOCKBAR_RIGHT);
}

const BOOL CSizingBar::IsSideTracking() const
{
...
CSize CSizingBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
...
CSize CSizingBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
    if (dwMode & (LM_HORZDOCK | LM_VERTDOCK)) // docked ?
    {
        ...
        return baseCSizingBar::CalcDynamicLayout(nLength, dwMode);
    }
...
void CSizingBar::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
    // force non-client recalc if moved or resized
    lpwndpos->flags |= SWP_FRAMECHANGED;

    baseCSizingBar::OnWindowPosChanging(lpwndpos);
...
// Mouse Handling
//
void CSizingBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
    if (m_pDockBar != NULL)
    {
        // start the drag
...
void CSizingBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
    if (m_pDockBar != NULL)
    {
        // toggle docking
...
void CSizingBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
    if (IsFloating())
    {
        baseCSizingBar::OnNcLButtonDown(nHitTest, point);
        return;
    }
...
void CSizingBar::OnNcLButtonUp(UINT nHitTest, CPoint point) 
{
    if (nHitTest == HTCLOSE)
        m_pDockSite->ShowControlBar(this, FALSE, FALSE); // hide

    baseCSizingBar::OnNcLButtonUp(nHitTest, point);
}

void CSizingBar::OnLButtonUp(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
        StopTracking();

    baseCSizingBar::OnLButtonUp(nFlags, point);
}

void CSizingBar::OnRButtonDown(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
        StopTracking();
    
    baseCSizingBar::OnRButtonDown(nFlags, point);
}

void CSizingBar::OnMouseMove(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
        OnTrackUpdateSize(point);
    
    baseCSizingBar::OnMouseMove(nFlags, point);
}

void CSizingBar::OnCaptureChanged(CWnd *pWnd) 
{
    if (m_bTracking && (pWnd != this))
        StopTracking();

    baseCSizingBar::OnCaptureChanged(pWnd);
}

void CSizingBar::OnNcCalcSize(BOOL bCalcValidRects,
...
void CSizingBar::NcPaintGripper(CDC* pDC, CRect rcClient)
{
...
void CSizingBar::OnPaint()
{
    // overridden to skip border painting based on clientrect
    CPaintDC dc(this);
}

UINT CSizingBar::OnNcHitTest(CPoint point)
{
    if (IsFloating())
        return baseCSizingBar::OnNcHitTest(point);
...
void CSizingBar::OnSettingChange(UINT uFlags, LPCTSTR lpszSection) 
{
    baseCSizingBar::OnSettingChange(uFlags, lpszSection);
...
/////////////////////////////////////////////////////////////////////////
// CSizingBar implementation helpers

void CSizingBar::StartTracking(UINT nHitTest)
{
    SetCapture();
...
void CSizingBar::OnTrackUpdateSize(CPoint& point)
{
    ASSERT(!IsFloating());
...
void CSizingBar::OnTrackInvertTracker()
{
    ASSERT(m_bTracking);
...
BOOL CSizingBar::GetEdgeRect(CRect rcWnd, UINT nHitTest,
                                    CRect& rcEdge)
{
    rcEdge = rcWnd;
    if (m_dwSCBStyle & SCBS_SHOWEDGES)
...
UINT CSizingBar::GetEdgeHTCode(int nEdge)
{
...
void CSizingBar::GetRowInfo(int& nFirst, int& nLast, int& nThis)
{
...
void CSizingBar::GetRowSizingBars(CSCBArr& arrSCBars)
{
    arrSCBars.RemoveAll();
    ...
    for (int i = nFirst; i <= nLast; i++)
    {
        ...
        if (FindSizingBar(pBar) >= 0)
            arrSCBars.Add((CSizingBar*)pBar);
    }
}

const int CSizingBar::FindSizingBar(CControlBar* pBar) const
{
...
BOOL CSizingBar::NegociateSpace(int nLengthAvail, BOOL bHorz)
{
...
void CSizingBar::AlignControlBars()
{
    int nFirst, nLast, nThis;
    GetRowInfo(nFirst, nLast, nThis);
...
void CSizingBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
    ...
    bNeedPaint |= ((m_biHide.bPushed ^ bWasPushed) || (m_biHide.bRaised ^ bWasRaised));//lint !e514

    if (bNeedPaint)
        SendMessage(WM_NCPAINT);
}

void CSizingBar::LoadState(LPCTSTR lpszProfileName)
{
    ASSERT_VALID(this);
...
void CSizingBar::SaveState(LPCTSTR lpszProfileName)
{
...
void CSizingBar::GlobalLoadState(LPCTSTR lpszProfileName)
{
    for (int i = 0; i < m_arrBars.GetSize(); i++)
        ((CSizingBar*) m_arrBars[i])->LoadState(lpszProfileName);
}

void CSizingBar::GlobalSaveState(LPCTSTR lpszProfileName)
{
    for (int i = 0; i < m_arrBars.GetSize(); i++)
        ((CSizingBar*) m_arrBars[i])->SaveState(lpszProfileName);
}
