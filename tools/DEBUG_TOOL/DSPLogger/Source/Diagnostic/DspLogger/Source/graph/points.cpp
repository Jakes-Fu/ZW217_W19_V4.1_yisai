...
CPointsCollection::CPointsCollection(BOOL b_sort_x/* = FALSE*/, BOOL b_keep_same_x/* = TRUE*/)
{
	bSortX = b_sort_x;
...
int CPointsCollection::AddPoint(double _x, double _y, BOOL bReScan, int* res, int _idx /*=0*/)
{
...
int CPointsCollection::GetNearestPoint(double _x, double _y, SSinglePoint* result)
{
    ...
    //_y not use it just find nearest to _x
    
    int size = GetSize();
    if (size == 0)  
        return -1;
    
    SSinglePoint fitpoint;
    for (int i=0; i<GetSize(); i++)
    {
        GetPoint(i, &fitpoint);
        if (_x - fitpoint.x <= 0)
