...
_TCHAR CFileDlg::SZ_SELED_DIR[ _MAX_PATH ] = _T("");
//////////////////////////////////////////////////////////////////////
...
int CALLBACK CFileDlg::BrowseProc( HWND hWnd, UINT uMsg, LPARAM lParam, LPARAM lpData )
{
	...
	{
		BOOL bSel = FALSE;
		if( _tcslen(SZ_SELED_DIR) == 2 ) // 盘符
		{
			_tcscat(SZ_SELED_DIR, _T("\\"));
			bSel = TRUE;
		}
		else
		{
			WIN32_FIND_DATA wfd;
			HANDLE hFind = ::FindFirstFile(SZ_SELED_DIR, &wfd);
		...
		if( bSel )
			::SendMessage(hWnd, BFFM_SETSELECTION, TRUE, (LPARAM)SZ_SELED_DIR);
	}
...
CString CFileDlg::GetSelectDir()
{
    ...
    browseInfo.lpszTitle      = _T("Please select a destination folder");   
	...
	_tcscpy(SZ_SELED_DIR,SZ_SEL_DIR);
