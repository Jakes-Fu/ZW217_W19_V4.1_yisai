/******************************************************************************
 * *vo_info.directx.c: Directx v2 or later DirectDraw interface for MPlayer
...
#define WNDCLASSNAME_WINDOWED	"MPlayer"
...
HWND hWndVideo=NULL;              //handle to the window
static HWND                 hWndVideoFS=NULL;           //fullscreen window
...
int vo_style = 0;
int vo_colorkey = 0x0000ff00;
int vo_border = 1;
int vo_ontop = 0;
HWND vo_parent = NULL;
int vo_event(int code);
...
static void draw_alpha(int x0, int y0, int w, int h, unsigned char *src, unsigned char *srca, int stride)
{
	...
	case IMGFMT_IYUV :
	case IMGFMT_YVU9 :
    	*vo_info.draw_alpha_yv12(w,h,src,srca,stride,((uint8_t *) image) + dstride*y0 + x0,dstride);
	break;
	case IMGFMT_YUY2 :
	    *vo_info.draw_alpha_yuy2(w,h,src,srca,stride,((uint8_t *) image)+ dstride*y0 + 2*x0 ,dstride);
    break;
    case IMGFMT_UYVY :
        *vo_info.draw_alpha_yuy2(w,h,src,srca,stride,((uint8_t *) image) + dstride*y0 + 2*x0 + 1,dstride);
    break;
	case IMGFMT_RGB15:	
    case IMGFMT_BGR15:
		*vo_info.draw_alpha_rgb15(w,h,src,srca,stride,((uint8_t *) image)+dstride*y0+2*x0,dstride);
    break;
    case IMGFMT_RGB16:
	case IMGFMT_BGR16:
        *vo_info.draw_alpha_rgb16(w,h,src,srca,stride,((uint8_t *) image)+dstride*y0+2*x0,dstride);
    break;
    case IMGFMT_RGB24:
	case IMGFMT_BGR24:
        *vo_info.draw_alpha_rgb24(w,h,src,srca,stride,((uint8_t *) image)+dstride*y0+4*x0,dstride);
    break;
    case IMGFMT_RGB32:
	case IMGFMT_BGR32:
        *vo_info.draw_alpha_rgb32(w,h,src,srca,stride,((uint8_t *) image)+dstride*y0+4*x0,dstride);
    break;
    }
	*/
}
...
void draw_osd(void)
{
    //*vo_info.draw_text(image_width,image_height,draw_alpha);
}
...
static uint32_t Directx_CreatePrimarySurface()
{
    ...
    if(*vo_info.vidmode)g_lpdd->lpVtbl->SetDisplayMode(g_lpdd,vm_width,vm_height,vm_bpp,*vo_info.refresh_rate,0);
    ...
    if(g_lpdd->lpVtbl->CreateSurface(g_lpdd,&ddsd, &g_lpddsPrimary, NULL )== DD_OK)
		mp_msg("<*vo_info.directx><INFO>primary surface created\n");
	else
	{
		mp_msg("<*vo_info.directx><FATAL ERROR>could not create primary surface\n");
		return 1;
	}
	return 0;
}
...
static uint32_t Directx_CreateOverlay(uint32_t imgfmt)
{
	...
	if(*vo_info.doublebuffering)   //tribblebuffering
	{
		if (g_lpdd->lpVtbl->CreateSurface(g_lpdd,&ddsdOverlay, &g_lpddsOverlay, NULL)== DD_OK)
		{
			mp_msg("<*vo_info.directx><INFO>overlay with format %s created\n",g_ddpf[i].img_format_name);
			...
			{
				mp_msg("<*vo_info.directx><FATAL ERROR>can't get attached surface\n");
				return 1;
			}
			return 0;
		}
		*vo_info.doublebuffering=0; //disable tribblebuffering
		mp_msg("<*vo_info.directx><WARN>cannot create tribblebuffer overlay with format %s\n",g_ddpf[i].img_format_name);
	} 
	//single buffer
	mp_msg("<*vo_info.directx><INFO>using singlebuffer overlay\n");
	...
	if(ddrval != DD_OK)
	{
	   if(ddrval == DDERR_INVALIDPIXELFORMAT)mp_msg("<*vo_info.directx><ERROR> invalid pixelformat: %s\n",g_ddpf[i].img_format_name);
       else mp_msg("<*vo_info.directx><ERROR>");
       switch(ddrval)
	   {
	      case DDERR_INCOMPATIBLEPRIMARY:
			 {mp_msg("incompatible primary surface\n");break;}
		  case DDERR_INVALIDCAPS:
			 {mp_msg("invalid caps\n");break;}
	      case DDERR_INVALIDOBJECT:
		     {mp_msg("invalid object\n");break;}
	      case DDERR_INVALIDPARAMS:
		     {mp_msg("invalid parameters\n");break;}
	      case DDERR_NODIRECTDRAWHW:
		     {mp_msg("no directdraw hardware\n");break;}
	      case DDERR_NOEMULATION:
		     {mp_msg("can't emulate\n");break;}
	      case DDERR_NOFLIPHW:
		     {mp_msg("hardware can't do flip\n");break;}
	      case DDERR_NOOVERLAYHW:
		     {mp_msg("hardware can't do overlay\n");break;}
	      case DDERR_OUTOFMEMORY:
		     {mp_msg("not enough system memory\n");break;}
	      case DDERR_UNSUPPORTEDMODE:
			 {mp_msg("unsupported mode\n");break;}  
		  case DDERR_OUTOFVIDEOMEMORY:
			 {mp_msg("not enough video memory\n");break;}
          default:
             mp_msg("create surface failed with 0x%x\n",ddrval);       
	   }
	   return 1;
	}
...
static uint32_t Directx_CreateBackpuffer()
{
	...
	{
		mp_msg("<*vo_info.directx><FATAL ERROR>can't create backpuffer\n");
		return 1;
	}
    mp_msg("<*vo_info.directx><INFO>backbuffer created\n");
	return 0;
}
...
static void uninit(void)
{
	...
	g_lpddclipper = NULL;
	mp_msg("<*vo_info.directx><INFO>clipper released\n");
	if (g_lpddsBack != NULL) g_lpddsBack->lpVtbl->Release(g_lpddsBack);
	g_lpddsBack = NULL;
	mp_msg("<*vo_info.directx><INFO>back surface released\n");
	if(*vo_info.doublebuffering && !nooverlay)
	{
		if (g_lpddsOverlay != NULL)g_lpddsOverlay->lpVtbl->Release(g_lpddsOverlay);
        g_lpddsOverlay = NULL;
		mp_msg("<*vo_info.directx><INFO>overlay surface released\n");
	}
	if (g_lpddsPrimary != NULL) g_lpddsPrimary->lpVtbl->Release(g_lpddsPrimary);
    g_lpddsPrimary = NULL;
	mp_msg("<*vo_info.directx><INFO>primary released\n");
	if(hWndVideoFS)DestroyWindow(hWndVideoFS);
	hWndVideoFS = NULL;
	if(hWndVideo) DestroyWindow(hWndVideo);
	hWndVideo = NULL;
	mp_msg("<*vo_info.directx><INFO>window destroyed\n");
	...
	colorbrush = NULL;
	mp_msg("<*vo_info.directx><INFO>GDI resources deleted\n");
	if (g_lpdd != NULL){
	    if(*vo_info.vidmode)g_lpdd->lpVtbl->RestoreDisplayMode(g_lpdd);
	    g_lpdd->lpVtbl->Release(g_lpdd);
	}  
	mp_msg("<*vo_info.directx><INFO>directdrawobject released\n");
	mp_msg("<*vo_info.directx><INFO>ddraw.dll freed\n");
	mp_msg("<*vo_info.directx><INFO>uninited\n");    
	vo_event(0);
}
...
{   
    mp_msg("<*vo_info.directx> adapter %d: ", adapter_count);

    if (!lpGUID)
    {
        mp_msg("%s", "Primary Display Adapter");
    }
    else
    {
        mp_msg("%s", lpDriverDescription);
    }
    
    if(adapter_count == *vo_info.adapter_num){
        MONITORINFO mi;
        if (!lpGUID)
...
static uint32_t Directx_InitDirectDraw()
{
	...
	mp_msg("<*vo_info.directx><INFO>Initing DirectDraw\n" );

	//load direct draw DLL: based on videolans code
    last_rect.left = 0xDEADC0DE;   // reset window position cache

	if(*vo_info.adapter_num){ //display other than default
        // enumerate all display devices attached to the desktop
        DirectDrawEnumerateExA(EnumCallbackEx, NULL, DDENUM_ATTACHEDSECONDARYDEVICES );

        if(*vo_info.adapter_num >= adapter_count)
            mp_msg("Selected adapter (%d) doesn't exist: Default Display Adapter selected\n",*vo_info.adapter_num);
    }

	// initialize DirectDraw and create directx v7 object
    if (DirectDrawCreateEx(selected_guid_ptr, (VOID**)&g_lpdd, &IID_IDirectDraw7, NULL ) != DD_OK )
    {
        mp_msg("<*vo_info.directx><FATAL ERROR>can't initialize ddraw\n");
		return 1;
    }
	...
	if(*vo_info.screenwidth && *vo_info.screenheight)
	{
	    vm_height=*vo_info.screenheight;
	    vm_width=*vo_info.screenwidth;
	}
    else 
    {
	...
	if(*vo_info.dbpp)vm_bpp=*vo_info.dbpp;
	else vm_bpp=ddsd.ddpfPixelFormat.dwRGBBitCount;

	if(*vo_info.vidmode){
		if (g_lpdd->lpVtbl->SetCooperativeLevel(g_lpdd, hWndVideo, DDSCL_EXCLUSIVE|DDSCL_FULLSCREEN) != DD_OK)
		{
	        mp_msg("<*vo_info.directx><FATAL ERROR>can't set cooperativelevel for exclusive mode\n");
            return 1;
		}                                
		...
		{
	        mp_msg("<*vo_info.directx><FATAL ERROR>can't set displaymode\n");
	        return 1;
		}
	    mp_msg("<*vo_info.directx><INFO>Inited adapter %i for %i x %i @ %i \n",*vo_info.adapter_num,vm_width,vm_height,vm_bpp);	
	    return 0;	
	}
	if (g_lpdd->lpVtbl->SetCooperativeLevel(g_lpdd, hWndVideo, DDSCL_NORMAL) != DD_OK) // or DDSCL_SETFOCUSWINDOW
     {
        mp_msg("<*vo_info.directx><FATAL ERROR>could not set cooperativelevel for hardwarecheck\n");
		return 1;
    }
    mp_msg("<*vo_info.directx><INFO>DirectDraw Inited\n");
	return 0;
}	
...
{   
	...
	if (!g_lpddsBack) return -1;

    if(*vo_info.fs || *vo_info.vidmode){
      aspect(&width,&height,A_ZOOM);
      rd.left=(*vo_info.screenwidth-width)/2;
      rd.top=(*vo_info.screenheight-height)/2;
      if(ShowCursor(FALSE)>=0)while(ShowCursor(FALSE)>=0){}
    } else {
      POINT pt;
      pt.x = 0;  //overlayposition relative to the window
      pt.y = 0;
      ClientToScreen(hWndVideo,&pt);  
      GetClientRect(hWndVideo, &rd);
	...
	if(!nooverlay)
	{
        ...
        if(rd.right > *vo_info.screenwidth)rs.right=((*vo_info.screenwidth-rd.left)*1000)/xstretch1000;
        if(rd.bottom > *vo_info.screenheight)rs.bottom=((*vo_info.screenheight-rd.top)*1000)/ystretch1000;
		...
		{
			if(capsDrv.dwFXCaps & DDFXCAPS_OVERLAYSHRINKXN)mp_msg("<*vo_info.directx><ERROR>can only shrinkN\n");
	        else mp_msg("<*vo_info.directx><ERROR>can't shrink x\n");
	        rd.right=rd.left+image_width;
		}
		else if((width > image_width)&& !(capsDrv.dwFXCaps & DDFXCAPS_OVERLAYSTRETCHX))
		{
			if(capsDrv.dwFXCaps & DDFXCAPS_OVERLAYSTRETCHXN)mp_msg("<*vo_info.directx><ERROR>can only stretchN\n"); 
	        else mp_msg("<*vo_info.directx><ERROR>can't stretch x\n");
	        rd.right = rd.left+image_width;
		}
		if((height < image_height) && !(capsDrv.dwFXCaps & DDFXCAPS_OVERLAYSHRINKY))
		{
			if(capsDrv.dwFXCaps & DDFXCAPS_OVERLAYSHRINKYN)mp_msg("<*vo_info.directx><ERROR>can only shrinkN\n");
	        else mp_msg("<*vo_info.directx><ERROR>can't shrink y\n");
	        rd.bottom = rd.top + image_height;
		}
		else if((height > image_height ) && !(capsDrv.dwFXCaps & DDFXCAPS_OVERLAYSTRETCHY))
		{
			if(capsDrv.dwFXCaps & DDFXCAPS_OVERLAYSTRETCHYN)mp_msg("<*vo_info.directx><ERROR>can only stretchN\n");
	        else mp_msg("<*vo_info.directx><ERROR>can't stretch y\n");
	        rd.bottom = rd.top + image_height;
		} 
    ...
    {
        g_lpddclipper->lpVtbl->SetHWnd(g_lpddclipper, 0,(*vo_info.fs && !*vo_info.vidmode)?hWndVideoFS: hWndVideo);
    }       
	
    if(!*vo_info.vidmode && !*vo_info.fs){
          RECT rdw=rd;
          if (vo_border)
...
//          printf("window: %i %i %ix%i\n",rdw.left,rdw.top,rdw.right - rdw.left,rdw.bottom - rdw.top);      
          ...
          SetWindowPos(hWndVideo,(vo_ontop)?HWND_TOPMOST:HWND_NOTOPMOST,rdw.left,rdw.top,rdw.right-rdw.left,rdw.bottom-rdw.top,SWP_NOOWNERZORDER); 
    }
    else SetWindowPos(*vo_info.vidmode?hWndVideo:hWndVideoFS,HWND_TOPMOST,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOOWNERZORDER|SWP_NOCOPYBITS);

    /*make sure the overlay is inside the screen*/
    if(rd.left<0)rd.left=0;
    if(rd.right>*vo_info.screenwidth)rd.right=*vo_info.screenwidth;
    if(rd.top<0)rd.top=0;
    if(rd.bottom>*vo_info.screenheight)rd.bottom=*vo_info.screenheight;
...
//    printf("overlay: %i %i %ix%i\n",rd.left,rd.top,rd.right - rd.left,rd.bottom - rd.top);
    ...
    if(FAILED(ddrval))
    {
        ...
        // perhaps shrinking the source size
	   	mp_msg("<*vo_info.directx><ERROR>UpdateOverlay failed\n" );
	  	mp_msg("<*vo_info.directx><ERROR>Overlay:x1:%i,y1:%i,x2:%i,y2:%i,w:%i,h:%i\n",rd.left,rd.top,rd.right,rd.bottom,rd.right - rd.left,rd.bottom - rd.top );
	  	mp_msg("<*vo_info.directx><ERROR>");
		switch (ddrval)
		{
			case DDERR_NOSTRETCHHW:
				{mp_msg("hardware can't stretch: try to size the window back\n");break;}
            case DDERR_INVALIDRECT:
				{mp_msg("invalid rectangle\n");break;}
			case DDERR_INVALIDPARAMS:
				{mp_msg("invalid parameters\n");break;}
			...
			case DDERR_INVALIDSURFACETYPE:
				{mp_msg("invalid surfacetype\n");break;}
			case DDERR_INVALIDOBJECT:
				{mp_msg("invalid object\n");break;}
			case DDERR_SURFACELOST:
				{
					mp_msg("surfaces lost\n");
					g_lpddsOverlay->lpVtbl->Restore( g_lpddsOverlay ); //restore and try again
			        g_lpddsPrimary->lpVtbl->Restore( g_lpddsPrimary );
			        ddrval = g_lpddsOverlay->lpVtbl->UpdateOverlay(g_lpddsOverlay,&rs, g_lpddsPrimary, &rd, dwUpdateFlags, &ovfx);   
					if(ddrval !=DD_OK)mp_msg("<*vo_info.directx><FATAL ERROR>UpdateOverlay failed again\n" );
					break;
				}
            default:
                mp_msg(" 0x%x\n",ddrval);      
...
static uint32_t Directx_CheckOverlayPixelformats()
{
    ...
    if (FAILED(ddrval))
	{
        mp_msg("<*vo_info.directx><ERROR>failed getting ddrawcaps\n");
		return 1;
	}
    if (!(capsDrv.dwCaps & DDCAPS_OVERLAY))
    {
		mp_msg("<*vo_info.directx><ERROR>Your card doesn't support overlay\n");
		return 1;
	}
    mp_msg("<*vo_info.directx><INFO>testing supported overlay pixelformats\n");
    ...
    do 
    {
        ...
        if (ddrval == DD_OK)
		{
			 mp_msg("<*vo_info.directx><FORMAT OVERLAY>%i %s supported\n",i,g_ddpf[i].img_format_name);
			 g_ddpf[i].drv_caps = VFCAP_CSP_SUPPORTED |VFCAP_OSD |VFCAP_CSP_SUPPORTED_BY_HW | VFCAP_HWSCALE_UP;
			 formatcount++;}
	    else mp_msg("<*vo_info.directx><FORMAT OVERLAY>%i %s not supported\n",i,g_ddpf[i].img_format_name);
		if (g_lpddsOverlay != NULL) {g_lpddsOverlay->lpVtbl->Release(g_lpddsOverlay);g_lpddsOverlay = NULL;}
	} while( ++i < NUM_FORMATS );
    mp_msg("<*vo_info.directx><INFO>Your card supports %i of %i overlayformats\n",formatcount, NUM_FORMATS);
	if (formatcount == 0)
	{
		mp_msg("<*vo_info.directx><WARN>Your card supports overlay, but we couldn't create one\n");
		mp_msg("<*vo_info.directx><INFO>This can have the following reasons:\n");
		mp_msg("<*vo_info.directx><INFO>- you are already using an overlay with another app\n");
		mp_msg("<*vo_info.directx><INFO>- you don't have enough videomemory\n");
		mp_msg("<*vo_info.directx><INFO>- *vo_info.directx doesn't support the cards overlay pixelformat\n");
		return 1;
	}
    if(capsDrv.dwFXCaps & DDFXCAPS_OVERLAYMIRRORLEFTRIGHT)mp_msg("<*vo_info.directx><INFO>can mirror left right\n"); //I don't have hardware which
    if(capsDrv.dwFXCaps & DDFXCAPS_OVERLAYMIRRORUPDOWN )mp_msg("<*vo_info.directx><INFO>can mirror up down\n");      //supports those send me one and I'll implement ;)
	return 0;		
}
...
{	
	...
	mp_msg("<*vo_info.directx><INFO>checking primary surface\n");
	...
	{
		mp_msg("<*vo_info.directx><FATAL ERROR>can't get pixelformat\n");
		return 1;
	}
    while ( i < NUM_FORMATS )
    {
	   if (g_ddpf[i].g_ddpfOverlay.dwRGBBitCount == ddpf.dwRGBBitCount)
	   {
		   ...
		   {
			   mp_msg("<*vo_info.directx><FORMAT PRIMARY>%i %s supported\n",i,g_ddpf[i].img_format_name);
	...
	if(formatcount==0)
	{
		mp_msg("<*vo_info.directx><FATAL ERROR>Unknown Pixelformat\n");
		return 1;
	}
	return 0;
}
...
static LRESULT CALLBACK WndProc(HWND hWndVideo, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
    {
	    ...
	    case WM_NCACTIVATE:
        {
            if(*vo_info.vidmode && adapter_count > 2) //only disable if more than one adapter.
			    return 0;
			break;
		}
		case WM_DESTROY:
        ...
        case WM_SYSCOMMAND:
		{
			...
			{   //kill screensaver etc. 
                ...
                mp_msg("<*vo_info.directx><INFO>killing screensaver\n" );
                return 0;                      
				case SC_MAXIMIZE:
					//if (!*vo_info.fs) control(VOCTRL_FULLSCREEN, NULL);
                return 0;                      
			}
			break;
		}
	...
	return DefWindowProc(hWndVideo, message, wParam, lParam);
}
...
static int preinit(const char *arg)
{
	...
	if(arg)
	{
		...
		{
			mp_msg("<*vo_info.directx><INFO>disabled overlay\n");
		    nooverlay = 1;
		}
	}
    ...
    wc.hbrBackground =  *vo_info.vidmode ? blackbrush : colorbrush;
    wc.lpszClassName =  WNDCLASSNAME_WINDOWED;
    wc.lpszMenuName  =  NULL;
    RegisterClass(&wc);
	if (!hWndVideo) {
		hWndVideo = CreateWindowEx(*vo_info.vidmode?WS_EX_TOPMOST:0,
			WNDCLASSNAME_WINDOWED,"", vo_parent ? WS_CHILD : WNDSTYLE,
			vo_parent ? 0 : CW_USEDEFAULT, vo_parent ? 0 : CW_USEDEFAULT, 0, 0,vo_parent,NULL,hInstance,NULL);
	}
    ...
    if(!*vo_info.vidmode)hWndVideoFS = CreateWindow(WNDCLASSNAME_FULLSCREEN,"MPlayer Fullscreen",WS_POPUP,monitor_rect.left,monitor_rect.top,monitor_rect.right-monitor_rect.left,monitor_rect.bottom-monitor_rect.top,hWndVideo,NULL,hInstance,NULL);			
    mp_msg("<*vo_info.directx><INFO>initial mplayer windows created\n");
	...
	{
		mp_msg("<*vo_info.directx><INFO>hardware supports overlay\n");
		nooverlay = 0;
 	} 
	else   //if we can't have overlay we create a backpuffer with the same imageformat as the primary surface
	{
       	mp_msg("<*vo_info.directx><INFO>using backpuffer\n");
		nooverlay = 1;
	}
 	mp_msg("<*vo_info.directx><INFO>preinit succesfully finished\n");
	return 0;
}
...
void flip_page(void)
{
	...
	if (*vo_info.doublebuffering) 
    {
		// flip to the next image in the sequence  
		dxresult = g_lpddsOverlay->lpVtbl->Flip( g_lpddsOverlay,NULL, DDFLIP_WAIT);
		if(dxresult == DDERR_SURFACELOST)
		{
			mp_msg("<*vo_info.directx><ERROR><*vo_info.directx><INFO>Restoring Surface\n");
			g_lpddsBack->lpVtbl->Restore( g_lpddsBack );
		    dxresult = g_lpddsOverlay->lpVtbl->Flip( g_lpddsOverlay,NULL, DDFLIP_WAIT);
		}
		if(dxresult != DD_OK)mp_msg("<*vo_info.directx><ERROR>can't flip page\n");
    }
	if(nooverlay) 
	{
		DDBLTFX  ddbltfx;
    ...
    if(*vo_info.directrendering && (dstride != ddsdsf.lPitch)){
      mp_msg("<*vo_info.directx><WARN>stride changed !!!! disabling direct rendering\n");
      *vo_info.directrendering=0;
    }
...
{
    if(mpi->flags&MP_IMGFLAG_READABLE) {mp_msg("<*vo_info.directx><ERROR>slow video ram\n");return VO_FALSE;} 
    if(mpi->type==MP_IMGTYPE_STATIC) {mp_msg("<*vo_info.directx><ERROR>not static\n");return VO_FALSE;}
    if((mpi->width==dstride) || (mpi->flags&(MP_IMGFLAG_ACCEPT_STRIDE|MP_IMGFLAG_ACCEPT_WIDTH)))
	{
        ...
        mp_msg( "<*vo_info.directx><INFO>Direct Rendering ENABLED\n");
        return VO_TRUE;
    } 
    return VO_FALSE;
}
...
static uint32_t put_image(mp_image_t *mpi){
	...
	{
		mp_msg("<*vo_info.directx><INFO>put_image: nothing to do: drawslices\n");
		return VO_TRUE;
    }
...
{
    RECT rd;
    *vo_info.screenwidth = monitor_rect.right - monitor_rect.left;
    *vo_info.screenheight = monitor_rect.bottom - monitor_rect.top;
    *vo_info.fs = options & 0x01;
	image_format =  format;
	image_width = width;
	image_height = height;
    ...
    if(*vo_info.vidmode){
	    *vo_info.screenwidth=vm_width;
	    *vo_info.screenheight=vm_height;
    }	
	aspect_save_screenres(*vo_info.screenwidth,*vo_info.screenheight);
    ...
    *vo_info.dy = 0;   
    ...
    if (g_lpddsPrimary != NULL) g_lpddsPrimary->lpVtbl->Release(g_lpddsPrimary);
    g_lpddsPrimary = NULL;
    mp_msg("<*vo_info.directx><INFO>overlay surfaces released\n");

    if(!*vo_info.vidmode){
        if(vo_geometry){
            *vo_info.dx= ( *vo_info.screenwidth - d_image_width ) / 2; *vo_info.dy=( *vo_info.screenheight - d_image_height ) / 2;    
            geometry(&*vo_info.dx, &*vo_info.dy, &d_image_width, &d_image_height, *vo_info.screenwidth, *vo_info.screenheight);
        }
        else {
            GetWindowRect(hWndVideo,&rd);
            *vo_info.dx=rd.left;
            *vo_info.dy=rd.top;
        }
        *vo_info.dx += monitor_rect.left; /* move position to global window space */
        *vo_info.dy += monitor_rect.top;
        rd.left = *vo_info.dx;
        rd.top = *vo_info.dy;
        rd.right = rd.left + d_image_width;
        rd.bottom = rd.top + d_image_height;
        if (vo_border && !vo_style) AdjustWindowRect(&rd,WNDSTYLE,FALSE);  
        SetWindowPos(hWndVideo,NULL, *vo_info.dx, *vo_info.dy,rd.right-rd.left,rd.bottom-rd.top,SWP_SHOWWINDOW|SWP_NOOWNERZORDER); 
    }
    else ShowWindow(hWndVideo,SW_SHOW); 
     
    if(*vo_info.fs && !*vo_info.vidmode)ShowWindow(hWndVideoFS,SW_SHOW);   
    if(*vo_info.vidmode)*vo_info.fs=0;    
	...
	if (!nooverlay && Directx_CreateOverlay(image_format))
	{
			if(format == primary_image_format)nooverlay=1; /*overlay creation failed*/
              ...
              mp_msg("<*vo_info.directx><FATAL ERROR>can't use overlay mode: please use -vo directx:noaccel\n");
			  return 1;
            }       
	}
	if(nooverlay)
	{
		...
		{
			mp_msg("<*vo_info.directx><FATAL ERROR>can't get the driver to work on your system :(\n");
			return 1;
		}
		mp_msg("<*vo_info.directx><INFO>back surface created\n");
		*vo_info.doublebuffering = 0;
		/*create clipper for nonoverlay mode*/
	    if(g_lpdd->lpVtbl->CreateClipper(g_lpdd, 0, &g_lpddclipper,NULL)!= DD_OK){mp_msg("<*vo_info.directx><FATAL ERROR>can't create clipper\n");return 1;}
        if(g_lpddclipper->lpVtbl->SetHWnd (g_lpddclipper, 0, hWndVideo)!= DD_OK){mp_msg("<*vo_info.directx><FATAL ERROR>can't associate clipper with window\n");return 1;}
        if(g_lpddsPrimary->lpVtbl->SetClipper (g_lpddsPrimary,g_lpddclipper)!=DD_OK){mp_msg("<*vo_info.directx><FATAL ERROR>can't associate primary surface with clipper\n");return 1;}
	    mp_msg("<*vo_info.directx><INFO>clipper succesfully created\n");
	}else{
		if(DD_OK != g_lpddsOverlay->lpVtbl->QueryInterface(g_lpddsOverlay,&IID_IDirectDrawColorControl,(void**)&g_cc))
			mp_msg("<*vo_info.directx><WARN>unable to get DirectDraw ColorControl interface\n");
	}
	Directx_ManageDisplay();
...
static int control(uint32_t request, void *data, ...)
{
    ...
    case VOCTRL_BORDER:
	        if(*vo_info.vidmode)
			{
				mp_msg("<*vo_info.directx><ERROR>border has no meaning in exclusive mode\n");
			}
	        else
			{
				if(vo_border) {
					vo_border = 0;
					SetWindowLong(hWndVideo, GWL_STYLE, WS_POPUP);
				} else {
					vo_border = 1;
					SetWindowLong(hWndVideo, GWL_STYLE, WNDSTYLE);
				}
    ...
    case VOCTRL_ONTOP:
	        if(*vo_info.vidmode)
			{
				mp_msg("<*vo_info.directx><ERROR>ontop has no meaning in exclusive mode\n");
			}
	        else
			{
    ...
    case VOCTRL_FULLSCREEN:
		{
	        if(*vo_info.vidmode)
			{
				mp_msg("<*vo_info.directx><ERROR>currently we do not allow to switch from exclusive to windowed mode\n");
			}
	        else
			{
			    if(!*vo_info.fs)
				{
					*vo_info.fs=1;
					ShowWindow(hWndVideoFS,SW_SHOW);
					ShowWindow(hWndVideo,SW_HIDE);
					SetForegroundWindow(hWndVideoFS);
				}
                else
				{
					*vo_info.fs=0;
					ShowWindow(hWndVideoFS,SW_HIDE);
					ShowWindow(hWndVideo,SW_SHOW);
				}  
	      			last_rect.left = 0xDEADC0DE;   // reset window position cache
				Directx_ManageDisplay();
...
void SetAddrPtr(vo_functions_t* vo)
{
	...
	vo->info = &info;
	*/
	vo->draw_frame = draw_frame;
	vo->draw_slice = draw_slice;
	vo->draw_osd = draw_osd;
	vo->flip_page = flip_page;
	vo->check_events = check_events;
	vo->config = config;
	vo->control = control;
	vo->preinit = preinit;
	vo->uninit = uninit;
