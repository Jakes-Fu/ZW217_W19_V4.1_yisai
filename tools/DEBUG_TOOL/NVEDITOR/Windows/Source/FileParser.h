...
public:
	...
	// add for MS\BYD check value
	BOOL CheckValue(TCHAR* strDes, TCHAR* szValue, ITEMTYPE_ENUM itemType);
	
	//for fix bin merge
	void SetWriteFix(TCHAR* szFixBin);
	
	// for hash header
	void SetHashHeaderPath(TCHAR* szHash);
...
protected:
	...
	public:
		BOOL ImportItem( CString strFile );	// used for command line mode only
		BOOL ImportNV(CString strFile); // used for samsung
