...
void CSuperGridCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if(lpDrawItemStruct->itemAction == ODA_DRAWENTIRE)
	{
		...
		else
		{
            ...
            crText = GetData(pSelItem)->GetTextClr();
		}
...
{
	UNREFERENCED_PARAMETER(iSubIconOffset);
	if(NULL == pDC || NULL == pSelItem)
	{
		return;
	}
	...
	if(pMsg->message == WM_KEYDOWN)
	{
		if(GetFocus()==this)
		{
			switch( pMsg->wParam )
			{
				case VK_LEFT:
					{		
						// [[ by Joly for bug 320350
						if( 0 == m_CurSubItem )
						{	
						   int iItem = GetSelectedItem();
					 	   pCurItemTmp = GetTreeItem(iItem);
						   if ( NULL != pCurItemTmp && !(IsCollapsed(pCurItemTmp)) && ItemHasChildren(pCurItemTmp))
						   {							
							   Collapse(pCurItemTmp);
							   OnItemCollapsed(pCurItemTmp);
						       InvalidateItemRect(iItem);							   
						   }
						   else//this branch still has problem.
						   {
							   pCurItemTmp = GetParentItem(pCurItemTmp);
							   if ( NULL != pCurItemTmp )
							   {							  
								   SelectNode(pCurItemTmp);
								   nScrollIndex = GetSelectedItem();
								   if ( -1 != nScrollIndex )
								   {
									   InvalidateItemRect(nScrollIndex);
								   }						  							   							
								}							   					  		   
						   }
						   return TRUE;
				...
				case VK_RIGHT:
					{   // [[ by Joly for Bug 320350
						if ( 0 == m_CurSubItem )
						{
							...
							{
								pCurItemTmp = GetTreeItem(iItem);
								if ( NULL != pCurItemTmp && ItemHasChildren(pCurItemTmp) )
								{							
									if(OnItemExpanding( pCurItemTmp, iItem ))
									{								 
										nScrollIndex = Expand( pCurItemTmp, iItem );
									    OnItemExpanded( pCurItemTmp, iItem );
										InvalidateItemRect(iItem);					
									}
									return TRUE;
								}							
...
void CSuperGridCtrl::MakeColumnVisible(int nCol)
{
	...
	int offset = 0;
	for(int i = 0; i<nColCount &&  pOrderarray[i] != nCol; i++)
...
/*	lpInfo->m_SubItems.FreeExtra();  //2015.6.1*/
...
void CSuperGridCtrl::OnModuleSelchanged(UINT nSelectModuleIndex)
{    
	UNREFERENCED_PARAMETER(nSelectModuleIndex);
}

void CSuperGridCtrl::OnItemSelchanged(UINT nSelectModuleIndex, UINT nSelectItemIndex)
{
	UNREFERENCED_PARAMETER(nSelectModuleIndex);
	UNREFERENCED_PARAMETER(nSelectItemIndex);
}

//@# Kai Liu 2003.3.4
// [
CSuperGridCtrl::CTreeItem* CSuperGridCtrl::InsertAnItem(CSuperGridCtrl::CTreeItem* pParent,
                              CSuperGridCtrl::CTreeItem* pCurItem,
                              CItemInfo* lpInfo,
                              BOOL bInsertAfter,
                              BOOL bUpdate)
{
    ...
    POSITION pos = pParent->m_listChild.Find(pCurItem);
    if(pos == NULL)
    {
        pParent->m_listChild.AddTail(pItem);
    }
    else
    {
        if(bInsertAfter)
        {
            pParent->m_listChild.InsertAfter(pos,pItem);
        }
        else
        {
