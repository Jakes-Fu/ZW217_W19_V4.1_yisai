...
class CItemInfo //should have called this LV_INSERTITEM ..what ever
{
     //Added by Leo.Feng
public:
    void  SetItemType(DWORD dwType){m_dwType = dwType;}
    DWORD GetItemType(){return m_dwType;}
...
public:
	CItemInfo():m_dwData(0),m_dwType((DWORD)(-1)),m_bCheck(0),m_iImage((const int)-1),m_lParam(NULL),m_clf((COLORREF)-1),m_enumCtrlType(edit),m_bCustItem(FALSE),m_bCustBase(FALSE),m_bChanged(FALSE){m_clfText = GetSysColor (COLOR_WINDOWTEXT);};
...
// 	}
	...
	// cust item
	void SetCustItem(BOOL bCustItem){ m_bCustItem = bCustItem;}
	BOOL IsCustItem(void) const{return m_bCustItem;}


	//cust base
	void SetCustBaseItem(BOOL bCustBase){ m_bCustBase = bCustBase;}
	BOOL IsCustBaseItem(void) const{return m_bCustBase;}
	...
	{
		//SetItemType(pItemInfo->GetItemType());
		//SetItemData(pItemInfo->GetItemData());
		...
		SetChanged(pItemInfo->GetChanged());
...
// 	}	
    ...
    COLORREF GetTextClr(){return m_clfText;};
    void SetTextClr(COLORREF clr){ m_clfText = clr;}
...
protected:
	...
	void SetCurSubItem(int nSubItem){m_CurSubItem = nSubItem;}
...
public:
    virtual void OnItemchanged(NMHDR* pNMHDR, LRESULT* pResult);
   	virtual void OnItemSelchanged(UINT nSelectModuleIndex,UINT nSelectItemIndex);
    virtual void OnModuleSelchanged(UINT nSelectModuleIndex);
    virtual CEdit* EditLabelEx(int nItem, int nCol);

    //@# Kai Liu 2003.3.4
    // [
protected:
    CSuperGridCtrl::CTreeItem* InsertAnItem(CSuperGridCtrl::CTreeItem* pParent,
