// NVWinGen.cpp : Defines the entry point for the console application.
//
...
#define VERSION_INFO  _T("NVGen_R2.18.4901\n")
...
BOOL ParseCommandLine(int argc, TCHAR* argv[])
{
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		else if (0 == _tcsicmp(argv[i],_T("-nvimage")))
		{
			if( (i+1)<argc )
			{
				_tcscpy(g_stCmd.szNvImgFile, argv[i + 1]);
				i += 1;
			}
			else
			{
                _tprintf(_T("-nvimage's parameter format is not correct. [-nvimage nvitem.bin]\n"));
                break;
			}
		}
...
const void* GetNVItemFromBlock( const unsigned char *pBlock, DWORD cbBlockLen, WORD wID, WORD *pLen )
{
	ASSERT( NULL != pBlock );
	ASSERT( NULL != pLen );
...
BOOL ImportNVImage( CFileParser* pParser,LPCTSTR pszNVimgFile )
{
	
	LPBYTE pBuf = NULL;
    DWORD dwSize = 0;	
	if(!LoadFileFromLocal(pszNVimgFile,pBuf,dwSize))
	{
		_tprintf(_T("Load nv image file fail.[%s]\r\n"), pszNVimgFile);
		return FALSE;
    }
	BOOL bOK = TRUE;
	int nCount = pParser->GetNVItemCount();
	for( int i=0; i<nCount; i++ )
	{
		WORD wLen;
		WORD wItemId = (WORD)pParser->GetNVItemID( i );
		
		const void *pItem = GetNVItemFromBlock( pBuf, dwSize, wItemId, &wLen );
		if ( NULL == pItem )
		{
			// if an error occured, we just report it 
			// and continue
			_tprintf(_T("Can't get NV Item: Id(%d),  Name(%s)\r\n"), wItemId,pParser->GetNVItemName(i));
			bOK = FALSE;
			break;
		}
		
		if ( !pParser->WriteNVItem( wItemId,( void* )pItem,wLen,TRUE) )
		{
			// if an error occured, we just report it
			// and continue
			_tprintf(_T("Interpret Nv Item %d fail!\r\n"), wItemId);
			bOK = FALSE;
			break;
		}
	}
...
BOOL DoCmdLine()
{
	...
	TCHAR sz_load_nvimage_file_failed[] = _T( "Load nvitem.bin failed\r\n" );
	...
	do 
	{
		...
		bRet = bIsXMLPrj ? pParser->LoadProjectXML(g_stCmd.strFileName,agError,g_stCmd.bCheckItemName) : pParser->LoadProject(g_stCmd.strFileName,agError,g_stCmd.bCheckItemName);
		if (!bRet)
		{
			break;
		}
		int nWarningCount = agError.GetSize();
		...
		//Load image
		if ( 0!= _tcslen(g_stCmd.szNvImgFile) && IsExistFile(g_stCmd.szNvImgFile) )
		{
			bRet = ImportNVImage( pParser,g_stCmd.szNvImgFile );
			if( !bRet )
			{
				_tprintf(_T("%s : %s"), sz_load_nvimage_file_failed, g_stCmd.szNvImgFile);
				break;
			}	
		}
		//import file
		...
		{
			// Import file 
			bRet = pParser->ImportSingleNVFile( g_stCmd.strImportFile );
			if( !bRet )
			{
