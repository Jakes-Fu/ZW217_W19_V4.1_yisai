...
BOOL ValueIsValid( TCHAR* lpszValue )
{
	...
	if (nLength>=2 && 0 == strValue.Left(1).CompareNoCase(_T("\"")))
	{
		//ASTRING ,such as "*******"
		if (0 == strValue.Right(1).CompareNoCase(_T("\"")))
		{
			return TRUE;
		}
	...
	if( nLength>=2 && 0== strValue.Left(2).CompareNoCase( _T( "0x" ) ) )
	{
                                ...
                                const TCHAR *szPrjDesc)
{
    ...
    else
    {
        ...
        if ( NULL == m_hPrjFile )
        {
            ...
            CString strErr;
            strErr.Format( _T("Can't open file %s!"), m_szPrjFileName );
                                ...
                                const TCHAR *szPrjDesc)
{
    ...
    {  
        //Notify that open file failed
        CString strErr;
        strErr.Format( _T("Creat IXMLDOMDocumentPtr fail!"));
        MessageBox(NULL, strErr, _T("Error"), MB_OK);
        return FALSE;
    }
...
BOOL CFileParser::SaveModule(INT nModuleIndex)
{
	...
	if( NULL == fpFile )
	{
		//Notify that open file failed
		CString strErr;
		strErr.Format( _T("Can't open file %s!"), m_CurrModule.szModuleFileName );
		MessageBox(NULL, strErr, _T("Error"), MB_OK);
		// ]]
		return FALSE;
	}
	...
	{      
		...
		{       
			...
			if(m_CurrModuleItem.bReadOnly || theApp.m_bWriteReadOnlyZeroItem)
			{
...
BOOL CFileParser::LoadProject(TCHAR *szPrjFileName,CStringArray& agWarning,BOOL bCheckItemName/* = TRUE*/)
{
	...
	else
	{
		...
		else
		{
			...
			if ( NULL == pszLine )
			{
				if( theApp.m_bCmdMode )
				{
					
				}
				else
				{
					::AfxMessageBox( _T("Invalid .prj file or the file corrupted!") );
				}
				::fclose( m_hPrjFile );
				return FALSE;
			}
			...
			{
				if ( ::AfxMessageBox( _T("This prj file is for customer only\n")
					_T("Do you really want to continue?"),
					MB_YESNO ) == IDNO )
				{
					::fclose( m_hPrjFile );
					return FALSE;
						...
						*(pResult + 1) = 0;
						...
						if(nVer == 1)
						{
							...
							else 
							{
								...
								{
									CString strError;
									strError.Format( _T("LoadModule %s failed..."), szLineContent );
	...
	{
		CString  strTmp;
		strTmp.Format(_T("Exist repeated IDs: %s\n"), m_strErrID);
...
BOOL CFileParser::LoadProjectXML(TCHAR *szPrjFileName,CStringArray& agWarning,BOOL bCheckItemName/* = TRUE*/)
{
	...
	else
	{
		...
		{
			CString strErr;
			strErr.Format( _T("Load file %s fail!"), m_szPrjFileName);
			MessageBox(NULL, strErr, _T("Error"), MB_OK);
			...
			for(int i=0;i<length;i++)
			{
				...
				if(pIXMLDOMNode)
				{
					...
					if(0==_tcscmp(s,_T("version")))
					{
						...
						{
							CString strError;
							strError.Format(_T("Current NV tool just support %s project's version,but this project version is %s,Please upgrade nv tool."),XML_VERSION,szPrjVer);
							AfxMessageBox(strError);
							return FALSE;
						}
					}
            ...
            {	
                    ...
                    ('\n' != *pszLine))
                {
                    ...
                    if(0 == (_tcscmp(szString, _T("MODULE"))))
                    {
						...
						if (NULL != _tcsstr(pszLine, _T("Calibration")))
						{
							m_bCalibration = TRUE;
						}
                    }                    
                    ...
                    else if(0 == (_tcscmp(szString, _T("END_ITEM"))))
                    {
						...
						//[[ add by Joly for static Nv
						if (-1 == m_CurrModuleItem.nParentItemIndex && m_bCalibration)
						{
							m_mapCalibrationData.insert(CMapData::value_type(m_CurrModuleItem.nItemID, m_CurrModuleItem.szItemName));
						}
						//]]
    ...
    for (int i = 0;i < num ;i++)
    {
        ...
        if(pNode)
        {
			...
			if ((int)nParentItemIndex == ROOT_ELEMENT)
			{
				...
				//[[ add by polo for Calibration Nv
				if ( m_bStaticFlag )
				{
					m_mapStaticData.insert(CMapData::value_type(m_CurrModuleItem.nItemID, m_CurrModuleItem.szItemName));
				}
...
BOOL CFileParser::LoadModuleXML(TCHAR *szModuleFileName)
{
    ...
    else
    {
        ...
        {   
            CString strErr;
            strErr.Format( _T("Load file %s fail!"), szModuleFileName);
            MessageBox(NULL, strErr, _T("Error"), MB_OK);
            ...
            for(int i=0;i<length;i++)
			{
                ...
                if(pIXMLDOMNode)
                {
					...
					if(0==_tcscmp(s,_T("version")))
                    {
						...
						{
							CString strError;
							strError.Format(_T("Current NV tool just support %s module's version,But this module [%s] version is %s,Please upgrade nv tool."),
                    ...
                    if(0==_tcscmp(s,_T("name")))
                    {
						...
						if (NULL != _tcsstr(m_CurrModule.szModuleName, _T("Calibration")))
						{
							m_bCalibration = TRUE;
						}
                    }
                    if(0==_tcscmp(s,_T("description")))
                    {
						...
						if (NULL != _tcsstr(m_CurrModule.szModuleDescription, _T("static")))
						{
							m_bStaticFlag = TRUE;
						}
                    }           
...
void CFileParser::SetModuleItem( int                nModuleIndex, 
								...
								BOOL               fSetModified )
{
    ModuleItem_List_Ptr pCurrModuleItemList = m_lstModuleItemPtr.GetAt (nModuleIndex);    
    ModuleItem tmpItem = pCurrModuleItemList->GetAt (nItemIndex);
	
    memcpy(&tmpItem, &NewModuleItem, sizeof(ModuleItem));	
    pCurrModuleItemList->SetAt (nItemIndex, tmpItem);
...
int CFileParser::InsertModuleItem(INT nModuleIndex, ModuleItem InsertModuleItem,INT nInsertAfter,BOOL bInsertAfter,BOOL bUpdateTree/* = FALSE*/) //lint -e578 
{
	ASSERT((UINT)nModuleIndex < m_nModuleCount);
	...
	if (-1 == InsertModuleItem.nParentItemIndex)
	{
		...
		{
			CString strTmp;
			strTmp.Format(_T("ItemID %d has existed, please input a unexisted ID."), InsertModuleItem.nItemID);
			AfxMessageBox(strTmp);
			return bReturnCode;
		}
		else
	...
	if (InsertModuleItem.szItemContent[0] != 0 && InsertModuleItem.szItemDescription != NULL)
	{
							  ...
							  DWORD        cbSize         )
{
    ...
    if( NULL == fpTarget )
    {
		if(!theApp.m_bCmdMode)
		{
			//Notify that open file failed
			CString strErr;
			strErr.Format( _T("Can't open file %s!"), m_szTargetFileName );
			MessageBox(NULL, strErr, _T("Error"), MB_OK);
			// ]]
		}
		SAFE_DELETE_ARRAY(m_szItemContent);
...
#define HASH_LENGTH  40   
	...
	if (m_bWriteFix)
	{
		...
		{
			CString strErr;
			strErr.Format(_T("Map file %s failed"), m_szFixBin);
			AfxMessageBox(strErr);
			fclose(fpTarget);
			SAFE_DELETE_ARRAY(m_szItemContent);
			return FALSE;
		}
    	DWORD dwFileSize = map.GetSize();
		
		AnalyzeFixBin((const unsigned char*)pvFile, dwFileSize, mapIDs);
		
		DWORD dwFixLength = dwFileSize - HASH_LENGTH - 4;
		memcpy(m_szItemContent+nNVImageLength, (const unsigned char*)pvFile + HASH_LENGTH + 4, dwFixLength);  //merge fix bin to project bin
		nNVImageLength += dwFixLength;

		if (!m_strErrID.IsEmpty())
		{
			CString strErr;
			strErr.Format(_T("Existed repeated IDs: %s\n"), m_strErrID);
			AfxMessageBox(strErr);
			fclose(fpTarget);
			SAFE_DELETE_ARRAY(m_szItemContent);
			return FALSE;
    ...
    if ( m_bWriteDsp )   // ]]
    {
		if ( GetFileSize(szDspFileName) >= MAX_TOTAL_ITEM_CONTENT)
		{
			if(!theApp.m_bCmdMode)
			{
				MessageBox(NULL,_T("DSP Bin size can't over 128K!"),_T("Error"), MB_OK);
			}
			else
			{
        ...
        {
            //Notify that can not open target file
            MessageBox(NULL,_T("Can't open DSP Code file, Please check the file is exist!"),_T("Error"), MB_OK);
...
#define DSP_TOTAL_SIZE 0x20000
		...
		{
			MessageBox(NULL,_T("Wrong DSP Code file, Please check the file!"),_T("Error"), MB_OK);
            fclose(fpTarget); 
			SAFE_DELETE_ARRAY(m_szItemContent);
			return FALSE;
		}
        ...
        if( nPos == -1 )
        {
			...
			else
			{
				...
				MessageBox(NULL, _T("[ERROR] Wrong DSP Code file, DSP ITEM1 [0X04] length can't over 64K!"), _T("Error"), MB_OK);
				_tprintf(_T("[ERROR] Wrong DSP Code file, DSP ITEM1 [0X04] length can't over 64K!"));
				fclose(fpTarget); 
				SAFE_DELETE_ARRAY(m_szItemContent);
				return FALSE;
        ...
        else
        {
			...
			else
			{
				...
				{
					strErr = _T("[ERROR] Wrong DSP Code file, DSP ITEM2-1 [0X04] length can't over 64K!");
				}

				if ((nRead - (int)lCurPos) >= FILE_TOTAL_SIZE)
				{
					strErr += _T("[ERROR] Wrong DSP Code file, DSP ITEM2-2 [0X08] length can't over 64K!");
				}
				AfxMessageBox(strErr);
				fclose(fpTarget); 
				SAFE_DELETE_ARRAY(m_szItemContent);
	...
	if ( ( NULL != lpAppendage ) && ( cbSize > 0 ) )
	{
        if( cbSize <= 4 )
        {
            AfxMessageBox( _T("Scinv.bin is invalid( size too small )!"), MB_OK | MB_ICONSTOP );
            fclose(fpTarget); // add by Joly
			SAFE_DELETE_ARRAY(m_szItemContent);
			return FALSE;
        }
    ...
    for(UINT nCurModuleIndex = 0; nCurModuleIndex < m_nModuleCount; nCurModuleIndex++)
    {
        ...
        for(int nCurItemIndex = 0; nCurItemIndex < GetModuleItemCount(nCurModuleIndex); nCurItemIndex++)
        {
			...
			{
				CString str;
				str.Format(_T("[ERROR] [ItemName: %s]:item ID [%d] is not valid."),
    ...
    {               
        ...
        for(int nCurItemIndex = 0; nCurItemIndex < nModuleItemCount; nCurItemIndex++)
        {
			...
			if (m_CurrModuleItem.nItemLength > 0xFFFF || 0 == m_CurrModuleItem.nItemLength)
			{
				CString str;
				Module  stCurModule = m_lstModule.GetAt(nCurModuleIndex);
				str.Format(_T("[ERROR] [NVM: %s] [ItemName: %s]:nv item data length [%d] is invalid."),GetStrFileTitle(stCurModule.szModuleFileName),
					m_CurrModuleItem.szItemName, m_CurrModuleItem.nItemLength);							
				AfxMessageBox(str);
				return 0;
			}
    ...
    for(nItemIndex = nParentIndex + 1; nItemIndex < nTotalModuleItemCount; nItemIndex++)
    {
        ...
        else
        {
				...
				(WSTRING_ITEM_TYPE == tmpModuleItem.eItemType ))
            {
				...
				if(STRUCT_ITEM_TYPE == tmpModuleItem.eItemType && nMaxLen>1)
...
int CFileParser::InsertModule(TCHAR *szMOduleFileName,CStringArray& agWarning,BOOL bCheckItemName/* = TRUE*/)
{
...
VOID CFileParser::UpdatedNVChangeFlag()
{
	int nIndex			= 0;
	int nTotalNVItems	= m_lstTopItem.GetSize();
	ModuleItem stNVItem;     
	for( nIndex = 0; nIndex < nTotalNVItems ; ++nIndex )
    {
        stNVItem = m_lstTopItem.GetAt (nIndex);
		if ( !stNVItem.bChanged )
		{
			m_currModuleItemList = m_lstModuleItemPtr.GetAt (stNVItem.nParentItemIndex);
			int nCurr = stNVItem.nItemIndex;
			if ( LONG_ITEM_TYPE < stNVItem.eItemType ) 
			{
				++nCurr;
			}
			...
			for(; nCurr < m_currModuleItemList->GetSize (); ++nCurr)
			{
				...
				if(stNVItem.nItemID == m_CurrModuleItem.nItemID )
				{
					if (m_CurrModuleItem.bChanged)
					{
						stNVItem.bChanged = m_CurrModuleItem.bChanged;
						m_lstTopItem.SetAt (nIndex , stNVItem);
						break;
					}
				}			
			}						
        ...
        if(nItemID == hTopItem.nItemID )
        {
            if(nNumberOfBytesToWrite == hTopItem.nItemLength )
            {
                ...
                for(; nCurr < m_currModuleItemList->GetSize (); nCurr++)
                {
					...
					if(m_CurrModuleItem.bReadOnly && NONE_PERMISSION == theApp.GetPermission())
					{
						continue;
					}
...
int CFileParser::GetNVItemCount( BOOL bCalcChangeFlag/* = FALSE */ )
{
...
BOOL CFileParser::IsChangedNVItem(int nIndex ) const
{
	return m_lstTopItem.GetAt( nIndex ).bChanged;
}
void CFileParser::ClearNVLoadState()
{
	int nTotalModule = m_lstModule.GetSize();
	for (INT i = 0; i < nTotalModule; ++i)
	{
		Module  tmpModerm = m_lstModule.GetAt(i);
		tmpModerm.bLoadFailed = FALSE;
		m_lstModule.SetAt(i, tmpModerm);
		
		ModuleItem_List_Ptr pCurModuleItemList = m_lstModuleItemPtr.GetAt(i);
		int nTotalItem = pCurModuleItemList->GetSize();
		for (INT j = 0; j < nTotalItem; ++j)
		{
			ModuleItem stCurrModuleItem = pCurModuleItemList->GetAt(j);
			
			stCurrModuleItem.bLoadFailed = FALSE;
			pCurModuleItemList->SetAt(j, stCurrModuleItem);
		}
	}
									...
									BOOL bBinaryMode /* = FALSE */ )
{
    ...
    if( NULL == pFile )
    {
		...
		else
		{
			...
			// ask the user whether to change the attribute
			CString string;
			string.Format( _T("%s is read-only.\nChange its attribute?"), szFileName );
			int ret = ::AfxMessageBox( string,
				MB_YESNO );
			...
			if ( IDYES == ret )
			{
				...
				{
					TRACE( _T("CFileParser::OpenFileForWrite : \n")
						_T("\tChange %s read-only attribute fail\n"),
						szFileName );
				}
#endif // _DEBUG
			}
...
unsigned int CFileParser::StrToInt(CString& strContent)
{
...
bool CFileParser::CompareItem(LPCTSTR lpszOpt1, LPCTSTR lpszOpt2)
{
...
BOOL CFileParser::ImportSingleNVFile( CString strFile )	// used for command line mode only
{
	ConstructItemTree();
	...
	try
	{
		CStdioFile file( strFile.operator LPCTSTR(),CFile::modeRead | CFile::typeText );
		CString strLine;
		CString strError = _T("");
		int nLineNo = 0;
		CString strText;
		while( file.ReadString( strLine ) )
		{
			...
			int nValueLen = strLine.GetLength() - nLeftLen;
			_tcscpy(szValue,strLine.Mid(nLeftLen,nValueLen));
			...
			if( bRet )
			{
				...
				{
					CString strTmpValue = TrimLeftRight(szValue);
					LPCTSTR pValue = strTmpValue.operator LPCTSTR();
					int nLen = _tcslen(pValue);
					if( nLen<2 || (pValue[0] != _T('\"') && pValue[nLen-1] != _T('\"')) )
					{
						bHasError = TRUE;		
						strText.Format( _T( "%s's value is invalid format string : %s\n" ),szPath,szValue);
						strError += strText;
					}
					else
					{
						if (mi.nArraySize > nLen - 2)
						{
							for(int i=1 ; i<nLen-1; ++i )
							{
								...
								{	
									TCHAR ch = strTmpValue.Mid(i,1).operator LPCTSTR()[0];
									cChildItem.bChanged = GetContent(cChildItem.szItemContent) != ch ? TRUE : FALSE;
									ZeroMemory(cChildItem.szItemContent,sizeof(cChildItem.szItemContent));
							...
							if (nChildItemIndex < mi.nArraySize)
							{
								...
								cEndChildItem.bChanged = CompareItem(cEndChildItem.szItemContent,_T("0x0"));
								if(cEndChildItem.bChanged)
								{
									ZeroMemory(cEndChildItem.szItemContent,sizeof(cEndChildItem.szItemContent));
						...
						{
							bHasError = TRUE;	
							strText.Format( _T( "%s's value length is invalid [nArraySize %d <= %d ]\n" ),szPath,mi.nArraySize,nLen-2);
							strError += strText;
						}
					}	
					...
					WSTRING_ITEM_TYPE != mi.eItemType )
				{
					mi.bChanged = CompareItem( mi.szItemContent,szValue);
					_tcscpy( mi.szItemContent,szValue );
					SetModuleItem( nModuleIndex,nItemIndex,mi );					
				}
				else
		...
		if(!strError.IsEmpty())
		{
			...
			else
			{
				...
				{
					theApp.WriteConsole( g_hConsole,strError,_tcslen( strError ),&lWrite,NULL );
				}
			}
		}
	}
...
UINT CFileParser::ReadOtherFiles(UINT& nNVImageLength)
{
	...
	if ( !strMsg.IsEmpty() )
	{
		...
		else
		{
			...
			MessageBox( NULL, strErrorMsg, _T("Error"), MB_OK);
		}
		return 1;
...
void CFileParser::GetCalibrationNVID(CMapData*& mapCalibrationNv)
{
	mapCalibrationNv = &m_mapCalibrationData;
	
}

// for fix bin merge
void CFileParser::SetWriteFix(LPCTSTR szFixBin)
{
...
void CFileParser::SetHashHeaderPath(LPCTSTR szHash)
{
...
#define FILE_NAME_LENGTH 20
	...
	for (int j = 0; j < nCountFile; ++j)
	{
		...
		int nLen = WideCharToMultiByte(CP_ACP, 0, strName, strName.GetLength(), NULL, 0, NULL, NULL);
	//	int nLen = strName.GetLength();
		char *pFileName = new char[nLen + 1];
		memset(pFileName,0,nLen + 1);
	//	char pFileName[FILE_NAME_LENGTH];
		WideCharToMultiByte(CP_ACP, 0, strName, strName.GetLength(), pFileName, nLen, NULL, NULL);
		pFileName[nLen] = '\0';
	//	pFileName = _tcstok(pFileName, szToken);
		...
		*(SHORT*)(m_szNvContent + m_dwNVLength) = 0x01;
		...
		{			
			CString strTmp;
			strTmp.Format(_T("[Warning] Not find \"nv_ver_flag\" in %s file."),vecFileName[j]);
			dlgMsg.m_agItem.Add(strTmp);
			if (theApp.m_bCmdMode)
			{
				strTmp += _T("\n");
				_tprintf(strTmp.operator LPCTSTR());
			}
		}		
	}
	if(!theApp.m_bCmdMode && dlgMsg.m_agItem.GetSize())
	{
		dlgMsg.m_strTitle.Format(_T("There were %d warning, Are you sure to continue?"),dlgMsg.m_agItem.GetSize());
		if(IDOK != dlgMsg.DoModal())
		{
			ClearNVHeader();
			DestructItemTree();
			return FALSE;
...
// 		ReportError(strErr);
		AfxMessageBox(_T("Wrong Path or No valid NV File"));
		ClearNVHeader();
		DestructItemTree();
		return FALSE;
	}
	...
	{
		AfxMessageBox(_T("Generate deltanv index fail."));
		ClearNVHeader();
		DestructItemTree();
		return FALSE;
	}
...
BOOL CFileParser::MergeModerm(const TCHAR* strPath, TCHAR* strFileName)
{
	...
	if (0 == _tcslen(strFileName))
	{
		...
		{
    		CString strErr;
    		strErr.Format(_T("Can't open file %s, write bin failed!"), strPathFile);
    		AfxMessageBox(strErr);
    		return FALSE;
		}
	...
	{
		CString strErr;
		strErr.Format(_T("Can't open file %s, write bin failed!"), strFileName);
		AfxMessageBox(strErr);
		return FALSE;
	}
	...
	{
		AfxMessageBox(_T("Not Found DetlaNV Mark in Moderm Bin!"));
		free(buff);
		return FALSE;
	}
	...
	{
		AfxMessageBox(_T("Morderm Bin Not Enough Memory!"));
		return FALSE;
	}
...
void CFileParser::ReportError(CString& strError)
{
	if(!theApp.m_bCmdMode)
	{
		AfxMessageBox(strError);
...
std::string CFileParser::TString2Astring(LPCTSTR lpszString)
{
	USES_CONVERSION;
	std::string str(T2A(lpszString));
	return str;
}

CString CFileParser::TrimLeftRight(LPCTSTR lpString)
{
	...
	case ASTRING_ITEM_TYPE:
		{
			...
			{
				CString strError;
				strError.Format(_T("Invalid format string : %s "),szValue);
				ReportError(strError);
				return FALSE;
			}
			std::string strValue = TString2Astring(szValue);
...
BOOL CFileParser::ImportDeltaNV(CString strFile)
{
	BOOL bHasError = FALSE;
		...
		while( file.ReadString( strLine ) )
		{
			...
			if( bRet )
			{
					...
					WSTRING_ITEM_TYPE != mi.eItemType )
				{
					...
					{
						CString strTmpValue = TrimLeftRight(szValue);
						if (!SetNVBin(mi, (TCHAR *)(strTmpValue.operator LPCTSTR()),tDeltaItem))
						{
							CString strText;
