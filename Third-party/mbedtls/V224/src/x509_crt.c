...
cleanup:
...
#else
    {
        ...
        #define FILE_MAX_PATH 256
        	SFS_HANDLE ffs_handler=0;
    	uint16 path_ptr[FILE_MAX_PATH+2] = 0;
        	uint8 f_path_ptr[FILE_MAX_PATH+2] = 0;

            SFS_FIND_DATA_T file_info = {0};
             
        	SPLIBC_StrToWstr(path, path_ptr);

    	if(0!=(ffs_handler=SFS_FindFirstFile(path_ptr,&file_info)))
            {
                SPLIBC_WstrToStr(file_info.name, f_path_ptr);
                snp_ret = mbedtls_snprintf( entry_name, sizeof entry_name,
                                    "%s/%s", path, f_path_ptr);
                ...
                if( t_ret < 0 )
                    ...
                    while( SFS_NO_ERROR!=SFS_FindNextFile(ffs_handler,&file_info) )
                    {
