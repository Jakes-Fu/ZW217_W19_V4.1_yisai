...
#define PAHOCOAP_PRIORITY (231)
#define PAHOCOAP_STACK_SIZE (1024 * 16)
#define PAHOCOAP_EVENT_QUEUE_SIZE (32)

BLOCK_ID Thread_start(void (*entry)(uint32, void*), void *argv, BOOLEAN flag)
{
    //if(flag == 1)
        return SCI_CreateThread("[P_send_COAP]", "[Q_send_COAP]", entry, 1, argv, PAHOCOAP_STACK_SIZE, PAHOCOAP_EVENT_QUEUE_SIZE, 
                                                PAHOCOAP_PRIORITY, SCI_PREEMPT, SCI_AUTO_START);
    //else
