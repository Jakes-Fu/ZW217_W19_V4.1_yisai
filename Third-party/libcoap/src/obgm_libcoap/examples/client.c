...
#define exit(x) do{ SCI_TRACE_LOW("exit with %d\n",x);result = -1;goto exit;} while(0)
...
method_t g_coap_method = 1;                    /* the method we are using in our requests */
...
clear_obs(coap_context_t *ctx,
  ...
  if (!coap_add_option(pdu,
      ...
      buf)) {
    debug("cannot add option Observe: %u", COAP_OBSERVE_CANCEL);
    goto error;
  }
...
#define HANDLE_BLOCK1(Pdu)                                        \
  ((g_coap_method == COAP_REQUEST_PUT || g_coap_method == COAP_REQUEST_POST) && \
   ((flags & FLAGS_BLOCK) == 0) &&                                \
...
message_handler(struct coap_context_t *ctx,
  ...
  if (COAP_RESPONSE_CLASS(received->hdr->code) == 2) {
    ...
    if (sent && coap_check_option(received, COAP_OPTION_SUBSCRIPTION, &opt_iter)) {
      debug("  coapdebug observation relationship established, set timeout to %d\n", obs_seconds);
    ...
    if (coap_get_data(received, &len, &databuf)) {
        debug("  coapdebug output get data!!!\n");
        
        num= coap_opt_block_num(block_opt);
        debug(" coapdebug output get data!!!,num=%d\n",num);
        append_to_output(databuf, len, num);
    }

      debug( "  coapdebug    block_opt -value  :  %s    . [%s   %d] \n",block_opt,__FUNCTION__, __LINE__);
      if(COAP_OPT_BLOCK_MORE(block_opt)) {
        /* more bit is set */
        debug(" coapdebug found the M bit, block size is %u, block nr. %u\n",
              COAP_OPT_BLOCK_SZX(block_opt),
              coap_opt_block_num(block_opt));

        /* create pdu with request for next block */
        pdu = coap_new_request(ctx, g_coap_method, NULL, NULL, 0); /* first, create bare PDU w/o any option  */
        if ( pdu ) {
...
set_blocksize(void) {
  ...
  if (g_coap_method != COAP_REQUEST_DELETE) {
    opt = g_coap_method == COAP_REQUEST_GET ? COAP_OPTION_BLOCK2 : COAP_OPTION_BLOCK1;
...
get_context(const char *node, const char *port, int secure) {
  ...
  if ( s != 0 ) {
    debug("  coapdebug   s  is : %d.     DNS error     [%s  %d]  \n",s,__FUNCTION__,__LINE__);
  ...
  for (rp = result; rp != NULL; rp = rp->ai_next) {
    ...
    if (rp->ai_addrlen <= sizeof(addr.addr)) {
      ...
      if (endpoint) {
        ...
        debug("  coapdebug   :cannot create endpoint.   [%s  %d]  \n",__FUNCTION__,__LINE__);        
        continue;
      }
    }
  }
  debug("  coapdebug   no context available for interface : 0x%x [%s  %d]  \n",node,__FUNCTION__,__LINE__);
  coap_free_context(ctx);
  ctx = NULL;
...
static void coap_client_dinit()
{
    ...
    g_coap_method = 1;                    /* the method we are using in our requests */
...
coap_client_main(int argc, char **argv) {
  ...
  //while ((opt = getopt(argc, argv, "Na:b:e:f:g:k:m:p:s:t:v:A:B:O:P:T:u:")) != -1) {
    ...
    case 'k' :
      ...
      break;
    case 'm' :
      g_coap_method = cmdline_method(optarg);
      break;
    case 'p' :
  ...
  if (optind < argc) {
    if (cmdline_uri(argv[optind]) < 0)
      debug("invalid CoAP URI\n");
    if (coap_uri_scheme_is_secure(&uri) && !coap_dtls_is_supported()) {
      debug("coapdebug URI scheme not supported in this version of libcoap\n");
      exit(1);
    }
  } else {
    usage( argv[0], PACKAGE_VERSION );
  ...
  if (res < 0) {
    debug("  coapdebug failed to resolve address\n");
    exit(-1);
  }
  ...
  if (!ctx) {    
    debug("  coapdebug    :cannot create context  [%s  %d]  \n",__FUNCTION__,__LINE__);
    goto finish;
  }
  if ((user_length < 0) || (key_length < 0)) {
    debug("Invalid user name or key specified\n");
    goto finish;
  }
  ...
  if (! (pdu = coap_new_request(ctx, g_coap_method, &optlist, payload.s, payload.length))) {
    goto finish;
  }
  ...
  if (pdu->hdr->type != COAP_MESSAGE_CON || tid == COAP_INVALID_TID)
    coap_delete_pdu(pdu);
  set_timeout(&max_wait, wait_seconds);
  debug(" coapdebug timeout is set to %d seconds   max_wait : %d\n", wait_seconds,max_wait);
 ...
 finish:
  debug(" coap  client_main  1 result = %d\n  ", result);   
  close_output();
  coap_delete_list(optlist);
  coap_free_context( ctx );
 exit:
  coap_client_dinit();
  debug(" coap client_main result = %d\n  ", result);  
  return result;
}
