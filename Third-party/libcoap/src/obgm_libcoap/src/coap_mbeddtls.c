...
{
    int fd ;
    struct coap_dtls_context_t * ctx ;
    int nbSent;
    in_port_t port;
    size_t offset;
...
{
    coap_dtls_context_t *dtls_context;
    int fd;
    int numBytes;
    ...
    dtls_context = (coap_dtls_context_t *)ctx;
    fd = dtls_context->sock_id;
    if (fd < 0)
        return (MBEDTLS_ERR_NET_INVALID_CONTEXT);
...
int mbeddtls_net_handshake_recv(void *ctx, unsigned char *buffer, size_t len, uint32_t timeout)
{
    ...
    coap_log(LOG_WARNING, "mbeddtls_net_handshake_recv  timeout1  :  %d  ,timeout: %d:%d", timeout,tv.tv_sec, tv.tv_usec);
    ...
    coap_dtls_new_context(void)
{
    ...
    dtls_context = coap_malloc(sizeof(struct coap_dtls_context_t));
...
void coap_dtls_free_context(struct coap_dtls_context_t *dtls_context)
{
    ...
    struct coap_dtls_context_t *tmp ;
...
{
    struct coap_dtls_context_t *ctxP;
...
unsigned int COAP_avRandom()
{
    return (((unsigned int)rand() << 16) + rand());
}

int COAP_ssl_random(void *p_rng, unsigned char *output, size_t output_len)
{
    ...
    while (rnglen > 0) 
    {
        *(output + rngoffset) = (unsigned char)COAP_avRandom() ;
        rngoffset++;
        rnglen--;
    }
    return 0;
}
#define COAP_TLS_READ_TIMEOUT 30000
int coap_dtls_cfg_context(coap_context_t *ctx, coap_address_t *addr, void *timer)
{
    ...
    //mbedtls_entropy_init(&dtls_context->entropy);    //modify by xiameng.yang
    ...
    //mbedtls_net_set_block(&dtls_context->server_fd);   //MODIFY BY XIAMENG.YANG
    ...
    idlen = coap_psk_set_identity(psk, psk_id, MBEDTLS_PSK_MAX_LEN);
    ...
    keylen = coap_psk_set_key(psk, psk_key, MBEDTLS_PSK_MAX_LEN);
    ...
    * but makes interop easier in this simplified example */
    ...
    #else
          mbedtls_ssl_conf_authmode(&dtls_context->conf, MBEDTLS_SSL_VERIFY_OPTIONAL);
    #endif


    
    mbedtls_ssl_conf_ca_chain(&dtls_context->conf, &dtls_context->cacert, NULL);
    mbedtls_ssl_conf_rng(&dtls_context->conf, COAP_ssl_random, NULL);  //modify by xaimeng.yang
    ...
    mbedtls_ssl_conf_read_timeout(&dtls_context->conf, COAP_TLS_READ_TIMEOUT);  //add by xaimeng.yang
    mbedtls_ssl_set_bio(&dtls_context->ssl, dtls_context, mbeddtls_net_send, mbeddtls_net_recv, mbeddtls_net_handshake_recv);
    /* 4. Handshake*/

#if 1
    if ((ret = mbedtls_ssl_handshake(&dtls_context->ssl)) != 0)
    {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE)
        {
            coap_log(LOG_WARNING, " failed ! 1 mbedtls_ssl_handshake returned %d  \n", -ret);            
        }
        coap_log(LOG_WARNING, " failed ! 2 mbedtls_ssl_handshake returned %d  \n", -ret);
        return ret;
    }
#else
