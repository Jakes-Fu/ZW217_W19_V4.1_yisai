...
// TODO:# warning "inet_ntop() not available, network addresses will not be included in debug output"
# endif /* WITH_CONTIKI */
  return 0;
#endif
}
...
coap_show_pdu(const coap_pdu_t *pdu) {
  ...
  debug("  coapdebug v:%d t:%s c:%s i:%04x {",
  ...
  for (i = 0; i < pdu->hdr->token_length; i++) {
    coap_log(COAP_DEBUG_FD, "%02x", pdu->hdr->token[i]);
  }
  coap_log(COAP_DEBUG_FD, "}");
  ...
  while ((option = coap_option_next(&opt_iter))) {
    ...
    debug(" coapdebug %s:%.*s", msg_option_string(opt_iter.type),
	    (int)buf_len, buf);
  }
