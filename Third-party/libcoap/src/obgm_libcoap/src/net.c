...
coap_new_context(
  ...
  coap_prng_init((void *) ((unsigned long)listen_addr ^ (unsigned long)clock_offset) ); // TODO:
  ...
  c->dtls_context = coap_dtls_new_context();
...
#if(defined HAVE_LIBTINYDTLS) || (defined WITH_MBEDDTLS)
...
coap_send_impl(coap_context_t *context, 
  ...
  if (is_dtls(local_interface)) {
    ...
    if (session == NULL) {
      ...
      coap_dtls_set_context(context, dst);
#endif
      coap_log(LOG_WARNING, "coap_send: goto coap_dtls_send\n");
      bytes_written = coap_dtls_send(context, pdu);
...
coap_send_confirmed(coap_context_t *context, 
  ...
  if (COAP_INVALID_TID == node->id) {
    debug(" coapdebug coap_send_confirmed: error sending pdu\n");
    coap_free_node(node);
    return COAP_INVALID_TID;
  }
...
coap_read_endpoint(coap_context_t *ctx, coap_endpoint_t *endpoint) {
  ...
  if (is_dtls(endpoint)) {
  result = coap_dtls_handle_message(ctx,endpoint,buffer,MAX_PACKET_SIZE);
  } else {
  bytes_read = ctx->network_read(endpoint, &packet);
  //debug( "  coapdebug  packet->src : %d      [%s   %d] \n",packet->src.addr.sa.sa_family,__FUNCTION__, __LINE__);
  ...
  if (bytes_read < 0) {
    ...
    if (bytes_read > 0) {
      ...
      //if (is_dtls(endpoint)) {
       //debug("go to coap_dtls_handle_message\n" );
       //result = coap_dtls_handle_message(ctx, endpoint, &remote, data, data_len);
      //} else {
      debug("go to   coapdebug coap_handle_message\n" );
      //debug( "  coapdebug    rcvd->remote family:  %d . [%s   %d] \n",remote.addr.sa.sa_family,__FUNCTION__, __LINE__);
...
#endif
  //debug( "  coapdebug    rcvd->remote family:  %d . [%s   %d] \n",node->remote.addr.sa.sa_family,__FUNCTION__, __LINE__);
...
handle_request(coap_context_t *context, coap_queue_t *node) {      
  ...
  if (!resource) {
	...
	info(" coapdebug create default response for %s\n", COAP_DEFAULT_URI_WELLKNOWN);
...
handle_response(coap_context_t *context, 
   ...
   * same token.
   */
   debug( "  coapdebug    rcvd->remote family:  %d . [%s   %d] \n",rcvd->remote.addr.sa.sa_family,__FUNCTION__, __LINE__);
...
coap_dispatch(coap_context_t *context, coap_queue_t *rcvd) {
...

  {
	...
	goto cleanup;
	 ...
	 str token ={0};  //= 	  { sent->pdu->hdr->token_length, sent->pdu->hdr->token };
        token.length = sent->pdu->hdr->token_length;
        token.s =  sent->pdu->hdr->token;
...
debug("  coapdebug   bool:%d   [%s  %d]  \n",( !context || (context->sendqueue == NULL)),__FUNCTION__,__LINE__);
