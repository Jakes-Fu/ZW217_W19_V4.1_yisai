...
/*  assert(optp); assert(*optp); 
  assert(length);*/
  if(optp == NULL ||length ==NULL)
  {
      debug( "  coapdebug    next_option_safe error   [%s   %d] \n",__FUNCTION__, __LINE__);
      return 0;
  }
...
/*  assert(data);
  ...
  //debug( "  coapdebug    length :%d    sizeof(coap_hdr_t) : %d. [%s   %d] \n",length,sizeof(coap_hdr_t),__FUNCTION__, __LINE__);
