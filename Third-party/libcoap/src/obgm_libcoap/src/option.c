...
#define ADVANCE_OPT(o,e,step) if ((e) < step) {			\
  ...
  //debug("  coapdebug     result->delta : %d  . [%s   %d] \n",result->delta,__FUNCTION__, __LINE__);
  //debug("  coapdebug     result->length : %d  . [%s   %d] \n",result->length,__FUNCTION__, __LINE__);

if(0)//&result->value != NULL)
{
    debug("  coapdebug     result->value : %s  . [%s   %d] \n",result->value,__FUNCTION__, __LINE__);
}    
