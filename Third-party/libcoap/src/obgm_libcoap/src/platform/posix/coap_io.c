...
coap_new_endpoint(const coap_address_t *addr, int flags) {
  ...
  if (!ep) {
    debug("coap_new_endpoint: malloc");
    goto error;
  }
  ...
  if (getsockname(sockfd, &ep->addr.addr.sa, &ep->addr.size) < 0) {
    debug( "coap_new_endpoint: cannot determine local address");
    goto error;
  }
...
/*  assert(local_interface);*/
  ...
  default:
    /* error */
    debug(" coapdebug  protocol not supported\n");
    return -1;
  }

  //zhangyi del for debug 20180906
  //return sendmsg(ep->handle.fd, &mhdr, 0);
  return sendto(ep->handle.fd, data, datalen, 0, (struct sockaddr *)&(dst->addr.sin), dst->size);
}
...
coap_network_read(coap_endpoint_t *ep, coap_packet_t **packet) {
  ...
  char msg_control[ (sizeof(struct sockaddr_storage))];
  struct msghdr mhdr;
  struct iovec iov[1];
...
len = recvfrom(ep->handle.fd,mhdr.msg_iov[0].iov_base,mhdr.msg_iov[0].iov_len,0,&(*packet)->src.addr.sa,NULL);
...
/*	assert(memcmp(&(*packet)->src.addr.st, mhdr.msg_name, (*packet)->src.size) == 0);*/
    ...
    if (!is_local_if(&ep->addr, &(*packet)->dst)) {
      debug(" coapdebug packet received on wrong interface, dropped\n");
      goto error;
    }
  }
