...
#elif defined(MBEDTLS) /* if defined(OPENSSL) */
#include "mbedtls/base64.h"
b64_size_t Base64_encode( char *out, b64_size_t out_len, const b64_data_t *in, b64_size_t in_len )
{
    b64_size_t olen = 0;
    b64_size_t ret = -1;
	if(0 == (ret = mbedtls_base64_encode((unsigned char *) out, out_len, &olen,in,in_len)))
        return olen;
    else
        return ret;
}
...
b64_size_t Base64_decode( b64_data_t *out, b64_size_t out_len, const char *in, b64_size_t in_len )
{
    b64_size_t olen = 0;
    b64_size_t ret = -1;
	if(0 == (ret = mbedtls_base64_decode(out, out_len, &olen,(unsigned char *)in,in_len)))
        return olen;
    else
        return ret;
}
