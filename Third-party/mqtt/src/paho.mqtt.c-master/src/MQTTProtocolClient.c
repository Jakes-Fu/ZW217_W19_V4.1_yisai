...
int MQTTProtocol_handlePublishes(void* pack, int sock)
{
	...
	Log(LOG_PROTOCOL, 11, " sock : %d ,clientID : %s  ,msgId : %d , qos :%d , retained :%d ,rc : %d , min_payload : %d , payload: %d ", sock, clientid, publish->msgId, publish->header.bits.qos,
...
int MQTTProtocol_handlePubacks(void* pack, int sock)
{
	...
	FUNC_ENTRY;
	client = (Clients*)(ListFindItem(bstate->clients, &sock, clientSocketCompare)->content);
	Log(LOG_PROTOCOL, 14, "sock  : %d  ,client->clientID : %s ,msgId : %d ", sock, client->clientID, puback->msgId);
	...
	if (ListFindItem(client->outboundMsgs, &(puback->msgId), messageIDCompare) == NULL)
		Log(TRACE_MIN, 3, "PUBACK  client->clientID : %s , msgId : %d", client->clientID, puback->msgId);
	else
	{
		Messages* m = (Messages*)(client->outboundMsgs->current->content);
		if (m->qos != 1)
			Log(TRACE_MIN, 4, " PUBACK client->clientID : %s , msgId : %d ,qos :%d  ", client->clientID, puback->msgId, m->qos);
		else
		{
			Log(TRACE_MIN, 6, "PUBACK  client->clientID : %s ,msgId : %d", client->clientID, puback->msgId);
			#if !defined(NO_PERSISTENCE)
...
int MQTTProtocol_handlePubrecs(void* pack, int sock)
{
	...
	FUNC_ENTRY;
	client = (Clients*)(ListFindItem(bstate->clients, &sock, clientSocketCompare)->content);
	Log(LOG_PROTOCOL, 15, "sock :  %d  ,clientID : %s ,  msgId :%d", sock, client->clientID, pubrec->msgId);
	...
	if (ListFindItem(client->outboundMsgs, &(pubrec->msgId), messageIDCompare) == NULL)
	{
		if (pubrec->header.bits.dup == 0)
			Log(TRACE_MIN, 3, "PUBREC  clientID : %s ,  msgId :%d", client->clientID, pubrec->msgId);
	}
	else
	{
		Messages* m = (Messages*)(client->outboundMsgs->current->content);
		...
		{
			if (pubrec->header.bits.dup == 0)
				Log(TRACE_MIN, 4, "PUBREC  clientID : %s ,  msgId :%d  , qos : %d", client->clientID, pubrec->msgId, m->qos);
		}
		else if (m->nextMessageType != PUBREC)
		{
			if (pubrec->header.bits.dup == 0)
				Log(TRACE_MIN, 5, "PUBREC  clientID : %s ,  msgId :%d ", client->clientID, pubrec->msgId);
		}
		else
		{
...
int MQTTProtocol_handlePubrels(void* pack, int sock)
{
	...
	FUNC_ENTRY;
	client = (Clients*)(ListFindItem(bstate->clients, &sock, clientSocketCompare)->content);
	Log(LOG_PROTOCOL, 17, "sock: %d ,clientID :%s  , msgId :%d", sock, client->clientID, pubrel->msgId);
	...
	if (ListFindItem(client->inboundMsgs, &(pubrel->msgId), messageIDCompare) == NULL)
	{
		if (pubrel->header.bits.dup == 0)
			Log(TRACE_MIN, 3, "PUBREL  ,clientID :%s  , msgId :%d", client->clientID, pubrel->msgId);
		else if (!Socket_noPendingWrites(sock))
			rc = SOCKET_ERROR; /* queue acks? */
		else
	...
	else
	{
		...
		if (m->qos != 2)
			Log(TRACE_MIN, 4, "PUBREL  ,clientID :%s  , msgId :%d , qos : %d", client->clientID, pubrel->msgId, m->qos);
		else if (m->nextMessageType != PUBREL)
			Log(TRACE_MIN, 5, "PUBREL ,clientID :%s  , msgId :%d  ", client->clientID, pubrel->msgId);
		else if (!Socket_noPendingWrites(sock))
		  rc = SOCKET_ERROR; /* queue acks? */
		else
		{
...
int MQTTProtocol_handlePubcomps(void* pack, int sock)
{
	...
	FUNC_ENTRY;
	client = (Clients*)(ListFindItem(bstate->clients, &sock, clientSocketCompare)->content);
	Log(LOG_PROTOCOL, 19, " sock : %d ,clientID :%s  , msgId :%d ", sock, client->clientID, pubcomp->msgId);

	/* look for the message by message id in the records of outbound messages for this client */
	if (ListFindItem(client->outboundMsgs, &(pubcomp->msgId), messageIDCompare) == NULL)
	{
		if (pubcomp->header.bits.dup == 0)
			Log(TRACE_MIN, 3, "PUBCOMP   ,clientID :%s  , msgId :%d ", client->clientID, pubcomp->msgId);
	}
	else
	{
		Messages* m = (Messages*)(client->outboundMsgs->current->content);
		if (m->qos != 2)
			Log(TRACE_MIN, 4, "PUBCOMP  ,clientID :%s  , msgId :%d   ,qos :%d", client->clientID, pubcomp->msgId, m->qos);
		else
		{
			if (m->nextMessageType != PUBCOMP)
				Log(TRACE_MIN, 5, "PUBCOMP  clientID :%s  , msgId :%d  ", client->clientID, pubcomp->msgId);
			else
			{
				Log(TRACE_MIN, 6, "PUBCOMP  clientID :%s  , msgId :%d  ", client->clientID, pubcomp->msgId);
				#if !defined(NO_PERSISTENCE)
					rc = MQTTPersistence_remove(client,
							(m->MQTTVersion >= MQTTVERSION_5) ? PERSISTENCE_V5_PUBLISH_SENT : PERSISTENCE_PUBLISH_SENT,
							m->qos, pubcomp->msgId);
		   ...
		   Socket_noPendingWrites(client->net.socket)) /* there aren't any previous packets still stacked up on the socket */
	{
		...
		if (regardless || difftime(now, m->lastTouch) > max(client->retryInterval, 10))
		{
			if (m->qos == 1 || (m->qos == 2 && m->nextMessageType == PUBREC))
			{
				...
				Log(TRACE_MIN, 7, "PUBLISH  clientID :%s  , socket : %d , msgId :%d  ", client->clientID, client->net.socket, m->msgid);
				publish.msgId = m->msgid;
				publish.topic = m->publish->topic;
				publish.payload = m->publish->payload;
				publish.payloadlen = m->publish->payloadlen;
				...
				{
					client->good = 0;
					Log(TRACE_PROTOCOL, 29, " clientID :%s  , socket : %d  , Socket_getpeer : %s ", client->clientID, client->net.socket,
			...
			{
				Log(TRACE_MIN, 7, "PUBREL   clientID :%s  , socket : %d , msgId :%d ", client->clientID, client->net.socket, m->msgid);
