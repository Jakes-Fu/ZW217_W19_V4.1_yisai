		...
		int MQTTVersion)
{
	...
	if ( !buf ) return -1;
	buf[0] = header.byte;//buf[ws_header] = header.byte;
	buf0len = 1 + MQTTPacket_encode(&buf[0 + 1], buflen);//buf0len = 1 + MQTTPacket_encode(&buf[ws_header + 1], buflen);
...
#endif
	rc = WebSocket_putdatas(net, buf, buf0len, count, &buffer, &buflen, &freeData);//rc = WebSocket_putdatas(net, &buf[ws_header], buf0len, count, &buffer, &buflen, &freeData);
		...
		int* frees, int MQTTVersion)
{
	...
	buf[0] = header.byte;//buf[ws_header] = header.byte;
	buf0len = 1 + MQTTPacket_encode(&buf[0 + 1], total);//buf0len = 1 + MQTTPacket_encode(&buf[ws_header + 1], total);
#if !defined(NO_PERSISTENCE)
...
#endif
	rc = WebSocket_putdatas(net, buf, buf0len, count, buffers, buflens, frees);//rc = WebSocket_putdatas(net, &buf[ws_header], buf0len, count, buffers, buflens, frees);
...
int MQTTPacket_send_disconnect(Clients* client, enum MQTTReasonCodes reason, MQTTProperties* props)
{
	...
	Log(LOG_PROTOCOL, 28, " client->net.socket  : %d ,client->clientID : %s ,rc : %d ", client->net.socket, client->clientID, rc);
	FUNC_EXIT_RC(rc);
	return rc;
}
...
int MQTTPacket_send_puback(int msgid, networkHandles* net, const char* clientID)
{
	...
	Log(LOG_PROTOCOL, 12, " net->socket : %d ,clientID : %s  ,msgid :%d ,rc : %d  ", net->socket, clientID, msgid, rc);
	FUNC_EXIT_RC(rc);
	return rc;
}
...
int MQTTPacket_send_pubrec(int msgid, networkHandles* net, const char* clientID)
{
	...
	Log(LOG_PROTOCOL, 13,  " net->socket : %d ,clientID : %s  ,msgid :%d ,rc : %d  ", net->socket, clientID, msgid, rc);
	FUNC_EXIT_RC(rc);
	return rc;
}
...
int MQTTPacket_send_pubrel(int msgid, int dup, networkHandles* net, const char* clientID)
{
	...
	Log(LOG_PROTOCOL, 16,  " net->socket : %d ,clientID : %s  ,msgid :%d ,rc : %d  ", net->socket, clientID, msgid, rc);
	FUNC_EXIT_RC(rc);
	return rc;
}
...
int MQTTPacket_send_pubcomp(int msgid, networkHandles* net, const char* clientID)
{
	...
	Log(LOG_PROTOCOL, 18,  " net->socket : %d ,clientID : %s  ,msgid :%d ,rc : %d  ", net->socket, clientID, msgid, rc);
	FUNC_EXIT_RC(rc);
	return rc;
}
...
int MQTTPacket_send_publish(Publish* pack, int dup, int qos, int retained, networkHandles* net, const char* clientID)
{
	...
	if (qos > 0 || pack->MQTTVersion >= 5)
	{
		...
		int frees[4] = {1, 0, 1, 0};
                  ...
                  lens[0]= 2;
                  lens[1]=strlen(pack->topic);
                  lens[2]=buflen;
		lens[3] = pack->payloadlen;
		if (qos > 0)
	...
	if (rc != TCPSOCKET_INTERRUPTED)
		free(topiclen);
	if (qos == 0)
		Log(LOG_PROTOCOL, 27,  " net->socket : %d ,clientID : %s  ,retained :%d ,rc : %d  ", net->socket, clientID, retained, rc);
	else
		Log(LOG_PROTOCOL, 10, " net->socket : %d ,clientID : %s  ,msgId : %d , qos :%d , retained :%d ,rc : %d , min_payload : %d , payload: %d ", net->socket, clientID, pack->msgId, qos, retained, rc,
