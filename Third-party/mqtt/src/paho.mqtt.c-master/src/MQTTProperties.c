  ...
  if (props->array)
  {
    ...
    switch (type)
    {
      ...
      case MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR:
        len = 2 + prop->value.LenString.data.len;
        props->array[props->count-1].value.LenString.data.data = datadup(&prop->value.LenString.data);
        if (type == MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
        {
          len += 2 + prop->value.LenString.value.len;
          props->array[props->count-1].value.LenString.value.data = datadup(&prop->value.LenString.value);
        }
        break;
    }
...
int MQTTProperty_write(char** pptr, MQTTProperty* prop)
{
  ...
  if (type >= MQTTPROPERTY_TYPE_BYTE && type <= MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
  {
    ...
    switch (type)
    {
      ...
      case MQTTPROPERTY_TYPE_BINARY_DATA:
      case MQTTPROPERTY_TYPE_UTF_8_ENCODED_STRING:
        writeMQTTLenString(pptr, prop->value.LenString.data);
        rc = prop->value.LenString.data.len + 2; /* include length field */
        break;
      case MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR:
        writeMQTTLenString(pptr, prop->value.LenString.data);
        writeMQTTLenString(pptr, prop->value.LenString.value);
        rc = prop->value.LenString.data.len + prop->value.LenString.value.len + 4; /* include length fields */
        break;
    }
  }
...
int MQTTProperty_read(MQTTProperty* prop, char** pptr, char* enddata)
{
  ...
  if (type >= MQTTPROPERTY_TYPE_BYTE && type <= MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
  {
    switch (type)
    {
      ...
      case MQTTPROPERTY_TYPE_UTF_8_ENCODED_STRING:
      case MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR:
        len = MQTTLenStringRead(&prop->value.LenString.data, pptr, enddata);
        prop->value.LenString.data.data = datadup(&prop->value.LenString.data);
        if (type == MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
        {
          len += MQTTLenStringRead(&prop->value.LenString.value, pptr, enddata);
          prop->value.LenString.value.data = datadup(&prop->value.LenString.value);
        }
        break;
    }
  }
...
DLLExport void MQTTProperties_free(MQTTProperties* props)
{
  ...
  for (i = 0; i < props->count; ++i)
  {
    ...
    case MQTTPROPERTY_TYPE_BINARY_DATA:
    	case MQTTPROPERTY_TYPE_UTF_8_ENCODED_STRING:
    	case MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR:
    	  free(props->array[i].value.LenString.data.data);
    	  if (type == MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
    	    free(props->array[i].value.LenString.value.data);
	  break;
    }
  }
  if (props->array)
