...
#endif
{
	...
	{	/* TCP connect completed. If SSL, send SSL connect */
		...
		if (ssl)
		{
			if (SSLSocket_setSocketForSSL(&aClient->net, aClient->sslopts, ip_address, addr_len) == 0)
			{
...
int MQTTProtocol_handlePingresps(void* pack, int sock)
{
	...
	Log(LOG_PROTOCOL, 21, " sock : %d , clientID :%s ", sock, client->clientID);
...
int MQTTProtocol_handleSubacks(void* pack, int sock)
{
	...
	FUNC_ENTRY;
	client = (Clients*)(ListFindItem(bstate->clients, &sock, clientSocketCompare)->content);
	Log(LOG_PROTOCOL, 23, "sock : %d ,clientID :%s  , msgId : %d", sock, client->clientID, suback->msgId);
...
int MQTTProtocol_handleUnsubacks(void* pack, int sock)
{
	...
	FUNC_ENTRY;
	client = (Clients*)(ListFindItem(bstate->clients, &sock, clientSocketCompare)->content);
	Log(LOG_PROTOCOL, 24, " sock : %d ,clientID :%s ,  msgId : %d", sock, client->clientID, unsuback->msgId);
