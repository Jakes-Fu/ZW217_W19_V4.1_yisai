...
static BLOCK_ID sendThread_id = 0;
static BLOCK_ID receiveThread_id = 0;

static SCI_MUTEX_PTR mqttasync_mutex = NULL;
static SCI_MUTEX_PTR socket_mutex = NULL;
static SCI_MUTEX_PTR mqttcommand_mutex = NULL;
static SCI_SEMAPHORE_PTR send_cond = NULL;

#ifdef _UNISOC_
static SCI_MUTEX_PTR g_pstMqttAsyncInitCountMutex = NULL;
static int g_iMqttAsyncInitCount = 0;
void MQTTAsync_init(void)
{
    if (NULL == g_pstMqttAsyncInitCountMutex)
    {
        g_pstMqttAsyncInitCountMutex = Thread_create_mutex("g_pstMqttAsyncInitCountMutex");
        SCI_TRACE_LOW("PahoMqtt: MQTTAsync_init() Create mutex of g_iMqttAsyncInitCount(resident memory,don't free). g_pstMqttAsyncInitCountMutex is 0x%p", g_pstMqttAsyncInitCountMutex);
    }
    Thread_lock_mutex(g_pstMqttAsyncInitCountMutex);
    g_iMqttAsyncInitCount++;
    SCI_TRACE_LOW("PahoMqtt: g_iMqttAsyncInitCount++, g_iMqttAsyncInitCount=%d", g_iMqttAsyncInitCount);
    Thread_unlock_mutex(g_pstMqttAsyncInitCountMutex);
    if (NULL == mqttasync_mutex)
        mqttasync_mutex = Thread_create_mutex("mqttasync_mutex");
    if (NULL == mqttcommand_mutex)
        mqttcommand_mutex = Thread_create_mutex("mqttcommand_mutex");
    if (NULL == socket_mutex)
        socket_mutex = Thread_create_mutex("socket_mutex");
    if (NULL == send_cond)
        send_cond = Thread_create_cond("send_cond");
    return ;
}

//Note:
///////First:  MQTTAsync_init()
///////second: MQTTAsync_uninit()
///////else : g_pstMqttAsyncInitCountMutex is NULL, crash
void MQTTAsync_uninit(void)
{
    Thread_lock_mutex(g_pstMqttAsyncInitCountMutex);
    g_iMqttAsyncInitCount--;
    SCI_TRACE_LOW("PahoMqtt: g_iMqttAsyncInitCount--, g_iMqttAsyncInitCount=%d", g_iMqttAsyncInitCount);
    if (0 == g_iMqttAsyncInitCount)
    {
        Thread_destroy_mutex(mqttasync_mutex);
        mqttasync_mutex = NULL;
        Thread_destroy_mutex(mqttcommand_mutex);
        mqttcommand_mutex = NULL;
...
static void MQTTAsync_lock_mutex(SCI_MUTEX_PTR amutex);
static void MQTTAsync_unlock_mutex(SCI_MUTEX_PTR amutex);
...
static int MQTTAsync_unpersistCommand(MQTTAsync_queuedCommand* qcmd)
{
	...
	FUNC_ENTRY;
	SCI_TRACE_LOW(key, "%s%u", PERSISTENCE_COMMAND_KEY, qcmd->seqno);
...
static int MQTTAsync_persistCommand(MQTTAsync_queuedCommand* qcmd)
{
	...
	if (aclient->c->MQTTVersion >= MQTTVERSION_5 && process) 	/* persist properties */
	{
		...
		SCI_TRACE_LOW(key, "%s%u", PERSISTENCE_V5_COMMAND_KEY, ++aclient->command_seqno);
	}
	else
		SCI_TRACE_LOW(key, "%s%u", PERSISTENCE_COMMAND_KEY, ++aclient->command_seqno);
...
void MQTTAsync_destroy(MQTTAsync* handle)
{
	...
	if (m->c)
	{
		...
		if (!ListRemove(bstate->clients, m->c))
			Log(LOG_ERROR, 0, " ListRemove ERROR");
		else
			Log(TRACE_MIN, 1, "saved_clientid : %s   ,saved_socket: %d", saved_clientid, saved_socket);
		free(saved_clientid);
	}
...
static int MQTTAsync_completeConnection(MQTTAsyncs* m, Connack* connack)
{
	...
	{
		Log(LOG_PROTOCOL, 1, "socket:%d, clientID:%s, rc:%c",m->c->net.socket, m->c->clientID, connack->rc);
...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
	...
	if (sendThread_state != STARTING && sendThread_state != RUNNING)
	{
		...
		Thread_start(MQTTAsync_sendThread, NULL,1);
		MQTTAsync_unlock_mutex(mqttasync_mutex);
	}
	if (receiveThread_state != STARTING && receiveThread_state != RUNNING)
	{
		...
		Thread_start(MQTTAsync_receiveThread, handle,0);
		MQTTAsync_unlock_mutex(mqttasync_mutex);
	}
...
static MQTTPacket* MQTTAsync_cycle(int* sock, unsigned long timeout, int* rc)
{
	...
	if ((*sock = SSLSocket_getPendingRead()) == -1)
	{
		...
		*sock = Socket_getReadySocket(0, &tp,(mutex_type)socket_mutex);
...
const char* MQTTAsync_strerror(int code)
{
  ...
  SCI_TRACE_LOW(buf, "Unknown error code %d", code);
  return buf;
}
