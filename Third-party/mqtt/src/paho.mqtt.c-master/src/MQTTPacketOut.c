	               ...
	               MQTTProperties* connectProperties, MQTTProperties* willProperties)
{
	...
	Log(LOG_PROTOCOL, 0, "client->net.socket : %d , client->clientID : %s  , client->cleansession : %d ,rc : %d", client->net.socket, client->clientID, client->cleansession, rc);
exit:
...
int MQTTPacket_send_pingreq(networkHandles* net, const char* clientID)
{
	...
	Log(LOG_PROTOCOL, 20, " net->socket : %d , clientID : %s , rc : %d ", net->socket, clientID, rc);
	FUNC_EXIT_RC(rc);
	return rc;
}
		...
		int msgid, int dup, Clients* client)
{
	...
	Log(LOG_PROTOCOL, 22, " client->net.socket  : %d ,client->clientID : %s , msgid : %d ,rc : %d ", client->net.socket, client->clientID, msgid, rc);
	if (rc != TCPSOCKET_INTERRUPTED)
		free(data);
	FUNC_EXIT_RC(rc);
	return rc;
...
int MQTTPacket_send_unsubscribe(List* topics, MQTTProperties* props, int msgid, int dup, Clients* client)
{
	...
	while (ListNextElement(topics, &elem))
		writeUTF(&ptr, (char*)(elem->content));
	rc = MQTTPacket_send(&client->net, header, data, datalen, 1, client->MQTTVersion);
	Log(LOG_PROTOCOL, 25,  " client->net->socket : %d ,clientID : %s  ,msgid :%d ,rc : %d  ", client->net.socket, client->clientID, msgid, rc);
	if (rc != TCPSOCKET_INTERRUPTED)
		free(data);
	FUNC_EXIT_RC(rc);
	return rc;
