...
static void atest_connect(void)
{
    ...
    rc = os_getaddrinfo("121.89.166.244", "6588", &addr_info, &addr_list);
    if (rc != 0)
    {
    ...
    {
        fd = os_socket(addr->ai_family, addr->ai_socktype, addr->ai_protocol);
        if (fd < 0)
        {
            continue;
        }
        if ((rc = os_connect(fd, addr->ai_addr, addr->ai_addrlen)) == 0)
        {
            break;
        }
        os_closesocket(fd);
...
static void atest_send(void)
{
    ...
    os_setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, (char *)&interval, sizeof(struct timeval));
    rc = os_send(fd, send_buf, BUFF_LENGTH, 0);
    if (!rc)
    {
        atest_disconnect();
...
static void atest_recv(void)
{
    ...
    rc = os_select(socket_fd + 1, &read_fds, NULL, NULL, &interval);
    if (!rc)
    {
        printf("time out\r\n");
        return;
    ...
    rc = os_recv(socket_fd, recv_buf, BUFF_LENGTH, 0);
    if (rc <= 0)
    {
        if (rc)
            printf("recv error(%d)!\r\n", rc);
...
static void atest_connect(void)
{
    ...
    rc = os_getaddrinfo("121.89.166.244", "6589", &addr_info, &addr_list);
    if (rc != 0)
    {
        printf("dns resolution failed\r\n");
        return;
...
static void atest_send(void)
{
    ...
    os_setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, (char *)&interval, sizeof(struct timeval));
    rc = os_sendto(fd, send_buf, BUFF_LENGTH, 0, &to_addr, sizeof(struct sockaddr));
    printf("udp send %d\r\n", rc);
}
...
static void atest_recv(void)
{
    ...
    rc               = os_recvfrom(socket_fd, recv_buf, BUFF_LENGTH, 0, &to_addr, &fromlen);
    printf("os_recv(%d):%s\r\n", rc, recv_buf);
    return;
}
