                             ...
                             uint8_t ** bufferP)
{
    ...
    case LWM2M_TYPE_FLOAT:
    {
        ...
        res = utils_floatToText_ex(dataP->value.asFloat, floatString, _PRV_STR_LENGTH * 2);
        if (res == 0) return -1;
                         ...
                         uint8_t ** bufferP)
{
    ...
    if (dataP->type == LWM2M_TYPE_OPAQUE && uriP->objectId == 19)
        *formatP = LWM2M_CONTENT_OPAQUE;
...
int lwm2m_data_serialize_onenet(lwm2m_uri_t * uriP,
                      int size,
                      lwm2m_data_t * dataP,
                      lwm2m_media_type_t * formatP,
                      uint8_t ** bufferP)
