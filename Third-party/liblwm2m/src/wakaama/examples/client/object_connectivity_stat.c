...
void conn_s_updateTxStatistic(lwm2m_context_t * lwm2mH, uint16_t txDataByte, bool smsBased)
{
    conn_s_data_t* myData;
    lwm2m_object_t *objectP = (lwm2m_object_t *)LWM2M_LIST_FIND(lwm2mH->objectList, LWM2M_CONN_STATS_OBJECT_ID);
    if (objectP == NULL)
        return;
...
void conn_s_updateRxStatistic(lwm2m_context_t * lwm2mH, uint16_t rxDataByte, bool smsBased)
{
    conn_s_data_t* myData;
    lwm2m_object_t *objectP = (lwm2m_object_t *)LWM2M_LIST_FIND(lwm2mH->objectList, LWM2M_CONN_STATS_OBJECT_ID);
    if (objectP == NULL)
...
static void free_object_conn_s(lwm2m_object_t * objectP)
{
...
lwm2m_object_t * get_object_conn_s(void* lwm2mH)
{
    ...
    if (NULL != connObj)
    {
        ...
        connObj->cleanFunc    = free_object_conn_s;
