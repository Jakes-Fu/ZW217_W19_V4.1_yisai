...
#ifdef CONFIG_LWM2M_ONENET_SUPPORT
static uint8_t prv_conn_m_discover(uint16_t instanceId,
                               int * numDataP,
                               lwm2m_data_t ** dataArrayP,
                               lwm2m_object_t * objectP)
{
...
static void free_object_conn_m(lwm2m_object_t * objectP)
{
...
lwm2m_object_t * get_object_conn_m(void* lwm2mH)
{
    ...
    if (NULL != connObj)
    {
        ...
        if(((lwm2m_context_t *)lwm2mH)->serverType == LWM2M_SERVER_ONENET)
            connObj->discoverFunc = prv_conn_m_discover;
#endif
        connObj->executeFunc = NULL;
        connObj->cleanFunc = free_object_conn_m;
