...
object_table_t internal_obj_table[] = {
    ...
    {LWM2M_DM_OBJECT_ID, get_object_DM_Onenet},
#endif
};
...
#ifdef CONFIG_LWM2M_ONENET_DM_SUPPORT
    {666, 0, 14, 1, 1, {{6601, 1, 1}, {6602, 1, 1}, {6603, 1, 1}, {6604, 1, 1}, {6605, 1, 1}, {6606, 1, 1}, {6607, 1, 1}, {6608, 1, 1}, {6609, 1, 1}, {6610, 1, 1}, {6611, 1, 1}, {6612, 1, 1}, {6613, 1, 1}, {6614, 1, 1}}},
#endif
    {0},
    {0},
    {0},
...
ipso_obj_t *lwm2m_get_ipso_obj(uint16_t objId, int16_t resId, ipso_res_t **ipso_res, uint8_t ref)
{
    ...
    for ( i = 0; i < max_size; i++)
        if (type_array[i].objId == objId)
        {
            ...
            {
                for (j = 0; j < type_array[i].resCount; j++)
...
{
    int max_size;
    ipso_obj_t *type_array;
    int i ;
    if (find_internal_object(objId) != -1)
        return true;

    max_size = sizeof(ipso_stdobj_table) / sizeof(ipso_stdobj_table[0]);
    type_array = ipso_stdobj_table;
...
lwm2m_ret_t lwm2m_ipso_register(ipso_obj_t *ipso_obj, uint8_t ref)
{
    ...
    if (newIndex != -1 && newIndex < max_size)
    {
...
int lwm2m_registNewRes(uint16_t objId, uint8_t *resString, uint8_t ref)
{
    ...
    for (i = index; i < max_size; i++)
    {
        if (type_array[i].objId == objId)
        {
            ...
            value = lwm2m_malloc(type_array[i].resCount * sizeof(void *));
            for (token = strtok_r((char *)resString, ";", &lasts); token != NULL && q < type_array[i].resCount; token = strtok_r(NULL, ";", &lasts))
            {
                ((int *)value)[q++] = (int)token;
            }

#ifdef CONFIG_LWM2M_ONENET_SUPPORT
            if (token != NULL || (q != type_array[i].resCount && lwm2m_configs[ref]->ResMode == 0))
#else
            if (token != NULL || q != type_array[i].resCount)
#endif
            {
                LOG_ARG("Error: resource string %d isn't match resource count %d", q, type_array[i].resCount);
                lwm2m_free(value);
                return -1;
            }

            for (j = 0; j < type_array[i].resCount && j < q; j++)
            {
                ...
                else
                {
                    bool isHasRes = false;
                    int temIndex ;
                    for ( temIndex = 0; temIndex < type_array[i].resCount; temIndex++)
                    {
...
static lwm2m_object_t *add_object(lwm2m_context_t *contextP, uint16_t objid, uint16_t instid)
{
    ...
    inst = LWM2M_LIST_FIND(targetP->instanceList, instid);
