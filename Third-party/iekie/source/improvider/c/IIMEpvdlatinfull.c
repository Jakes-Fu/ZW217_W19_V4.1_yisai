...
#include "IIMEstring.h"//IIMEString_Copy		ZHMCH
...
DECLARE_PROVIDER_FUNCTIONS(IIMELatinFullProvider)

static IIME_RESULT EventHandle_Smart(IIMELatinFullProvider *pProvider,
                                   IIME_UINT hEvent,
                                   IIME_UINT iMsg,
                                   IIME_IMM_CONTENT *pContent);

static IIME_RESULT EventHandle_Multitap(IIMELatinFullProvider *pProvider,
                                   IIME_UINT hEvent,
                                   IIME_UINT iMsg,
                                   IIME_IMM_CONTENT *pContent);

static int PostProcess(unsigned short *pBuffer, IIMELatinFullProvider *pProvider);

static IIME_RESULT Smart_KeyHandle(IIMELatinFullProvider *pThis, IIME_UINT iMsg);
static IIME_RESULT Smart_CharHandle(IIMELatinFullProvider *pThis, IIME_UINT iMsg);
static IIME_RESULT Smart_TimerHandle(IIMELatinFullProvider *pThis);

static IIME_RESULT Multitap_KeyHandle(IIMELatinFullProvider *pThis, IIME_UINT iMsg);
static IIME_RESULT Multitap_CharHandle(IIMELatinFullProvider *pThis, IIME_UINT iMsg);
static IIME_RESULT Multitap_TimerHandle(IIMELatinFullProvider *pThis);

static IIME_RESULT Smart_Commit(IIMELatinFullProvider *pThis, int idx);
static IIME_RESULT Multitap_Commit(IIMELatinFullProvider *pThis, int idx);

static void LatinFull_FillCandidate(IIMELatinFullProvider *pThis);
static void LatinFull_ClearCandidate(IIMELatinFullProvider *pThis);
static void LatinFull_ResetPageInfo(IIMELatinFullProvider *pThis);
static void LatinFull_Recognize(IIMELatinFullProvider *pThis);
static void LatinFull_ResetMultiKey(IIMELatinFullProvider *pThis);
static void LatinFull_Reset(IIMELatinFullProvider *pThis);

static IIME_RESULT Smart_AddChar(IIMELatinFullProvider *pThis);
static IIME_RESULT Smart_DeleteChar(IIMELatinFullProvider *pThis);
static IIME_RESULT Smart_DeleteAll(IIMELatinFullProvider *pThis);
static IIME_RESULT Smart_NaviCandidate(IIMELatinFullProvider *pThis, IIME_UINT iMsg);
static void Smart_CheckCase(IIMELatinFullProvider *pThis);

static void LatinFull_GetSymList(IIMELatinFullProvider *pThis, LDB_CHAR* pMultiList, int* pCount);

static const QWERTY_KEYMAP* LatinFull_FindKeymap(IIMELatinFullProvider *pThis, const IIME_QWERTY_INFO *pQwertyInfo);
static int LatinFull_KeymapKey(IIMELatinFullProvider *pThis, unsigned short key);
static const unsigned short * LatinFull_KeymapValue(IIMELatinFullProvider *pThis, unsigned short key);

/* for english, Hinglish, Indonesian, malay, swahili*/
static void Basic_GetMultitype(
    IIMELatinFullProvider     *pThis, 
    IN LDB_CHAR             key,
    IN LDB_CHAR             prev_char,
	OUT LDB_CHAR*			pMultiList,
    OUT int*                count);

static int Basic_ValidateInput(IIMELatinFullProvider *pThis, LDB_CHAR c);
...
static int Thai_ValidateInput(IIMELatinFullProvider *pThis, LDB_CHAR c);
#endif /* KMX_LDB_THAI */
...
static int Arabic_Hebrew_ValidateInput(IIMELatinFullProvider *pThis, LDB_CHAR c);
...
static int Arabic_Normalize(unsigned short *pBuffer, IIMELatinFullProvider *pThis);
...
static int Latin2_ValidateInput(IIMELatinFullProvider *pThis, LDB_CHAR c);
static int Latin2_Normalize(unsigned short *pBuffer, IIMELatinFullProvider *pThis);
...
static int Latin3_ValidateInput(IIMELatinFullProvider *pThis, LDB_CHAR c);
static int Vietnamese_Normalize(unsigned short *pBuffer, IIMELatinFullProvider *pThis);
#endif
...
//daodejing
    ...
    defined(KMX_LDB_TAMIL) || defined(KMX_LDB_PUNJABI) || defined(KMX_LDB_GUJARATI) || defined(KMX_LDB_ORIYA) || \
    defined(KMX_LDB_KANNADA) || defined(KMX_LDB_MALAYALAM) || defined(KMX_LDB_KHMER) || defined(KMX_LDB_LAO) || \
    defined(KMX_LDB_MYANMAR) || defined(KMX_LDB_NEPALI) || defined(KMX_LDB_ASSAMESE)|| defined(KMX_LDB_BODO) || \
    defined(KMX_LDB_DOGRI) || defined(KMX_LDB_SANSKRIT) || defined(KMX_LDB_KASHMIRI) || defined(KMX_LDB_KONKANI) || \
    defined(KMX_LDB_MAITHILI) || defined(KMX_LDB_MANIPURI) || defined(KMX_LDB_SANTALI) || defined(KMX_LDB_SOMALI) || defined(KMX_LDB_SINDHI)
...
static int Amharic_ValidateInput(IIMELatinFullProvider *pThis, LDB_CHAR c);
#endif
...
Provider_Com* IIMELatinFullProvider_Create(
    ...
    IIME_INT                  idMethod)
{
	...
	IIMELatinFullProvider *pThis = IIME_MALLOC(sizeof(IIMELatinFullProvider));
	memset(pThis, 0, sizeof(IIMELatinFullProvider));

   	/* initialize interface functions*/
	INITIALIZE_PROVIDER_INTERFACE(IIMELatinFullProvider);
...
//daodejing
    ...
    defined(KMX_LDB_MYANMAR) || defined(KMX_LDB_NEPALI) || defined(KMX_LDB_ASSAMESE) || \
    defined(KMX_LDB_NEPALI) || defined(KMX_LDB_BODO) || defined(KMX_LDB_DOGRI) || \
    defined(KMX_LDB_SANSKRIT) || defined(KMX_LDB_KASHMIRI) || defined(KMX_LDB_KONKANI) || defined(KMX_LDB_MAITHILI) || \
...
void IIMELatinFullProvider_Delete(
    Provider_Com*           pProvider)
{
	IIMELatinFullProvider *pThis = (IIMELatinFullProvider *)pProvider;
	if (pThis)
	{
...
static IIME_RESULT EventHandle_IIMELatinFullProvider(Provider_Com *pProvider,
                                   IIME_UINT hEvent,
                                   IIME_UINT iMsg,
                                   IIME_IMM_CONTENT *pContent)
{
...
static IIME_RESULT SetOptions_IIMELatinFullProvider(Provider_Com *pProvider,
...
static IIME_RESULT GetOptions_IIMELatinFullProvider(Provider_Com *pProvider,
                                  IIME_IMM_OPTIONS *pOption)
{
    return IIME_ERROR_SUCCESS;
}

static IIME_RESULT AddPhrase_IIMELatinFullProvider(Provider_Com* pProvider, unsigned short* szPhrase, int nLen)
{
    return IIME_ERROR_UNKNOWN;
}

static IIME_RESULT CheckAsso_IIMELatinFullProvider(Provider_Com* pProvider, unsigned short wContext, unsigned short* szAsso, int* pNumber)
{
    return IIME_ERROR_UNKNOWN;
}

static IIME_RESULT QueryProperties_IIMELatinFullProvider(Provider_Com* pProvider, int* pProperties)
{
    return IIME_ERROR_UNKNOWN;
}
...
static IIME_RESULT EventHandle_Smart(IIMELatinFullProvider *pThis, 
                                   IIME_UINT hEvent,
                                   IIME_UINT iMsg,
                                   IIME_IMM_CONTENT *pContent)
{
...
static void Smart_CheckCaseByContext(IIMELatinFullProvider *pThis, const unsigned short *text, int length)
{
...
static int PostProcess(unsigned short *pBuffer, IIMELatinFullProvider *pThis)
{
    int length = 0;
    switch (pThis->m_language)
	{
//daodejing
    ...
    defined(KMX_LDB_NEPALI) || defined(KMX_LDB_SINHALA) || defined(KMX_LDB_BODO) || defined(KMX_LDB_DOGRI) || \
    defined(KMX_LDB_SANSKRIT) || defined(KMX_LDB_KASHMIRI) || defined(KMX_LDB_KONKANI) || defined(KMX_LDB_MAITHILI) || \
    defined(KMX_LDB_MANIPURI) || defined(KMX_LDB_SANTALI) || defined(KMX_LDB_SOMALI) || defined(KMX_LDB_SINDHI)
	case IIME_LANG_HINDI:
	case IIME_LANG_NEPALI:
...
#if defined(KMX_LDB_ARABIC) || defined(KMX_LDB_URDU) || defined(KMX_LDB_PERSIAN)
	case IIME_LANG_ARABIC:
    case IIME_LANG_URDU:
    case IIME_LANG_PERSIAN:
...
static IIME_RESULT EventHandle_Multitap(IIMELatinFullProvider *pThis, 
                                   IIME_UINT hEvent,
                                   IIME_UINT iMsg,
                                   IIME_IMM_CONTENT *pContent)
{
    ...
    {
        int len_old, len_new;
        unsigned short pBuffer[MAX_ITEM_LENGTH] = {0};//L"\0";
        if (pre_char)
        {
            pBuffer[0] = pre_char;
            pBuffer[1] = 0;
...
static int Vietnamese_Normalize(unsigned short *buf, IIMELatinFullProvider *pThis)
{
    int i, j;
...
static int Arabic_Normalize(unsigned short *buf, IIMELatinFullProvider *pThis)
{
	...
	extern int kmx_is_valid_sinhala_word_start_char(IN LDB_CHAR	c);
	extern int kmx_is_valid_sinhala_follow_char(IN LDB_CHAR c1, IN LDB_CHAR c2);
#endif
...
static int Indic_ValidateInput(IIMELatinFullProvider *pThis, LDB_CHAR c)
{
	if (LatinFull_KeymapKey(pThis, c))
		return 1;
