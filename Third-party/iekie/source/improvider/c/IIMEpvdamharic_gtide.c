...
static void FillRowCandidate(AmharicProvider *pThis, IIME_WCHAR iMsg);
                                   ...
                                   IIME_IMM_CONTENT *pContent)
{
    ...
    if (pThis->m_nCand)
    {
        ...
        if (pThis->m_iStatus == En_Row || pThis->m_iStatus == En_Colomn || pThis->m_iStatus == En_Lead)
...
//            FillColomnCandidate(pThis, iMsg);
        ...
        case IIME_VK_SELECT:
            {
                IIME_BOOL bCommit = CommitByNumber(pThis, 0xFFFF);
                if (bCommit)
...
static void FillRowCandidate(AmharicProvider *pThis, IIME_WCHAR iMsg)
{
    ...
    if (iMsg == 0xFFFF)
        c1 = pThis->m_pCandidate[pThis->m_iHotCand];
    else{
        int offset = iMsg - IIME_VK_1;
        if (offset < pThis->m_nCand)
            c1 = pThis->m_pCandidate[offset];
        else{
            pThis->m_iStatus = En_Lead;
            return;
        }
