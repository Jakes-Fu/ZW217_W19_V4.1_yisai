...
uint32 ATV_IIC_Write(uint32 slave_addr, uint32 cmd, uint16 data)
{
    ...
    i2c_handle_atv  = s_lcm_func_table->sensor_geti2chandler();
    s_lcm_func_table->i2c_halwrite(i2c_handle_atv, NULL, reg_val, iic_length);	
...
uint32 ATV_IIC_Read(uint32 slave_addr, uint32 cmd, uint16* data)
{
   ...
   i2c_handle_atv  = s_lcm_func_table->sensor_geti2chandler();
   s_lcm_func_table->i2c_halwrite(i2c_handle_atv, NULL, reg_val, iic_length);
   s_lcm_func_table->i2c_halread(i2c_handle_atv, NULL, red_val, iic_length);
