...
LOCAL ERR_LCD_E ILI9328_Init(void)
{
	...
	s_lcm_func_table->trace("qinss LCD: in ILI9328_reset");

	s_lcm_func_table->lcd_reset();//reset lcd throught lcdc	
	s_lcm_func_table->lcd_delayms(50);
	...
	s_lcm_func_table->lcd_delayms(200);                        // Dis-charge capacitor power voltage
	...
	s_lcm_func_table->lcd_delayms(50);                         // Delay 50ms
	...
	s_lcm_func_table->lcd_delayms(30);
	//--------------- write to GRAM ---------------// 
...
{
	s_lcm_func_table->trace("qinss LCD: in ILI9328_EnterSleep, is_sleep = %d", is_sleep);
	if ( is_sleep ) // enter sleep mode.
	{

	  	LCD_CtrlData_ILI9328(0x0007, 0x0131); // Set D1=0, D0=1
    	s_lcm_func_table->lcd_delayms(10);
...
LOCAL void ILI9328_Close(void)
{
	s_lcm_func_table->trace("qinss LCD: in ILI9328_Close");
...
{
	s_lcm_func_table->trace("qinss LCD: in ILI9328_GetOperations");
