...
struct ApuEvent_t *ApuEventQueue = NULL;//[ APU_EVENT_MAX ];
int  cur_event;
WORD entertime;
...
BYTE *wave_buffers[5] = {0};//[5][735];      /* 44100 / 60 = 735 samples per sync */
...
int InfoNES_pAPU_fpic_init()
{
	int idx = 0;
	ApuEventQueue = (struct ApuEvent_t *)NesMallocFromWRE(APU_EVENT_MAX * sizeof(struct ApuEvent_t));
	if (NULL == ApuEventQueue)
		return -1;
	wave_buffers[0] = (BYTE *)NesMallocFromWRE(736 * sizeof(BYTE));
	wave_buffers[1] = (BYTE *)NesMallocFromWRE(736 * sizeof(BYTE));
	wave_buffers[2] = (BYTE *)NesMallocFromWRE(736 * sizeof(BYTE));
	wave_buffers[3] = (BYTE *)NesMallocFromWRE(736 * sizeof(BYTE));
	wave_buffers[4] = (BYTE *)NesMallocFromWRE(736 * sizeof(BYTE));
	if ((NULL == wave_buffers[0])
		||(NULL == wave_buffers[1])
		||(NULL == wave_buffers[2])
		||(NULL == wave_buffers[3])
		||(NULL == wave_buffers[4]))
		return -1;
	pAPUSoundRegs[idx++] = ApuWriteC1a;
	pAPUSoundRegs[idx++] = ApuWriteC1b;
	pAPUSoundRegs[idx++] = ApuWriteC1c;
	pAPUSoundRegs[idx++] = ApuWriteC1d;
	pAPUSoundRegs[idx++] = ApuWriteC2a;
	pAPUSoundRegs[idx++] = ApuWriteC2b;
	pAPUSoundRegs[idx++] = ApuWriteC2c;
	pAPUSoundRegs[idx++] = ApuWriteC2d;
	pAPUSoundRegs[idx++] = ApuWriteC3a;
	pAPUSoundRegs[idx++] = ApuWriteC3b;
	pAPUSoundRegs[idx++] = ApuWriteC3c;
	pAPUSoundRegs[idx++] = ApuWriteC3d;
	pAPUSoundRegs[idx++] = ApuWriteC4a;
	pAPUSoundRegs[idx++] = ApuWriteC4b;
	pAPUSoundRegs[idx++] = ApuWriteC4c;
	pAPUSoundRegs[idx++] = ApuWriteC4d;
	pAPUSoundRegs[idx++] = ApuWriteC5a;
	pAPUSoundRegs[idx++] = ApuWriteC5b;
	pAPUSoundRegs[idx++] = ApuWriteC5c;
	pAPUSoundRegs[idx++] = ApuWriteC5d;
	
	pulse_waves[0] = pulse_87;
	pulse_waves[1] = pulse_75;
	pulse_waves[2] = pulse_50;
	pulse_waves[3] = pulse_25;
	return 0;
}

void InfoNES_pAPU_fpic_finalize()
{
	NES_FREE(ApuEventQueue);
	NES_FREE(wave_buffers[0]);
	NES_FREE(wave_buffers[1]);
	NES_FREE(wave_buffers[2]);
	NES_FREE(wave_buffers[3]);
	NES_FREE(wave_buffers[4]);
}
/*-------------------------------------------------------------------*/
	...
	for ( i = 0; i < ApuSamplesPerSync; i++ ) 
	{
		...
		*/
		...
		if ( ApuC1SweepOn && ApuC1SweepShifts )     
		{
			...
			while ( ApuC1SweepPhase < 0 ) /*lint !e685*//*lint !e568*/
			{
...
void ApuRenderingWave2( void )
{
	...
	for ( i = 0; i < ApuSamplesPerSync; i++ ) 
	{
		...
		/* Envelope decay at a rate of ( Envelope Delay + 1 ) / 240 secs */
		ApuC2EnvPhase -= 4;
		while ( ApuC2EnvPhase < 0 )/*lint !e685*//*lint !e568*/
		{
		...
		*/
		...
		if ( ApuC2SweepOn && ApuC2SweepShifts )     
		{
			...
			while ( ApuC2SweepPhase < 0) /*lint !e685*//*lint !e568*/
			{
...
//				ApuC3b = ApuEventQueue[event].data;
			...
			case 2:
				...
				if ( ApuC3Freq ) 
				{
					ApuC3Skip = ApuTriangleMagic / ApuC3Freq; /*lint !e573*/
				} 
				else
				{
					ApuC3Skip = 0;  
...
void ApuRenderingWave4(void)
{
	...
	for ( i = 0; i < ApuSamplesPerSync; i++ ) 
	{
		...
		/* Envelope decay at a rate of ( Envelope Delay + 1 ) / 240 secs */
		ApuC4EnvPhase -= 4;
		while ( ApuC4EnvPhase < 0 )/*lint !e568*//*lint !e685*/
		{
		...
		if ( ApuCtrlNew & 0x08 ) 
		{
			...
			if ( ApuC4Index > 0x1fffffff ) 
			{
				...
				{ 
					ApuC4Sr |= ((!(ApuC4Sr & 1)) ^ (!(ApuC4Sr & 4))) << 5;/*lint !e514*/
				}
				else 
				{
					ApuC4Sr |= ((!(ApuC4Sr & 1)) ^ (!(ApuC4Sr & 16))) << 9;/*lint !e514*/
				}
				ApuC4Sr >>= 1;
			}
...
void ApuRenderingWave5(Nes_Global_T *pGData)
{
	unsigned int i;
    int cycles = 0;
    int event = 0;
    ...
    for ( i = 0; i < ApuSamplesPerSync; i++ ) 
	{
		...
		if( ApuC5DmaLength ) 
		{
			ApuC5Phaseacc -= (int)ApuCycleRate;
			...
			while( ApuC5Phaseacc < 0 ) 
			{
				...
				if( !( ApuC5DmaLength & 7 ) ) 
				{
					ApuC5CurByte = K6502_Read(pGData, ApuC5Address );
...
//						ApuC5Enable = 0;
				...
				// positive delta
				if( ApuC5CurByte & ( 1 << ((ApuC5DmaLength&7)^7)) ) /*lint !e504*/
				{
...
void InfoNES_pAPUVsync(Nes_Global_T *pGData)
{
	ApuRenderingWave1();
	ApuRenderingWave2();
	ApuRenderingWave3();
	ApuRenderingWave4();
	ApuRenderingWave5(pGData);
