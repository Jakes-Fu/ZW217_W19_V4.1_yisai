...
{
  Map188_Dummy = (BYTE *)NesMallocFromWRE(0x2000 * sizeof(BYTE));
  if (NULL == Map188_Dummy)
  	return;
  /* Initialize Mapper */
...
void Map188_Write(Nes_Global_T *pGData, WORD wAddr, BYTE byData )
{
  ...
  if ( byData )
  {
    if ( byData & 0x10 )
    {
      ...
      byData <<= 1;
      pGData->ROMBANK0 = ROMPAGE( ( byData + 16 ) % ( NesHeader.byRomSize << 1 ) );
      pGData->ROMBANK1 = ROMPAGE( ( byData + 17 ) % ( NesHeader.byRomSize << 1 ) );
    }
  } 
  else 
  {
    ...
    {
      pGData->ROMBANK0 = ROMPAGE( 14 );
      pGData->ROMBANK1 = ROMPAGE( 15 );
    } else {
