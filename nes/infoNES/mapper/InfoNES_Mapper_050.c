...
void Map50_Init(Nes_Global_T *pGData)
{
  ...
  /* Set SRAM Banks */
  pGData->SRAMBANK = ROMPAGE( 15 % ( NesHeader.byRomSize << 1 ) );

  /* Set ROM Banks */
  pGData->ROMBANK0 = ROMPAGE( 8 % ( NesHeader.byRomSize << 1 ) );
  pGData->ROMBANK1 = ROMPAGE( 9 % ( NesHeader.byRomSize << 1 ) );
  pGData->ROMBANK2 = ROMPAGE( 0 % ( NesHeader.byRomSize << 1 ) );
  pGData->ROMBANK3 = ROMPAGE( 11 % ( NesHeader.byRomSize << 1 ) );
...
void Map50_Apu(Nes_Global_T *pGData, WORD wAddr, BYTE byData )
{
  if ( ( wAddr & 0xE060 ) == 0x4020 )
  {
    ...
    else
    {
      ...
      pGData->ROMBANK2 = ROMPAGE( byDummy % ( NesHeader.byRomSize << 1 ) );
    }
  }
}
