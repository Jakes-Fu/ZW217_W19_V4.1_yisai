 ...
 ** 						   Macro Define
 **---------------------------------------------------------------------------*/
#define HI253_I2C_ADDR_W		0x40
#define HI253_I2C_ADDR_R			0x41
 ...
 **---------------------------------------------------------------------------*/
LOCAL uint32 set_hi253_ae_awb_enable(uint32 ae_enable, uint32 awb_enable);
LOCAL uint32 set_hi253_ae_enable(uint32 enable);
LOCAL uint32 set_hi253_hmirror_enable(uint32 enable);
LOCAL uint32 set_hi253_vmirror_enable(uint32 enable);
LOCAL uint32 set_hi253_preview_mode(uint32 preview_mode);
LOCAL uint32 _hi253_Power_On(uint32 power_on);
LOCAL uint32 HI253_Identify(uint32 param);
LOCAL uint32 HI253_Before_Snapshot(uint32 param);  
LOCAL uint32 HI253_After_Snapshot(uint32 param);

LOCAL uint32 HI253_set_brightness(uint32 level);
LOCAL uint32 HI253_set_contrast(uint32 level);
LOCAL uint32 HI253_set_sharpness(uint32 level);
LOCAL uint32 HI253_set_saturation(uint32 level);
LOCAL uint32 HI253_set_image_effect(uint32 effect_type);

LOCAL uint32 HI253_read_ev_value(uint32 value);
LOCAL uint32 HI253_write_ev_value(uint32 exposure_value);
LOCAL uint32 HI253_read_gain_value(uint32 value);
LOCAL uint32 HI253_write_gain_value(uint32 gain_value);
LOCAL uint32 HI253_read_gain_scale(uint32 value);
LOCAL uint32 HI253_set_frame_rate(uint32 param);
LOCAL uint32 HI253_set_work_mode(uint32 mode);
LOCAL uint32 HI253_set_whitebalance_mode(uint32 mode);
LOCAL uint32 set_hi253_video_mode(uint32 mode);
LOCAL uint32 set_hi253_ev(uint32 level);
LOCAL uint32 _hi253_GetResolutionTrimTab(uint32 param);
...
/*lint -save -e533 */
__align(4) const SENSOR_REG_T HI253_YUV_640X480[]=
{
{0x01, 0xf9}, //sleep on
{0x08, 0x0f}, //Hi-Z on
{0x01, 0xf8}, //sleep off
...
__align(4) SENSOR_REG_T HI253_YUV_1280X960[]=
{  
{0x01, 0xf1}, //sleep off	
{0x0e, 0x00}, //PLL off
{0x03, 0x00},
...
__align(4) SENSOR_REG_T HI253_YUV_1600X1200[]=
{
/*
{0x01, 0xf1}, //sleep off	
//{0x0e, 0x03}, //PLL off
 ...
 **---------------------------------------------------------------------------*/
LOCAL void HI253_WriteReg( uint8  subaddr, uint8 data )
{
#if 0
		...
		if(HI253_WRITE_DELAY!=cmd[0])
			{
		     I2C_WriteCmdArr(HI253_I2C_ADDR_W, cmd, 2, SCI_TRUE);
			}
		else
            {
		      if(cmd[1] > 0x80)
	...
	//SCI_TRACE_LOW:"SENSOR: HI253_WriteReg reg/value(%x,%x) !!"
	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_1677_112_2_18_0_31_0_852,(uint8*)"dd", subaddr, data);
#endif
...
LOCAL uint8 HI253_ReadReg( uint8  subaddr)
{
	uint8 value = 0;
#if 0    
	
	#ifndef	_USE_DSP_I2C_
		I2C_WriteCmdArr(HI253_I2C_ADDR_W, &subaddr, 1, SCI_TRUE);
		I2C_ReadCmd(HI253_I2C_ADDR_R, &value, SCI_TRUE);
	#else
		value = (uint16)DSENSOR_IICRead((uint16)subaddr);
	#endif
	
    //SCI_TRACE_LOW:"SENSOR: HI253_ReadReg reg/value(%x,%x) !!"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_1696_112_2_18_0_31_0_853,(uint8*)"dd", subaddr, value);
#endif
...
LOCAL uint32 HI253_Identify(uint32 param)
{
	...
	//SCI_TRACE_LOW:"HI253_Identify: it is HI253"
	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_1737_112_2_18_0_31_0_854,(uint8*)"");
...
LOCAL uint32 _hi253_Power_On(uint32 power_on)
{
    ...
    uint32 				reset_width=g_hi253_yuv_info.reset_pulse_width;	    
    ...
    //SCI_TRACE_LOW:"SENSOR: _hi253_Power_On(1:on, 0:off): %d"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_1777_112_2_18_0_31_0_855,(uint8*)"d", power_on);
...
LOCAL uint32 _hi253_GetResolutionTrimTab(uint32 param)
{
    return (uint32)s_HI253_Resolution_Trim_Tab;
}
LOCAL uint32 set_hi253_ae_enable(uint32 enable)
{
        if(0x00==enable)
	...
	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_1802_112_2_18_0_31_0_856,(uint8*)"d", enable);
...
LOCAL uint32 set_hi253_ae_awb_enable(uint32 ae_enable, uint32 awb_enable)
{
        ...
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_1833_112_2_18_0_31_0_857,(uint8*)"dd", ae_enable, awb_enable);

	return 0;
}


LOCAL uint32 set_front_sensor_yrotate_HI253(void)
{
    uint8 iFilpY;
    uint8 ss = 0;
    //#if 1
    //PUBLIC uint8 Get_sensor_direction_ud_front(void);
    //if(Get_sensor_direction_ud_front() == 1)
    //{
...
LOCAL uint32 set_back_sensor_yrotate_HI253(void)
{
    uint8 iFilpY;
    uint8 ss = 0;
#if 1
    //PUBLIC uint8 Get_sensor_direction_ud_back(void);
    //if(Get_sensor_direction_ud_back() == 1)
    //{
    HI253_WriteReg(0x03,0x00);
    iFilpY = HI253_ReadReg(0x11);
    ss = iFilpY & 0x02;
...
LOCAL uint32 set_hi253_hmirror_enable(uint32 enable)
{
	...
	//SCI_TRACE_LOW:"set_hi253_hmirror_enable: enable = %d"
	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_1903_112_2_18_0_31_0_858,(uint8*)"d", enable);

	return 0;
}

LOCAL uint32 set_front_sensor_xrotate_HI253(void)
{
    uint8 iFilpX,iHV_Mirror;
    uint8 ss = 0;
    //PUBLIC uint8 Get_sensor_direction_lr_front(void);

    //if(Get_sensor_direction_lr_front() == 1 ) //·´×ª
    //{
    HI253_WriteReg(0x03,0x00);
    ...
    {
    iHV_Mirror |= 0x01;
    //HI704_WriteReg(0x03,0x02);    
    //iFilpX =  HI704_ReadReg(0x1A) & 0xEF;
    //}
    ...
    //HI704_WriteReg(0x03,0x02);    
    //HI704_WriteReg(0x1A, iFilpX);			
    }
    return 0;
}

LOCAL uint32 set_back_sensor_xrotate_HI253(void)
{

    uint8 iFilpX,iHV_Mirror;
    uint8 ss = 0;
    //PUBLIC uint8 Get_sensor_direction_lr_back(void);

    //if(Get_sensor_direction_lr_back() == 1 ) //·´×ª
    //{
    HI253_WriteReg(0x03,0x00);
    iHV_Mirror = HI253_ReadReg(0x11);
    ss = iHV_Mirror & 0x01;
...
LOCAL uint32 set_hi253_vmirror_enable(uint32 enable)
{
	...
	//SCI_TRACE_LOW:"set_hi253_vmirror_enable: enable = %d"
	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_1984_112_2_18_0_31_1_859,(uint8*)"d", enable);
	return 0;
}
...
/******************************************************************************/
__align(4) const SENSOR_REG_T HI253_brightness_tab[][3]=
{
    {
    {0x03,0x10}, //-3
    {0x40,0xb0}, 
...
LOCAL uint32 HI253_set_brightness(uint32 level)
{
   ...
   for(i = 0; (0xFF != sensor_reg_ptr[i].reg_addr) || (0xFF != sensor_reg_ptr[i].reg_value); i++)
	...
	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2049_112_2_18_0_31_1_860,(uint8*)"d", level);

	return 0;
}


__align(4) const SENSOR_REG_T HI253_contrast_tab[][3]=
{
    {
    {0x03,0x10},  ///-3
    {0x48,0x50}, 
...
LOCAL uint32 set_hi253_preview_mode(uint32 preview_mode)
{
	//SCI_TRACE_LOW:"set_hi253_preview_mode: preview_mode = %d"
	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2138_112_2_18_0_31_1_861,(uint8*)"d", preview_mode);
...
//		
/******************************************************************************/
__align(4) const SENSOR_REG_T HI253_image_effect_tab[][7]=
{
    {
    {0x03,0x10},   //default
    {0x11,0x03},
...
LOCAL uint32 HI253_set_image_effect(uint32 effect_type)
{
    ...
    //SCI_TRACE_LOW:"HI253_set_image_effect: effect_type = %d"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2268_112_2_18_0_31_1_862,(uint8*)"d", effect_type);
...
#define PLL_ADDR_    0x0e
    ...
    //SCI_TRACE_LOW:"HI253_After_Snapshot "
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2278_112_2_18_0_31_1_863,(uint8*)"");
#if 0
...
LOCAL uint32 HI253_Before_Snapshot(uint32 param)
{      
    //SCI_TRACE_LOW:"sensor:HI253_Before_Snapshot"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2294_112_2_18_0_31_1_864,(uint8*)"");
    ...
    {
        //SCI_TRACE_LOW:"HI253_Before_Snapshot:dont set any"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2306_112_2_18_0_31_1_865,(uint8*)"");
    }

    //SCI_TRACE_LOW:"sensor:HI253_Before_Snapshot2"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2309_112_2_18_0_31_1_866,(uint8*)"");
...
/******************************************************************************/
__align(4) const SENSOR_REG_T HI253_mode_tab[][9]=
{
  {
  {0x03, 0x20},
  {0x10, 0x1c},
...
LOCAL uint32 HI253_set_work_mode(uint32 mode)
{
	...
	//SCI_TRACE_LOW:"sensor:HI253_set_work_mode: mode = %d"
	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2397_112_2_18_0_31_1_867,(uint8*)"d", mode);
	return 0;
}

__align(4) const SENSOR_REG_T HI253_WB_mode_tab[][11]=
{
    //AUTO
    {
		{0x03, 0x22},
...
LOCAL uint32 HI253_set_whitebalance_mode(uint32 mode )
{
	...
	//SCI_TRACE_LOW:"HI253_set_whitebalance_mode: mode = %d"
	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2489_112_2_18_0_31_2_868,(uint8*)"d", mode);
	return 0;
...
//		 
/******************************************************************************/
__align(4) const SENSOR_REG_T HI253_video_mode_nand_tab[][52]=
{
 {
    {0x01, 0xf1}, //sleep off	
    {0x0e, 0x00}, //PLL off
...
//    	s_preview_mode = DCAMERA_ENVIRONMENT_MAX;
    ...
    //SCI_TRACE_LOW:"SENSOR: set_hi253_video_mode: mode = %d"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2653_112_2_18_0_31_2_869,(uint8*)"d", mode);
    return 0;
}
__align(4) const SENSOR_REG_T HI253_ev_tab[][3]=
{
...
LOCAL uint32 set_hi253_ev(uint32 level)
{
    ...
    //Sensor_WriteReg(0x03,0x10);
   // Sensor_WriteReg(0x12,(Sensor_ReadReg(0x12)|0x10));//make sure the Yoffset control is opened.
    ...
    //SCI_TRACE_LOW:"SENSOR: set_hi253_ev: level = %d"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_HI253_2720_112_2_18_0_31_2_870,(uint8*)"d", level);
