...
#define DSP_CHROMA_LEVEL_CUR      (ATV_DSP_INT_COEF_LEVEL_2)
#define DSP_FRAMEB_LEVEL_CUR      (ATV_DSP_INT_COEF_LEVEL_2)
#define DSP_SHARP_LEVEL_CUR       (ATV_DSP_INT_COEF_LEVEL_2)


#ifdef DSP_LUMMINANCE_LEVEL_CUR
...
#elif (DSP_LUMMINANCE_LEVEL_CUR == ATV_DSP_INT_COEF_LEVEL_5)
...
#if 0 // def RDA5888_GPIO_I2C_ENABLE // invalid for SPRD.
// ---------------------------------------------------------------------------
// RDA5888 IIC DRIVER
// ---------------------------------------------------------------------------
...
static void rda_i2c_delay(uint32 time)
{
	while(time--);
}
...
uint32 RDA_IIC_Write(uint8 rega, uint16 data)
{
    uint8 reg_val[2];
    uint32 iic_length=0x02;
    uint32 i2c_handle_atv;

    reg_val[0]=(data>>0x08)&0xff;
    reg_val[1]=data&0xff;

    //reg_val[0]=data&0xff;
    //reg_val[1]=(data>>0x08)&0xff;


    i2c_handle_atv  = Sensor_GetI2cHnadler();
    I2C_HAL_Write(i2c_handle_atv, &rega, reg_val, iic_length);	

    return 0;
}

uint32 RDA_IIC_Read(uint8 rega, uint16* data)
{
    uint8 red_val[2];
    uint32 iic_length=0x02;
    uint32 i2c_handle_atv;
   ...
   i2c_handle_atv  = Sensor_GetI2cHnadler();
   I2C_HAL_Read(i2c_handle_atv, &rega, red_val, iic_length);
   
   *data = (uint16)((red_val[0]<<0x08)|(red_val[1]));

    //*data = (uint16)((red_val[1]<<0x08)|(red_val[0]));
    return 0;
}
...
#if (defined(DSP_CHROMA_LEVEL_CUR) || defined(DSP_FRAMEB_LEVEL_CUR))
	//	set coefficients of chroma&framebuffer.
	for (i = 0; i < 8; i++)
	{
	    temp = 0x0;
#if defined(DSP_CHROMA_LEVEL_CUR)
		temp = (m_wChromaCoefTable[i]<<12);
#endif
#if defined(DSP_FRAMEB_LEVEL_CUR)
...
#ifdef RDA5888_AUDSOFTMUTE_ENABLE
    RDA5888_FieldWriteReg(0x082, 0x0002, 0x1);
#endif
...
{
    RDAEXT_DebugPrint(RDA5888_TRACE_MOD  "[rdamtv]RDA_Inital_Dig Begin\n");
	...
	//RDA5888_WriteReg(0x129,0x08ab); //hsync_delay_sel & clk_inv  for SPRD_CUSTOMER_P2
	...
	RDA5888_WriteReg(0x129,0x09c9); // 9'h129,16'h09cb; // for SPRD Platform
	RDA5888_WriteReg(0x12f,0x0c00); // 9'h12f,16'h0c00; by xu, iq swap = 1
	RDA5888_WriteReg(0x133,0x0586); // 9'h133,16'h0586; *
	RDA5888_WriteReg(0x135,0x03cc); // 9'h135,16'h03cc; h_blank_end, cell without shell
	RDA5888_WriteReg(0x137,0x035f); // 9'h137,16'h035f;
	...
	RDA5888_WriteReg(0x17b,0x0900); // 9'h17b,16'h4000; // open the it601 interlace mode.0x4000  0x4800
#else
	#ifdef RDA5888_HSYNCHOLD_ENABLE
		RDA5888_WriteReg(0x17b,0x4000); // 9'h17b,16'h4000; // open the it601 interlace mode.0x4000  0x4800
	#else
		RDA5888_WriteReg(0x17b,0x0000); // 9'h17b,16'h4000; // close the it601 interlace mode.//0x0000 //0x0900
	#endif
#endif
...
#ifdef  BB_GPIO_HOLD_RDA5888
	RDA5888_WriteReg(0x1c4,0x0200); // 9'h1c4,16'h0248;  0x0248
#else
	#ifdef RDA5888_HSYNCHOLD_ENABLE
		RDA5888_WriteReg(0x1c4,0x0248); // 9'h1c4,16'h0248;  0x0248
	#else
		RDA5888_WriteReg(0x1c4,0x0200); // 9'h1c4,16'h0248; added by xu 01152010r, no  delay hvsync for green arise when swing ana
	#endif
#endif
...
#define SEEK_TV_THRESHOLD        75 // (defaul: 65) modify for sprd
#define K1                       1
#define K2                       2
...
#if 0 // test air freq.
    dwVFreq = 184250000; // set fixed freq, for test.  527250000
    eVideoStd = RDAMTV_VSTD_PAL_D;
#endif
