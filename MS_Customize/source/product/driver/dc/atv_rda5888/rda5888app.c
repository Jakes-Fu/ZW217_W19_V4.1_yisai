...
#define CHANNEL_FILTER_THRESHOLD   150 //145 modify for sprd.
...
extern void RDAAPP_TimerHandler(uint32);
...
//  none.
// ----------------------------------------------------------------------------
void RDAAPP_RealTimeProcess(void)
{
...
void RDAAPP_VideoEnhance(unsigned int buffer1, unsigned int buffer2, unsigned int size)
{
    ...
    if (g_nVideoEnhanceEnable)
    {
		...
		// image sharp process. size/8 for half of screen.
		if (m_nImageSharpEn)
		{
		    ImageSharp_asm((unsigned short *)buffer1, size/4);
		    //ImageSharpProcess((unsigned short *)buffer1, size);
		}
#endif
...
void RDAAPP_ATVFrameProcess(unsigned int buffer1, unsigned int buffer2, unsigned int w,unsigned int h)
{
    RDAAPP_VideoEnhance(buffer1,buffer2,w*h*2);
}
