...
#define berg_ID_W25X16       0x3015
#define berg_ID_W25X32       0x3016
#define berg_ID_W25X64       0x3017

/* Status1/Status2 register bit map */
#define berg_SR_WIP      (1 << 0)    /* Write-in-Progress */

#define berg_SR_QE       (1 << 9)    /* Quad-enable */
...
#define berg_TIMEOUT_CMD    100


struct berg_spi_flash_params {
    u16    id;
...
struct berg_spi_flash {
    struct spi_flash flash;
    const struct berg_spi_flash_params *params;
};

static __inline struct berg_spi_flash *
to_berg_spi_flash(struct spi_flash *flash)
{
    return container_of(flash, struct berg_spi_flash, flash); /*lint !e413*/
}

#define berg_REG1_LOCK_MSK 0xFC  //SRP0,SEC,TB,BP2,BP1,BP0
#define berg_REG2_LOCK_MSK 0x41   //CMP,SRP1
#define berg_REG3_LOCK_MSK 0x4  //WPS


static const struct spi_flash_lock_desc berg32mbit_lock_table[] = {
...
static const struct spi_flash_lock_desc berg64mbit_lock_table[] = {
/*S7:spr0, S6:sec, S5:tb, S4:bp2, S3:bp1, S2:bp0, S1:wel, S0:busy,*/
/* CMP=0 */
...
static const struct spi_flash_lock_desc berg128mbit_lock_table[] = {
/*S7:spr0, S6:sec, S5:tb, S4:bp2, S3:bp1, S2:bp0, S1:wel, S0:busy,*/
/* CMP=0 */
...
static const struct spi_flash_lock_desc berg256mbit_lock_table[] = {
...
static const struct berg_spi_flash_params berg_spi_flash_table[] = {
    {
...
static int berg_qpi_4read(struct spi_flash *flash)
{ 
    u8 cs = flash->spi->cs;
...
static int berg_spi_write(struct spi_flash *flash, u32 offset,
                    size_t len, const void *buf)
{ 
    u8 cmd = 0x32; // CMD_4PP
    flash->spi_rw_mode = WR_CMD_1BIT|WR_ADDR_1BIT|WR_DATA_4BIT;
    spiflash_cmd_program(flash, offset,len, buf, cmd);
    return 0;//coverity warnning
}

static int berg_spi_read(struct spi_flash *flash)
{ 
    u8 cs = flash->spi->cs;
...
static int berg_enter_4io(struct spi_flash *flash)
{
    struct berg_spi_flash *sfd = to_berg_spi_flash(flash);
    int ret=0;
    ...
    debug("SF: berg_enter_qpi:sts1=0x%x,sts2=0x%x\n", status1,status2);

    ret=spiflash_write_enable (flash);
    
    cmd = CMD_W25_WRSR;
    dout = (status2<<8)|(status1)|(berg_SR_QE);
    spiflash_cmd_write(flash, &cmd,1, &dout,2);
    spiflash_cmd_wait_ready(flash, berg_TIMEOUT_CMD);

    return ret;
}   

static int berg_enter_qpi(struct spi_flash *flash)
{
    struct berg_spi_flash *sfd = to_berg_spi_flash(flash);
    int ret=0;
    u8 cmd=0;
...
/*lint -save -e533 */   
static int berg_exit_qpi(struct spi_flash *flash)
{
    int ret=0;
    ret=spiflash_cmd_exit_qpi(flash, CMD_EXIT_QPI);
    return ret;//coverity warnning
}

static int berg_work_configure(struct spi_flash *flash)
{
    int ret=0;
    struct berg_spi_flash *sfd = to_berg_spi_flash(flash);
    ...
    if(QPI_MODE == sfd->params->support_qpi) {
        ret=berg_enter_qpi(flash);
    } else {
        ret=berg_enter_4io(flash);
    }

    debug("SF: berg_work_cofigure: work_mode=%d\n", flash->work_mode);
    return ret;//coverity warnning
}
/*lint -restore*/
#if 0
int berg_erase(struct spi_flash *flash, u32 offset, size_t len)
{
    struct berg_spi_flash *sfd = to_berg_spi_flash(flash);
    ...
    debug("SF: berg_erase: len=0x%x as blocks=%d sectors=%d\n",\
...
#endif
 int berg_lock(struct spi_flash *flash, u32 offset, size_t len)
{
    u8 cmd=0;
    u8 status=0;
    u8 status2=0;
    ...
    if(SPIFLASH_SIZE_32Mbit == flash->size) {
        lock_table = berg32mbit_lock_table;
        table_size = ARRAY_SIZE(berg32mbit_lock_table);
    } else if(SPIFLASH_SIZE_64Mbit == flash->size) {
        lock_table = berg64mbit_lock_table;
        table_size = ARRAY_SIZE(berg64mbit_lock_table);
    } else if (SPIFLASH_SIZE_128Mbit == flash->size) {
        lock_table = berg128mbit_lock_table;
        table_size = ARRAY_SIZE(berg128mbit_lock_table);
    } else if (SPIFLASH_SIZE_256Mbit == flash->size) {
        lock_table = berg256mbit_lock_table;
        table_size = ARRAY_SIZE(berg256mbit_lock_table);
    } else {
        debug("SF: Unsupported with berg Size 0x%x\n", flash->size);
        return -1;
    }
    ...
    if (i == table_size) {
        debug("SF: Unsupported berg Lock @0x%x len=0x%x\n", offset, len);
        return -1;
    }
	...
	//step1,check status register_1,and set status register_1 	
        if ((status & berg_REG1_LOCK_MSK) != ((lock_desc->reg1_value)& 0xFF))
        {
	        status &= (~berg_REG1_LOCK_MSK);
	...
	//step2,check status register_2,and set status register_2
        if ((status2 & berg_REG2_LOCK_MSK) != ((lock_desc->reg2_value)& 0xFF))
        {
	        status2 &= (~berg_REG2_LOCK_MSK);
	...
	//step3,check status register_3,and set status register_3
		...
		if((status3&berg_REG3_LOCK_MSK) == berg_REG3_LOCK_MSK)//check WPS	
		{
			status3 &= (~berg_REG3_LOCK_MSK);
	        spiflash_write_enable (flash);
            cmd = CMD_W25_WRSR3; 
	    ...
	    if ((status2 & berg_REG2_LOCK_MSK) != 0)
        {
		...
		if(((status3&berg_REG3_LOCK_MSK) == berg_REG3_LOCK_MSK) && (0x17 != (flash->id & 0xff)))//check WPS	
		{
			status3 &= (~berg_REG3_LOCK_MSK);
            spiflash_write_enable (flash);
            cmd = CMD_W25_WRSR3; 
 ...
 */
 int berg_unlock(struct spi_flash *flash, u32 offset, size_t len)
{
    u8 cmd=0;
    u8 status=0;
...
struct berg_spi_flash stm_berg_obj[2] = {0};
struct spi_flash *spi_flash_probe_berg(struct spi_slave *spi, u8 *idcode)
{
    const struct berg_spi_flash_params *params;
    struct berg_spi_flash *sfd=NULL;
    u32 page_size;
    u32 i;

    for (i = 0; i < ARRAY_SIZE(berg_spi_flash_table); i++) {
        params = &berg_spi_flash_table[i];
        if (params->id == ((idcode[1] << 8) | idcode[2]))
            break;
    }

    if (i == ARRAY_SIZE(berg_spi_flash_table)) {
        debug("SF: Unsupported berg ID %02x%02x\n",
                idcode[1], idcode[2]);
        return NULL;
    }

    //stm = SCI_ALLOC(sizeof(struct berg_spi_flash));
    if(spi->cs == 0)
        sfd = &stm_berg_obj[0];
    else if(spi->cs == 1)
        sfd = &stm_berg_obj[1];    
    ...
    if(QPI_MODE == sfd->params->support_qpi)
        ...
        sfd->flash.write = berg_spi_write;
    
    sfd->flash.erase = berg_erase;
    sfd->flash.read = spiflash_read_common;
    sfd->flash.reset = spiflash_reset;

    sfd->flash.erase_chip = spiflash_erase_chip;    
    sfd->flash.enter_4read= berg_qpi_4read;  
    sfd->flash.enter_read= berg_spi_read;  
    sfd->flash.lock= berg_lock;  
    sfd->flash.unlock= berg_unlock;
    ...
    berg_work_configure(&(sfd->flash));

    spiflash_set_clock(&(sfd->flash), sfd->params->read_freq_max);
    return &sfd->flash;
}

#endif //#if defined (CONFIG_SPI_FLASH_berg) || !defined (CONFIG_SPI_FLASH)
