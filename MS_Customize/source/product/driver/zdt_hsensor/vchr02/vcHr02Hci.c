...
#define Drive_Version  V_1_9_2

static uint16 lastAdjCount = 0;
static uint32 lastRtcCount = 0,lastRtcValue = 0;
static uint8  ledCurrent[SLOTCHNU],ledMaxCurrent[SLOTCHNU] = {0x6f,0x6f,0x6f};
static uint8	pdRes[SLOTCHNU],pdResMax[SLOTCHNU] = {7,7,7};
static uint8  pdResSet[SLOTCHNU],ppgGain[SLOTCHNU];
static uint8  recordEnv[SLOTCHNU] = {0,0,0};
static uint16 count_pdRes[SLOTCHNU],count_QaPdRes[SLOTCHNU];
static uint8  adjustFlag[2][64];
static uint8  slotCount,sampleIndex,totalSlot = 0,readIndex = 0, readCnt = 0;
static uint8  fifoWriteIndex = 0,fifoReadIndex = 0x80,fifoIntDiv = 0;
static uint8  enFifo,enSlot[SLOTCHNU];
static uint16 oscCnt = 0,cntCalOsc = 0;
static uint32 intDiffRtc[8] = {0},intDiffAdj[8] = {0};
static uint8  oscDifSetting = 2;//oscDifSetting = 2£¬mean diffrtc <= 2
static BOOLEAN slot0EnvIsExceedFlag,slot1EnvIsExceedFlag;

extern int HeartRateValue;

#if SportMotionEn
/* SportMotion */
int32 gsensor_abs_xyz_sum_last_tick;
int32 gsensor_abs_xyz_sum_current_tick;
int32 gsensor_abs_xyz_diff_value;
 ...
 *
 **/
void green_led_off_state_gsensor_abs_sum_diff_func(int32 x_axis_value, int32 y_axis_value, int32 z_axis_value) 
{
	int32 x_axis_value_current = x_axis_value;
	int32 y_axis_value_current = y_axis_value;
	int32 z_axis_value_current = z_axis_value;
	
	gsensor_abs_xyz_sum_last_tick = gsensor_abs_xyz_sum_current_tick;
	if(x_axis_value<0)
	{
		x_axis_value_current = x_axis_value;
	}
	if(y_axis_value<0)
	{
...
/*
 * @brief
 * Function:Initialize vcHr02's register Settings 
 *
 **/
static vcHr02Ret_t vcHr02RegInit(vcHr02_t *pvcHr02)
{
	...
	uint8 i = 0;
	uint8 regConfig[17] = {0x01,0x3F,0x9A,0x40,0x03,0x1F,0x00,0x00,0x80,0x00,0x57,0x37,0x07,0x16,0x56,0x16,0x00};
	...
	ret |= VCHR02_WriteRegisters(VCREG11,pvcHr02->regConfig,17);
	
	pvcHr02->regConfig[0] = 0x80| regConfig[0];
	ret |= VCHR02_WriteRegisters(VCREG11,pvcHr02->regConfig,1);
...
vcHr02Ret_t vcHr02StopSample(vcHr02_t *pvcHr02)
{
    ...
    ret |= VCHR02_WriteRegisters(VCREG11, (uint8 *)&modeCtrl, VCREG11_SIZE);
 ...
 *
 **/
static vcHr02Ret_t vcHr02AddAdjustFlag(uint8 slotNum)
{
    vcHr02Ret_t ret = VCHR02RET_ISOK;
	...
	{
		ret |= VCHR02_ReadRegisters(VCREG3,(uint8 *)&fifoWriteIndex,VCREG3_SIZE);
...
*
**/
static vcHr02Ret_t vcHr02OutPutPPGData(vcHr02_t *pvcHr02,uint8 StartIndex, uint16 StopIndex,uint8 IndexFlag)
{
	uint16 i = 0;
    uint8 dataLength = 0;
	uint8 sampleData[128];
    vcHr02Ret_t ret = VCHR02RET_ISOK;
	
	dataLength = StopIndex - StartIndex;
	ret |= VCHR02_ReadRegisters(StartIndex,sampleData,dataLength);
...
extern uint32 vcHr02GetRtcCountFromMCU(void)//yangyu add
{
    uint64 rtcCount = 0;
    rtcCount = SCI_GetTickCount()*32768/1000;
    return rtcCount;
 ...
 *
 **/
uint16 vcHr02CalculateOSCFreq(vcHr02_t *pvcHr02)
{
	uint8 clkAdjCnt[4]={0},newFreDiv[2] = {0},i = 0;	
	uint32 rtcCount[2];
	uint16 adjCount[2],freDiv[2]={0};
	uint16 newDiv,f20kOsc;

	BOOLEAN diffCountIsOk = TRUE;
	BOOLEAN diffIsOK;
	
	/* Get McuRtcCount */
	rtcCount[0] = vcHr02GetRtcCountFromMCU();	
	VCHR02_ReadRegisters(VCREG7,clkAdjCnt,4);	
	/* Get McuRtcCount */
	   ...
	   if(diffCountIsOk)
	   	{
			for(i = 0; i < 2; i++)
			{
			freDiv[i] =((uint32)(intDiffAdj[2*i+2] - intDiffAdj[2*i]))* pvcHr02->mcuOscValue/pvcHr02->vcSampleRate/((uint32)(intDiffRtc[2*i+2] - intDiffRtc[2*i])) - 1;
			...
			if(diffIsOK)
			{
				...
				newFreDiv[1] = newDiv;
				VCHR02_WriteRegisters(VCREG15,newFreDiv,VCREG15_SIZE);
		...
		case VCHR02WEARST_ISWEAR:
#if BiometricEn
			if((pvcHr02->sampleData.psValue < VCPS_TH)||(pvcHr02->sampleData.envValue[2] > VCENV_TH)||(slot0EnvIsExceedFlag == TRUE)||(slot1EnvIsExceedFlag == TRUE)||(HeartRateValue == -1))

			{
				...
				if(--pvcHr02->unWearCnt <= 0) 
				{
                    ...
                    pvcHr02->vcPsFlag = 1;
					ret |= VCHR02_WriteRegisters(VCREG11,(uint8 *)&newSoltChNu, VCREG11_SIZE);
					ret |= VCHR02RET_ISWEARTOUNWEAR;
				}
			}
#else
		#if (VCHR02ISVC32S || VCHR02ISVC52S || VCHR02ISVC9201)
					if((pvcHr02->sampleData.psValue < VCPS_TH)||(pvcHr02->sampleData.envValue[2] > VCENV_TH))
		#else
					if((pvcHr02->sampleData.psValue < VCPS_TH)||(pvcHr02->sampleData.envValue[2] > VCENV_TH)||(slot0EnvIsExceedFlag == TRUE)||(slot1EnvIsExceedFlag == TRUE))
		#endif
			{
				/* 3 times */
				if(--pvcHr02->unWearCnt <= 0) 
 ...
 *
 **/
static  vcHr02Ret_t vcHr02ReadFifo(vcHr02_t *pvcHr02,uint8 *len)
{
	uint32 rtcValue = 0;
	uint8 dateLength = 0;
 ...
 *
 **/
static vcHr02Ret_t vcHr02EnvAdjust(vcHr02_t *pvcHr02,uint8 slotNum)
{
	vcHr02Ret_t ret = VCHR02RET_ISOK;
	...
	if((pvcHr02->sampleData.envValue[slotNum] > VCENV_TH) && enSlot[slotNum])
	{
		...
		if(pdRes[slotNum] != 0)
		{
			...
			ret |= VCHR02_WriteRegisters((VCREG20 + slotNum),(uint8 *)&(pvcHr02->regConfig[slotNum + 10]), 1);
		}
		else
		{
 ...
 *
 **/
static vcHr02Ret_t vcHr02OvloadAdjust(vcHr02_t *pvcHr02,uint8 slotNum)
{
	uint8 overLoad = 0;
	uint8 ovload[SLOTCHNU];
	vcHr02Ret_t ret = VCHR02RET_ISOK;
	
	ret|= VCHR02_ReadRegisters(VCREG1, (uint8 *)&overLoad, VCREG1_SIZE);
	...
	if((overLoad&(pvcHr02->regConfig[0]&0x07))!= 0)
	{
		if(ovload[slotNum]&& enSlot[slotNum])
		{
			if(ledCurrent[slotNum] != 0)
			{
				...
				ret|= VCHR02_WriteRegisters((VCREG17 + slotNum), (uint8 *)&(pvcHr02->regConfig[slotNum+7]),1);	
			}
			else
			{
 ...
 *
 **/
static vcHr02Ret_t vcHr02PPgLedCurAndPdResAdjust(vcHr02_t *pvcHr02,uint8 slotNum,uint8 ledcur,uint8 pdres)
{
	uint8  oldPdRes,oldLedCurrent;
	uint8  newPdRes,newLedCurrent;
	vcHr02Ret_t ret = VCHR02RET_ISOK;
 ...
 *
 **/
static vcHr02Ret_t vcHr02PpgAdjust(vcHr02_t *pvcHr02,uint8 slotNum)
{
	uint8  ppgDataAddress, ppgDataAddress_EnFifo;
	uint8  ppgDate[2];
	uint16 ppgCurValue[2];
	uint8  IndexGap;
	vcHr02Ret_t ret = VCHR02RET_ISOK;
	...
	ret|= VCHR02_ReadRegisters(ppgDataAddress,ppgDate, 2);
 ...
 *
 **/
static vcHr02Ret_t vcHr02AdjustPDResMax(vcHr02_t *pvcHr02,uint8 slotNum)
{
	uint8 minEnv[SLOTCHNU];
	vcHr02Ret_t ret = VCHR02RET_ISOK;
...
{
	uint8 vcRegisterValue[6] = {0};
	vcHr02Ret_t ret = VCHR02RET_ISOK;

	ret |= VCHR02_ReadRegisters(VCREG1,vcRegisterValue,6); 
	...
	ret |= VCHR02_ReadRegisters(VCREG17,vcRegisterValue,6); 
...
vcHr02Ret_t  vcHr02GetSampleValues(vcHr02_t *pvcHr02,uint8 *DataLen)
{
	...
	int16 secondCnt = pvcHr02->vcSampleRate*180/(fifoIntDiv+1);//3min
	//int16 secondCnt = pvcHr02->vcSampleRate*10/(fifoIntDiv+1);//10s
	...
	switch(pvcHr02->workMode)
	{
		case VCWORK_MODE_HRWORK:
			...
			SCI_TRACE_LOW("vchr02 vcHr02GetSampleValues vcFifoReadFlag=%d,oscCheckFinishFlag=%d" ,pvcHr02->vcFifoReadFlag,pvcHr02->oscCheckFinishFlag);
			...
			{	
				...
				SCI_TRACE_LOW("vchr02 vcHr02GetSampleValues oscCheckFinishFlag=%d" ,pvcHr02->oscCheckFinishFlag);
				if(pvcHr02->oscCheckFinishFlag)
				{
					#if (VCWEAR_READ_FIFO_MODE == VCWEAR_READ_FIFO_BY_INT)
					SCI_TRACE_LOW("vchr02 vcHr02GetSampleValues 1");
		...
		case VCWORK_MODE_SPO2WORK:
	        ...
	        if(pvcHr02->oscCheckFinishFlag)
				{
					#if (VCWEAR_READ_FIFO_MODE == VCWEAR_READ_FIFO_BY_INT)
					SCI_TRACE_LOW("vchr02 vcHr02GetSampleValues 2");
					ret |= vcHr02ReadFifo(pvcHr02,DataLen);
					#endif
				}
...
//     read pvcHr02->sampleData.psValue
		...
		case VCWORK_MODE_NOISEWORK:
			ret |= vcHr02ReadStatus(pvcHr02);
		#if (VCWEAR_READ_FIFO_MODE == VCWEAR_READ_FIFO_BY_INT)	
    		SCI_TRACE_LOW("vchr02 vcHr02GetSampleValues 3");
					ret |= vcHr02ReadFifo(pvcHr02,DataLen);				
		#endif
			break;
		case VCWORK_MODE_LPDETECTION:
	...
	SCI_TRACE_LOW("vchr02 vcHr02GetSampleValues ret=%d", ret);
	return ret;
}
...
vcHr02Ret_t vcHr02SoftReset(vcHr02_t *pvcHr02)
{
	...
	ret |= VCHR02_WriteRegisters(VCREG10, (uint8 *)&Reset, VCREG10_SIZE);
	oscCnt = 0;  
