...
#----------------------------------------------#
# Only packet for SC6531EFM
#----------------------------------------------#

BEGIN{push @INC,'./make/perl_script/perl_pm'};
BEGIN{push @INC,'C:/perl/perl_pm'};
...
if($number ne 15){
    die "\nInvalid parameters, param number must be 15.\n\n";
}
...
my $index = 4;
my @param;
my $preloadudisk_support_omitFlag;
my $eraseudisk_support_omitFlag;
my $fota_file_flag;

if(${ARGV[12]} ne "")
{
    $preloadudisk_support_omitFlag  = 1;
    $eraseudisk_support_omitFlag = 0;
}
else
{
    $preloadudisk_support_omitFlag  = 0;
    $eraseudisk_support_omitFlag = 1;
}
...
#-----------------------------------------------#
#ARGV[$index+3]:bootloader
#ARGV[$index+4]:stone_bin
#-----------------------------------------------#
...
@param = (
   ...
   ["FDL"           , ${ARGV[$index+0]}  , 0x0101   , 1        , 0        , 0x00800100    , 0xFFFFFFFF  ],
   ["FDL2"          , ${ARGV[$index+1]}  , 0x0101   , 1        , 0        , 0x80008000    , 0xFFFFFFFF  ],
   ["NV"            , ${ARGV[$index+2]}  , 1        , 1        , 0        , 0x90000001    , 0xFFFFFFFF  ],
   ["PS"            , ${ARGV[$index+4]}  , 1        , 1        , 0        , 0x80000003    , 0xFFFFFFFF  ],
   ["MMIRes"        , ${ARGV[$index+5]}  , 1        , 1        , 1        , 0x90000004    , 0xFFFFFFFF  ],
   ["EraseUdisk"    , ""                 , 0        , 0        , 1        , 0x90000005    , 0xFFFFFFFF  ],
   ["UDISK"         , ${ARGV[$index+6]}  , 1        , 1        , 1        , 0x90000006    , 0xFFFFFFFF  ],
   ["OperatorData"  , ${ARGV[$index+7]}  , 1        , 1        , 0        , 0x90000007    , 0xFFFFFFFF  ],
   ["FLASH"         , ""                 , 0        , 1        , 0        , 0x90000003    , 0xFFFFFFFF  ],
   ["PhaseCheck"    , ""                 , 0        , 1        , 0        , 0x90000002    , 0xFFFFFFFF  ],
   #config must be the last one
   [""              , $config            , 2        , 0        , 0        , 0x0           , 0xFFFFFFFF  ],
);
}
elsif ( (${ARGV[$index+5]} eq "") )
{
@param = (
   #ID              , file_path          , file_flag, check_flag, omit_flag, addr1        , addr2 
   ["FDL"           , ${ARGV[$index+0]}  , 0x0101   , 1        , 0        , 0x00800100    , 0xFFFFFFFF  ],
   ["FDL2"          , ${ARGV[$index+1]}  , 0x0101   , 1        , 0        , 0x80008000    , 0xFFFFFFFF  ],
   ["NV"            , ${ARGV[$index+2]}  , 1        , 1        , 0        , 0x90000001    , 0xFFFFFFFF  ],
   ["BOOTLOADER"    , ${ARGV[$index+3]}  , 1        , 1        , 0        , 0x80000000    , 0xFFFFFFFF  ],
   ["PS"            , ${ARGV[$index+4]}  , 1        , 1        , 0        , 0x80000003    , 0xFFFFFFFF  ],
   ["MMIRes"        , ${ARGV[$index+5]}  , 1        , 1        , 1        , 0x90000004    , 0xFFFFFFFF  ],
   ["EraseUdisk"    , ""                 , 0        , $eraseudisk_support_omitFlag, 1     , 0x90000006    , 0xFFFFFFFF  ],
   ["UDISK"         , ${ARGV[$index+6]}  , 1        , 1        , 1        , 0x90000006    , 0xFFFFFFFF  ],
   ["OperatorData"  , ${ARGV[$index+7]}  , 1        , 1        , 0        , 0x90000100    , 0xFFFFFFFF  ],
   ["preload_udisk" , ${ARGV[$index+8]}  , 1        , $preloadudisk_support_omitFlag, 1   , 0x90000006    , 0xFFFFFFFF  ],
   ["FLASH"         , ""                 , 0        , 1        , 0        , 0x90000003    , 0xFFFFFFFF  ],
   ["PhaseCheck"    , ""                 , 0        , 1        , 0        , 0x90000002    , 0xFFFFFFFF  ],
...
@param = (
   ...
   ["NV"            , ${ARGV[$index+2]}  , 1        , 1        , 0        , 0x90000001    , 0xFFFFFFFF  ],
   ["BOOTLOADER"    , ${ARGV[$index+3]}  , 1        , 1        , 0        , 0x80000000    , 0xFFFFFFFF  ],
   ["FOTA_BOOTLOADER"		, ${ARGV[$index+10]} , 1        , $fota_file_flag,        , 1        , 0x80000002, 0xFFFFFFFF  ],
   ["PS"            , ${ARGV[$index+4]}  , 1        , 1        , 0        , 0x80000003    , 0xFFFFFFFF  ],
   ["MMIRes"        , ${ARGV[$index+5]}  , 1        , 1        , 0        , 0x90000004    , 0xFFFFFFFF  ],
   ["CP"            , ${ARGV[$index+6]}  , 1        , 1        , 0        , 0x90000101    , 0xFFFFFFFF  ],  
   ["EraseUdisk"    , ""                 , 0        , $eraseudisk_support_omitFlag, 1     , 0x90000006    , 0xFFFFFFFF  ],
   ["UDISK"         , ""                 , 1        , 1        , 1        , 0x90000006    , 0xFFFFFFFF  ],
   ["OperatorData"  , ${ARGV[$index+7]}  , 1        , 1        , 0        , 0x90000100    , 0xFFFFFFFF  ],
   ["preload_udisk" , ${ARGV[$index+8]}  , 1        , $preloadudisk_support_omitFlag, 1   , 0x90000006    , 0xFFFFFFFF  ],
   ["IMS_TOGGLE"    , ""                 , 0        , 1        , 0        , 0x90000112    , 0xFFFFFFFF  ],
   ["CONFIG_NV"     , ""                 , 0        , 1        , 0        , 0x90000113    , 0xFFFFFFFF  ],
   ["FLASH"         , ""                 , 0        , 1        , 0        , 0x90000003    , 0xFFFFFFFF  ],
   ["PhaseCheck"    , ""                 , 0        , 1        , 0        , 0x90000002    , 0xFFFFFFFF  ],
   #config must be the last one
   [""              , $config            , 2        , 0        , 0        , 0x0           , 0xFFFFFFFF  ],
...
sub CheckParam
{
	...
	for ($j = 0; $j < $file_count; $j++)
	{
		...
		$strings += " $param[$j][0] $param[$j][1] $param[$j][2] $param[$j][3] $param[$j][4]";
	}
