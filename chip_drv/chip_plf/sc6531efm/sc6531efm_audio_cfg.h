...
#define VBC_REQ_ID_ADDR(_req_offset) (DMA_CTL_REG_BASE + 0x2020 + (_req_offset)*0x04)
...
static __inline void sprd_codec_mclk(uint32 en)
{
    if( en )
    {
          ANA_REG_OR(ANA_ARM_CLK_EN0,ANA_CLK_AUD_SCLK_EN);
     	  ANA_REG_OR(ANA_ARM_CLK_EN0,ANA_CLK_AUD_IF_6P5M_EN);
    }
    else
    {
          ANA_REG_AND(ANA_ARM_CLK_EN0,~ANA_CLK_AUD_SCLK_EN);
          ANA_REG_AND(ANA_ARM_CLK_EN0,~ANA_CLK_AUD_IF_6P5M_EN);
    }
}
...
#define IIS_REQ_ID_ADDR(_req_offset) (DMA_CTL_REG_BASE + 0x2010 + (_req_offset)*0x04)
