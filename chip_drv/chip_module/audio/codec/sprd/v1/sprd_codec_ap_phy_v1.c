/******************************************************************************
 ** File Name:      sprd_codec_ap_phy_v1.c                                    *
 ** Author:         ken.kuang                                                 *
 ** DATE:           12/12/2011                                                *
...
#include "sprd_codec_ap_phy_v1.h"
...
#define SPKR_REPLACE_SPKL 0
#if !(SPKR_REPLACE_SPKL)
#define SPKR_OUTPUT_SAME_AS_SPKL 1
#endif
...
{
    _sprd_codec_ap_en_op(en,&(audif_ptr->audif_enb),AUDIF_DAC_EN);
}

PUBLIC void __sprd_codec_ap_adc_en(uint32 en,uint32 chan)
{
    _sprd_codec_ap_en_op(en,&(audif_ptr->audif_enb),AUDIF_ADC_EN);
}
...
LOCAL void _sprd_codec_ap_stereo_open_state(uint32 en,uint32* p_l_val,uint32* p_r_val)
{
    ...
    reg1 = SPRD_CODEC_AP_REG_GET((uint32)(&(audif_ptr->daocr2)));
    if( ((reg1 & DAC_ADCPGAR_BUFN_SPN) ) )
    {
        r_val |= ADC_PGAR_BUFN_EN;
    }
    if( ((reg1 & DAC_ADCPGAR_BUFP_SPP) )
    ...
    if( ((reg1 & DAC_ADCPGAL_BUFN_SPN) )
     || ((reg & DAC_ADCPGAL_BUFN_HPR) )
    )
    {
        l_val |= ADC_PGAL_BUFN_EN;
    }
    if( ((reg1 & DAC_ADCPGAL_BUFP_SPP) )
...
PUBLIC void __sprd_codec_ap_ear_mixer(uint32 en)
{
    ...
    _sprd_codec_ap_chan_en(en,AUDIO_CHAN_L,&(audif_ptr->daocr4),l_val,0);
}
...
{
#if (SPKR_REPLACE_SPKL)
    __sprd_codec_ap_aor_mixer(en,chan);
#else
    uint32 l_val;
    uint32 r_val;
...
{
#if (SPKR_REPLACE_SPKL)
    __sprd_codec_ap_aor_mixer_adcbuf(en,chan);
#else
    uint32 l_val;
    uint32 r_val;
...
PUBLIC void __sprd_codec_ap_aor_mixer(uint32 en,uint32 chan)
{
    uint32 l_val;
    uint32 r_val;
    l_val = (DAC_DACL_AORN | DAC_DACL_AORP);
    r_val = (DAC_DACR_AORN | DAC_DACR_AORP);
    _sprd_codec_ap_chan_en(en,chan,&(audif_ptr->daocr3),l_val,r_val);
}
...
PUBLIC void __sprd_codec_ap_aor_mixer_adcbuf(uint32 en,uint32 chan)
{
    uint32 l_val;
    uint32 r_val;
    l_val = (DAC_ADCPGAL_BUFN_AORN | DAC_ADCPGAL_BUFP_AORP);
    r_val = (DAC_ADCPGAR_BUFN_AORN | DAC_ADCPGAR_BUFP_AORP);
    _sprd_codec_ap_chan_en(en,chan,&(audif_ptr->daocr3),l_val,r_val);
    __sprd_codec_ap_adc_pga_buf_pwr(en,chan);
}
...
{
    _sprd_codec_ap_en_op(en,&(audif_ptr->dcr1),AUD_AOLR_EN);
}
...
PUBLIC void __sprd_codec_ap_auxmic_bias(uint32 en)
{
    ...
    SPRD_CODEC_AUXMICBIAS_SLP((uint32)(!en));
}
