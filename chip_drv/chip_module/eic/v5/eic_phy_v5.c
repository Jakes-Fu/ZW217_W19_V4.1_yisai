...
LOCAL EIC_CALLBACK s_eic_latch_int_table[ EIC_LATCH_SIZE ] = {0};
LOCAL EIC_CALLBACK s_eic_async_int_table[ EIC_ASYNC_SIZE ] = {0};
LOCAL EIC_CALLBACK s_eic_sync_int_table[ EIC_SYNC_SIZE ] = {0};
LOCAL EIC_CALLBACK s_eica_dbnc_int_table[ EICA_DBNC_SIZE ] = {0};
LOCAL EIC_CALLBACK s_eica_latch_int_table[ EIC_LATCH_SIZE ] = {0};
...
/* EIC_DBNC/EIC_SYNC/EIC_ASYNC share the NO11 interrupt channel in SC6531 */
PUBLIC ISR_EXE_T EIC_ShareIsr( uint32 data )
{
    EICD_DbncIsr(data);
    EIC_A_SYNCIsr(data);
    return ISR_DONE;
...
PUBLIC void EIC_PHY_Open( void )
{
    ...
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_95_112_2_17_23_4_20_949,(uint8*)"");
    
    // Open D-die eic module
    SCI_ASSERT (DEVICE_Find ("EIC_D", &s_dev_eic_ddie) == CLOCK_SUCCESS); /*assert verified*/    
    DEVICE_Open(s_dev_eic_ddie);
    SCI_ASSERT (DEVICE_Find ("EIC_A", &s_dev_eic_adie) == CLOCK_SUCCESS); /*assert verified*/    
    DEVICE_Open(s_dev_eic_adie); 

    ISR_RegHandler (TB_ANA_EIC_INT, (TB_ISR)EICA_DbncIsr); 
    ...
    //enable EIC_latch INT and register its callback   
    ISR_RegHandler(TB_SPECIAL_LATCH_INT, (TB_ISR)EIC_LatchIsr);  
    CHIPDRV_EnableIRQINT( TB_SPECIAL_LATCH_INT ); 

#if defined(CHIP_VER_6531) || defined(CHIP_VER_6531EFM)
    ISR_RegHandler(TB_EIC_SYNC_ASYNC_INT, (TB_ISR)EIC_ShareIsr);   
    CHIPDRV_EnableIRQINT( TB_EIC_SYNC_ASYNC_INT); 
#else //CHIP_VER_6530
    // EIC_DBNC share int with GPIO, so GPIO_GpiHandler will call EICD_DbncIsr
    CHIPDRV_EnableIRQINT( TB_GPIO_EICDBC_INT);    

    // enable EIC_async/ECI_SYNC INT and register its callback     
    ISR_RegHandler(TB_EIC_SYNC_ASYNC_INT, (TB_ISR)EIC_A_SYNCIsr);   
    CHIPDRV_EnableIRQINT( TB_EIC_SYNC_ASYNC_INT);   
#endif
...
PUBLIC void EIC_PHY_Close( void )
{
    ...
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_133_112_2_17_23_4_20_950,(uint8*)"");
                           ...
                           BOOLEAN is_en)
{
    ...
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_149_112_2_17_23_4_20_951,(uint8*)"dd",chan_id, is_en);

    if(grp_type == EIC_DBNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_DBNC_INT_EN;
    else if(grp_type == EIC_LATCH)
        addr = eic_group_cfg_ptr->base_addr + EIC_LATCH_INT_EN; 
    else if(grp_type == EIC_ASYNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_ASYNC_INT_EN;            
    else if(grp_type == EIC_SYNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_SYNC_INT_EN;
    else
        SCI_PASSERT(0, ("EIC_PHY_SetTrg not support grptype=%d.",grp_type));/*assert verified*/  
...
/******************************************************************************/
//  Description:    Get the eic_dbnc(gpi) data.Must be masked in EIC_DBNC_DMSK
//  Author:         lichd
//  Note:           Only eic_dbnc support this feature
/******************************************************************************/
PUBLIC void EIC_PHY_MskData( EIC_GROUP_DES_T* eic_group_cfg_ptr, uint32 chan_id)
{
    ...
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_209_112_2_17_23_4_20_953,(uint8*)"d",chan_id);
    ...
    {
        SCI_PASSERT(0, ("EIC_PHY_GetData not support _TrgEn."));/*assert verified*/
    }
}
...
/******************************************************************************/
//  Description:    Set trig condition for the interrupt of a eic's channel
//  Author:         lichd
//  Note:           
/******************************************************************************/
                            ...
                            EIC_TRG_TYPE_E trg_type_e )
{
    ...
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_251_112_2_17_23_4_20_955,(uint8*)"dd",chan_id,trg_type_e);

    if(grp_type == EIC_DBNC)
    {
        addr = eic_group_cfg_ptr->base_addr + EIC_DBNC_INT_POL;
    ...
    {
        addr = eic_group_cfg_ptr->base_addr + EIC_LATCH_INT_POL;
        if(HIGH_LEVEL == trg_type_e)
        {
            EIC_REG_AND(addr, ~(uint32)(1<<chan_id)); 
        }
    ...
    else if(grp_type == EIC_ASYNC)
    {
        switch( trg_type_e )
        {
            case HIGH_LEVEL: 
            {
                addr = eic_group_cfg_ptr->base_addr + EIC_ASYNC_INT_MODE;
                EIC_REG_OR(addr, (1<<chan_id));
                addr = eic_group_cfg_ptr->base_addr + EIC_ASYNC_INT_POL;
                EIC_REG_OR(addr, (1<<chan_id));
                break;
            }
            case LOW_LEVEL:
            ...
            case RISING_EDGE:
            {
                addr = eic_group_cfg_ptr->base_addr + EIC_ASYNC_INT_BOTH;
                EIC_REG_AND(addr, ~(uint32)(1<<chan_id));
    ...
    else if(grp_type == EIC_SYNC)
    {
        switch( trg_type_e )
        {
            case HIGH_LEVEL: 
            {
                addr = eic_group_cfg_ptr->base_addr + EIC_SYNC_INT_MODE;
                EIC_REG_OR(addr, (1<<chan_id));
                addr = eic_group_cfg_ptr->base_addr + EIC_SYNC_INT_POL;
                EIC_REG_OR(addr, (1<<chan_id));
                break;
            }
            case LOW_LEVEL:
            ...
            case RISING_EDGE:
            {
                addr = eic_group_cfg_ptr->base_addr + EIC_SYNC_INT_BOTH;
                EIC_REG_AND(addr, ~(uint32)(1<<chan_id));
        ...
        SCI_PASSERT(0, ("EIC_PHY_SetTrg not support grptype."));/*assert verified*/        
}

/******************************************************************************/
//  Description:    Start trig after your configuration
//  Author:         lichd
//  Note:           Before the first int will be occured when the trig condition met,
//                  you must set StartTrg bit of a channel. 
/******************************************************************************/
PUBLIC void EIC_PHY_StartTrg( EIC_GROUP_DES_T* eic_group_cfg_ptr, uint32 chan_id )
{
    ...
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_411_112_2_17_23_4_21_956,(uint8*)"d",chan_id);
    ...
    {
        SCI_PASSERT(0, ("EIC_PHY_StartTrg not sup EIC_LATCH."));/*assert verified*/
    }

}

/******************************************************************************/
//  Description:    Set debounce count(ms) of a eic's channel
//  Author:         lichd
//  Note:           Only eic_dbnc support this feature          
/******************************************************************************/
PUBLIC void EIC_PHY_SetDbnc( EIC_GROUP_DES_T* eic_group_cfg_ptr, uint32 chan_id, uint32 cnt)
{
    ...
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_432_112_2_17_23_4_21_957,(uint8*)"dd",chan_id,cnt);
    ...
    {
        SCI_PASSERT(0, ("EIC_LATCH not support seting Debounce."));/*assert verified*/
    }
}
...
/******************************************************************************/
//  Description:    Get whether the int of a channel is met.
//                  TRUE -- INT; FALSE -- NO INT
//  Author:         lichd
//  Note:                  
/******************************************************************************/
PUBLIC uint32 EIC_PHY_GetIntSts( EIC_GROUP_DES_T* eic_group_cfg_ptr, uint32 chan_id)
{
    ...
    if(grp_type == EIC_DBNC)
        ...
        addr = eic_group_cfg_ptr->base_addr + EIC_LATCH_INT_RAWSTS; 
    else if(grp_type == EIC_ASYNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_ASYNC_INT_RAWSTS;            
    else if(grp_type == EIC_SYNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_SYNC_INT_RAWSTS;
    else
        SCI_PASSERT(0, ("EIC_PHY_GetIntSts not support grptype=%d.",grp_type));/*assert verified*/    
  
    return (EIC_REG_GET(addr) & (1<<chan_id));  
}

PUBLIC uint32 EIC_PHY_GetMskIntSts( EIC_GROUP_DES_T* eic_group_cfg_ptr, uint32 chan_id)
{
    uint32 addr     = 0;
    uint32 grp_type = eic_group_cfg_ptr->type;
    ...
    if(grp_type == EIC_DBNC)
        ...
        addr = eic_group_cfg_ptr->base_addr + EIC_LATCH_INT_MSKSTS; 
    else if(grp_type == EIC_ASYNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_ASYNC_INT_MSKSTS;            
    else if(grp_type == EIC_SYNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_SYNC_INT_MSKSTS;
    else
        SCI_PASSERT(0, ("EIC_PHY_GetMskIntSts not support grptype=%d.",grp_type));/*assert verified*/    
...
/******************************************************************************/
//  Description:    Clear the int flag of a channel.
//  Author:         lichd
//  Note:                  
/******************************************************************************/
PUBLIC void EIC_PHY_ClrIntSts( EIC_GROUP_DES_T* eic_group_cfg_ptr, uint32 chan_id)
{
    ...
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_542_112_2_17_23_4_21_958,(uint8*)"d",chan_id);

    if(grp_type == EIC_DBNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_DBNC_INT_CLR;
    else if(grp_type == EIC_LATCH)
        addr = eic_group_cfg_ptr->base_addr + EIC_LATCH_INT_CLR; 
    else if(grp_type == EIC_ASYNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_ASYNC_INT_CLR;            
    else if(grp_type == EIC_SYNC)
        addr = eic_group_cfg_ptr->base_addr + EIC_SYNC_INT_CLR;
    else
        SCI_PASSERT(0, ("EIC_PHY_SetTrg not support grptype=%d.",grp_type));/*assert verified*/  
...
PUBLIC ISR_EXE_T EICD_DbncIsr( uint32 data )
{
    ...
    for( i=0; i<EIC_DBNC_SIZE; i++)
    {
        ...
        if( EIC_PHY_GetMskIntSts(eic_group_cfg_ptr, i) )
        {
            ...
            {    
                SCI_PASSERT(0, ("EICD_DbncIsr chan[%d]'s callback is PNULL.", i));/*assert verified*/
            }
            ...
            SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_589_112_2_17_23_4_21_959,(uint8*)"d", i);
...
{ 
    SCI_ASSERT( chan_id < EIC_DBNC_SIZE); /*assert to do*/

    //EICDRV_TraceLow:"EICD_Dbnc_RegCallback"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_758_112_2_17_23_4_21_961,(uint8*)"");
    ...
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,EIC_PHY_V5_766_112_2_17_23_4_21_962,(uint8*)"");
    }
}
