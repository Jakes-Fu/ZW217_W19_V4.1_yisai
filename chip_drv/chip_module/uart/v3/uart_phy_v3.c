...
//Global controls.
LOCAL __inline void UART_GLOBAL_Reg (uint32 port, BOOLEAN enable)
{
    switch (port)
    {
        case UART_COM0:
...
{
    {ARM_UART0_BASE, ARM_UART1_BASE, ARM_UART2_BASE, 0},/*lint !e785 confirmed by xuepeng*/
...
PUBLIC void UART_InitUartLowLevel (uint32 port, uint32 baudrate)
{
    ...
    REG32 (uart_base + ARM_UART_IEN)   = 0x00;
    REG32 (uart_base + ARM_UART_CTL0)  = 0x1C;
    REG32 (uart_base + ARM_UART_CTL1)  = 0x00;
    REG32 (uart_base + ARM_UART_CLKD0) = (CHIP_GetAPBClk() + (baudrate / 2)) / (baudrate);
    REG32 (uart_base + ARM_UART_CLKD1) = 0x00;
}
                                  ...
                                  UART_INIT_PARA_T *para)
{
    ...
    if (DS_WAKEUP_ENABLE == para->ds_wakeup_en)
    {
        ...
        if (port == UART_COM0)
        {
            if (type == DS_WAKEUP_BY_CTS)
            {
                REG32 (INT_UINT_CTL) |= ( (0x1<<1) | (0x1<<17));
            }
            else if (type == DS_WAKEUP_BY_RXD)
            {
                REG32 (INT_UINT_CTL) |= ( (0x1<<0) | (0x1<<16));
            }
        }
        else if (type == DS_WAKEUP_BY_RXD && port == UART_COM1)
        {
            REG32 (INT_UINT_CTL) |= ( (0x1<<2) | (0x1<<18));
        }
        else if (type == DS_WAKEUP_BY_RXD && port == UART_COM2)
        {
            REG32 (INT_UINT_CTL) |= ( (0x1<<3) | (0x1<<19));
        }
...
//  Note:          NONE                                                      *
/*****************************************************************************/
void UART_PHY_SetWordSize (uint32 port, UART_BYTE_SIZE_E byte_size)
{
    uint32 ctl0;
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:          NONE                                                      *
/*****************************************************************************/
void UART_PHY_SetStopBits (uint32 port, UART_STOP_BIT_E stop_bits)
{
    uint32 ctl0;
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:          NONE                                                      *
/*****************************************************************************/
void UART_PHY_SetTxWaterMark (uint32 port, uint16 tx_watermark)
{
    uint32 watermark;
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:          NONE                                                      *
/*****************************************************************************/
uint16 UART_PHY_GetTxWaterMark (uint32 port)
{
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:          NONE                                                      *
/*****************************************************************************/
void UART_PHY_SetRxWaterMark (uint32 port, uint16 rx_watermark)
{
    uint32 watermark;
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:          NONE                                                      *
/*****************************************************************************/
uint16 UART_PHY_GetRxWaterMark (uint32 port)
{
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:           NONE
/*****************************************************************************/
uint32 UART_PHY_GetTxFifoCnt (uint32 port)
{
    uint32 tx_fifo_cnt;
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:           NONE
/*****************************************************************************/
BOOLEAN UART_PHY_GetIsTxFifoBusy (uint32 port)
{
    return 0;
}
...
//  Note:           NONE
/*****************************************************************************/
uint32 UART_PHY_GetRxFifoCnt (uint32 port)
{
    uint32 rx_fifo_cnt;
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:          NONE                                                      *
/*****************************************************************************/
void UART_PHY_SetFlowCtl (uint32 port, UART_FLOW_CTR_E is_enable)
{
    uint32 flow_ctl;
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:          NONE                                                      *
/*****************************************************************************/
void UART_PHY_SetDmaMode (uint32 port, BOOLEAN is_enable)
{
    uint32 ctl1;
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:          NONE                                                      *
/*****************************************************************************/
void UART_PHY_SetSelfLoop (uint32 port, BOOLEAN is_enable)
{
    uint32 ctl1;
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
//  Note:          NONE
/*****************************************************************************/
uint32 UART_PHY_GetFifoSize (uint32 port)
{
...
//  Note:           NONE
/*****************************************************************************/
void  UART_PHY_WaiTxFifoEmpty (uint32 port, uint32 fifo_size)
{
    uint32   cur_tick     = SCI_GetTickCount();
    uint32   next_tick   = SCI_GetTickCount();
    volatile UART_REG_S *p = (volatile UART_REG_S *) (hw_config.baseAddr[port-UART_COM0]);
...
void UART_PHY_Arbitrate (uint32 pord_id, uint is_ARM)
{
    if (is_ARM)
    {
        if (1 == pord_id)
        {
            ...
            REG32 (GR_PCTL) &= ~MISC0_UART1_MUX_SEL;
        }
        else
        {
...
serial_config_funs serial_set_func =
{
...
serial_get_funs serial_get_func =
{
