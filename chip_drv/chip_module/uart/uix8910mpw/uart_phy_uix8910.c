...
PUBLIC void UART_InitUartLowLevel (uint32 port, uint32 baudrate)
{
    ...
    p->uart_conf = (ARM_UART_TXRST_TX_FIFO_RESET | ARM_UART_RXRST_RX_FIFO_RESET);
    UART_PHY_BaudSet(p, baudrate);
}
    ...
    while (!threadReadBuffer (&port))
    {
        ...
        if (s_uart_port[port].callback != NULL)
        {
            if (mask_sts & (uint32) ARM_UART_TX_INT)
            {
                ...
                uart_reg_ptr->uart_conf &= ~ (ARM_UART_TX_IE_TX_IE_ENABLE);
...
LOCAL int UART_PHY_BaudSet(HWP_ARM_UART_T *puart, uint32 baudrate)
{    
    uint32 n_set, n_temp;    
    ...
    {       
        ...
        {           
            baud_div_temp = UART_BAUD_BASE_CLK/(n_temp*baudrate);           
            remain = UART_BAUD_BASE_CLK%(n_temp*baudrate);          
            if(remain > (n_temp*baudrate/2))            
            {               
                baud_div_temp++;            
            }                       
            baudrate_real = UART_BAUD_BASE_CLK/(n_temp * baud_div_temp);            
                                  ...
                                  UART_INIT_PARA_T *para)
{
    ...
    p->uart_conf &= ~ (ARM_UART_TOUT_IE_TOUT_IE_ENABLE | ARM_UART_TX_IE_TX_IE_ENABLE | ARM_UART_RX_IE_RX_IE_ENABLE);

    if (para->parity_enable != PARITY_DISABLE)
    {
        config |= (ARM_UART_CHECK_CHECK_ENABLE | (para->parity << 1));
    }

    config |= ((ONE_AND_HALF_BIT == para->stop_bits) || (TWO_STOP_BIT == para->stop_bits)) ? (ARM_UART_STOP_BIT_2BIT_STOP) : (ARM_UART_STOP_BIT_1BIT_STOP);
    ...
    {
        config |= (ARM_UART_TOUT_HWFC_TOUT_HWFC_ENABLE | ARM_UART_RX_TRIG_HWFC_RX_TRIG_HWFC_ENABLE | ARM_UART_HWFC_HWFC_ENABLE);     // enable Rx/Tx hardware_flow control
    }
    ...
    config |= (ARM_UART_TX_IE_TX_IE_ENABLE | ARM_UART_RX_IE_RX_IE_ENABLE | ARM_UART_TOUT_IE_TOUT_IE_ENABLE);
...
LOCAL BOOLEAN UART_PHY_IsHwFlowControl (HWP_ARM_UART_T *p)
{
    ...
    if (p->uart_conf & (ARM_UART_HWFC_HWFC_ENABLE | ARM_UART_RX_TRIG_HWFC_RX_TRIG_HWFC_ENABLE | ARM_UART_TOUT_HWFC_TOUT_HWFC_ENABLE))
    {
        condition = TRUE;
    }
...
uint32 UART_PHY_WriteData (uint32 port, const uint8 *buffer, uint32 length, BOOLEAN irq_en)
{
    ...
    if (irq_en)
    {
        p->uart_conf |= (ARM_UART_TX_IE_TX_IE_ENABLE);
    }
...
void UART_PHY_SetBaudSpeed (uint32 port, uint32 baudrate)
{
    ...
    UART_PHY_BaudSet(p, baudrate_tmp);
}
...
void UART_PHY_SetParity (uint32 port, UART_PARITY_E parity)
{
    ...
    ctl0 = (p->uart_conf & ~ARM_UART_PARITY_ODD_PARITY);
    ctl0 |= (parity << 1);
    p->uart_conf = ctl0;
}
...
void UART_PHY_SetParityEnable (uint32 port, UART_PARITY_SET_E is_enable)
{
    ...
    ctl0 = (p->uart_conf & ~ARM_UART_CHECK_CHECK_ENABLE);
    ctl0 |= (is_enable << 0);
    p->uart_conf = ctl0;
}
...
UART_PARITY_SET_E UART_PHY_GetParityEnable (uint32 port)
{
    ...
    ctl0 = (p->uart_conf & ARM_UART_CHECK_CHECK_ENABLE) >> 0;
...
PUBLIC void UART_PHY_SetStopBits (uint32 port, UART_STOP_BIT_E stop_bits)
{
    ...
    ctl0 = (p->uart_conf & ~ARM_UART_STOP_BIT_2BIT_STOP);
    ctl0 |= ((ONE_AND_HALF_BIT == stop_bits) || (TWO_STOP_BIT == stop_bits)) ? (ARM_UART_STOP_BIT_2BIT_STOP) : (ARM_UART_STOP_BIT_1BIT_STOP);
    p->uart_conf = ctl0;
}
...
PUBLIC void UART_PHY_SetFlowCtl (uint32 port, UART_FLOW_CTR_E is_enable)
{
    ...
    {
        p->uart_conf |= (uint32) (ARM_UART_HWFC_HWFC_ENABLE | ARM_UART_RX_TRIG_HWFC_RX_TRIG_HWFC_ENABLE | ARM_UART_TOUT_HWFC_TOUT_HWFC_ENABLE);
    }
    else
    {
        p->uart_conf &= ~ (uint32) (ARM_UART_HWFC_HWFC_ENABLE | ARM_UART_RX_TRIG_HWFC_RX_TRIG_HWFC_ENABLE | ARM_UART_TOUT_HWFC_TOUT_HWFC_ENABLE);
...
PUBLIC void UART_PHY_SetDmaMode (uint32 port, BOOLEAN is_enable)
{
    ...
    if (is_enable)
    {
        p->uart_conf &= ~ (uint32) (ARM_UART_RX_IE_RX_IE_ENABLE | ARM_UART_TX_IE_TX_IE_ENABLE | ARM_UART_TOUT_IE_TOUT_IE_ENABLE);
    }
    else
    {
        p->uart_conf |= (uint32) (ARM_UART_RX_IE_RX_IE_ENABLE | ARM_UART_TX_IE_TX_IE_ENABLE | ARM_UART_TOUT_IE_TOUT_IE_ENABLE);
...
PUBLIC void UART_PHY_RxIntEnable (uint32 port, BOOLEAN is_enable)
{
    ...
    if (is_enable)
    {
        p->uart_conf |= (ARM_UART_RX_IE_RX_IE_ENABLE | ARM_UART_TOUT_IE_TOUT_IE_ENABLE);
    }
    else
    {
        p->uart_conf &= ~ (ARM_UART_RX_IE_RX_IE_ENABLE | ARM_UART_TOUT_IE_TOUT_IE_ENABLE);
