...
)
{
    ...
    if ( (arm_ahb_clk_level < freq_cont_ptr->min_clk_lvl) || (arm_ahb_clk_level > freq_cont_ptr->max_clk_lvl))
    {
        valid = SCI_FALSE;
    }
    else
...
)
{
    ...
    if (freq_type_index < FREQ_INDEX_CUSTOMIZED_BASE)
    {
        ...
        while (app_clk_lvl_table_ptr[i].freq_index != FREQ_INDEX_MAX)
        {
            if (app_clk_lvl_table_ptr[i].freq_index == freq_type_index)
            {
                arm_ahb_clk_level = app_clk_lvl_table_ptr[i].clk_lvl;
                break;
            }
            else
            {
...
)
{
    ...
    if (_FREQ_PHY_IsChngFreqEnable())
    {
        ...
        ahb_clk_to_set = clk_lvl_table_ptr[arm_ahb_clk_level-1].ahb_clk;
