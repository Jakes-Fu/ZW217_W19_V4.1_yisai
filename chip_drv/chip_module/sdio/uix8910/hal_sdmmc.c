...
void hal_set_sdmmcId(uint8 cardId)
{
    ...
    } else if (cardId == MCD_CARD_ID_1) {
        g_sdmmcId = MCD_CARD_ID_1;
        g_hwpSdmmcCtrl = hwp_sdmmc2;
    }
}
  ...
  HAL_ERR_T hal_SdmmcTransfer(HAL_SDMMC_TRANSFER_T* transfer)
{
#if 1
    uint8 channel = 0;
    HAL_APIFC_REQUEST_ID_T ifcReq = HAL_IFC_NO_REQWEST;
    uint32 length = 0;
    uint32 lengthExp = 0;
    ...
    ifcReq = HAL_IFC_SDMMC1_RX;
    switch (transfer->direction)
    {
        case HAL_SDMMC_DIRECTION_READ:
            ifcReq = (g_sdmmcId == MCD_CARD_ID_1) ? HAL_IFC_SDMMC2_RX : HAL_IFC_SDMMC1_RX;
            break;

        case HAL_SDMMC_DIRECTION_WRITE:
            ifcReq = (g_sdmmcId == MCD_CARD_ID_1) ? HAL_IFC_SDMMC2_TX : HAL_IFC_SDMMC1_TX;
            break;
...
//hal_ApIfcOpen;
    channel = hal_ApIfcTransferStart(ifcReq, transfer->sysMemAddr,
  ...
  BOOLEAN hal_SdmmcTransferDone(void)
{
    ...
    HAL_APIFC_REQUEST_ID_T ifcReqId = HAL_IFC_NO_REQWEST;
    SCI_DisableIRQ();
    ...
    {
     ifcReqId = (g_sdmmcId == MCD_CARD_ID_1) ? HAL_IFC_SDMMC2_RX : HAL_IFC_SDMMC1_RX;
        ...
        if ((hwp_sdmmc->SDMMC_INT_STATUS & SDMMC_DAT_OVER_INT)
               && hal_ApIfcGetTc(ifcReqId, g_halSdmmcReadCh) == 0)
        {
            // Transfer is over
            hwp_sdmmc->SDMMC_INT_CLEAR = SDMMC_DAT_OVER_CL;
            hal_ApIfcChannelRelease(ifcReqId, g_halSdmmcReadCh);
    ...
    {
     ifcReqId = (g_sdmmcId == MCD_CARD_ID_1) ? HAL_IFC_SDMMC2_TX : HAL_IFC_SDMMC1_TX;
        // Operation in progress is a write.
        // The SDMMC module itself can know it has finished
       // sdmmc_event(0xdd1100a1);sdmmc_event(g_halSdmmcWriteCh);
        if ((hwp_sdmmc->SDMMC_INT_STATUS & SDMMC_DAT_OVER_INT)
               && hal_ApIfcGetTc(ifcReqId, g_halSdmmcWriteCh) == 0)
        {
            // Transfer is over
            hwp_sdmmc->SDMMC_INT_CLEAR = SDMMC_DAT_OVER_CL;
            hal_ApIfcChannelRelease(ifcReqId, g_halSdmmcWriteCh);
  ...
  HAL_ERR_T hal_SdmmcStopTransfer(HAL_SDMMC_TRANSFER_T* transfer)
{
    ...
    {
     ifcReqId = (g_sdmmcId == MCD_CARD_ID_1) ? HAL_IFC_SDMMC2_RX : HAL_IFC_SDMMC1_RX;
        hal_ApIfcChannelFlush(ifcReqId, g_halSdmmcReadCh);
        while(!hal_ApIfcChannelIsFifoEmpty(ifcReqId, g_halSdmmcReadCh));
        hal_ApIfcChannelRelease(ifcReqId, g_halSdmmcReadCh);
        g_halSdmmcReadCh = HAL_UNKNOWN_CHANNEL;
    }
    else
    {
    ifcReqId = (g_sdmmcId == MCD_CARD_ID_1) ? HAL_IFC_SDMMC2_TX : HAL_IFC_SDMMC1_TX;
        hal_ApIfcChannelFlush(ifcReqId, g_halSdmmcWriteCh);
        while(!hal_ApIfcChannelIsFifoEmpty(ifcReqId, g_halSdmmcWriteCh));
        hal_ApIfcChannelRelease(ifcReqId, g_halSdmmcWriteCh);
        g_halSdmmcWriteCh = HAL_UNKNOWN_CHANNEL;
    }
    return HAL_ERR_NO;
