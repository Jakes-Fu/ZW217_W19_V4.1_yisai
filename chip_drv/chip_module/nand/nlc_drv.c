...
#define NF_TIMEOUT_VAL      70                 //the timeout value is 70ms
#define NF_TIMEOUT0_VAL		4
#define NF_TIMEOUT1_VAL		20
#define PAGE_SIZE_S         512
#define SPARE_SIZE_S        16
#define PAGE_SIZE_L         2048
#define SPARE_SIZE_L        64
#define NF_READ_TRY_TIMES	3
...
#ifdef __FOTA_COMPILE__ //close vm in fota.
#undef VM_SUPPORT
#endif
...
#define NFC_WAIT_COMMAND_FINISH                                                    \
    {                                                                              \
        ...
        {                                                                          \
            _NF_RegSave();															\
            SCI_PASSERT(0,("NFC timeout happened!"));/*assert to do*/              \
...
#define MAX_SAVED_NFC_REG								(17)
#define NAND_TIMEOUT_DBG_SUPPORT
#else
#define MAX_SAVED_NFC_REG								(15)
#endif
/******************************************************************************
                          Struct Definition
******************************************************************************/
...
LOCAL NF_STATE_E s_nf_state1 = NF_STATE_IDLE;
LOCAL ERR_NF_E   s_nf_operation_status = ERR_NF_SUCCESS;
LOCAL BOOLEAN    s_conv_flag = SCI_FALSE;
LOCAL uint32 s_nf_reg_saved[MAX_SAVED_NFC_REG];
LOCAL BOOLEAN s_NandHwOnDieEccEnable = FALSE;
...
******************************************************************************/
/******************************************************************************/
//  Description:   save the nand controller register for dbg use
//  Parameter:
//      None
//  Return:
//      None
/******************************************************************************/
LOCAL void _NF_RegSave()
{
	s_nf_reg_saved[0] = REG32(NFC_CMD);
	s_nf_reg_saved[1] = REG32(NFC_STR0);
	s_nf_reg_saved[2] = REG32(NFC_STR1);
	s_nf_reg_saved[3] = REG32(NFC_STR2);
	s_nf_reg_saved[4] = REG32(NFC_STR3);
	s_nf_reg_saved[5] = REG32(NFC_END0);
	s_nf_reg_saved[6] = REG32(NFC_END1);
	s_nf_reg_saved[7] = REG32(NFC_END2);
	s_nf_reg_saved[8] = REG32(NFC_END3);
	s_nf_reg_saved[9] = REG32(NFC_PARA);
	s_nf_reg_saved[10] = REG32(NFC_ID_STATUS);
	s_nf_reg_saved[11] = REG32(NFC_RB);
	s_nf_reg_saved[12] = REG32(NFC_INT_SRC);
	s_nf_reg_saved[13] = REG32(NFC_TIMEOUT);	
	s_nf_reg_saved[14] = REG32(NFC_CMD_SET);
#ifdef SC6800H_PLATFORM	
	s_nf_reg_saved[15] = REG32(LCM_STATUS); //LCM status
	s_nf_reg_saved[16] = REG32(LCM_TRANS); //LCM transfer control
#endif	
}
...
LOCAL void _NF_StateSync (void)
{
    ...
    switch (state)
    {
        ...
        case NF_STATE_WRITE_WAIT:
            {
                uint32 nfc_cmd;
                uint32 counter;

                NFC_WAIT_COMMAND_FINISH /*lint !e527 comfirmed by xuepeng*/
        ...
        case NF_STATE_ERASE:
        case NF_STATE_COPY_START:

            TM_SendTestPointRequest (0x6e616e64, 6);
...
LOCAL ERR_NF_E nc_ecc_trans (uint8 *pEccIn, uint8 *pEccOut,uint8 nSct)
{
...
LOCAL ERR_NF_E _nand_mc_excute_cmd (uint16 cmd, Nand_MC_ECC_E ecc_flag_e, Nand_MC_BUSWIDTH_E bus_width_e)
{
    ...
    CHIP_REG_OR (NFC_CMD_SET, NFC_MC_MODE_ENABLE);

    CHIP_REG_SET (NFC_CMD, val);

    return ERR_NF_SUCCESS;
}


LOCAL ERR_NF_E _nand_reset_mc (void)
{
    _nand_mc_init();

    _nand_mc_storebuf (NAND_MC_ASMNOP (0xf));

    _nand_mc_excute_cmd (NAND_MC_ASMCMD (NF_RESET), NAND_MC_ECC_DISABLE, NAND_MC_BUSWIDTH_X8);

    SCI_Sleep (2);

    return ERR_NF_FAIL;//add return value for pclint warning
}
...
)
{
    ...
    _nand_mc_excute_cmd (NAND_MC_ASMNOP (1), NAND_MC_ECC_DISABLE, bus_width_e); /*lint !e778 confirmed by xuepeng*/

    _NF_SetState (NF_STATE_WRITE_WAIT);
    ...
    if (NF_STATE_WRITE_WAIT!=_NF_GetState())
    {
...
LOCAL ERR_NF_E _nand_read_s_mc (uint32 page_addr, BOOLEAN bReadSpare, BOOLEAN bReadMain)
{
    uint32 nfc_cmd;
    uint32 counter;
    NAND_CTL_REG_T  *nandctl_ptr= (NAND_CTL_REG_T *) NAND_CTL_BASE;
    uint32 nand_addr = 0;
    uint8  col_addr0, col_addr1, row_addr0, row_addr1;

    nand_addr = page_addr;

    col_addr0= nand_addr & 0x00ff;
    nand_addr = nand_addr>>9;

    col_addr1 =nand_addr & 0x00ff;
    nand_addr = nand_addr>>8;
    ...
    if (SCI_TRUE ==  bReadMain)
    {
        _nand_mc_storebuf (NAND_MC_ASMRBLOCK (2));
    }

    if (SCI_TRUE ==  bReadSpare)
    {
        _nand_mc_storebuf (NAND_MC_ASMRWORD (0x10));
    }
#ifdef NAND_TIMEOUT_DBG_SUPPORT	
	if(SCI_FALSE ==  bReadMain) //add a switch command
	{
		_nand_mc_storebuf (NAND_MC_ASMCMD (NF_READ_1ST));
	}
#endif	
    _nand_mc_storebuf (NAND_MC_ASMNOP (1)); /*lint !e778 confirmed by xuepeng*/
    ...
    if (SCI_TRUE ==  bReadMain)
    {
        _nand_mc_excute_cmd (NAND_MC_ASMCMD (NF_READ_1ST), NAND_MC_ECC_ENABLE, NAND_MC_BUSWIDTH_X8);
    }
    else if (SCI_TRUE ==  bReadSpare)
    {
        _nand_mc_excute_cmd (NAND_MC_ASMCMD (NF_READ_SPARE), NAND_MC_ECC_DISABLE, NAND_MC_BUSWIDTH_X8);
    }
#ifndef NAND_TIMEOUT_DBG_SUPPORT
    //Read nfc_cmd until the command has been executed completely.
    NFC_WAIT_COMMAND_FINISH /*lint !e527 comfirmed by xuepeng*/
#endif
    return ERR_NF_SUCCESS;
...
LOCAL ERR_NF_E _MT29F4G_ecc_correct(struct _HwECCCtrl *ctl)
{
    ...
    {
        return ERR_NF_REWRITE_RECOMMAND;
    }
    else
    {
        return ERR_NF_SUCCESS;
...
PUBLIC ERR_NF_E NANDCTL_Init (uint32 ahb_clk)
{
    ...
    REG32 (NFC_PARA) =  ORIGINAL_NAND_TIMING | 0xf<<4;

    //Clear all Nand INT
    REG32 (NFC_INT_SRC) |= (BIT_0|BIT_4|BIT_5);
...
{
    uint32 counter, i, temp, nCycle,nfc_cmd;
    BOOLEAN bFind;
    ...
    else
    {
        ...
        REG32 (NFC_PARA) =  g_Nand_Ahb_Timing[i].timing_param;/*lint !e661 confirmed by xuepeng*/
        //REG32 (NFC_PARA) =  ORIGINAL_NAND_TIMING| (nCycle<<4);
    }
...
#ifdef NAND_TIMEOUT_DBG_SUPPORT
/******************************************************************************/
//  Description:   clear the nand command 
//  Parameter:
//      None
//  Return:
//      None
/******************************************************************************/
LOCAL void _NANDCTL_CmdClear()
{
	int i;
	REG32(NFC_CMD_CLR) = 1;
	//wait for a while
...
/******************************************************************************/
PUBLIC ERR_NF_E _NANDCTL_Wait(uint32 time)
{
    ...
    nfc_cmd = nandctl_ptr->cmd;
	counter=SCI_GetTickCount();  
	while((nfc_cmd&(BIT_31))&&(SCI_GetTickCount()-counter< time))
	{
		nfc_cmd=nandctl_ptr->cmd;
	}
	if(nandctl_ptr->cmd&(BIT_31)) 
...
//      ERR_NF_SUCCESS    Read nand successfully
/******************************************************************************/
LOCAL ERR_NF_E _NANDCTL_WaitTimeAndCmdClear(uint32 time)
{
	uint32 nfc_cmd;
	uint32 counter;		
	NAND_CTL_REG_T  *nandctl_ptr= (NAND_CTL_REG_T *) NAND_CTL_BASE;
    ...
    nfc_cmd = nandctl_ptr->cmd;
	counter=SCI_GetTickCount();  
	while((s_nf_state1 != NF_STATE_CMD_CLR) && (nfc_cmd&(BIT_31))&&(SCI_GetTickCount()-counter< time))
	{
		nfc_cmd=nandctl_ptr->cmd;
	}
	if(nandctl_ptr->cmd&(BIT_31))
...
)
{
    ...
    if(s_nf_state1 != NF_STATE_READ_WAIT)
    {
    	for(rd_cnt = 0; rd_cnt < NF_READ_TRY_TIMES; rd_cnt ++)
    	{
    		NLC_DISABLEIRQ
	    	_nand_read_s_mc (page_addr, SCI_TRUE, SCI_TRUE);
			ret = _NANDCTL_Wait(NF_TIMEOUT0_VAL);	
			if(ERR_NF_SUCCESS != ret) 
			{
				s_nf_state1 = NF_STATE_READ_WAIT;
				NLC_RESTOREIRQ
				ret = _NANDCTL_WaitTimeAndCmdClear(NF_TIMEOUT1_VAL);	
				NLC_DISABLEIRQ
				if((s_nf_state1 != NF_STATE_READ_WAIT) || (ERR_NF_SUCCESS != ret))
				{
					_NANDCTL_CmdClear();
					s_nf_state1 = NF_STATE_IDLE;
					NLC_RESTOREIRQ	
					continue;
				}
			}
			s_conv_flag = SCI_TRUE;
			NandCopy ( (uint8 *) (NFC_MBUF_ADDR + PAGE_SIZE_S), (uint8 *) pSBuf,SPARE_SIZE_S);
			s_conv_flag = SCI_FALSE;
			if (NULL!=pEcc)
			{
				*pEcc_val = EndianConv_32 (nandctl_ptr->ecc0);
	            ...
	            if ( (pEcc[0]!=pSBuf[g_Nand_Para.eccpos]) ||
	                    (pEcc[1]!=pSBuf[g_Nand_Para.eccpos+1]) ||
	                    (pEcc[2]!=pSBuf[g_Nand_Para.eccpos+2]))
	            {
	            	s_nf_state1 = NF_STATE_IDLE;
	            	NLC_RESTOREIRQ
	                continue;
			...
			NandCopy ( (uint8 *) NFC_MBUF_ADDR, (uint8 *) pMBuf,PAGE_SIZE_S);
			s_nf_state1 = NF_STATE_IDLE;
			NLC_RESTOREIRQ
			break;
    	}
		if(NF_READ_TRY_TIMES == rd_cnt)
		{
			_NF_RegSave();
			SCI_PASSERT(0,("NFC svc read timeout happened!"));/*assert to do*/ 
		}
    }
	else
	{
		s_nf_state1 = NF_STATE_CMD_CLR;
		//in abort mode don't need disable irq
		for(rd_cnt = 0; rd_cnt < NF_READ_TRY_TIMES; rd_cnt ++)
		{
			_NANDCTL_CmdClear();
			_nand_read_s_mc (page_addr, SCI_TRUE, SCI_TRUE);
			ret = _NANDCTL_Wait(NF_TIMEOUT1_VAL); //abort mode, wait long time
			if(ERR_NF_SUCCESS == ret)
			{
				s_conv_flag = SCI_TRUE;
				NandCopy ( (uint8 *) (NFC_MBUF_ADDR + PAGE_SIZE_S), (uint8 *) pSBuf,SPARE_SIZE_S);
		...
		{
			_NF_RegSave();
			SCI_PASSERT(0,("NFC abort read timeout happened!"));/*assert to do*/ 
		}
	}
    return ERR_NF_SUCCESS;
...
)
{
    ...
    if(s_nf_state1 != NF_STATE_READ_WAIT)
    {
    	...
    	{
    		NLC_DISABLEIRQ
	    	_nand_read_s_mc (page_addr, SCI_FALSE, SCI_TRUE);
			ret = _NANDCTL_Wait(NF_TIMEOUT0_VAL);	
			if(ERR_NF_SUCCESS != ret) 
			{
				s_nf_state1 = NF_STATE_READ_WAIT;
			...
			NandCopy ( (uint8 *) NFC_MBUF_ADDR,pMBuf,PAGE_SIZE_S);
			s_nf_state1 = NF_STATE_IDLE;
			NLC_RESTOREIRQ
			break;
    	}
	...
	else
	{
		...
		for(rd_cnt = 0; rd_cnt < NF_READ_TRY_TIMES; rd_cnt ++)
		{
			_NANDCTL_CmdClear();
			_nand_read_s_mc (page_addr, SCI_FALSE, SCI_TRUE);
			ret = _NANDCTL_Wait(NF_TIMEOUT1_VAL); //abort mode, wait time long
			if(ERR_NF_SUCCESS == ret)
			{
				NandCopy ( (uint8 *) NFC_MBUF_ADDR,pMBuf,PAGE_SIZE_S);
				break;
		...
		{
			_NF_RegSave();
			SCI_PASSERT(0,("NANDCTL_M_Read_S abort read timeout happened!"));/*assert to do*/ 
		}
	}
    return ERR_NF_SUCCESS;
}
...
)
{
    ...
    if(s_nf_state1 != NF_STATE_READ_WAIT)
    {
    	...
    	{
    		NLC_DISABLEIRQ
	    	_nand_read_s_mc (page_addr, SCI_TRUE, SCI_FALSE);
			ret = _NANDCTL_Wait(NF_TIMEOUT0_VAL);	
			if(ERR_NF_SUCCESS != ret) 
			{
				s_nf_state1 = NF_STATE_READ_WAIT;
			...
			s_conv_flag = SCI_TRUE;
			NandCopy ( (uint8 *) (NFC_MBUF_ADDR), (uint8 *) pSBuf,SPARE_SIZE_S);
			s_conv_flag = SCI_FALSE;
			s_nf_state1 = NF_STATE_IDLE;
			NLC_RESTOREIRQ
			break;
...
PUBLIC ERR_NF_E NANDCTL_MS_Write_S (uint32 page_addr,uint8 *pMBuf,uint8 *pSBuf,uint8 ecc_en)
{
    ...
    _NF_SetState (NF_STATE_WRITE_START);
...
{
    uint32 cmd,nfc_cmd;
    uint32 counter,rd_cnt;
    BOOLEAN bReadNand = FALSE;
    NAND_CTL_REG_T  *nandctl_ptr= (NAND_CTL_REG_T *) NAND_CTL_BASE;
    ...
    ERR_NF_E mRet = ERR_NF_SUCCESS;
    ERR_NF_E sRet = ERR_NF_SUCCESS;

    NLC_DISABLEIRQ//set start_addr and end_addr
    ...
    if (NULL!=pEcc)
    {
        for (rd_cnt = 0; (rd_cnt<=3) && (!bReadNand); rd_cnt ++)
        {
            bReadNand = TRUE;
            ...
            s_conv_flag = SCI_TRUE;
            NandCopy ( (uint8 *) NFC_SBUF_ADDR,pSBuf,nSct*16);
            s_conv_flag = SCI_FALSE;
                ...
                switch (nSct)
                {
                    ...
                    case 2:
                        ...
                        if ( (pEcc[0]!=pSBuf[g_Nand_Para.eccpos]) ||
                                (pEcc[1]!=pSBuf[g_Nand_Para.eccpos+1]) ||
                                (pEcc[2]!=pSBuf[g_Nand_Para.eccpos+2]) ||
                                (pEcc[4]!=pSBuf[g_Nand_Para.eccpos+16]) ||
                                (pEcc[5]!=pSBuf[g_Nand_Para.eccpos+17]) ||
                                (pEcc[6]!=pSBuf[g_Nand_Para.eccpos+18]))
                        {
                            bReadNand = FALSE;
                        }
                    ...
                    case 3:
                        ...
                        if ( (pEcc[0]!=pSBuf[g_Nand_Para.eccpos]) ||
                                ...
                                (pEcc[5]!=pSBuf[g_Nand_Para.eccpos+17]) ||
                                (pEcc[6]!=pSBuf[g_Nand_Para.eccpos+18]) ||
                                (pEcc[8]!=pSBuf[g_Nand_Para.eccpos+32]) ||
                                (pEcc[9]!=pSBuf[g_Nand_Para.eccpos+33]) ||
                                (pEcc[10]!=pSBuf[g_Nand_Para.eccpos+34]))
                        {
                            bReadNand = FALSE;
                        }
                        break;
                    case 4:
                        ...
                        if ( (pEcc[0]!=pSBuf[g_Nand_Para.eccpos]) ||
                                ...
                                (pEcc[9]!=pSBuf[g_Nand_Para.eccpos+33]) ||
                                (pEcc[10]!=pSBuf[g_Nand_Para.eccpos+34]) ||
                                (pEcc[12]!=pSBuf[g_Nand_Para.eccpos+48]) ||
                                (pEcc[13]!=pSBuf[g_Nand_Para.eccpos+49]) ||
                                (pEcc[14]!=pSBuf[g_Nand_Para.eccpos+50]))
                        {
                            bReadNand = FALSE;
                        }
                        break;
    ...
    { 
        ...
        if(s_NandHwOnDieEccEnable)
        {
		        mRet = NANDCTL_ONDIE_ECCCorrect();
		        if(mRet != ERR_NF_SUCCESS)
		        {
		           continue;
		        }
		        break; // if read and check sucessfully
		    }
		    else
		    {
		    	break; //if ondie ecc not enable, only read once and don't check
		    }
		}
    	for (rd_cnt = 0; (rd_cnt <= 3); rd_cnt ++)
		{		
        ...
        if(s_NandHwOnDieEccEnable)
        {
            	sRet = NANDCTL_ONDIE_ECCCorrect();
        	    if(sRet != ERR_NF_SUCCESS)
    	        {
					continue;
            	}
            	break; // if read and check sucessfully
        ...
        NandCopy ( (uint8 *) NFC_SBUF_ADDR,pSBuf,nSct*16);
        s_conv_flag = SCI_FALSE;
        if((ERR_NF_SUCCESS == mRet) && (ERR_NF_SUCCESS == sRet))
        {
        	ret = ERR_NF_SUCCESS;
        }
        else
    ...
    NandCopy ( (uint8 *) NFC_MBUF_ADDR,pMBuf,nSct*512);
...
LOCAL void _nand_copy (uint8 *pSrc,  //Source Address
...
#elif defined(CHIP_ENDIAN_DEFAULT_LITTLE) && defined(CHIP_ENDIAN_LITTLE) && defined(NAND_ENDIAN_SWITCH)
    uint32 i, d;
    char *pD = (char *) &d;
                    ...
                    )
{
    ...
    for (;;)
    {
        _NF_SetState (NF_STATE_COPY_START);

        _nand_copy (pSrc, pDst, len);

        if (NF_STATE_COPY_END != _NF_GetState())
        {
            _NF_SetState (NF_STATE_COPY_END);
            break;
        }
        else if (NF_STATE_WRITE_START == state)
        {
            REG32 (NFC_ECC_EN) =0x0;
            NANDCTL_Reset();
            REG32 (NFC_ECC_EN) =0x1;
