...
LOCAL void _ArmSrcSet (const char *SrcName)
{
    ...
    else if (strcmp ("CLK_249M6", SrcName) == 0)
    {
        tmp |= 2<<CLK_MCU_SEL_SHIFT;
    }
    else if (strcmp ("CLK_312M", SrcName) == 0)
    {
...
{
    CHIP_REG_AND(APB_CLK_CFG1, ~CLK_SDIO_SEL_MSK);
   
    if (strcmp ("CLK_48M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG1, 0<<CLK_SDIO_SEL_SHIFT);
    }
    else if (strcmp ("CLK_78M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG1, 1<<CLK_SDIO_SEL_SHIFT);
    }
    else if (strcmp ("CLK_89M1", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG1, 2<<CLK_SDIO_SEL_SHIFT);
    }
    else if (strcmp ("CLK_104M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG1, 3<<CLK_SDIO_SEL_SHIFT);
    }
    else
    {
        SCI_PASSERT (SCI_FALSE,  /*assert verified*/
...
LOCAL void _I2sSrcDivSet (int Div)
{
    ...
    CHIP_REG_AND(APB_CLK_CFG1, ~CLK_IIS_DIV_MCU_MSK);
	CHIP_REG_OR(APB_CLK_CFG1, Div<<CLK_IIS_DIV_MCU_SHIFT);
}
...
LOCAL void _Uart1SrcSet (const char *SrcName)
{
    ...
    {
        CHIP_REG_AND(APB_CLK_CFG1, ~CLK_UART1_SEL_MCU);
    }
    else if (strcmp ("26M_XTL", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG1, CLK_UART1_SEL_MCU);
    }
    else
    {
        SCI_PASSERT (SCI_FALSE,  /*assert verified*/
...
LOCAL void _Uart2SrcSet (const char *SrcName)
{
    if (strcmp ("26M_XTL", SrcName) == 0)
    {
        CHIP_REG_AND(APB_CLK_CFG0, ~CLK_UART2_SEL);
    }
    else if (strcmp ("CLK_104M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG0, CLK_UART2_SEL);
    }
    else
    {
        SCI_PASSERT (SCI_FALSE,  /*assert verified*/
...
{
    CHIP_REG_AND(APB_CLK_CFG0, ~CLK_ZIP0_SEL_MSK);
		
    if (strcmp ("26M_XTL", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG0, 0x0UL<<CLK_ZIP0_SEL_SHIFT);
    }
    else if (strcmp ("CLK_208M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG0, 0x1UL<<CLK_ZIP0_SEL_SHIFT);
    }
    else if (strcmp ("CLK_104M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG0, 0x2UL<<CLK_ZIP0_SEL_SHIFT);
    }		
    else
    {
        SCI_PASSERT (SCI_FALSE,  /*assert verified*/
 ...
 * _Lzma1SrcSet
 ******************************************************************************/
LOCAL void _Lzma1SrcSet (const char *SrcName)
{
    CHIP_REG_AND(APB_CLK_CFG0, ~CLK_ZIP1_SEL_MSK);
		
    if (strcmp ("26M_XTL", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG0, 0x0UL<<CLK_ZIP1_SEL_SHIFT);
    }
    else if (strcmp ("CLK_208M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG0, 0x1UL<<CLK_ZIP1_SEL_SHIFT);
    }
    else if (strcmp ("CLK_104M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG0, 0x2UL<<CLK_ZIP1_SEL_SHIFT);
    }		
    else
    {
        SCI_PASSERT (SCI_FALSE,  /*assert verified*/
                     (err_string, "_Lzma1SrcSet", SrcName, "26M_XTL, CLK_104M,CLK_208M"));
    }
}
...
{
    CHIP_REG_AND(APB_CLK_CFG1, ~CLK_ADI_SEL_MCU_MSK);
    if (strcmp ("CLK_26M", SrcName) == 0)
    {
        CHIP_REG_AND(APB_CLK_CFG1, 0x0UL<<CLK_ADI_SEL_MCU_SHIFT);
    }
    else if (strcmp ("CLK_78M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG1, 0x1UL<<CLK_ADI_SEL_MCU_SHIFT);
    }
    else if (strcmp ("CLK_104M", SrcName) == 0)
    {
        CHIP_REG_OR(APB_CLK_CFG1, 0x2UL<<CLK_ADI_SEL_MCU_SHIFT);
    }
    else
    {
        SCI_PASSERT (SCI_FALSE,  /*assert verified*/
 ...
 * _IIC2Enable
 ******************************************************************************/
LOCAL void _IIC2Enable (void)
{
    /* Enable keypad apb and rtc clock */
    //CLOCK_LOG_TRACE:"[_IIC2Enable]"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SC6530_CLOCK_CFG_633_112_2_17_23_3_47_538,(uint8*)"");

    CHIP_REG_SET (APB_EB0_SET,  I2C2_EB_SET);
}

/******************************************************************************
 * _IIC2Disable
 ******************************************************************************/
LOCAL void _IIC2Disable (void)
{
    /* Enable keypad apb and rtc clock */
    //CLOCK_LOG_TRACE:"[_IIC2Enable]"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SC6530_CLOCK_CFG_643_112_2_17_23_3_47_539,(uint8*)"");

    CHIP_REG_SET (APB_EB0_CLR,  I2C2_EB_CLR);
}

/******************************************************************************
 * _IIC2Reset
 ******************************************************************************/
LOCAL void _IIC2Reset (void)
{
    //CLOCK_LOG_TRACE:"[_IIC2Reset]"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SC6530_CLOCK_CFG_652_112_2_17_23_3_47_540,(uint8*)"");

    /* Reset SPI1 clock */
    CHIP_REG_SET (APB_RST0_SET,  I2C2_SOFT_RST_SET);
    DELAY_CYCLE(1000)

    CHIP_REG_SET (APB_RST0_CLR,  I2C2_SOFT_RST_CLR);
    DELAY_CYCLE(1000)
}
 ...
 * _Uart2Reset
 ******************************************************************************/
LOCAL void _Uart2Reset (void)
{
    //CLOCK_LOG_TRACE:"[_Uart1Reset]"
    //SCI_TRACE_ID(TRACE_TOOL_CONVERT,SC6530_CLOCK_CFG_1296_112_2_17_23_3_48_595,(uint8*)"");

    /* Reset uart1 clock */
    CHIP_REG_SET (APB_RST0_SET,  UART2_SOFT_RST_SET_MCU);
    DELAY_CYCLE(1000)

    CHIP_REG_SET (APB_RST0_CLR,  UART2_SOFT_RST_CLR_MCU);
    DELAY_CYCLE(1000)
}
...
LOCAL void _LzmaEnable (void)
{
    ...
    CHIP_REG_SET(AHB_EB0_SET, ZIP0_EB_SET);
}
...
LOCAL void _LzmaDisable (void)
{
    ...
    CHIP_REG_SET (AHB_EB0_CLR,  ZIP0_EB_CLR);
}
...
LOCAL void _LzmaReset (void)
{
    ...
    CHIP_REG_SET(AHB_RST0_SET, ZIP0_SOFT_RST_SET);
    DELAY_CYCLE(1000)

    CHIP_REG_SET(AHB_RST0_CLR, ZIP0_SOFT_RST_CLR);
    DELAY_CYCLE(1000)   
}
 ...
 * _LzmaEnable
 ******************************************************************************/
LOCAL void _Lzma1Enable (void)
{
    /* Enable Sdio apb clock */
    //CLOCK_LOG_TRACE:"[_Lzma2Enable]"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SC6530_CLOCK_CFG_1435_112_2_17_23_3_48_607,(uint8*)"");
	
    CHIP_REG_SET(AHB_EB0_SET, ZIP1_EB_SET);
}

/******************************************************************************
 * _LzmaDisable
 ******************************************************************************/
LOCAL void _Lzma1Disable (void)
{
    /* Disable Sdio apb clock */
    //CLOCK_LOG_TRACE:"[_Lzma2Disable]"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SC6530_CLOCK_CFG_1445_112_2_17_23_3_48_608,(uint8*)"");
      
    CHIP_REG_SET (AHB_EB0_CLR,  ZIP1_EB_CLR);
}

/******************************************************************************
 * _LzmaReset
 ******************************************************************************/
LOCAL void _Lzma1Reset (void)
{
    //CLOCK_LOG_TRACE:"[_Lzma2Reset]"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,SC6530_CLOCK_CFG_1454_112_2_17_23_3_48_609,(uint8*)"");
   
    CHIP_REG_SET(AHB_RST0_SET, ZIP1_SOFT_RST_SET);
    DELAY_CYCLE(1000)

    CHIP_REG_SET(AHB_RST0_CLR, ZIP1_SOFT_RST_CLR);
    DELAY_CYCLE(1000)   
}
...
LOCAL const CLOCK system_clock[] =
{
    ...
    {
    "MPLL",    1248000000,  "26M_XTL",    SCI_TRUE, _MpllEnable, _MpllDisable, 
    },
    {
    "CLK_624M",624000000,  "MPLL",       SCI_TRUE, SCI_NULL, SCI_NULL, 
    },
    {
    "CLK_312M",312000000,  "MPLL",       SCI_TRUE, SCI_NULL, SCI_NULL, 
    },
    {
    ...
    {
    "CLK_26M_MPLL",26000000,  "MPLL",       SCI_TRUE, SCI_NULL, SCI_NULL,    
    },
    {
    "CLK_13M_MPLL",  13000000,  "MPLL",       SCI_TRUE, SCI_NULL, SCI_NULL,    
    },
    {
    "CLK_6.5M_MPLL",  6500000,  "MPLL",       SCI_TRUE, SCI_NULL, SCI_NULL,    
    },
    {
    "CLK_249M6",249600000,  "MPLL",       SCI_TRUE, SCI_NULL, SCI_NULL, 
    },
    {
    "CLK_124M8",124800000,  "MPLL",       SCI_TRUE, SCI_NULL, SCI_NULL,    
    },
    {
    ...
    {
    "CLK_24M",  24000000,  "MPLL",       SCI_TRUE, SCI_NULL, SCI_NULL,        
    },
    {
...
{
    "ARM",        312000000,      "ARM",      0,            SCI_TRUE,
    SCI_NULL,     SCI_NULL,      _ArmSrcSet, SCI_NULL,     SCI_NULL,
},
{
    "AHB",        312000000,      "ARM",      1,            SCI_TRUE,
...
{
     "UART2",     SCI_NULL,       "APB",      0,           SCI_TRUE,      
     _Uart2Enable,_Uart2Disable,  _Uart2SrcSet,SCI_NULL,   _Uart2Reset,      
},
{
...
{
     "I2C2",      SCI_NULL,       "APB",      0,           SCI_TRUE,     
     _IIC2Enable, _IIC2Disable,   SCI_NULL,   SCI_NULL,    _IIC2Reset,     
},
{
...
{
      "LZMA1",     208000000,       "AHB",      0,           SCI_TRUE,      
      _Lzma1Enable,_Lzma1Disable,   _Lzma1SrcSet,SCI_NULL,    _Lzma1Reset,     
},
{
