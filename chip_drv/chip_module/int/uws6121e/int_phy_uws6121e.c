...
static void INT_PHY_IRQWholePrioritySet(void)
{
	...
	setIntPriority(SYS_IRQ_ID_TIMER1_OS, 0xFF);
	real_priority_level = readIntPriority(SYS_IRQ_ID_TIMER1_OS);
	real_priority_level &= 0xFF;
	setPriorityMask(real_priority_level);
	setIntPriority(SYS_IRQ_ID_TIMER1_OS, 0);
}
...
LOCAL void PMIC_HisrFunc (uint32 i, void *d)
{
	switch(PMIC_INT_ID)
	{
		case ADC_INT:
			...
			break;
		case RTC_INT:
			RTCHisrFunc(PMIC_INT_ID, NULL);
			INT_PHY_PMIC_RTC_EN();
			break;
		case WDG_INT:
			WDG_TimerHandler(PMIC_INT_ID);
			INT_PHY_PMIC_WDG_EN();
			break;
		case EIC_INT:
			EICD_DbncIsr(PMIC_INT_ID);
			INT_PHY_PMIC_EIC_EN();
			break;
		case TMR_INT:
			INT_PHY_PMIC_TMR_EN();
...
PUBLIC ISR_EXE_T PMIC_Isr_Handle( uint32 data )
{
    ...
    SCI_TRACE_LOW("ANA_INTC_MASK_STATUS = 0x%x", int_status);
    for( i=0; i<INTC_EN_SIZE; i++)
    {
        if( PMIC_ISR_PHY_GetMskIntSts(int_status, i) )
        {
              /* disable interrupt to avoid interrupt continus */
		//INT_PHY_PMIC_DISABLE_ALL();
             result = PMIC_ISR_PHY_SELESCT(i);
        }
    }
    //INT_PHY_PMIC_ABLE_ALL();
    return result;
}
...
PUBLIC uint32 PMIC_ISR_PHY_SELESCT(uint32 chan_id)
{
	...
	switch(chan_id)
	{
		case ADC_INT:
			PMIC_INT_ID = ADC_INT;
			INT_PHY_PMIC_ADC_UNEN();
			break;
		case RTC_INT: //use
			PMIC_INT_ID = RTC_INT;
			INT_PHY_PMIC_RTC_UNEN();
			result = RTC_Handler(chan_id);
			if(ISR_DONE == result)
			{
				INT_PHY_PMIC_RTC_EN();
			}
		...
		case WDG_INT: //use
			PMIC_INT_ID = WDG_INT;
			INT_PHY_PMIC_WDG_UNEN();
			result = CALL_HISR;
			break;
		case EIC_INT: //use
			PMIC_INT_ID = EIC_INT;
			INT_PHY_PMIC_EIC_UNEN();
			result = CALL_HISR;
			break;
		case TMR_INT:
			PMIC_INT_ID = TMR_INT;
			INT_PHY_PMIC_TMR_UNEN();
			break;
		case CAL_INT:
			PMIC_INT_ID = CAL_INT;
			INT_PHY_PMIC_CAL_UNEN();
			break;
		case PSM_INT:
			PMIC_INT_ID = PSM_INT;
			INT_PHY_PMIC_PSM_UNEN();
			break;
		default:
			break;
	}
...
PUBLIC void INT_PHY_PMIC_INIT (void)
{
	...
	//EIC INTC OPEN
	//Enable the PCLK of EIC
	INT_PHY_PMIC_PCLK_EIC_EN();

	//Enable RTC clock of EIC
	INT_PHY_PMIC_RTC_EIC_EN();
	...
	ISR_RegHandler_Ex (TB_PMIC_INT, (TB_ISR) PMIC_Isr_Handle, PMIC_HisrFunc, CHIPDRV_HISR_PRIO_2, NULL);
	
       CHIPDRV_EnableIRQINT(TB_PMIC_INT);
}

//CLK
PUBLIC void INT_PHY_PMIC_RTC_CLK_DISABLE_ALL (void)
{
	ANA_REG_AND(ANA_RTC_CLK_EN0_RTC, 0x0);
}
PUBLIC void INT_PHY_PMIC_RTC_CLK_ABLE_ALL (void)
{
	ANA_REG_OR(ANA_RTC_CLK_EN0_RTC, 0xFFFF);
}

//CLK Enable
PUBLIC void INT_PHY_PMIC_RTC_ARCH_EN (void)
{
	ANA_REG_OR(ANA_RTC_CLK_EN0_RTC, RTC_ARCH_EN);
}

PUBLIC void INT_PHY_PMIC_RTC_RTC_EN (void)
{
	ANA_REG_OR(ANA_RTC_CLK_EN0_RTC, RTC_RTC_EN);
}

PUBLIC void INT_PHY_PMIC_RTC_WDG_EN (void)
{
	ANA_REG_OR(ANA_RTC_CLK_EN0_RTC, RTC_WDG_EN);
}

PUBLIC void INT_PHY_PMIC_RTC_EIC_EN (void)
{
	ANA_REG_OR(ANA_RTC_CLK_EN0_RTC, RTC_EIC_EN);
}

PUBLIC void INT_PHY_PMIC_RTC_EFS_EN (void)
{
	ANA_REG_OR(ANA_RTC_CLK_EN0_RTC, RTC_EFS_EN);
}

//CLK Disable
PUBLIC void INT_PHY_PMIC_RTC_ARCH_UNEN (void)
{
	ANA_REG_AND(ANA_RTC_CLK_EN0_RTC, ~RTC_ARCH_EN);
}

PUBLIC void INT_PHY_PMIC_RTC_RTC_UNEN (void)
{
	ANA_REG_AND(ANA_RTC_CLK_EN0_RTC, ~RTC_RTC_EN);
}

PUBLIC void INT_PHY_PMIC_RTC_WDG_UNEN (void)
{
	ANA_REG_AND(ANA_RTC_CLK_EN0_RTC, ~RTC_WDG_EN);
}

PUBLIC void INT_PHY_PMIC_RTC_EIC_UNEN (void)
{
	ANA_REG_AND(ANA_RTC_CLK_EN0_RTC, ~RTC_EIC_EN);
}

PUBLIC void INT_PHY_PMIC_RTC_EFS_UNEN (void)
{
	ANA_REG_AND(ANA_RTC_CLK_EN0_RTC, ~RTC_EFS_EN);
}

//INTC
PUBLIC void INT_PHY_PMIC_ABLE_ALL (void)
{
	ANA_REG_OR(ANA_INTC_EN, ANA_ALL_PIMC_INT_ABLE);
}

PUBLIC void INT_PHY_PMIC_DISABLE_ALL (void)
{
	ANA_REG_AND(ANA_INTC_EN, ANA_ALL_PIMC_INT_DISABLE);
}

// INTC Enable
PUBLIC void INT_PHY_PMIC_ADC_EN (void)
{
	ANA_REG_OR(ANA_INTC_EN, ANA_ADC_INT_EN);
}

PUBLIC void INT_PHY_PMIC_RTC_EN (void)
{
	ANA_REG_OR(ANA_INTC_EN, ANA_RTC_EN_INT);
}

PUBLIC void INT_PHY_PMIC_WDG_EN (void)
{
	ANA_REG_OR(ANA_INTC_EN, ANA_WDG_INT_EN);
}

PUBLIC void INT_PHY_PMIC_EIC_EN (void)
{
	ANA_REG_OR(ANA_INTC_EN, ANA_EIC_INT_EN);
}

PUBLIC void INT_PHY_PMIC_TMR_EN (void)
{
	ANA_REG_OR(ANA_INTC_EN, ANA_TMR_INT_EN);
}

PUBLIC void INT_PHY_PMIC_CAL_EN (void)
{
	ANA_REG_OR(ANA_INTC_EN, ANA_CAL_INT_EN);
}

PUBLIC void INT_PHY_PMIC_PSM_EN (void)
{
	ANA_REG_OR(ANA_INTC_EN, ANA_PSM_INT_EN);
}

// INTC disable
PUBLIC void INT_PHY_PMIC_ADC_UNEN (void)
{
	ANA_REG_AND(ANA_INTC_EN, ~ANA_ADC_INT_EN);
}
PUBLIC void INT_PHY_PMIC_RTC_UNEN (void)
{
	ANA_REG_AND(ANA_INTC_EN, ~ANA_RTC_EN_INT);
}
PUBLIC void INT_PHY_PMIC_WDG_UNEN (void)
{
	ANA_REG_AND(ANA_INTC_EN, ~ANA_WDG_INT_EN);
}
PUBLIC void INT_PHY_PMIC_EIC_UNEN (void)
{
	ANA_REG_AND(ANA_INTC_EN, ~ANA_EIC_INT_EN);
}
PUBLIC void INT_PHY_PMIC_TMR_UNEN (void)
{
	ANA_REG_AND(ANA_INTC_EN, ~ANA_TMR_INT_EN);
}
PUBLIC void INT_PHY_PMIC_CAL_UNEN (void)
{
	ANA_REG_AND(ANA_INTC_EN, ~ANA_CAL_INT_EN);
}
PUBLIC void INT_PHY_PMIC_PSM_UNEN (void)
{
	ANA_REG_AND(ANA_INTC_EN, ~ANA_PSM_INT_EN);
}

//PLCK
PUBLIC void INT_PHY_PMIC_PCLK_ABLE_ALL (void)
{
	ANA_REG_OR(ANA_MODULE_EN0_RTC, ANA_PCLK_ABLE_ALL);
}

PUBLIC void INT_PHY_PMIC_PCLK_DISABLE_ALL (void)
{
	ANA_REG_AND(ANA_MODULE_EN0_RTC, ANA_PCLK_DISABLE_ALL);
}
...
{
	ANA_REG_OR(ANA_MODULE_EN0_RTC, ANA_PSM_TOPA_EN);
}

PUBLIC void INT_PHY_PMIC_PCLK_RTC_TOPA_EN (void)
{
	ANA_REG_OR(ANA_MODULE_EN0_RTC, ANA_RTC_TOPA_EN);
}

PUBLIC void INT_PHY_PMIC_PCLK_IOMUX_EN (void)
{
	ANA_REG_OR(ANA_MODULE_EN0_RTC, ANA_IOMUX_EN);
}
...
{
	ANA_REG_AND(ANA_MODULE_EN0_RTC, ~ANA_PSM_TOPA_EN);
}
PUBLIC void INT_PHY_PMIC_PCLK_RTC_TOPA_UNEN (void)
{
	ANA_REG_AND(ANA_MODULE_EN0_RTC, ~ANA_RTC_TOPA_EN);
}
PUBLIC void INT_PHY_PMIC_PCLK_IOMUX_UNEN (void)
{
	ANA_REG_AND(ANA_MODULE_EN0_RTC, ~ANA_IOMUX_EN);
}
/*****************************************************************************/
...
#define GIC_DIST_CTRL				0x1000
#define GIC_DIST_CTR				0x1004
#define GIC_DIST_IIDR				0x1008
#define GIC_DIST_IGROUP				0x1080
#define GIC_DIST_ENABLE_SET			0x1100
#define GIC_DIST_ENABLE_CLEAR		0x1180
#define GIC_DIST_PENDING_SET		0x1200
#define GIC_DIST_PENDING_CLEAR		0x1280
#define GIC_DIST_ACTIVE_SET			0x1300
#define GIC_DIST_ACTIVE_CLEAR		0x1380
#define GIC_DIST_PRI				0x1400
#define GIC_DIST_TARGET				0x1800
#define GIC_DIST_CONFIG				0x1c00
#define GIC_DIST_SOFTINT			0x1f00
#define GIC_DIST_SGI_PENDING_CLEAR	0x1f10
#define GIC_DIST_SGI_PENDING_SET	0x1f20
...
PUBLIC void INT_PHY_GicDistSuspend(void)
{
	...
	uint32 dist_base;//unsigned long
	int i;


	gic_irqs  = NB_SYS_IRQ;
	dist_base = 0x00800000;
...
//        gic->saved_spi_target[i] &= gicd_target_cpid_mask[i];
	...
	REG_WRITE(0xFF, 0x00802004);
	REG_WRITE(GICD_ENABLE, dist_base + GIC_DIST_CTRL);
	REG_WRITE(GICC_ENABLE, 0x00802000);
}
