...
PUBLIC uint32 INT_PHY_IrqHandleDefaultFunc (uint32 LogicBit)
{
    ...
    ana_die_id = ANA_REG_GET (ANA_ADIE_CHIP_ID);

    if (ana_die_id == ANA_V1_CHIP_ID)         ///A die G1
    {
        if (LogicBit < TB_ADC_INT)
        {
            CHIPDRV_DisableIRQINT (LogicBit);
        }
        else
        {
            if (LogicBit != TB_ANA_GPIO_INT)
            {
                ANA_REG_AND (sub_int_ctrl[LogicBit - TB_ADC_INT].mod_int_ctrl_addr,
                             ~sub_int_ctrl[LogicBit - TB_ADC_INT].int_msk);
            }
            else
            {
                ANA_REG_SET ( (ANA_GPIN_PG0_BASE + GPI_IE),  0);
                ANA_REG_SET ( (ANA_GPIO_PG0_BASE + GPIO_IE), 0);
                ANA_REG_SET ( (ANA_GPIO_PG1_BASE + GPIO_IE), 0);
            }
        }
    }
    else if (ana_die_id == ANA_V2_CHIP_ID)    ///A die G2
    {
        CHIPDRV_DisableIRQINT (LogicBit);
    }
    else                                            ///error
...
PUBLIC void INT_PHY_GetIrqIntStatus (void)
{
    ...
    if (IrqUnitRegValue[0]& (1 << TB_ANA_DIE_INT))
    {
...
PUBLIC void INT_PHY_Init (void)
{
    ...
    if (ana_die_id == ANA_V1_CHIP_ID)
    {
        for (i = 0; i < ANA_MAX_INT_NUM; i++)
        {
            //disable all interrupt
            if (i != (TB_ANA_GPIO_INT - TB_ADC_INT))
            {
                ANA_REG_AND (sub_int_ctrl[i].mod_int_ctrl_addr, ~sub_int_ctrl[i].int_msk);
            }
            else
            {
                ANA_REG_SET ( (ANA_GPIN_PG0_BASE + GPI_IE), 0);
    ...
    else if (ana_die_id == ANA_V2_CHIP_ID)
    {
    ...
    CHIPDRV_EnableIRQINT (TB_ANA_DIE_INT);

    //register DSP FIQ Handler for DSP overrun assert
    status = FIQ_RegHandler (TB_DSP_INT1, DSP_FiqHandler);

    SCI_PASSERT ( (FIQ_SUCCESS == status || FIQ_ISR_ASSIGNED == status),  /*assert verified*/
                  ("%s, %d, fiq regist status: %d", __MODULE__, __LINE__, status));

    CHIPDRV_EnableFIQINT (TB_DSP_INT1);
...
PUBLIC void INT_PHYFiqDebug (void)
{
    ...
    TM_SendTestPointRequest (_tx_pc_val_before_fiq, CHIP_REG_GET (0x8b000010));
}

/*****************************************************************************/
//  Description:    This function is used to get dsp overrun assert string
//                  according to the assert id.
//  Author:         Andy.Chen
//  Note:
//****************************************************************************/
LOCAL uint8 *_get_dsp_passert_string (  // return:      string pointer
    uint32 id)              // parameter:   assert id
{
    uint32 start_tag, end_tag, cur_tag;
    uint32 i, num;
    uint8  *strptr = NULL;
    DSP_ASSERT_INFO_PTR_T cur_position;

    num = sizeof (s_dsp_assert_table) /sizeof (DSP_ASSERT_INFO_T);
    SCI_ASSERT (0 != num);/*assert verified*/

    /* first compare the id, whether is the head or the tail,
       we can also do not do the two compare,
       but that will make the logic in for-loop more complicated */
    if (id == s_dsp_assert_table[0].id)
    {
        strptr = s_dsp_assert_table[0].str;
    }
    else if (id == s_dsp_assert_table[num-1].id)
    {
        strptr = s_dsp_assert_table[num-1].str;
    }

    /* second search the id in the table, as the id is not continuous, we can not
       search by a base-id. in order to increase efficiency, we always check the
       medium in the table, then shorten the range, until we find the same id*/
    else if ( (id > s_dsp_assert_table[0].id) && (id < s_dsp_assert_table[num-1].id))
    {
        start_tag = 0;                      // |----------|----------|
        end_tag   = num - 1;                //start=0    cur      end=num-1

        for (i = 1; i <= num; i *= 2)
        {
            cur_tag   = (start_tag + end_tag) /2;
            cur_position = (DSP_ASSERT_INFO_PTR_T) &s_dsp_assert_table[cur_tag];

            if (id < cur_position->id)
            {
                end_tag = cur_tag;          // |-----|----|----------|
                continue;                   // start    end
            }
            else if (id > cur_position->id)
            {
                start_tag = cur_tag;        // |----------|-----|----|
                continue;                   //            start    end
            }
...
//add for sc8800g, used to handle dsp overrun asset
//this function is moved from deep_sleep_v3.c
//@tim.luo
void DSP_FiqHandler (uint32 fiq_num)
{
    char *str;

    // Clear dsp FIQ
    * (volatile uint32 *)    GR_IRQ |= BIT_3;

    // DSP clears Resp_done to 0 whenever get ready to send CMD to ARM.
    // Otherwise if Resp_done!= 0, then do nothing.
    if (0 == cmd_ptr->DSPRes_block.Resp_done)
    {
        switch (cmd_ptr->DSPRes_block.Cmd_ID)
        {
            case DSP_OVERRUN_REQ:
                {
                    str = (char *) _get_dsp_passert_string ( (uint32) (cmd_ptr->DSPRes_block.SeqNr));

                    if (NULL != str)
                        SCI_PASSERT (SCI_FALSE, ("DSP OVERRUN:0x%X:%s",  /*assert verified*/
                                                 cmd_ptr->DSPRes_block.SeqNr,str));
                    else
                        SCI_PASSERT (SCI_FALSE, ("DSP OVERRUN:0x%X",  /*assert verified*/
                                                 cmd_ptr->DSPRes_block.SeqNr));
                }
                break;
            case DSP_DEACTIVE_WAKEUP_REQ:
                {
                    SCI_ASSERT (0);/*assert to do*/
                }
                break;
            ...
            default:
                SCI_PASSERT (SCI_FALSE, ("DSP_FiqHandler: default branch 0x%x",  /*assert to do*/
                                         cmd_ptr->DSPRes_block.Cmd_ID));
                //here we avoid the fail in calibration mode, disable assert!
                break;
        }

        // Read completed, DSP querying for changing
        cmd_ptr->DSPRes_block.Resp_done = 1; /*lint !e527 confirmed by xuepeng*/

    }
    // In case of repeat CMD from DSP, or DSP just send an interrupt
    // to wake up ARM in speacial applications
    else
    {
        // Test point for Debug
        TM_SendTestPointRequest (0x091022, cmd_ptr->DSPRes_block.Cmd_ID);
    }
}
