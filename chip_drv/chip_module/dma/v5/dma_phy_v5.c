...
PUBLIC void DMA_PHY_SetChnPause (uint32 channel, BOOLEAN is_pause)
{
    ...
    if(is_pause)
    {
        dma_chn_ctl_ptr->full_chn_pause |= BIT_0;
    }
    else
    {
        dma_chn_ctl_ptr->full_chn_pause &= ~BIT_0;
...
{
    uint32 reqx_uid_base = DMA_CTL_REG_BASE + 0x2000 + (uid-1)*0x04;
...
PUBLIC void DMA_PHY_SetChnWorkMode (uint32 channel, DMA_CHN_WORKMODE_E workmode)
{
    ...
    switch (workmode)
    {
        case DMA_CHN_WORKMODE_NORMAL:
            dma_chn_ctl_ptr->full_chn_cfg &= ~BIT_4;
            break;
...
{
    uint32 frag_cfg  = 0;
    uint32 chip_id = CHIP_GetHWChipID();
    
    {
        ...
        switch (chnparam->workmode)
        {
            ...
            case DMA_CHN_WORKMODE_LINKLIST:
                SCI_DisableIRQ();
                dma_chn_ctl_ptr->full_chn_cfg |= BIT_4; //Linklist_en
                dma_chn_ctl_ptr->full_chn_int  |= BIT_3; //Linklist_int_en
    ...
    SCI_ASSERT((chnparam->src_address) >= FLASH_GetEndAddr()); /*assert to do*/ 
    SCI_ASSERT((channel < DMA_CHANNEL_MAX) && (channel >= DMA_CHANNEL_MIN)); /*assert to do*/

    if(0x65300000 == chip_id)
    {
        //for sc6530
       //endian_type and  endian_switchmode
        switch (chnparam->endian_switchmode)
        {
            case DMA_ENDIANSWITCHMODE_UN:
                frag_cfg |= (uint32) SC6530_DMA_CHx_ENDIANSWMODESEL_UN;
                break;
            case DMA_ENDIANSWITCHMODE_FULL:
                frag_cfg |= (uint32) SC6530_DMA_CHx_ENDIANSWMODESEL_FULL;
                break;
            case DMA_ENDIANSWITCHMODE_MODE0:
                frag_cfg |= (uint32) SC6530_DMA_CHx_ENDIANSWMODESEL_MODE0;
                break;
            case DMA_ENDIANSWITCHMODE_MODE1:
                frag_cfg |= (uint32) SC6530_DMA_CHx_ENDIANSWMODESEL_MODE1;
                break;
            default:
                SCI_ASSERT (0);/*assert verified*/
                break;
        ...
        switch (chnparam->src_datawidth)
        {
            case DMA_DATAWIDTH_BYTE:
                frag_cfg |= (uint32) SC6530_DMA_CHx_SRC_DATAWIDTH_BYTE;
                break;
            case DMA_DATAWIDTH_HALFWORD:
                frag_cfg |= (uint32) SC6530_DMA_CHx_SRC_DATAWIDTH_HALFWORD;
                break;
            case DMA_DATAWIDTH_WORD:
                frag_cfg |= (uint32) SC6530_DMA_CHx_SRC_DATAWIDTH_WORD;
                break;
            default:
                SCI_ASSERT (0);/*assert verified*/
                break;
        ...
        {
            case DMA_DATAWIDTH_BYTE:
                frag_cfg |= (uint32) SC6530_DMA_CHx_DEST_DATAWIDTH_BYTE;
                break;
            case DMA_DATAWIDTH_HALFWORD:
                frag_cfg |= (uint32) SC6530_DMA_CHx_DEST_DATAWIDTH_HALFWORD;
                break;
            case DMA_DATAWIDTH_WORD:
                frag_cfg |= (uint32) SC6530_DMA_CHx_DEST_DATAWIDTH_WORD;
                break;
            default:
                SCI_ASSERT (0);/*assert verified*/
                break;
        ...
        {
            frag_cfg |= SC6530_DMA_CHx_ADDFIXED_ENABLE;
            frag_cfg |= SC6530_DMA_CHx_ADDFIXEDON_SRC;
        }
        else if(chnparam->src_step != 0 && chnparam->dest_step == 0)
        {
            frag_cfg |= SC6530_DMA_CHx_ADDFIXED_ENABLE;
            frag_cfg |= SC6530_DMA_CHx_ADDFIXEDON_DEST;		
        }
        else
        {
    ...
    if(g_channel[channel].type == DMA_CHN_TRSF_TYPE_STANDARD) 
    {
        ...
        if(0x65300000 == chip_id)
        {
            ...
            switch (chnparam->requestmode)
            {
                case DMA_CHN_REQMODE_FRAGMENT:
                    frag_cfg |= (uint32) SC6530_DMA_CHx_REQMODE_FRAGMENT;
                    break;    
                case DMA_CHN_REQMODE_NORMAL:
                    frag_cfg |= (uint32) SC6530_DMA_CHx_REQMODE_FRAGMENT;
                    break;
                case DMA_CHN_REQMODE_TRASACTION:
                    frag_cfg |= (uint32) SC6530_DMA_CHx_REQMODE_BLOCK;
                    break;
                case DMA_CHN_REQMODE_LIST:
                    frag_cfg |= (uint32) SC6530_DMA_CHx_REQMODE_LIST;
                    break;
                default:
                    SCI_ASSERT (0);/*assert verified*/
                    break;
        ...
        //fragment length
        frag_cfg |=
            (chnparam->block_length << SC6530_DMA_CHx_FRAGLENGTH_OFFSET) & SC6530_DMA_CHx_FRAGLENGTH_MASK;	        
        }
        else
        {
        //fragment length
...
PUBLIC uint32 DMA_PHY_StdChn_Transfered_Data (uint32 channel)
{
    ...
    if (g_channel[channel].type == DMA_CHN_TRSF_TYPE_STANDARD) 
    {
        if (0x65300000 == chip_id)
        {
            length = (dma_chn_ctl_ptr->std_chn_blk_len & 0xffff0000) >> 16;
        }
        else
        {
	    /*6500 and 6530c's DMA ip version is r3p0*/		
