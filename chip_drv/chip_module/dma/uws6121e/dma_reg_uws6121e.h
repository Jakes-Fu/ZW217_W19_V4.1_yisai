...
#ifndef _DMA_REG_UWS6121E_H_
#define _DMA_REG_UWS6121E_H_
...
#define REG_CP_DMA_BASE               0x12040000
typedef volatile struct
{
    ...
    uint32_t                          __18[2];				//0x00000018
    uint32_t                          axidma_ch_irq_distr;          //0x00000020
    uint32_t Reserved_00000024[7];                 //0x00000024
    struct
    {
        uint32_t                      conf;                         //0x00000040
        uint32_t                      map;                          //0x00000044
        uint32_t                      saddr;                        //0x00000048
        uint32_t                      daddr;                        //0x0000004C
        uint32_t                      count;                        //0x00000050
        uint32_t                      countp;                       //0x00000054
        uint32_t                      status;                       //0x00000058
        uint32_t                      sgaddr;                       //0x0000005C
        uint32_t                      sgconf;                       //0x00000060
        uint32_t				set;					//0x00000064
        uint32_t				clr;					//0x00000068
        REG32 Reserved_00000024[5];             //0x00000024
    } axidma_ch[12];
} HWP_ARM_AXIDMA_T;

#define hwp_dma                    ((HWP_ARM_AXIDMA_T*) REG_ACCESS_ADDRESS(REG_DMA_BASE))
#define hwp_cp_dma			((HWP_ARM_AXIDMA_T*) REG_ACCESS_ADDRESS(REG_CP_DMA_BASE))
...
#define ARM_AXIDMA_GEN_REG_SECURITY_EN (1<<6)
//axidma_delay
...
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<1)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<2)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<3)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<4)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<5)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<6)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<7)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<8)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<9)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<10)
#define ARM_AXIDMA_CH0_IRQ_EN0 (1<<11)
...
#define ARM_AXIDMA_SG_COUNT_COUNT(n)	(((n)&0xFFFF)<<0)
#define ARM_AXIDMA_SG_SUSPEND      (1<<20)
#define ARM_AXIDMA_OUNT_FINISH_STA	(1<<21)
#define ARM_AXIDMA_OUNTP_FINISH_STA	(1<<22)
