...
PUBLIC uint32 KPD_Init (void)
{
    ...
    /* Reset kpd*/
    CHIP_REG_OR (GR_SOFT_RST, SWRST_KPD_RST);
    for (i=0; i<10; i++){;}
    CHIP_REG_AND (GR_SOFT_RST, ~SWRST_KPD_RST);
...
LOCAL void ChangeState (KPD_KEY_T *key_ptr)
{
    ...
    if (TB_KPD_RELEASED == key_ptr->state)
    {
        ...
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,KPD_PHY_V0_419_112_2_17_23_5_3_1037,(uint8*)"d", key_ptr->key_code);
    }
...
LOCAL uint16 GetKeycode (KPD_NUM_E key_num)
{
    ...
    {
        case KPD_NUM_1:
            key_code = ( ( (kpd_ptr->key_status & KPD1_COL_CNT) >> (8* (key_num-KPD_NUM_1))) << 3) |
                       ( (kpd_ptr->key_status & KPD1_ROW_CNT) >> (8* (key_num-KPD_NUM_1) +4));
            break;
        case KPD_NUM_2:
            key_code = ( ( (kpd_ptr->key_status & KPD2_COL_CNT) >> (8* (key_num-KPD_NUM_1))) << 3) |
                       ( (kpd_ptr->key_status & KPD2_ROW_CNT) >> (8* (key_num-KPD_NUM_1) +4));
            break;
        case KPD_NUM_3:
            key_code = ( ( (kpd_ptr->key_status & KPD3_COL_CNT) >> (8* (key_num-KPD_NUM_1))) << 3) |
                       ( (kpd_ptr->key_status & KPD3_ROW_CNT) >> (8* (key_num-KPD_NUM_1) +4));
            break;
        case KPD_NUM_4:
            key_code = ( ( (kpd_ptr->key_status & KPD4_COL_CNT) >> (8* (key_num-KPD_NUM_1))) << 3) |
                       ( (kpd_ptr->key_status & KPD4_ROW_CNT) >> (8* (key_num-KPD_NUM_1) +4));
            break;
        default:
            break;
...
PUBLIC uint32 CheckPowerButtonState (void)
{
    ...
    status = GPIO_GetValue (163);
    SCI_RestoreIRQ();

    //sc8800g power key status:
    // pressed :   0
    // released:   1
    return (status?SCI_FALSE:SCI_TRUE);
#else
    return (CHIP_REG_GET (GPI_PG0_BASE) & BIT_3) >> 3;
#endif
}
