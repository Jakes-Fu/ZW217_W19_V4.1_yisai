...
#include "aonifc_reg.h"
#include "aonifc_drvapi.h"
...
HAL_AONIFC_REQUEST_ID_T g_halAonIfcChannelOwner[AON_IFC_STD_CHAN_NB];

UINT8   g_aonifc_ch[AONIFC_REQUESTID_NUM] = {AONIFC_UART2_TX_CH, AONIFC_UART2_RX_CH, AONIFC_UART3_TX_CH, AONIFC_UART3_RX_CH, AONIFC_DBG_UART_RX_CH, AONIFC_DBG_UART_TX_CH};
...
// =============================================================================
void hal_AonIfcOpen(void)
{
    uint8 channel;

    // Initialize the channel table with unknown requests. This is more
    // of a sanity initialization.
    for (channel = 0; channel < AON_IFC_STD_CHAN_NB; channel++)
    {
        g_halAonIfcChannelOwner[channel] = HAL_IFC_NO_REQWEST;
    }
}
...
// =============================================================================
HAL_AONIFC_REQUEST_ID_T hal_AonIfcGetOwner(uint8 channel)
{
    ...
    IFC_ASSERT(channel < AON_IFC_STD_CHAN_NB, channel);

    return g_halAonIfcChannelOwner[channel];
}
...
// =============================================================================
void hal_AonIfcChannelRelease(HAL_AONIFC_REQUEST_ID_T requestId, uint8 channel)
{
    uint32 status;
    ...
    if (g_halAonIfcChannelOwner[channel] == requestId)
    {
...
// =============================================================================
void hal_AonIfcChannelFlush(HAL_AONIFC_REQUEST_ID_T requestId, uint8 channel)
{
    uint32 status;
...
// =============================================================================
BOOLEAN hal_AonIfcChannelIsFifoEmpty(HAL_AONIFC_REQUEST_ID_T requestId, uint8 channel)
{
    uint32 status;
...
// =============================================================================
uint8 hal_AonIfcTransferStart(HAL_AONIFC_REQUEST_ID_T requestId, uint8* memStartAddr, uint32 xferSize, HAL_IFC_MODE_T ifcMode)
{
    uint32 status;
    ...
    HAL_ASSERT(xferSize < (1<<AON_IFC_TC_LEN),
               ("HAL IFC: Transfer size too large: %d", xferSize));

    HAL_ASSERT((requestId < AONIFC_REQUESTID_NUM), ("aonifc requestid %d is error", requestId));
...
#error
    ...
    channel = g_aonifc_ch[requestId];
   SCI_TRACE_LOW("rui.zhang1 channel=%d",channel);
    HAL_ASSERT((channel < AON_IFC_STD_CHAN_NB), ("aonifc requestId %d ch %d is error", requestId, channel));
    ...
    g_halAonIfcChannelOwner[channel]     = requestId;
...
// =============================================================================
uint32 hal_AonIfcGetTc(HAL_AONIFC_REQUEST_ID_T requestId, uint8 channel)
{
    uint32 status;
    uint32 tc = 0;
...
// =============================================================================
void hal_AonIfcExtendTransfer(uint8 channel, uint16 xferSize)
{
    // Here, we consider the transfer as previously finished.
    if (channel == HAL_UNKNOWN_CHANNEL) return;
...
// =============================================================================
uint8 hal_AonIfcGetChannel(HAL_AONIFC_REQUEST_ID_T requestId, HAL_IFC_MODE_T ifcMode)
{
    uint8 channel;
    uint32 status;
    ...
    if (channel >= AON_IFC_STD_CHAN_NB)
    {
        hal_SysExitCriticalSectionifc(status);
...
// =============================================================================
void hal_AonIfcSetStartAddress(uint8 channel, CONST uint8* startAddress)
{
    // channel number too big
    IFC_ASSERT(channel < AON_IFC_STD_CHAN_NB, channel);
    HAL_ASSERT(hwp_aonIfc->std_ch[channel].tc == 0, ("Attempt to change an IFC start_address "
...
uint32 hal_AonIfcGetRunStatus(HAL_AONIFC_REQUEST_ID_T requestId, uint8 channel)
{
    uint32 status;
    uint32 tc = 0;
