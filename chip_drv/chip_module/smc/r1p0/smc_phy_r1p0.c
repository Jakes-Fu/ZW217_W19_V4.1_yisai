...
{
	smc_cfg_info.smc_cfg->cfg0		 = 0x11110000;
	smc_cfg_info.smc_cfg->dl[0]		 = 0x0;
	smc_cfg_info.smc_cfg->dl[1]		 = 0x0;
	smc_cfg_info.smc_cfg->cfg1		 = 0x00000010;

	smc_cfg_info.smc_cfg->cfgx_cs1[0]= 0x00904FF0;
	smc_cfg_info.smc_cfg->cfgx_cs1[1]= 0x0151FFFF;
	smc_cfg_info.smc_cfg->cfgx_cs1[2]= 0x00A0744F;
	smc_cfg_info.smc_cfg->cfgx_cs1[3]= 0x0;
}
...
PUBLIC void SMC_Config(SMC_CFG_PTR_T smc_cfg_ptr)
{
	...
	if(smc_cfg_ptr->mem_type == DDR_pSRAM)
	{
		...
		SMC_CLR_OR(smc_cfg_ptr->cfg2,SMC_DLL_CPST_THR_MASK,(0x2 << SMC_DLL_CPST_THR_SHIFT));
    	...
    	smc_cfg_ptr->dl_dly_sel[0] = 0x0;
    	smc_cfg_ptr->dl_dly_sel[1] = 0x0;
    	smc_cfg_ptr->dl_dly_sel[2] = 0x3E;
    	smc_cfg_ptr->dl_dly_sel[3] = 0x30;
    	smc_cfg_ptr->dl_dly_sel[4] = 0x30;
    	smc_cfg_ptr->dl_dly_sel[5] = 0x30;
    	smc_cfg_ptr->dl_dly_sel[6] = 0x30;
#endif
	}
...
#else
	/*Config DLL*/
	for(smc_cfg_ptr->index = 2;smc_cfg_ptr->index < 7;smc_cfg_ptr->index++)
	{
		smc_cfg_ptr->smc_cfg->dl[smc_cfg_ptr->index] |= (smc_cfg_ptr->mem_type << SMC_DLX_DLY_CPST_EN_SHIFT) | 
														(smc_cfg_ptr->dl_dly_sel[smc_cfg_ptr->index] << SMC_DLX_DLY_SEL_SHIFT);
	}
