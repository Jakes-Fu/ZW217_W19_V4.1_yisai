...
struct udc_s
{
    ...
    UdcLinkList_t *rx_linklist;     ///RX_dma linklist
    UdcLinkList_t *tx_linklist;     ///TX_dma linklist
    musbFifo_t rx_fifo;
    musbFifo_t tx_fifo;
 ...
 */
static  void udcPlatSetPower(udc_t *udc, bool on)
{
    if (CHECK_UDC_PLAT_OPS(udc, setpower))
        udc->plat_ops.setpower(udc, on);
}
 ...
 */
static  void udcPlatSetClock(udc_t *udc, bool on)
{
    if (CHECK_UDC_PLAT_OPS(udc, setclk))
        udc->plat_ops.setclk(udc, on);
}
 ...
 * @param udc   the udc
 */
static  void udcPlatSuspend(udc_t *udc)
{
 ...
 * @param udc   the udc
 */
static  void udcPlatResume(udc_t *udc)
{
 ...
 * @param udc   the udc
 */
static  void udcPlatEnable(udc_t *udc)
{
    if (CHECK_UDC_PLAT_OPS(udc, enable))
        udc->plat_ops.enable(udc);
}
 ...
 * @param udc   the udc
 */
static  void udcPlatDisable(udc_t *udc)
{
    if (CHECK_UDC_PLAT_OPS(udc, disable))
        udc->plat_ops.disable(udc);
}
 ...
 *      - (-1)  fail
 */
static  int udcEpQueue(udc_t *udc, usbEp_t *ep, usbXfer_t *xfer)
{
 ...
 * @param xfer  the usb transfer
 */
static  void udcEpDequeue(udc_t *udc, usbEp_t *ep, usbXfer_t *xfer)
{
 ...
 * @param ep    the usb endpoint
 */
static  void udcEpDequeueAll(udc_t *udc, usbEp_t *ep)
{
 ...
 *      - other usb endpoint
 */
static  usbEp_t *udcEpAlloc(udc_t *udc, usb_endpoint_descriptor_t *desc)
{
 ...
 * @param ep    the usb endpoint
 */
static  void udcEpFree(udc_t *udc, usbEp_t *ep)
{
    if (CHECK_UDC_OPS(udc, ep_free) && ep != NULL)
        udc->ops.ep_free(udc, ep);
}
 ...
 *      - other the xfer
 */
static  usbXfer_t *udcXferAlloc(udc_t *udc)
{
 ...
 * @param xfer  the xfer
 */
static  void udcXferFree(udc_t *udc, usbXfer_t *xfer)
{
    if (CHECK_UDC_OPS(udc, xfer_free) && xfer)
        udc->ops.xfer_free(udc, xfer);
}
 ...
 *      - other success
 */
static  int udcEpEnable(udc_t *udc, usbEp_t *ep)
{
 ...
 * @param ep    the endpoint
 */
static  void udcEpDisable(udc_t *udc, usbEp_t *ep)
{
    if (CHECK_UDC_OPS(udc, ep_disable) && ep != NULL)
        udc->ops.ep_disable(udc, ep);
}
 ...
 */
static  void udcEpStall(udc_t *udc, usbEp_t *ep, bool halt)
{
 ...
 */
static  void udcGdbPollIntr(udc_t *udc)
{
 ...
 *      - 0         success
 */
static  int udevDriverSetup(udevDrv_t *drv, const usb_device_request_t *setup)
{
 ...
 *      - 0     success
 */
static  int udevDriverBind(udevDrv_t *drv)
{
 ...
 * @param drv   the driver
 */
static  void udevDriverUnbind(udevDrv_t *drv)
{
 ...
 * @param drv   the driver
 */
static  void udevDriverStart(udevDrv_t *drv)
{
 ...
 * @param drv   the driver
 */
static  void udevDriverStop(udevDrv_t *drv)
{
 ...
 * @param drv   the driver
 */
static  void udevDriverConnect(udevDrv_t *drv)
{
 ...
 * @param drv   the driver
 */
static  void udevDriverDisconnect(udevDrv_t *drv)
{
 ...
 * @param drv   the driver
 */
static  void udevDriverSuspend(udevDrv_t *drv)
{
 ...
 * @param drv   the driver
 */
static  void udevDriverResume(udevDrv_t *drv)
{
