...
#include "pwm_reg_v5.h"
#include "pwm_drvapi.h"
#include "deepsleep_drvapi.h"
#include "../../timer/timer_phy.h"
...
PUBLIC void PWM_Config (int pwm_num,  uint32  freq,  uint16  duty_cycle)
{
    ...
    base_addr = PWMx_BASE ( (uint32) pwm_num);
    ...
    if (freq <= PWM_MIN_FREQ (pwm_clk))
    {
        ...
        pwm_bit = GEN_CLR_PWMx (pwm_num);
        CHIP_REG_SET (GEN_CLR_PWM, pwm_bit);
        return;
    }
    ...
    if (XTL_E == pwm_clk_source)
    {
    	// 26M
    	uint32 value = 0;
        value = CHIP_REG_GET(CGM_PWM_CFG);
		value &= ~CGM_PWM_SEL_MSK;
		value |= 1<<CGM_PWM_SEL_SHIFT;
		CHIP_REG_SET(CGM_PWM_CFG, value);
    }
    else
    {
    	// 32k rtc
    ...
    pwm_bit = GEN_SET_PWMx (pwm_num);
    CHIP_REG_SET (GEN_SET_PWM, pwm_bit);

    /*set tone parameters*/
    CHIP_REG_SET(base_addr + PWM_PAT_LOW, PWM_PAT_LOW_MSK);
    CHIP_REG_SET(base_addr + PWM_PAT_HIGH,PWM_PAT_HIGH_MSK);
...
PUBLIC uint32 PWM_Ioctl (uint32 pwm_num, uint32 cmd, uint32 *arg)
{
    ...
    SCI_ASSERT (PWM_NUM > pwm_num);/*assert verified*/
