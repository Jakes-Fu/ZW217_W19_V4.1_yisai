 ...
 ******************************************************************************/
#ifndef _TPC_REG_V0_H_
#define _TPC_REG_V0_H_
/*----------------------------------------------------------------------------*
 **                         Dependencies                                      *
 **------------------------------------------------------------------------- */
...
#define TPC_SAMPLE_CTRL                     (TPC_REG_BASE + 0x0004)
#define TPC_BOUNCE_CTRL                     (TPC_REG_BASE + 0x0008)
#define TPC_FILTER_CTRL                     (TPC_REG_BASE + 0x000C)
#define TPC_CALC_CTRL                       (TPC_REG_BASE + 0x0010)
#define TPC_CALC_X_COEF                     (TPC_REG_BASE + 0x0014)
#define TPC_CALC_Y_COEF                     (TPC_REG_BASE + 0x0018)
#define TPC_INT_EN                          (TPC_REG_BASE + 0x001C)
#define TPC_INT_STAT                        (TPC_REG_BASE + 0x0020)
#define TPC_INT_SRC                         (TPC_REG_BASE + 0x0024)
#define TPC_INT_CLR                         (TPC_REG_BASE + 0x0028)
#define TPC_INT_CTRL                        (TPC_REG_BASE + 0x002C)
#define TPC_BUF_DATA                        (TPC_REG_BASE + 0x0030)
#define TPC_Z_BUF_DATA                      (TPC_REG_BASE + 0x0034)
...
typedef union tpc_calc_x_coef_tag
{
    struct tpc_x_coef_ctrl_map
    {
        ...
        volatile unsigned int x_coef_b            :11;//x_coef_b is S11, it is in [-1024, 1023] ;
...
typedef union tpc_calc_y_coef_tag
{
    struct tpc_y_coef_ctrl_map
    {
        ...
        volatile unsigned int reserved_1          :6;//Reserved ;
        volatile unsigned int y_coef_b            :11;//y_coef_b is S11, it is in [-1024, 1023] ;
        volatile unsigned int reserved_2          :5;//Reserved ;
#else
        volatile unsigned int reserved_2          :5;//Reserved ;
        volatile unsigned int y_coef_b            :11;//y_coef_b is S11, it is in [-1024, 1023] ;
...
typedef union tpc_z_buf_data_tag
{
    ...
    {
#if defined(CHIP_ENDIAN_LITTLE) && !defined (_BIG_ENDIAN)
        volatile unsigned int z2                  :10;//Z1 ADC value ;
        volatile unsigned int reserved_1          :6;//Reserved ;
        volatile unsigned int z1                  :10;//Y ADC or position value ;
        volatile unsigned int reserved_2          :6;//Z2 ADC value ;
#else
        volatile unsigned int reserved_2          :6;//Z2 ADC value ;
        volatile unsigned int z1                  :10;//Y ADC or position value ;
        volatile unsigned int reserved_1          :6;//Reserved ;
...
#define TPC_REG_NUM                 (sizeof(TP_REG_T)/sizeof(uint32))
/**----------------------------------------------------------------------------*
