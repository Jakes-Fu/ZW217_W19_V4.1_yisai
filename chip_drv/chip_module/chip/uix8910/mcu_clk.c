...
static void sys_clk_autogate_cfg(void)
{
	...
	hwp_sysCtrlAp->CLK_AP_APB_MODE &= ~(SYS_CTRL_MODE_CLK_AP_APB(0x1FFFFFF) & \
				...
				(~SYS_CTRL_MODE_AP_APB_CLK_ID_SCID1_MANUAL) & \
				(~SYS_CTRL_MODE_AP_APB_CLK_ID_SCID2_MANUAL) & \
...
void clk_axibus_set(uint32 clk)
{
	uint32 div;
	div = clk_get_regval_by_freq(clock_default_config.cpu_pll_freq, clk);
	hwp_sysCtrlAp->CFG_PLL_SYS_AXI_DIV = div;
       hwp_sysCtrlAp->CFG_PLL_SYS_AXI_DIV |= SYS_CTRL_CFG_PLL_SYS_AXI_DIV_UPDATE;
}
