...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.rdamicro.com/svn/developing1/Sources/chip/branches/gallite441/hal/src/hal_spi.c $ //
//    $Author: jingyuli $                                                        //
...
// FULLY REACTIVE without OS Driver before revision 6540
// Rq: The Lock function has been removed, but it could
// easily be reimplemented if needed.
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_8910) || (CHIP_ASIC_ID == CHIP_ASIC_ID_8910M)
#define  SYS_IRQ_SPI1    SYS_IRQ_ID_SPI_1
#endif
...
/// @param sysFreq Newly applied system frequency
// =============================================================================
PRIVATE VOID hal_SpiUpdateAllDividers(HAL_SYS_FREQ_T sysFreq)
{
    HAL_SPI_ID_T i;
    for(i=HAL_SPI; i<HAL_SPI_QTY; i++)
    {
...
PUBLIC BOOL hal_SpiActivateCs(HAL_SPI_ID_T id, HAL_SPI_CS_T csNum)
{
    ...
    if (doActivate)
    {
        ...
        hal_SysRequestFreq((HAL_SYS_FREQ_USER_ID_T)HAL_SYS_FREQ_SPI, suitedSysFreq, hal_SpiUpdateAllDividers);
        ...
        g_halSpiPropArray[id].activeStatus = (HAL_SPI_ACTIVE_T)csNum;
...
PUBLIC VOID hal_SpiForcePin(HAL_SPI_ID_T id, HAL_SPI_PIN_T pin,  HAL_SPI_PIN_STATE_T state)
{
    ...
    //HAL_ASSERT(pin >= HAL_SPI_PIN_CS0,"Invalid SPI pin %d (negative).", pin);   /*clean warning*/
...
PROTECTED VOID hal_SpiIrqHandler(UINT8 interruptId)
{
    ...
    HAL_SPI_ID_T id = (HAL_SPI_ID_T)(interruptId - SYS_IRQ_SPI1);
    HAL_ASSERT(id < HAL_SPI_QTY, "Improper SPI id %d ! \n"
               " Your system only has %d SPI", id, HAL_SPI_QTY);
