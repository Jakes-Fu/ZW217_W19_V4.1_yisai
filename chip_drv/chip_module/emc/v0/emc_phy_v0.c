...
PUBLIC void EMC_SysStartUp (void)
{
    ...
    {
        volatile uint32 j,arm_ahb_clk;
        //MPLL is open already in RomCode
        j = REG32 (AHB_CLK_CFG0);

        //j |= 0xC << 4;               //Src_sel_emc:4'b1100 : MPLL_DIV2:

        if(0x68000000 == REG32 (CHIP_TYPE))
    {
        //set CLK_EMC to 200MHz
        j &= ~ (BIT_7 | BIT_6 | BIT_5 | BIT_4);
        j |= 0xC << 4;//j |= 0x8 << 4;               //Src_sel_emc:4'b1100 : MPLL_DIV2:
        arm_ahb_clk = ARM_CLK_100M;
    }
    else
    {
        //set CLK_EMC to 180MHz
        j |= BIT_4;               //Src_sel_emc to PLL_DIV4
        arm_ahb_clk = ARM_CLK_90M;
    }
    
        j &= ~ (BIT_9 | BIT_8);      //Div_cfg_emc:0x0
        //set CLK_ARM to 200MHz
        j &= ~ (BIT_2 | BIT_1);      //Div_cfg_arm:0x01
        //set CLK_AHB to 100MHz
        j |= BIT_3;
        //set Src_sel_arm to  MPLL/2(200Mhz)
        j |= BIT_0;
        REG32 (AHB_CLK_CFG0) = j;
