...
)
{
    ...
    switch (blk_hal)
    {
        case EFUSE_UID_START_ID:
            ...
            max_id = EFUSE_PHY_BLK_ID_3;
            break;
        case EFUSE_HASH_START_ID:
            min_id = EFUSE_PHY_BLK_ID_4;
            max_id = EFUSE_PHY_BLK_ID_11;
            break;
        case EFUSE_MAX_ID:
...
)
{
    ...
    {
         /* 
            block 0~3:      uid for first 4 block, 
            blcok 4~11:     hash value for middle 8 block,
            blcok 12~15:    reserved.
         */
        case EFUSE_MIN_ID:      
        ...
        case EFUSE_HASH_END_ID:
            phy_blk_id = EFUSE_PHY_BLK_ID_11;
            break;
        case EFUSE_MAX_ID:
...
PUBLIC void EFUSE_PHY_InitReset (void)
{
    ...
    CHIP_REG_SET (APB_RST0_SET, EFUSE_SOFT_RST_SET);

    for (i = 0; i < 100; i++) {};

    CHIP_REG_SET (APB_RST0_CLR, EFUSE_SOFT_RST_CLR);
}
...
)
{
    ...
    if (is_enable)
    {
        ...
        CHIP_REG_SET (ANA_PROT_CTL2, 0xc686);//efuse protect
        CHIP_REG_OR (ANA_MISC_CTL6, EFS_2P5V_PWR_ON);//efuse 2.5 power on
    ...
    else
    {
        ...
        CHIP_REG_AND (ANA_MISC_CTL6, ~EFS_2P5V_PWR_ON);
        CHIP_REG_SET (ANA_PROT_CTL2, 0);
