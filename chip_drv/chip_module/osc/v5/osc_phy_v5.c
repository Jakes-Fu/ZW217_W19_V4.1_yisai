    ...
    if (isEnable)
    {
        switch (clk_src)
        {
            ...
            case CLOCK_DCXO_32K:
                ...
                OSC_Delay (3);
                OSC_REG_AND (ANA_LDO_PD_SET0,~LDO_DCXO_PD);
                OSC_Delay (3);
                OSC_REG_OR (ANA_LDO_PD_RST0,LDO_DCXO_PD_RST);
                OSC_Delay (3);
                break;
            case CLOCK_EXTE_32K:
                OSC_REG_AND (ANA_XTL_CTL1,~CLK_OSC32K_PD_SET); //open external 32k crystal
                OSC_Delay (3);
                OSC_REG_OR (ANA_XTL_CTL1,CLK_OSC32K_PD_CLR);
                OSC_Delay (3);
                break;
            default:
                break;
    ...
    else
    {
        switch (clk_src)
        {
            ...
            case CLOCK_DCXO_32K:
                ...
                OSC_Delay (3);
                OSC_REG_AND (ANA_LDO_PD_RST0,~LDO_DCXO_PD_RST);
                OSC_Delay (3);
                OSC_REG_OR (ANA_LDO_PD_SET0,LDO_DCXO_PD);
                OSC_Delay (3);
                break;
            case CLOCK_EXTE_32K:
                OSC_REG_AND (ANA_XTL_CTL1,~CLK_OSC32K_PD_CLR); //close external 32k clock
                OSC_Delay (3);
                OSC_REG_OR (ANA_XTL_CTL1,CLK_OSC32K_PD_SET);
                OSC_Delay (3);
                break;
            default:
                break;
...
{
    uint16 src_sel[4];
    src_sel[0] = (uint16) (OSC_REG_GET (ANA_MISC_CTL8) & CLK_SELF32K_SEL_SET);
    src_sel[1] = (uint16) (OSC_REG_GET (ANA_MISC_CTL8) & CLK_SELF32K_SEL_CLR);
    src_sel[2] = (uint16) (OSC_REG_GET (ANA_MISC_CTL9) & CLK_EXT32K_SEL_SET);
    src_sel[3] = (uint16) (OSC_REG_GET (ANA_MISC_CTL9) & CLK_EXT32K_SEL_CLR);
...
PUBLIC void OSC_PHY_SwitchClkSource (CLOCK_SOURCE_E clk_src)
{
    ...
    {
        case CLOCK_SELF_32K:
            OSC_REG_AND (ANA_MISC_CTL8,~CLK_SELF32K_SEL_CLR); //用K2开关选择self 32k
            OSC_Delay (3);
            OSC_REG_OR (ANA_MISC_CTL8,CLK_SELF32K_SEL_SET);
            OSC_Delay (3);
            OSC_REG_AND (ANA_MISC_CTL9,~CLK_EXT32K_SEL_SET); //用K3开关来控制不选择external 32k
            OSC_Delay (3);
            OSC_REG_OR (ANA_MISC_CTL9,CLK_EXT32K_SEL_CLR);
            OSC_Delay (3);
            break;
        case CLOCK_DCXO_32K:
            OSC_REG_AND (ANA_MISC_CTL8,~CLK_SELF32K_SEL_SET); //用K2开关选择 DCXO 32k
            OSC_Delay (3);
            OSC_REG_OR (ANA_MISC_CTL8,CLK_SELF32K_SEL_CLR);
            OSC_Delay (3);
            OSC_REG_AND (ANA_MISC_CTL9,~CLK_EXT32K_SEL_SET); //用K3开关控制不选择external 32k
            OSC_Delay (3);
            OSC_REG_OR (ANA_MISC_CTL9,CLK_EXT32K_SEL_CLR);
            OSC_Delay (3);
            break;
        case CLOCK_EXTE_32K:
            OSC_REG_AND (ANA_MISC_CTL9,~CLK_EXT32K_SEL_CLR); //用K3开关选择external 32k
            OSC_Delay (3);
            OSC_REG_OR (ANA_MISC_CTL9,CLK_EXT32K_SEL_SET);
            OSC_Delay (3);
            break;
        default:
            break;
...
PUBLIC void OSC_PHY_DCXO_ChangeMode (DCXO_WORKMODE_E dcxo_mode)
{
    switch (dcxo_mode)
    {
        case NORMALCURRENT_MODE:
            OSC_REG_OR (ANA_LDO_PD_RST0,LDO_DCXO_LOW_CURRENT_MODE_RST);
            break;
        case LOWCURRENT_MODE:
            OSC_REG_OR (ANA_LDO_PD_SET0,LDO_DCXO_LOW_CURRENT_MODE_SET);
            break;
        default:
            break;
    }
...
{
    OSC_REG_OR (ANA_MISC_CTL2,DCXO_MODE_SWT1);
    OSC_REG_AND (ANA_MISC_CTL2,~DCXO_MODE_SWT2);
...
PUBLIC void OSC_PHY_EnableRefAutoSleep(DCXO_REF_E dcxo_ref,BOOLEAN isEnable)
{
	if(isEnable)
	{
		...
		{
			case DCXO_REF1:
				OSC_REG_OR(ANA_MISC_CTL8,DCXO_REF1_XTL_AUTO_EN);
				break;
			case DCXO_REF2:
				OSC_REG_OR(ANA_MISC_CTL8,DCXO_REF2_XTL_AUTO_EN);
				break;
			default:
				break;
		}
	...
	else
	{
		...
		{
			case DCXO_REF1:
				OSC_REG_AND(ANA_MISC_CTL8,~DCXO_REF1_XTL_AUTO_EN);
				break;
			case DCXO_REF2:
				OSC_REG_AND(ANA_MISC_CTL8,~DCXO_REF2_XTL_AUTO_EN);
				break;
			default:
				break;
		}		
