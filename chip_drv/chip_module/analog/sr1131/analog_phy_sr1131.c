...
PUBLIC void _ANA_SetLCMBrightness (uint32  value)
{
    ...
    if ( (lcm_onoff_sts & WHTLED_PWR_ON) == 0)
    {
...
PUBLIC void _ANA_SetLCMBackLight (uint32  value)
{
    ...
    if (ANA_DEV_CLOSE == value)
    {
        ...
        _ANA_SetValByStep(&lcm_level_info);
         ...
         //close lcm backlight
        ANA_REG_AND (ANA_LED_CTL0, ~ (WHTLED_PWR_ON));
    }
    else
    {
        ...
        //open lcm backlight
        ANA_REG_OR (ANA_LED_CTL0, WHTLED_PWR_ON);
...
LOCAL void _ANA_SetFlashLight (uint32 value)
{
    if (ANA_DEV_CLOSE == value)
    {
        ...
        ANA_REG_AND (ANA_LED_CTL0, ~(IB5_SW_ON_OFF|IB4_SW_ON_OFF));    
    }
    else
    {
        //open white-led IB4/5 output
        ANA_REG_OR (ANA_LED_CTL0, IB5_SW_ON_OFF|IB4_SW_ON_OFF);  
    }
