 ...
 */
  ...
  rs->alpha_to = (data_t *)SCI_ALLOC_APP(sizeof(data_t)*(rs->nn+1));
  if(rs->alpha_to == NULL){
    SCI_FREE(rs);
    rs = NULL;
    goto done;
  }
  rs->index_of = (data_t *)SCI_ALLOC_APP(sizeof(data_t)*(rs->nn+1));
  if(rs->index_of == NULL){
    SCI_FREE( rs->alpha_to);
    SCI_FREE(rs);
    rs = NULL;
    goto done;
  }
  ...
  rs->genpoly = (data_t *)SCI_ALLOC_APP(sizeof(data_t)*(nroots+1));
  if(rs->genpoly == NULL){
    SCI_FREE( rs->alpha_to);
    SCI_FREE( rs->index_of);
    SCI_FREE( rs);
...
RS *init_rs(int symsize, int gfpoly, int fcr, int prim, int nroots, int pad)
{
...
#endif
	SCI_TRACE_LOW("QRcode_encodeMask init_rs enter");
	...
	SCI_TRACE_LOW("QRcode_encodeMask init_rs_char before");
