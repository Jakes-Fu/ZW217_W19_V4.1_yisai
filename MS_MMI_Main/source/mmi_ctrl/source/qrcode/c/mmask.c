...
LOCAL void MMask_writeFormatInformation(int version, int width, unsigned char *frame, int mask, QRecLevel level)
{
	unsigned int format;
	unsigned char v;
	int i;
...
unsigned char *MMask_mask(int version, unsigned char *frame, QRecLevel level)
{
	...
	mask = (unsigned char *)SCI_ALLOC_APP(width * width);
