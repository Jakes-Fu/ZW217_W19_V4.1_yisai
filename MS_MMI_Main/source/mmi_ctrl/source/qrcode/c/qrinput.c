...
static QRinput_List *QRinput_List_newEntry(QRencodeMode mode, int size, const unsigned char *data)
{
    ...
    {
        SCI_TRACE_LOW(" QRinput_List_newEntry QRinput_check return ");
        return NULL;
    }
    SCI_TRACE_LOW(" QRinput_List_newEntry");

    entry = (QRinput_List  *)SCI_ALLOC_APP( sizeof(QRinput_List) );
    memset(entry, 0, sizeof(QRinput_List));
    if(entry == NULL) return NULL;
    SCI_TRACE_LOW(" QRinput_List_newEntry malloc entry ");

    entry->mode = mode;
    entry->size = size;
    if(size > 0)
    {
        entry->data = (unsigned char  *)SCI_ALLOC_APP(size);
        ...
        SCI_TRACE_LOW(" QRinput_List_newEntry malloc entry->data ");
        memcpy(entry->data, data, size);
    }
    entry->bstream = NULL;
    entry->next = NULL;
    SCI_TRACE_LOW(" QRinput_List_newEntry end ");
    return entry;
}
...
static QRinput_List *QRinput_List_dup(QRinput_List *entry)
{
    ...
    n = (QRinput_List *)SCI_ALLOC_APP(sizeof(QRinput_List));
    if(n == NULL) return NULL;

    n->mode = entry->mode;
    n->size = entry->size;
    n->data = (unsigned char *)SCI_ALLOC_APP(n->size);
    if(n->data == NULL)
    {
        SCI_FREE(n);
        return NULL;
...
{
    QRinput *input;
    SCI_TRACE_LOW("enter QRinput_new2");
    if(version < 0 || version > QRSPEC_VERSION_MAX || level > QR_ECLEVEL_H)
    {
        SCI_TRACE_LOW("version error return");
        return NULL;
    }
    SCI_TRACE_LOW("QRinput_new2 befor malloc");
    input = (QRinput *)SCI_ALLOC_APP(sizeof(QRinput));
    memset(input, 0, sizeof(QRinput));
    if(input == NULL) return NULL;
    SCI_TRACE_LOW("QRinput_new2 after malloc");
    input->head = NULL;
    input->tail = NULL;
...
{
    SCI_TRACE_LOW(" QRinput_appendEntry start ");
    if(input->tail == NULL)
    {
        input->head = entry;
        input->tail = entry;
    ...
    entry->next = NULL;
    SCI_TRACE_LOW(" QRinput_appendEntry end ");
}
...
int QRinput_append(QRinput *input, QRencodeMode mode, int size, const unsigned char *data)
{
    ...
    SCI_TRACE_LOW("QRinput_append");

    entry = QRinput_List_newEntry(mode, size, data);
    if(entry == NULL)
    {
        SCI_TRACE_LOW("QRinput_append QRinput_List_newEntry NULL");
        return -1;
    }
...
static int QRinput_appendPaddingBit(BitStream *bstream, QRinput *input)
{
    ...
    if(padlen > 0)
    {
        padbuf = (unsigned char *)SCI_ALLOC_APP(padlen);
        if(padbuf == NULL)
        {
            ret = -1;
            goto DONE;
...
static QRinput_InputList *QRinput_InputList_newEntry(QRinput *input)
{
    ...
    entry = (QRinput_InputList *)SCI_ALLOC_APP(sizeof(QRinput_InputList));
    if(entry == NULL) return NULL;
...
QRinput_Struct *QRinput_Struct_new(void)
{
    ...
    s = (QRinput_Struct *)SCI_ALLOC_APP(sizeof(QRinput_Struct));
...
static int QRinput_List_shrinkEntry(QRinput_List *entry, int bytes)
{
    ...
    data = (unsigned char *)SCI_ALLOC_APP(bytes);
