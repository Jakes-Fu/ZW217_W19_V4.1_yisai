...
BitStream *BitStream_new(void)
{
    ...
    bstream = (BitStream *)SCI_ALLOC_APP(sizeof(BitStream));
    if(bstream == NULL)
    {
        return NULL;
    } 
...
static int BitStream_allocate(BitStream *bstream, int length)
{
    ...
    data = (unsigned char *)SCI_ALLOC_APP(length);
    if(data == NULL) 
    {
        return -1;
    }
...
int BitStream_append(BitStream *bstream, BitStream *arg)
{
    ...
    data = (unsigned char *)SCI_ALLOC_APP(bstream->length + arg->length);
    if(data == NULL) 
    {
        return -1;
    }
...
unsigned char *BitStream_toByte(BitStream *bstream)
{
    ...
    data = (unsigned char *)SCI_ALLOC_APP((size + 7) / 8);
    if(data == NULL) 
    {
        return NULL;
    }
