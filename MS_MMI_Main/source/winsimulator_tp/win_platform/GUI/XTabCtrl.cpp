...
BOOL CXTabCtrl::SelectTabByName(LPCTSTR lpszTabCaption)
{
	...
	for (int i=0; i < GetItemCount(); i++)
	{
		...
		if (_tcscmp(lpszTabCaption, item.pszText) == 0)
			return SelectTab(i);
	}
...
{	
	...
	for (int i=0; i < GetItemCount(); i++)
	{
		...
		if(pWnd != NULL)
		{
			...
			//pWnd->SetWindowPos(NULL,rect.left,rect.top, rect.Width()+(cx-rcOrg.Width()),rect.Height() + (cy-rcOrg.Height()),SWP_DRAWFRAME);
			
			pWnd->GetClientRect(&rect);
			int w = rect.Width();
			int h = rect.Height();
			DWORD dwSize= MAKELONG(w,h);
			pWnd->PostMessage(WM_SIZE,nType,(LPARAM)dwSize);
