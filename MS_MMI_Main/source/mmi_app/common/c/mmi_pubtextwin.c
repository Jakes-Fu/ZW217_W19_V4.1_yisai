...
/*                         TYPE AND CONSTANT                                 */
/*---------------------------------------------------------------------------*/
//pub text win infomation
typedef struct
{
    MMIPUB_TYPE_E           type;
    PROCESSMSG_FUNC         user_func;                  //用户传入的回调
    MMI_HANDLE_T            ctrl_handle;                //text handle
...
/*****************************************************************************/
LOCAL MMI_RESULT_E HandlePubTextWinMsg(
                                       MMI_WIN_ID_T      win_id, 
                                       ...
                                       )
{
    ...
    MMIPUB_TEXT_INFO_T*  win_info_ptr = MMIPUB_GetWinAddDataPtr(win_id);
    MMIPUB_TEXT_NOTIFY_T notify       = {0};

    if ( PNULL == win_info_ptr || MMIPUB_TEXT != win_info_ptr->type )
    {
    ...
    if ( MMI_RESULT_FALSE == result )
    {
        ...
        case MSG_OPEN_WINDOW:
            MMK_SendMsg( win_info_ptr->dst_handle, MSG_PUBTEXT_OPEN, &notify );
        ...
        case MSG_APP_WEB:
        case MSG_CTL_MIDSK:
            MMK_SendMsg( win_info_ptr->dst_handle, MSG_PUBTEXT_OK, &notify );
            break;
            
        case MSG_APP_CANCEL:
        case MSG_CTL_CANCEL:
            MMK_SendMsg( win_info_ptr->dst_handle, MSG_PUBTEXT_CANCEL, &notify );
            MMK_CloseWin( win_id );
            break;
...
/*****************************************************************************/
//  Description : open text win
//  Global resource dependence : 
//  Author: James.Zhang
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIPUB_OpenTextWin(
                                  MMIPUB_TEXT_PARAM_T* param
                                  )
{
    ...
    {    
        ...
        MMIPUB_TEXT_INFO_T* win_info_ptr  = PNULL;
        ...
        {            
            ...
            win_info_ptr = SCI_ALLOC_APP(sizeof(MMIPUB_TEXT_INFO_T));
            SCI_MEMSET(win_info_ptr,0,sizeof(MMIPUB_TEXT_INFO_T));
            
            //set user data
            win_info_ptr->type = MMIPUB_TEXT;
            ...
            win_create.func = HandlePubTextWinMsg;
            ...
            if( 0 != ( win_handle = MMK_CreateWindow( &win_create ) ) )
            {
                ...
                GUITEXT_INIT_DATA_T    text_data    = {0};
                ...
                GUITEXT_SetString( win_info_ptr->ctrl_handle, param->string.wstr_ptr, param->string.wstr_len, FALSE );
        ...
        {
            //SCI_TRACE_LOW:"MMIPUB_OpenTextWin:win_id %d is already open!"
            SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMI_PUBTEXTWIN_245_112_2_18_1_58_39_52,(uint8*)"d", win_id);
        }
    }
