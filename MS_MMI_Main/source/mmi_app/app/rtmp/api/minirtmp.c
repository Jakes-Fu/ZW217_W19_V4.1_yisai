...
#if !(defined(_WIN32) || defined(REAL_WATCH_RTOS))
#include <poll.h>
#endif
...
int minirtmp_write(MINIRTMP *r, uint8_t *data, int size, uint32_t timestamp, int is_video, int keyframe, int stream_hdrs)
{
    ...
    if (r->flv_buf_size < (size + 32))
    {
        ...
        if (r->flv_buf)
            rtmp_free(r->flv_buf);
        r->flv_buf = rtmp_malloc(r->flv_buf_size);
    }

    flv_size = format_flv(r->flv_buf, data, size, is_video ? 9 : 8, timestamp, timestamp, keyframe, stream_hdrs);
...
int minirtmp_read(MINIRTMP *r)
{
    ...
    if (!RTMP_IsConnected(r) || !RTMP_ReadPacket(r->rtmp, &r->rtmpPacket))
        return MINIRTMP_EOF;
    if (RTMPPacket_IsReady(&r->rtmpPacket))
    {
        r->packet_reveived = 1;
...
int minirtmp_init(MINIRTMP *r, const char *url, int stream, RMTP_QUIT_CALLBACK cb)
{
    memset(r, 0, sizeof(*r));
    r->rtmp = RTMP_Alloc();
    RTMP_Init(r->rtmp);
	r->rtmp->quit_streaing = cb;
