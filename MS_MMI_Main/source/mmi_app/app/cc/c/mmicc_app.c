...
#define MMICC_ECC_MAX_LEN   MN_MAX_SIM_ECC_LEN
#elif defined MODEM_SUPPORT_RDA
#define MMICC_ECC_MAX_LEN   MN_MAX_ECC_LEN
#endif
...
LOCAL void CC_CallAlertingInd(DPARAM param)
{
    ...
    case MN_CALL_REMOTE_USER_ALERTING:
        ...
        {//w217 只支持volte电话
            StopLocalNetRing();
            SCI_TRACE_LOW("volte call: CC_CallAlertingInd() Remote ring: %d ",s_call_context.pscall_info.is_volte_codec_on);
                if(!s_call_context.pscall_info.is_volte_codec_on)
                {
                //in fact, volte codec is already enable when receive APP_MN_CALL_EARLY_MEDIA_IND
                CC_EnableVoLTECodec(TRUE);
...
#elif MODEM_SUPPORT_RDA
        {
            SCI_TRACE_LOW("volte call: CC_CallAlertingInd() local ring: %d ",pSig->alerting_type);
            HandleMoMuteAndLocalRingState(APP_MN_ALERTING_IND);
        }
#endif
        {
...
LOCAL void CC_ParseMTNumberFromSipUri(DPARAM param)
{
    ...
    uint8 tel_num_len = 0;
    uint8 trans_len = 0;
    uint8 uri_addr[MN_MAX_NUMBER_STRING_LEN+1];

    SCI_TRACE_LOW("CC_ParseMTNumberFromSipUri pSig->calling_num_present=%d",pSig->calling_num_present);

    //号码信息保存在uri_addr字段中
    if (!pSig->calling_num_present)
    {
#ifdef MODEM_SUPPORT_SPRD
    SCI_MEMCPY(uri_addr,pSig->uri_addr,MN_IMS_MAX_URI_STRING_LEN);
#elif MODEM_SUPPORT_RDA
    SCI_MEMCPY(uri_addr,pSig->calling_num.calling_uri,MN_MAX_NUMBER_STRING_LEN);
#endif
        //check if it is  uri number or not
        if (strlen((char*)uri_addr) > 0)
        {
            SCI_TRACE_LOW("CC_ParseMTNumberFromSipUri   uri_addr = %s",uri_addr);
            memset(&uri_number,0,sizeof(URI_NUMBER_T));
            if (CC_GetUriPhoneNumber(uri_addr , &uri_number))
            {
                                 ...
                                 )
{
   ...
   is_emergency_call_in_local = CC_IsEmergencyInLocalEmergencyList(tele_num, tele_len, &is_local_exisit, dual_sys);
    ...
    {    
        ...
        {
            uint8 ecc_code[MMICC_ECC_MAX_LEN*2 + 2] = {0};
            uint8 tel_num_len = 0;
            ...
            tel_num_len = MMIAPICOM_GenNetDispNumber(
                ...
                (uint8)(MMICC_ECC_MAX_LEN),
                (uint8*)sim_emc.call_codes[i].ecc_code,
                ecc_code,
                (uint8)(MMICC_ECC_MAX_LEN*2+2)
                );
    ...
    for (j = 0; j < MMI_DUAL_SYS_MAX; j++)
    {
        if (MMIAPIPHONE_GetSimExistedStatus(j))
        {
            ...
            if (0 < sim_emc.validate_num)
            {
                ...
                {
                    uint8 ecc_code[MMICC_ECC_MAX_LEN*2 + 2] = {0};
                    wchar ecc_code_wstr[MMICC_ECC_MAX_LEN*2 + 2] = {0};
                    uint8 tel_num_len = 0;
                    ...
                    tel_num_len = MMIAPICOM_GenNetDispNumber(
                        ...
                        temp_emergency_call_len = MIN(tel_num_len, MMICC_ECC_MAX_LEN*2);
                    ...
                    {
                        *emergency_call_len_ptr = MIN(tel_num_len, MMICC_ECC_MAX_LEN*2);
                        is_input_emergency_call = TRUE;
   ...
   )
{
    ...
    if (TRUE == MMIAPICC_IsEmergencyNum(tele_num,tele_len,dual_sys,TRUE))
    {
        SCI_TRACE_LOW("make ecc call");
        s_cc_mo_error_type = CC_MO_ECC_CALL;
        return CC_RESULT_ERROR;
    }
#endif
   ...
   //parse call type
    ...
    )
    {
           ...
           || MMI_PARSE_NORMAL_CALL_TYPE_E == parse_result)
       {
            ...
            if (!MMIAPIPHONE_GetVolteStatus(dual_sys))
            {
                SCI_TRACE_LOW("MakeCall not support ip call");
                s_cc_mo_error_type = CC_MO_NO_VOLTE_CALL;
                return CC_RESULT_ERROR;
            }
...
#endif  
{
    ...
    case CC_WAITING_STATE:
		...
		if(1 != MMICC_GetCallNum())
		{
			...
			{			
#ifndef MMI_MULTI_SIM_SYS_NONE			
				SCI_TRACE_LOW("MMIAPIBT_HfuHangup");
                s_call_context.operate_waiting_call = RELEASE_ALL_ACTIVE_AND_ACCEPT;
                MMIAPIBT_HfuHangup();
#else
                MMICC_OpenCCMenu();
    ...
    case CC_INCOMING_CALL_STATE:
        ...
        if(MMICC_GetBtCallFlag())
        {
            bt_status = MMIAPIBT_HfuAnswerCall();
            if(BT_PENDING != bt_status && BT_SUCCESS != bt_status)
           {
                SCI_TRACE_LOW("ccapp.c:MMICC_AnswerCall() MMIAPIBT_HfuAnswerCall fail");
                return FALSE;
           }
        }
        else
   ...
   )
{
    ...
    uint32 i = 0;
    BOOLEAN is_emergency_call = FALSE;
    MMIPB_BCD_NUMBER_T * pb_num_ptr = {0};
    BOOLEAN result = FALSE;*/
    ...
    if(CC_INVALID_INDEX != s_bt_current_call_index && MMICC_GetBtCallFlag())
    {
        SCI_TRACE_LOW("ccapp.c: MMIAPICC_MakeBtCall()  s_call_context.call_number= %d,s_bt_current_call_index=%d",s_call_context.call_number,s_bt_current_call_index);
        MMIPUB_OpenAlertWarningWin(TXT_DIAL_FAILED);
        return CC_RESULT_ERROR;
    }
#endif
    ...
    MMICC_SetBtCallFlag(TRUE);
    GetWantMoCallInfo(MN_DUAL_SYS_1, tele_num, tele_len, PNULL, PNULL, PNULL, &s_call_context.want_mo_call);

    bt_status = MMIAPIBT_HfuDialNumber(tele_num);
    if(BT_PENDING != bt_status && BT_SUCCESS != bt_status)
    {
        MMICC_SetBtCallFlag(FALSE);
        ClearWantCallInfo();
