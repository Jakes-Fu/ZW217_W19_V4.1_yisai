...
** 
****************************************************************************/
#ifndef _MMIZDTHTTP_WINTAB_C_
#define _MMIZDTHTTP_WINTAB_C_
...
#define  ZDTHTTP_TEST_MENU_LEN  20
#define  ZDTHTTP_TEST_MENU_NUM  5
const uint8 zdthttp_test_menu_text[ZDTHTTP_TEST_MENU_NUM][ZDTHTTP_TEST_MENU_LEN]= 
{
    "Trace Del",
    "Trace Save",
    "Log Enable",
...
LOCAL MMI_RESULT_E HandleZDTHTTP_TestMainMenuWinMsg (
                                      MMI_WIN_ID_T   win_id, 	// 窗口的ID
                                      MMI_MESSAGE_ID_E 	msg_id, 	// 窗口的内部消息ID
                                      DPARAM            param		// 相应消息的参数
                                      );

WINDOW_TABLE( MMIZDTHTTP_TEST_MENU_WIN_TAB ) = 
{
    WIN_FUNC((uint32) HandleZDTHTTP_TestMainMenuWinMsg),    
    WIN_ID( MMIZDTHTTP_TEST_MENU_WIN_ID ),
    WIN_TITLE(TXT_ZDTHTTP_TEST),
    //WIN_STATUSBAR,
    WIN_SOFTKEY(TXT_COMMON_OK, TXT_NULL, STXT_RETURN),
    CREATE_LISTBOX_CTRL( GUILIST_TEXTLIST_E, MMIZDTHTTP_TEST_MENU_CTRL_ID),
    END_WIN
};

BOOLEAN MMIZDTHTTP_OpenTestMenuWin(void)
{
    MMK_CreateWin((uint32*)MMIZDTHTTP_TEST_MENU_WIN_TAB,PNULL);
}
                                            ...
                                            )
{
    ...
    MMI_CTRL_ID_T            ctrl_id = MMIZDTHTTP_TEST_MENU_CTRL_ID;
    uint16                    node_id = 0;
    ...
    case MSG_OPEN_WINDOW:
        {
            ...
            GUILIST_SetMaxItem(ctrl_id, ZDTHTTP_TEST_MENU_NUM,FALSE);   
            
            for ( i = 0; i < ZDTHTTP_TEST_MENU_NUM; i++ )
            {
                item_t.item_style    = GUIITEM_STYLE_ONE_LINE_ICON_TEXT;
                item_t.item_data_ptr = &item_data;
                copy_len = SCI_STRLEN(zdthttp_test_menu_text[i]);
                ...
                GUI_GB2UCS(item_data.item_content[1].item_data.text_buffer.wstr_ptr, zdthttp_test_menu_text[i],copy_len );
