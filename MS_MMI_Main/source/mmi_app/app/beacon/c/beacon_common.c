...
BOOLEAN           g_beacon_is_charge = FALSE;
uint8                 g_beacon_is_bat_low = 0;
uint8                 g_beacon_is_bat_full = 0;
BOOLEAN           g_beacon_is_auto_answer = FALSE;
BOOLEAN           g_beacon_bt_is_on = FALSE;


#define BEACON_POWER10or16_GRADE  8

const uint32 g_dwaBeaconPower10[BEACON_POWER10or16_GRADE] = 
{
    1, 10, 100, 1000, 10000, 100000, 1000000, 10000000
};

// Power of 16
const uint32 g_dwaBeaconPower16[BEACON_POWER10or16_GRADE] = 
{
...
uint8 BEACON_Nibble2HexChar(uint8 bNibble)
{
    uint8  bRtn = '0';
...
uint8 BEACON_HexChar2Nibble(uint8 bHexChar)
{
    uint8  bRtn = 0;
...
uint8 BEACON_GetNextPara(T_BT_GET_PARA_TYPE tType, uint8** pCmd, uint16* pLen, uint32* pNum, uint8* pStr, uint16 wMaxStrLen)
{
    uint8  bRtn = 1, c, bHaveNextComma = FALSE, bDoubleQuotes = 0, MaxNumLen = BEACON_POWER10or16_GRADE;
    int    i, L = 0, EndIndex = 0;

    for(i=0; i < *pLen; i++)
    {
        if(tType == BT_GET_STR_WITH_DQ)
        {
            if(bDoubleQuotes < 2)
            {
                if(*(*pCmd + i) == ',')
    ...
    if(EndIndex)
    {
        if(tType == BT_GET_NUM_DEC)  // Get decimal number
        {
            *pNum = 0;
        ...
        else if(tType == BT_GET_NUM_HEX)  // Get hexadecimal number
        {
            *pNum = 0;
            for(i=EndIndex-1; i>=0; i--)  // Scan from the last digit
            {
                ...
                if( (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F'))
                {
                    if(MaxNumLen)
                    {
                        *pNum += ( BEACON_HexChar2Nibble(c) * g_dwaBeaconPower16[L++] );
                        MaxNumLen --;
                    }
                }
                else
        ...
        else    // Get string
        {
            ...
            for(i=0; i<EndIndex; i++)
            {
                ...
                {
                    if(tType==BT_GET_STR_WITH_DQ && *(*pCmd + i)=='"')
                    {
                        bDoubleQuotes ++;
                        continue;
                    }
            ...
            if(tType==BT_GET_STR_WITH_DQ && bDoubleQuotes!=2)
...
uint16 BEACON_UCS_Str16_to_uint16(
                           uint8*  src_ptr, //[IN]  String的源指针
                           uint16  src_len, //[IN]  源String的长度，byte数
                           ...
                           )
{
    ...
    len = BEACON_Str16_to_Int(src_ptr,src_len,tmp_buf);
    len = len/2;
    for(i = 0; i < len; i++)
    {
        if(i < dest_max_len)
...
uint32  BEACON_Get_DayTimeSecond(void)
{
    uint32 daysecond;
    SCI_TIME_T          time;
    TM_GetSysTime(&time);
...
uint32  BEACON_Get_TimeSecond(void)
{
    SCI_TM_T sys_time = {0};
    uint32   second = 0;
...
BOOLEAN BEACON_GetIsCharge(void)
{
    return g_beacon_is_charge;
}

void BEACON_SetIsCharge(BOOLEAN is_charge)
{
    if(g_beacon_is_charge != is_charge)
    {
        g_beacon_is_charge = is_charge;
#ifdef BEACON_LED_SUPPORT
        if(is_charge)
        {
...
uint8 BEACON_GetIsBatLow(void)
{
    return g_beacon_is_bat_low;
}

void BEACON_SetIsBatLow(uint8 is_low)
{
    if(g_beacon_is_bat_low != is_low)
    {
        g_beacon_is_bat_low = is_low;
#ifdef BEACON_PLAT_YX_SUPPORT
...
uint8 BEACON_GetIsBatFull(void)
{
    return g_beacon_is_bat_full;
}

void BEACON_SetIsBatFull(uint8 is_full)
{
    if(g_beacon_is_bat_full != is_full)
    {
        g_beacon_is_bat_full = is_full;
#ifdef BEACON_LED_SUPPORT
        if(is_full && g_beacon_is_charge)
        {
...
BOOLEAN BEACON_GetIsAutoAnswer(void)
{
    return g_beacon_is_auto_answer;
}

void BEACON_SetIsAutoAnswer(BOOLEAN is_auto)
{
    g_beacon_is_auto_answer = is_auto;
    return;
}

BOOLEAN BEACON_GetIsBTOn(void)
{
    return g_beacon_bt_is_on;
}

void BEACON_SetIsBTOn(BOOLEAN is_on)
{
    g_beacon_bt_is_on = is_on;
    return;
}

BOOLEAN BEACON_SIM_Exsit(void)
{
...
#if 1
BOOLEAN  BEACON_DBFile_Write(char * path,char * filename, uint8 * data_ptr, uint32  data_len)
{
...
BOOLEAN  BEACON_DBFile_Exist(char * path, char * filename)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    wchar * file_dev_ptr;
...
uint32  BEACON_DBFile_GetSize(char * path, char * filename)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    wchar * file_dev_ptr;
...
BOOLEAN  BEACON_DBFile_Read(char * path, char * filename, void *buffer, uint32 bytes_to_read, uint32 *bytes_read)
{
    BOOLEAN ret = FALSE;
...
BOOLEAN  BEACON_DBFile_RemoveFile(char * path, char * filename)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    wchar * file_dev_ptr;
...
BOOLEAN  BEACON_DBFile_RemoveDir(char * path	)
{
    BOOLEAN ret = FALSE;
...
BOOLEAN BEACON_File_MemFull(BOOLEAN is_tf)
{
...
BOOLEAN BEACON_File_Save(const char * file_name,uint8  * data_ptr, uint32 data_len,BOOLEAN is_add)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    uint16   tmp_len = 0;
...
BOOLEAN BEACON_File_Remove(const char * file_name)
{	
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    uint16   tmp_len = 0;
...
BOOLEAN BEACON_File_Read(const char * file_name,uint8  * data_ptr, uint32  data_len, uint32 * read_len)
{
    BOOLEAN ret = FALSE;
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
...
BOOLEAN BEACON_File_Exsit(const char * file_name)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    uint16   tmp_len = 0;
...
BOOLEAN BEACON_Dir_Exsit(const char * dir_path)
{
    SFS_HANDLE	hFind    = PNULL;
    SFS_FIND_DATA_T sfsfind             = {0};
...
BOOLEAN BEACON_File_RemoveDirFile(const char * dir_path)
{
    SFS_HANDLE	hFind    = PNULL;
    SFS_FIND_DATA_T sfsfind             = {0};
    BOOLEAN		result              = FALSE;
...
MMIFILE_HANDLE  BEACON_File_Open(const char * file_name,BOOLEAN is_read)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    uint16   tmp_len = 0;
...
uint32  BEACON_File_Write(MMIFILE_HANDLE file_handle,uint8  * data_ptr, uint32 data_len)
{
    uint16 j = 0;
    MMIFILE_ERROR_E file_ret = SFS_NO_ERROR;
    uint32 transbyte = 0;
...
void  BEACON_File_Close(MMIFILE_HANDLE file_handle)
{
    if(file_handle != SFS_INVALID_HANDLE)
    {
        MMIFILE_CloseFile(file_handle);
...
uint32  BEACON_File_GetSize(const char * file_name)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    uint16   tmp_len = 0;
...
BOOLEAN  BEACON_File_Rename(const char * file_name, const char * new_filename)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
