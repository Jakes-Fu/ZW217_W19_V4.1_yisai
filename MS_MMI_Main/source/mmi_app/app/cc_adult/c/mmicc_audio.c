...
PUBLIC BOOLEAN    MMIAPICC_SwitchAudioDevice( MMICC_AUDIO_OPER_E oper, MMICC_AUDIO_DEV_E dev)
{
    ...
    {
#ifdef UAL_BT_HFU_SUPPORT
        BOOLEAN  is_bt_audio_connected = ual_bt_hfu_aud_is_conn();

        SCI_TRACE_LOW("ccapp_a hfu SwitchAudioDevice, is_bt_audio_connected = %d", is_bt_audio_connected);

        if( oper == MMICC_AUDIO_OPER_ENABLE )
        {
            if(dev == MMICC_AUDIO_DEV_BT)
            {
                result = ual_bt_hfu_trans_aud_conn(BT_AUDIO_CONN_AG_TO_HF);
                SCI_TRACE_LOW("ccapp_a hfu SwitchAudioDevice enable bt, BT_TransferAudioConn(BT_AUDIO_CONN_AG_TO_HF)");
            }

            else
            {
                SCI_TRACE_LOW("ccapp_a hfu SwitchAudioDevice enable, is_bt_audio_connected is  %d", is_bt_audio_connected);
            }
        }
        else if(oper == MMICC_AUDIO_OPER_DISABLE)
        {
            if(dev == MMICC_AUDIO_DEV_BT)
            {
                result = ual_bt_hfu_trans_aud_conn(BT_AUDIO_CONN_HF_TO_AG);
                SCI_TRACE_LOW("ccapp_a hfu SwitchAudioDevice diable bt, BT_TransferAudioConn(BT_AUDIO_CONN_HF_TO_AG)");
            }

            else
            {
                SCI_TRACE_LOW("ccapp_a hfu SwitchAudioDevice diable, dev is  %d", dev);
            }
        }
        else
        {
            SCI_TRACE_LOW("ccapp_a hfu SwitchAudioDevice oper invalid, oper is  %d", oper);
        }
#endif
    }
    ...
    SCI_TRACE_LOW("ccapp_a SwitchAudioDevice, result = %d", result);
...
PUBLIC BOOLEAN MMIAPICC_SetCallVolume(uint8 volume)
{
    ...
    if( 1 == is_phone )
    {
        ...
        if (MMIAPIBT_GetActivatedHS(&address))
        {
            ...
            {
                result = MMIAPICC_SetBtSpeakerVol(volume);// Bug 1941478
#ifndef WIN32
                SCI_TRACE_LOW("%s: set volume result = %d ", __FUNCTION__, result);
#endif
                return result;
            }
        }
#endif
        handle = GetAudioHandle();
    ...
    {
#ifdef UAL_BT_HFU_SUPPORT
        result = ual_bt_hfu_set_volume(UAL_BT_HFU_AUD_LOCAL, (uint32)volume);
        MMIAPISET_SetCallVolume(volume);
#endif
    }
#ifndef WIN32
...
PUBLIC uint8 MMIAPICC_GetCallVolume(void)
{
    ...
    {
#ifdef UAL_BT_HFU_SUPPORT
         volume = (uint8)ual_bt_hfu_get_volume();
#endif
    }

#ifndef WIN32
    SCI_TRACE_LOW("%s: volume = %d ", __FUNCTION__,volume);
#endif
    return volume;
}
/*****************************************************************************/
...
PUBLIC MMICC_AUDIO_DEV_E    MMICC_GetCurrentDev ( void )
{
    ...
    else
    {
        ...
        if( TRUE == ual_bt_hfu_aud_is_conn() )
        {
            dev = MMICC_AUDIO_DEV_BT;
        }
        else
    ...
    SCI_TRACE_LOW("%s: current dev = %d ", __FUNCTION__,dev);
...
PUBLIC MMISET_CALL_RING_TYPE_E MMIAPICC_StartRingOrVibrate(BOOLEAN is_vt_call)
{
...
#endif
    {
        ...
        //MMICC_GetCurrentCallNumber(&pb_num, FALSE, FALSE); //影响BT来电响铃
    }    
...
/*****************************************************************************/
//  Description : 设置蓝牙设置的音量
//  Global resource dependence :
//  Author: fangfang.yao
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPICC_SetBtSpeakerVol(uint8 value)
{
    BOOLEAN result  = FALSE;
    BT_STATUS return_res = BT_SUCCESS;

    return_res = MMIAPIBT_SetVolume(BT_CHANNEL_SPK, value);
    if((BT_SUCCESS == result)||(BT_PENDING == result))
    {
        result = TRUE;
    }
    else
    ...
    SCI_TRACE_LOW("MMIAPICC_SetBtSpeakerVol result = %d",result);
    return result;
}
