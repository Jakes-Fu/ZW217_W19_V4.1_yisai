...
/*****************************************************************************
** File Name:      mmi_livewallpaper_collision.c                                    *
** Author:         Arvin.wu                                                       *
** Date:           18/06/2011                                                *
...
******************************************************************************/
#ifndef _MMI_LIVEWALLPAPER_COLLISION_C_
#define _MMI_LIVEWALLPAPER_COLLISION_C_
#if defined(LIVE_WALLPAPER_SUPPORT_COLLISION) 
...
#define BLOCK_MEM_POOL_SIZE_COLLISION (800 * 1024)   // Set a  MEM POOL for different livewallpaper respectively;
/*---------------------------------------------------------------------------*/
/*                          TYPE AND CONSTANT                                */
/*---------------------------------------------------------------------------*/

typedef struct _COLLISION_WALLPAPER_PARAM_T_
{
    SE_HANDLE se_handle;
...
}COLLISION_WALLPAPER_PARAM_T;
/**--------------------------------------------------------------------------*
 **                         LOCAL DEFINITION                                 *
 **--------------------------------------------------------------------------*/

LOCAL COLLISION_WALLPAPER_PARAM_T s_Collision_wallpaper_param = {0};
LOCAL SE_COLLISION_RENDER_IN_T s_param_input = {0};
LOCAL SE_COLLISION_RENDER_OUT_T s_param_output = {0};
/**--------------------------------------------------------------------------*
 **                         EXTERNAL DECLARE                                 *
 **--------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
...
/*****************************************************************************/
//  Description :帖更新算法端函数。
//  Global resource dependence : 
//  Author: Glen.Li
//  Note:
/*****************************************************************************/
LOCAL void SE_Render_Frame_Update_Step(uint16*target_buf_ptr); 
/*****************************************************************************/
 ...
 **                         GLOBAL DEFINITION                                *
 **--------------------------------------------------------------------------*/
PUBLIC const MMI_LIVE_WALLPAPER_ITEM_INFO_T g_collision_Instance =
{
    CollisionInit,
    CollisionConstruct,
    CollisionDrawBg,
...
LOCAL BOOLEAN CollisionInit(void)
{
    ...
    MMK_SetWinAngle( focus_win_id, LCD_ANGLE_0, FALSE );  //keep the collision dynamic wallpaper's lcd angle as 0 whenever the actual lcd angle is ;
            
    if(0 == s_Collision_wallpaper_param.se_handle)
    {   
        uint32 i = 0;
        
        GUILCD_GetLogicWidthHeight( GUI_MAIN_LCD_ID, &(s_Collision_wallpaper_param.width), &(s_Collision_wallpaper_param.height) );

        GUIRES_GetImgWidthHeight(&bg_width
                                                                , &bg_height
                                                                , IMAGE_SET_DYNAMIC_WALLPAPER_COLLISION_BACKGROUND_JPG_FORMAL_1
                                                                , VIRTUAL_WIN_ID);
        GUIRES_GetImgWidthHeight(&box_width
                                                                , &box_height
                                                                , IMAGE_SET_DYNAMIC_WALLPAPER_COLLISION_BOX_1
                                                                , VIRTUAL_WIN_ID);
        GUIRES_GetImgWidthHeight(&ball_width
                                                                , &ball_height
                                                                , IMAGE_SET_DYNAMIC_WALLPAPER_COLLISION_BALL_1
                                                                , VIRTUAL_WIN_ID);
      ...
      //  if(0 != VIRTUAL_WIN_ID )    
        {
            box_ptr = MMI_GetLabelImage(IMAGE_SET_DYNAMIC_WALLPAPER_COLLISION_BOX_1, 
                                            VIRTUAL_WIN_ID, 
                                            &box_size);
        ...
        s_Collision_wallpaper_param.collision_mode = SE_COLLISION_MODE_EGG;
        s_Collision_wallpaper_param.render_type = SE_COLLISION_IOCTRL_UPDATE;
        s_Collision_wallpaper_param.bg_picture_format = BACKGROUND_PICTURE_FORMAT;
        s_Collision_wallpaper_param.total_ball_num = COLL_ICON_NUM_TOTAL;
        s_Collision_wallpaper_param.update_ball_num = BALL_UPDATE_NUM; 
        s_Collision_wallpaper_param.initial_ball_num = INITIAL_BALL_NUM;

        init_param.collision_mode = s_Collision_wallpaper_param.collision_mode;
        init_param.icon_max_num = s_Collision_wallpaper_param.total_ball_num;
        init_param.bg_data_format = IMGREF_FORMAT_RGB565;
        init_param.bg_width =s_Collision_wallpaper_param.width;
        init_param.bg_height = s_Collision_wallpaper_param.height;
        ...
        s_Collision_wallpaper_param.current_ball_num = 1;
	...
	init_param.icon_img[0].img.polycon.vertex_point_ptr[3].y= -25;
        ...
        for(i=1; i<(init_param.icon_img_num); i++)
        {
            ...
            ball_ptr = MMI_GetLabelImage(IMAGE_SET_DYNAMIC_WALLPAPER_COLLISION_BALL_1 + i - 1
                                            ,VIRTUAL_WIN_ID
                                            ,&ball_size);
        ...
        s_Collision_wallpaper_param.se_handle = SE_Create(SE_ID_COLLISION,&init_param);
        if(s_Collision_wallpaper_param.se_handle > 0)
        {
            b_result = TRUE;
        }
        else
...
LOCAL void CollisionCalculate(LW_CAL_PARAM *lw_cal_param)
{
    ...
    static uint16 isfirstimeupdate = FALSE;
    ...
    SCI_MEMSET(&s_param_input,0,sizeof(s_param_input));
    SCI_MEMSET(&s_param_output,0,sizeof(s_param_output));
    
    SCI_MEMCPY((void*)(lw_cal_param->dest_buf_info.buf_ptr), (void*)(lw_cal_param->src_buf_info.buf_ptr), SCR_SIZE);
    if(SE_COLLISION_MODE_EGG == s_Collision_wallpaper_param.collision_mode)
    {
#if defined MMI_ISTYLE_SUPPORT     
        if ( (MMISET_IDLE_ISTYLE == MMIAPISET_GetIdleStyle()) )
        {
            if( (!isfirstimeupdate) 
                || (MMILIVEWALLPAPER_PREVIEW_WIN_ID == focus_win_id) 
                && (s_Collision_wallpaper_param.current_ball_num<= COLLISION_BALL_AUTOADD_MAX_NUM))
            {
                uint16 i=0;            
                isfirstimeupdate =TRUE;
                
                for(i=0; i < COLLISION_BALL_AUTOADD_MAX_NUM - s_Collision_wallpaper_param.current_ball_num + 1; i++)
                {
                    s_Collision_wallpaper_param.tp_icon_point.click_action = SE_ACTION_ADD;
                    s_Collision_wallpaper_param.render_type = SE_COLLISION_IOCTRL_CLICK;
                    s_Collision_wallpaper_param.tp_icon_point.click_state = SE_CLICK_UP; 
                    s_Collision_wallpaper_param.tp_icon_point.centre_pos.x = 100;
                    s_Collision_wallpaper_param.tp_icon_point.centre_pos.y = 150;
                    
                    s_param_input.type = s_Collision_wallpaper_param.render_type;   
                    s_param_input.param.click_param.click_action = s_Collision_wallpaper_param.tp_icon_point.click_action;
                    s_param_input.param.click_param.click_state = s_Collision_wallpaper_param.tp_icon_point.click_state;
                    s_param_input.param.click_param.centre_pos.x = s_Collision_wallpaper_param.tp_icon_point.centre_pos.x;
                    s_param_input.param.click_param.centre_pos.y = s_Collision_wallpaper_param.tp_icon_point.centre_pos.y;

                    if(0 != s_Collision_wallpaper_param.se_handle)
                    {
                        ret = SE_Render(s_Collision_wallpaper_param.se_handle, &s_param_input, &s_param_output);
                    }

                    if(SE_COLLISION_IOCTRL_CLICK == s_Collision_wallpaper_param.render_type)
                    {
                        s_Collision_wallpaper_param.render_type = SE_COLLISION_IOCTRL_UPDATE;
                    }
                    
                    SE_Render_Frame_Update_Step(lw_cal_param->dest_buf_info.buf_ptr);     
                    
                    s_Collision_wallpaper_param.current_ball_num++;
                }
            }
        }
#endif        
        if(SE_COLLISION_IOCTRL_CHG_GRAVITY == s_Collision_wallpaper_param.render_type)
        {
            s_param_input.type = s_Collision_wallpaper_param.render_type;
            s_param_input.param.chg_gravity.direction = s_Collision_wallpaper_param.gravity_direction;
        ...
        else if(SE_COLLISION_IOCTRL_CLICK == s_Collision_wallpaper_param.render_type)
        {
            s_param_input.type = s_Collision_wallpaper_param.render_type;   
            s_param_input.param.click_param.click_action = s_Collision_wallpaper_param.tp_icon_point.click_action;
            s_param_input.param.click_param.click_state = s_Collision_wallpaper_param.tp_icon_point.click_state;
        ...
        case MSG_GSENSOR:
        {
            switch( (uint32 )param)
            {
                case MMK_GSENSOR_STATUS_LEFT_SIDE:
                    s_Collision_wallpaper_param.gravity_direction = SE_GRAVITY_RIGHT;
                    s_Collision_wallpaper_param.render_type = SE_COLLISION_IOCTRL_CHG_GRAVITY;
                    break;
                case MMK_GSENSOR_STATUS_RIGHT_SIDE:
                    s_Collision_wallpaper_param.gravity_direction = SE_GRAVITY_LEFT;
                    s_Collision_wallpaper_param.render_type = SE_COLLISION_IOCTRL_CHG_GRAVITY;
                    break;
                case MMK_GSENSOR_STATUS_UP_SIDE:
                    s_Collision_wallpaper_param.gravity_direction = SE_GRAVITY_DOWN;
                    s_Collision_wallpaper_param.render_type = SE_COLLISION_IOCTRL_CHG_GRAVITY;
                    break;
                case MMK_GSENSOR_STATUS_DOWN_SIDE:
                    s_Collision_wallpaper_param.gravity_direction = SE_GRAVITY_UP;
                    s_Collision_wallpaper_param.render_type = SE_COLLISION_IOCTRL_CHG_GRAVITY;
                    break;
                default:
                    break;
...
LOCAL void CollisionDeinit(void)
{
    if(s_Collision_wallpaper_param.se_handle > 0)
    {
        SE_Destory(s_Collision_wallpaper_param.se_handle);

        s_Collision_wallpaper_param.se_handle = 0;
        s_Collision_wallpaper_param.win_handle = 0;
        
        s_Collision_wallpaper_param.current_ball_num = 0;  
        s_Collision_wallpaper_param.update_ball_num = 0;  
        s_Collision_wallpaper_param.total_ball_num = 0;   
...
LOCAL void SaveCollisionTPPoint(GUI_POINT_T pt, MMI_MESSAGE_ID_E msg_id )
{
    if( (s_Collision_wallpaper_param.current_ball_num < s_Collision_wallpaper_param.total_ball_num))
    {
        switch(msg_id)
        {
#ifdef TOUCH_PANEL_SUPPORT //IGNORE9527
            case MSG_TP_PRESS_DOWN:
                s_Collision_wallpaper_param.tp_icon_point.click_state = SE_CLICK_DOWN; 
                break;
#endif //TOUCH_PANEL_SUPPORT //IGNORE9527
#ifdef TOUCH_PANEL_SUPPORT //IGNORE9527
            case MSG_TP_PRESS_UP:
        ...
        s_Collision_wallpaper_param.tp_icon_point.centre_pos.x = pt.x;
        s_Collision_wallpaper_param.tp_icon_point.centre_pos.y = pt.y;
    }
}
...
{
    SE_RESULT_E ret = SE_RESULT_SUCCESS;
    s_param_input.param.update_param.bg_img_addr = (uint32)target_buf_ptr; 
    s_param_input.param.update_param.bg_img_size = s_Collision_wallpaper_param.width * s_Collision_wallpaper_param.height * 2;
    s_param_input.param.update_param.bg_data_format = s_Collision_wallpaper_param.bg_picture_format;
    s_param_input.type = s_Collision_wallpaper_param.render_type;
    s_param_input.param.update_param.icon_img_num = 0;
#if defined MMI_ISTYLE_SUPPORT 
    if ( MMISET_IDLE_ISTYLE == MMIAPISET_GetIdleStyle() )
    {
        s_param_input.param.update_param.icon_img_num = 0;
    ...
    if(ret)
    {
        //may do exception in future, if SE_Render failed, it only means that algorithm can't update new frame, but still need to keep all the buffer,initialized content;
    }
}
/*****************************************************************************/
//  Description : 获得动态壁纸类型
...
LOCAL uint32 CollisionGetType(void)
{
    return MMI_LIVE_WALLPAPER_TYPE_COLLISION;
}
/*****************************************************************************/
//  Description : 获得动态壁纸ThumbNail信息
//  Global resource dependence : 
...
LOCAL void CollisionGetThumbNailInfo(LW_THUMBNAIL_INFO *thumbnail_info_ptr)
{
    if(thumbnail_info_ptr)
    {
        ...
        thumbnail_info_ptr->image_id = IMAGE_WALLPAPER_COLLISION_THUMBNAIL;

        thumbnail_info_ptr->is_not_string = FALSE;
        thumbnail_info_ptr->text_id = TXT_SET_WALLPAPER_COLLISION;
        thumbnail_info_ptr->instance_mem_pool_size = BLOCK_MEM_POOL_SIZE_COLLISION;
    }
}
#endif //LIVE_WALLPAPER_SUPPORT_COLLISION

#endif //_MMI_LIVEWALLPAPER_COLLISION_C_
