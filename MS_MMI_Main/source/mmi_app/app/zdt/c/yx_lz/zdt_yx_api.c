...
int YX_LOCTION_SOS_CallBack (uint8 loc_ok,void * pLoctionData)
{
    YX_API_LOCTION_End(loc_ok,YX_LOCTION_TYPE_SOS);
    return 0;
}
...
BOOLEAN YX_LOCTION_Start(YX_APP_T *pMe,uint16 type,uint8 * pParam,uint16 ParamLen)
{
    ...
    else
    {
        ...
        else if(YX_LOCTION_TYPE_WL == type)
        {
            ...
            YX_LOCTION_Link_AddData(pMe,type,pParam,ParamLen);
                #ifdef ZDT_GPS_SUPPORT
                    MMIZDTGPS_API_LocStart(time_s,YX_LOCTION_WL_CallBack);
                #else
                    #ifdef ZDT_WIFI_SUPPORT
        ...
        else if(YX_LOCTION_TYPE_SOS == type)
        {
            if(YX_LOCTION_Link_TypeExsit(pMe,type))
            {
                is_exsit =  TRUE;
            ...
            {
                #if 0 //def ZDT_GPS_SUPPORT
                    MMIZDTGPS_API_LocStart(time_s,YX_LOCTION_SOS_CallBack);
                #else
                    #ifdef ZDT_WIFI_SUPPORT
                        MMIZDTWIFI_API_LocStart(10,YX_LOCTION_SOS_CallBack);
                    #endif
                #endif
            }
        }
...
int YX_LOCTION_EndHandle(YX_APP_T *pMe,DPARAM param)
{
    ...
    switch(type)
    {
        ...
        case YX_LOCTION_TYPE_SOS:
            {
                YX_LOCTION_LINK_DATA_T cur_data = {0};
                while(YX_LOCTION_Link_GetByType(pMe,&cur_data,YX_LOCTION_TYPE_SOS))
                {
                    YX_Net_Send_REPORT_LOCATION_INFO(pMe,loc_ok);
                    if(cur_data.pParam != NULL)
                    {
...
BOOLEAN YX_API_LOCTION_SOS_Start(void)
{
    YX_LOCTION_Start(&g_yx_app,YX_LOCTION_TYPE_SOS,NULL,0);
    return TRUE;
}
...
uint16  g_yx_netsms_content[301] = {0};
uint16  g_yx_net_netsms_id = 0;
uint32  g_yx_netsms_content_len = 0;

PUBLIC BOOLEAN YX_API_NetSms_Stop(void)
{
...
BOOLEAN YX_API_NetSms_PlayLast(void)
{
    ...
    if(YX_DB_NETSMS_GetNewUnReadNum(&last_id))
    {
        ...
        g_yx_netsms_content_len = GUI_UTF8ToWstr(g_yx_netsms_content,300,yx_DB_NetSms_Reclist[last_id].netsms_content,yx_DB_NetSms_Reclist[last_id].netsms_content_len);
...
PUBLIC BOOLEAN YX_API_NetSms_PlayIndex(uint8 index)
{
    ...
    YX_API_NetSms_Stop();
    SCI_MEMSET(g_yx_netsms_content,0,sizeof(g_yx_netsms_content));
    g_yx_netsms_content_len = GUI_UTF8ToWstr(g_yx_netsms_content,300,yx_DB_NetSms_Reclist[index].netsms_content,yx_DB_NetSms_Reclist[index].netsms_content_len);
    YX_DB_NETSMS_ChangStatusToRead(index);
    #ifdef ZDT_LED_SUPPORT
    ZDT_Led_Start_Call_Incoming_Over();
    #endif
    ZDT_LOG("YX_API_NetSms_PlayLast last_id = %d,content_len=%d,playtimes=%d",index,g_yx_netsms_content_len,yx_DB_NetSms_Reclist[index].netsms_playtimes);
