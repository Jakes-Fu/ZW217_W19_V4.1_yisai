...
static uint16 s_yx_voc_play_vol = 4;
static uint16 s_yx_voc_rcv_idx = 0;

BOOLEAN YX_Voice_Play_Valid(void);
#ifdef TOUCH_PANEL_SUPPORT
extern void  ZdtApp_YX_VocMenu_DrawStatus(YX_APP_T * pMe,YXVocRspTypeStatus type, uint16 err_id);
extern void  ZdtApp_YX_VocMenu_RecordRsp(YX_APP_T * pMe,YXVocRspTypeStatus type, uint16 err_id);
#endif
...
BOOLEAN YX_Voice_SendAlert(YX_APP_T * pMe,uint16 wParam)
{
    #if 0
    ZDT_LOG("YX_Voice_SendAlert wParam=%d",wParam);
    if(wParam == 0) //成功
    {
...
//yangyu add begin
#define SEND_RCV_VOC_FILE_RECORD_TXT_DIR "D:\\YXVOICEDB"
#define SEND_RCV_VOC_FILE_RECORD_TXT "D:\\YXVOICEDB\\yx_voice_record.txt"


PUBLIC void YX_VocFilePathDBReadToBuffer();
LOCAL void YX_VocFilePathSaveFullBufToTxt(char* full_data, uint32 data_len);

PUBLIC void YX_VocFilePathTxtDel()
{
    ZDT_File_Remove((const char *)SEND_RCV_VOC_FILE_RECORD_TXT);
}

PUBLIC void YX_VocFilePathTxtInitOnBoot()
{


}
PUBLIC void YX_VocFileCheckAndRemoveOverItem()
{
    uint32 read_len = 0;
    uint8 buffer[(MAX_YX_VOC_SAVE_SIZE + 10)*50] = {0};
    uint8 buffer2[(MAX_YX_VOC_SAVE_SIZE + 10)*50] = {0};
    char line_data[(MAX_YX_VOC_SAVE_SIZE + 10)][50] = {0};//notice ,same with txt_line_path_data[20][50]
    ...
    if(ZDT_File_Read((const uint8*)SEND_RCV_VOC_FILE_RECORD_TXT , buffer, (MAX_YX_VOC_SAVE_SIZE + 10)*50, &read_len))
    {
    ...
    if(buffer && SCI_STRLEN(buffer) > 0)
    {
        ...
        for(i = 0 ; i < (MAX_YX_VOC_SAVE_SIZE + 10) ; i++) // total will only save 20 lines, over 20 will delete
        {
            if(line_data[i] && SCI_STRLEN(line_data[i]) > 0 && ZDT_File_Exsit(line_data[i]))
            {
                valid_line_count++;
                valid_line[j++] = i;
                //ZDT_LOG("YX_VocFileCheckAndRemoveOverItem line data=%s", line_data[i]);

            }
            
        }
        //ZDT_LOG("YX_VocFileCheckAndRemoveOverItem valid_line_count=%d", valid_line_count);
        ...
        if(valid_line_count > MAX_YX_VOC_SAVE_SIZE)
        {
            ...
            {
                //ZDT_LOG("YX_VocFileCheckAndRemoveOverItem,i=%d, remove=%s", i, line_data[valid_line[i]]);
            ...
            {
                 //ZDT_LOG("YX_VocFileCheckAndRemoveOverItem, j=%d, line_data=%s", j, line_data[valid_line[i]]);
                 SCI_MEMCPY(buffer2 + j, line_data[valid_line[i]], SCI_STRLEN(line_data[valid_line[i]]));
                 j += SCI_STRLEN(line_data[valid_line[i]]);
                 buffer2[j] = '\n'; //next line
                 j += 1;
            }
            //ZDT_LOG("YX_VocFileCheckAndRemoveOverItem, j=%d, buffer2=%s", j, buffer2);
            YX_VocFilePathSaveFullBufToTxt(buffer2, j);
...
LOCAL void YX_VocFilePathSaveFullBufToTxt(char* full_data, uint32 data_len)
{
    ...
    //MMIAPICOM_WstrToStr(data_full_name, data_save);
    //ZDT_LOG("YX_VocFilePathSaveFullBufToTxt len =%d, data_full_=%s \n", data_len, full_data);

    tmp_len = GUI_GBToWstr(full_path, (const uint8*)SEND_RCV_VOC_FILE_RECORD_TXT_DIR, SCI_STRLEN(SEND_RCV_VOC_FILE_RECORD_TXT_DIR));
    full_path_len = tmp_len;
    if(!MMIFILE_IsFileExist(full_path, full_path_len))
    {
        ret = MMIFILE_CreateDir(full_path, full_path_len);
    }
    tmp_len = GUI_GBToWstr(full_path, (const uint8*)SEND_RCV_VOC_FILE_RECORD_TXT, SCI_STRLEN(SEND_RCV_VOC_FILE_RECORD_TXT));
    full_path_len = tmp_len;
    if(MMIFILE_IsFileExist(full_path, full_path_len))
    {
       ZDT_File_Remove((const uint8*)SEND_RCV_VOC_FILE_RECORD_TXT); 
    }  
...
LOCAL void YX_VocFilePathSaveToTxt(char* data_full_name)
{
    ...
    //MMIAPICOM_WstrToStr(data_full_name, data_save);
    //ZDT_LOG("YX_VocFilePathSaveToTxt data_full_name=%s \n", data_full_name);
    ...
    ZDT_File_Close(file_handle);
    //YX_VocFilePathDBReadToBuffer();
    YX_VocFileCheckAndRemoveOverItem();
...
BOOLEAN YX_DB_Voc_Init(YX_APP_T* pMe)
{
    ...
    sprintf( full_path_name,"D:\\YXVocRcv\\R1_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"111",3,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name);
    time_ms++;
    
    sprintf( full_path_name,"D:\\YXVocRcv\\R2_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"222",3,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name);
    time_ms++;
    
    sprintf( full_path_name,"D:\\YXVocSend\\S1_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"111",3,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name);
    time_ms++;
    
    sprintf( full_path_name,"D:\\YXVocSend\\S2_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"222",3,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name);
    time_ms++;
     sprintf( full_path_name,"D:\\YXVocSend\\S3_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"333",3,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name);
    time_ms++;
    sprintf( full_path_name,"D:\\YXVocSend\\S4_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
...
//ui_line_data must be [20][50]
PUBLIC void YX_VocFilePathDBReadToBuffer(char **ui_line_data, uint16 *real_lines)
{
    uint32 read_len = 0;
    uint8 buffer[MAX_YX_VOC_SAVE_SIZE*50] = {0};
    
    char line_data[MAX_YX_VOC_SAVE_SIZE][50] = {0};//notice ,same with txt_line_path_data[20][50]
    uint16 line_index = 0;
    uint16 i = 0, j = 0 ;
    uint16 cr_index = 0;

    if(ZDT_File_Read((const uint8*)SEND_RCV_VOC_FILE_RECORD_TXT , buffer, MAX_YX_VOC_SAVE_SIZE*50 , &read_len))
    {
        //ZDT_LOG("YX_VocFilePathDBReadToBuffer success buffer=%s", buffer);
        ZDT_LOG("YX_VocFilePathDBReadToBuffer success ");
    }
    ...
    if(buffer && SCI_STRLEN(buffer) > 0)
    {
        ...
        *real_lines = line_index;
        SCI_MEMCPY(ui_line_data , line_data, MAX_YX_VOC_SAVE_SIZE*50*sizeof(char));
        for(i = 0 ; i < MAX_YX_VOC_SAVE_SIZE ; i++) // total will only save 20 lines, over 20 will delete
        {
            if(line_data[i] && SCI_STRLEN(line_data[i]) > 0)
            {
                //ZDT_LOG("YX_VocFilePathDBReadToBuffer line data=%s", line_data[i]);
...
{
    return ZDT_IsStartWith(full_name, (const uint8*)YX_VOICE_SEND_DIR);
}

PUBLIC BOOLEAN YX_VOC_IsRcvFile(char* full_name)
{
    return ZDT_IsStartWith(full_name, (const uint8*)YX_VOICE_RCV_DIR);
}


//yangyu end
static BOOLEAN YX_VOC_RcvFileNameBuild(YX_APP_T * pMe,YX_VOC_RCV_FILE_DATA_T * pRcvData)
{
    uint32 time_ms = ZDT_Get_TimeSecond();
    uint16 voc_idx = pRcvData->voc_idx;
    ZDT_LOG("YX_VOC_RcvFileNameBuild voc_idx=%d,time=%ld",voc_idx,time_ms);

    SCI_MEMSET(pMe->m_yx_pFileRcvVocName, 0, MAX_YX_VOC_FILENAME_SIZE+1);

    sprintf( pMe->m_yx_pFileRcvVocName,
             "%s\\R%d_%ld.amr",
             YX_VOICE_RCV_DIR,voc_idx,time_ms);
    return TRUE;
}

static BOOLEAN YX_VOC_SendFileNameBuild(YX_APP_T * pMe)
{
    uint32 time_ms = ZDT_Get_TimeSecond();
    ...
    sprintf( pMe->m_yx_pFileSendVocName,
             "%s\\S%d_%ld.amr",
             YX_VOICE_SEND_DIR,pMe->m_yx_voc_data_idx+1,time_ms);
...
BOOLEAN YX_Voice_FileSendAllRemove(YX_APP_T * pMe)
{
    if(pMe->m_yx_eRecorderStatus == YX_RECOREDER_STOP
        && pMe->m_yx_voc_is_sending == FALSE
        )
    {
        ZDT_File_RemoveDirFile(YX_VOICE_SEND_DIR);
    }
    return TRUE;
}

BOOLEAN YX_Voice_FileRcvAllRemove(YX_APP_T * pMe)
{
    if(pMe->m_yx_eRecorderStatus == YX_RECOREDER_STOP
        && pMe->m_yx_voc_is_playing == FALSE
        )
    {
        ZDT_File_RemoveDirFile(YX_VOICE_RCV_DIR);
        pMe->m_yx_pDB_vocrcv_last_CurList.list_id = 0;
    }
    return TRUE;
}
...
int  YX_VOC_RcvFile_Save(YX_APP_T *pMe,YX_VOC_RCV_FILE_DATA_T * pRcvData)
{
    int res = AEE_SUCCESS;
    MMIFILE_HANDLE file_handle;
    uint8 pack_sum = 0;
    ...
    if(pack_sum > 0)
    {
        YX_VOC_RcvFileNameBuild(pMe,pRcvData);
...
{
    if(YX_VOCRCDSTOP_SUCCESS == err_id)
    {
        MMIZDT_UpdateTalkWin(YX_RECOREDER_DISP_RECORD_OK,PNULL);
    }
    else
    {
        if(YX_VOCRCDSTOP_ERR_LOWTIME == err_id)
        {
            MMIZDT_UpdateTalkWin(YX_RECOREDER_DISP_RECORD_FAIL,"RStop Err LowTime");
        }
        else
        {
            char tmp_buf[50] = {0};
            sprintf(tmp_buf,"RStop Err %d",err_id);
            MMIZDT_UpdateTalkWin(YX_RECOREDER_DISP_RECORD_FAIL,tmp_buf);
        }
    }
...
BOOLEAN YX_Voice_Record_CallBack(YX_APP_T * pMe,YXVocRecordErrStatus err_code)
{
    ...
    {
        MMIZDT_UpdateTalkWin(YX_RECOREDER_DISP_RECORDING,PNULL);
    }
    else
    {
...
BOOLEAN YX_Voice_Send_CallBack(YX_APP_T * pMe,YXVocSendErrStatus err_code)
{
    ...
    {
        MMIZDT_UpdateTalkWin(YX_RECOREDER_DISP_SEND_OK,PNULL);
        YX_VocFilePathSaveToTxt(pMe->m_yx_pFileSendVocName);//yangyu add
    }
    else
    {
        char tmp_buf[50] = {0};
        sprintf(tmp_buf,"SendErr:%d",err_code);
        MMIZDT_UpdateTalkWin(YX_RECOREDER_DISP_SEND_FAIL,tmp_buf);
        MMIZDT_TinyChatSendCallBack(FALSE);
    }
    return TRUE;
}
...
PUBLIC BOOLEAN YX_Voice_Record_PlayCur(void)
{
    ...
    {
        srv_result = MMIRECORDSRV_Play(g_yx_app.m_yx_pRecordMedia, 8);
...
static BOOLEAN YX_Voice_PlayFileGetLastName(char *pInBuffer,char *pOutBuffer)
{
    uint16 len = 0;
    char* p;
    ...
    strcat(pOutBuffer,YX_VOICE_RCV_LAST_FILENAME);
   
    ZDT_LOG("YX_Voice_PlayFileGetLastName %s to %s",pInBuffer,pOutBuffer);

    return TRUE;
}

static BOOLEAN YX_Voice_PlayFileRemove(YX_APP_T * pMe)
{
    BOOLEAN res = FALSE;
    uint32 cur_rcv_num = YX_DB_GetVocRcvNum();
    char szLastName[MAX_YX_VOC_FILENAME_SIZE] = {0};
    
    if(cur_rcv_num > 1)
    {
        ZDT_File_Remove((char *)pMe->m_yx_pDB_vocrcv_CurList.r_name);
        YX_DB_VocRcvDel(pMe,pMe->m_yx_pDB_vocrcv_CurList.list_id);
    }
    else if(cur_rcv_num == 1)
    {
        pMe->m_yx_pDB_vocrcv_last_CurList = pMe->m_yx_pDB_vocrcv_CurList;
        YX_Voice_PlayFileGetLastName(pMe->m_yx_pDB_vocrcv_last_CurList.r_name,szLastName);
        ZDT_File_Remove(szLastName);
        ZDT_File_Rename(pMe->m_yx_pDB_vocrcv_CurList.r_name,szLastName);
        SCI_MEMCPY(pMe->m_yx_pDB_vocrcv_last_CurList.r_name,szLastName,MAX_YX_VOC_FILENAME_SIZE);
        pMe->m_yx_pDB_vocrcv_last_CurList.list_id = 1;
        YX_DB_VocRcvDel(pMe,pMe->m_yx_pDB_vocrcv_CurList.list_id);
    }
    return TRUE;
}
...
static void YX_Voice_Play_HandleEnd(BOOLEAN is_end)
{
    ...
    if (is_end )
    {
        ...
        {               
            ...
            {
                 MMIZDT_SendSigTo_APP(ZDT_APP_SIGNAL_YX_VOC_PLAYSTOP,NULL,0);
                if(YX_Voice_Play_Valid() && YX_DB_GetVocRcvNum() > 0)
                {
                    MMIZDT_SendSigTo_APP(ZDT_APP_SIGNAL_YX_VOC_FILEPLAY,NULL,0);
                }
            }
...
int32 YX_Net_Send_TK_VocFile_Start(YX_APP_T *pMe)
{
    ...
    MMIZDT_UpdateTalkWin(YX_RECOREDER_DISP_SENDING,file_name);
...
int YX_Net_Receive_TK_VocFile(YX_APP_T *pMe,uint8 * pFiledata,uint16 DataLen)
{
    ...
    if(pFiledata != NULL && DataLen > 18)
    {
        ...
        {            
            res = YX_VOC_RcvFile_Save(pMe,&voc_rcv_data);
            
            if(res == AEE_SUCCESS)
            {
                result = 1;
                YX_DB_VocRcvAdd(pMe,pMe->m_yx_pFileRcvVocName);
                                ...
                                )
{
    ...
    if(pMe->m_yx_eRecorderStatus == YX_RECOREDER_RECORD)
    {
        ...
        {
            char tmp_buf[50] = {0};
            sprintf(tmp_buf,"TimerOut %d",(YX_VOICE_RECORD_TIME_OUT - pMe->m_yx_voc_send_times));
...
BOOLEAN YX_Voice_Record(YX_APP_T * pMe)
{
    ...
    build_file = YX_VOC_SendFileNameBuild(pMe);
    if(FALSE == build_file)
    {
...
BOOLEAN YX_Voice_FilePlay_Start(YX_APP_T * pMe)
{
    ...
    if(YX_DB_VocRcv_ListGetFirstByTime(pMe,&pMe->m_yx_pDB_vocrcv_CurList))
    {
        need_play = 1;
    }
    else if(pMe->m_yx_pDB_vocrcv_last_CurList.list_id > 0)
    {
        pMe->m_yx_pDB_vocrcv_CurList = pMe->m_yx_pDB_vocrcv_last_CurList;
        need_play = 2;
    }
    if(need_play)
    {
        int play_res = 0;
        play_res = YX_Voice_FilePlay(pMe,pMe->m_yx_pDB_vocrcv_CurList.r_name);
        if(0 == play_res)
        {
            char file_name[100] = {0};
            pMe->m_yx_voc_is_playing = TRUE;
            s_yx_voc_is_playing = TRUE;
            Ret = TRUE;
            YX_Voice_Play_CallBack(pMe,YX_FILEPLAY_SUCCESS);
            YX_Func_GetFileName(pMe->m_yx_pDB_vocrcv_CurList.r_name,file_name);
        ...
        else
        {
            ...
            if(s_voc_is_play_all)
            {
                if(YX_Voice_Play_Valid() && need_play == 1)
                {
                    MMIZDT_SendSigTo_APP(ZDT_APP_SIGNAL_YX_VOC_FILEPLAY,NULL,0);
                }
                else
            ...
            YX_Voice_Play_CallBack(pMe,YX_FILEPLAY_ERR);
        }
    }
    else
    {
        ZDT_LOG("YX_Voice_FilePlay_Start OVER %d",YX_DB_GetVocRcvNum());
        YX_Voice_FilePlay_Stop(pMe);
    }
    return Ret;
...
BOOLEAN YX_Voice_FilePlay_Stop(YX_APP_T * pMe)
{
    ...
    {
        if(pMe->m_yx_pDB_vocrcv_last_CurList.list_id == 0)
        {
            YX_Voice_FileRcvAllRemove(pMe);
        }
    ...
    {
        MMIZDT_UpdateTalkTPWin();
        //ZdtApp_YX_VocMenu_RecordRsp(pMe,YX_VOCRSP_TYPE_PLAYSTOP,0);
    }
#endif
    return TRUE;
}
