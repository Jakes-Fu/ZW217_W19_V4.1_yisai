...
//0: 不是亲情号码>0:亲情号码
static uint16 YX_SMS_IsFromSOS(uint8 * pNum)
{    
    #if 0
    uint16 love_idx = 0;
    int sos_idx = 0;
...
int YX_SMS_Receive_IP(uint8 * pNum,uint8 * pContent,uint16 ContentLen)
{
    int ret = 0;
    ...
    //设置成功
    uint16 sms_t_1[10] = {0x8BBE,0x7F6E,0x6210,0x529F ,0x000D,0x000A,0x0};
    // 服务器地址和端口:
    uint16 sms_t_2[10] = {0x670D,0x52A1,0x5668,0x5730,0x5740,0x548C,0x7AEF,0x53E3,0x003A,0x0};
    ...
    sprintf((char *)tmp_buf,"%s,%d\r\nIMEI:%s", yx_DB_Set_Rec.ip_domain,yx_DB_Set_Rec.ip_port,g_zdt_phone_imei);
...
int YX_SMS_Get_IP(uint8 * pNum)
{
    ...
    SCI_MEMSET(tmp_buf,0,sizeof(tmp_buf));
    SCI_MEMSET(tmp_wbuf,0,sizeof(tmp_wbuf));
    sprintf((char *)tmp_buf,"SOS:%s\r\n",yx_DB_Sos_Reclist[0].sos_num);
    MMIAPICOM_StrToWstr(tmp_buf,tmp_wbuf);
    MMIAPICOM_Wstrcat(sms_buf,tmp_wbuf);
...
static BOOLEAN YX_SMS_SOS_ContentValid(uint8 * pNum,uint8 * pData,int DataLen,BOOLEAN is_ucs2)
{
    ...
    //YX_Func_strupr((char *)msg_str);
    if(msg_len == 2 &&  strncmp( (char *)msg_str, "DW", 2 ) == 0)
    {
    ...
    else if(msg_len == 6 &&  strncmp( (char *)msg_str, "SDMSON", 6 ) == 0)
    {
        YX_API_SeNetOpen();
    ...
    else if(msg_len > 6 &&  strncmp( (char *)msg_str, "SETIP:", 6) == 0)
    {
        ...
        YX_SMS_Receive_IP(pNum,msg_str+6,msg_len-6);
        ret = TRUE;
    }
...
extern BOOLEAN YX_SMS_IsValid(uint8 * pNum,uint8 * pData,int DataLen,BOOLEAN is_ucs2)
{
    ...
    if(server_status == YX_SMS_SERVER_HEAD)
    {
        ...
        YX_SMS_ServerValid(pNum,pData+12,DataLen-12);
        ret = TRUE;
    }
    else if(server_status == YX_SMS_SERVER_TAIL)
    {
        ZDT_LOG("YX SMS Server Tail");
        YX_SMS_ServerValid(pNum,pData,DataLen-12);
        ret = TRUE;
    }
    else
    {
    #ifdef YX_DB_NEED_SOS
        if(0 != YX_SMS_IsFromSOS(pNum))
        {
