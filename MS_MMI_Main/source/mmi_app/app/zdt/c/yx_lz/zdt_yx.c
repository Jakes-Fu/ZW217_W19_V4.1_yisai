...
extern int YX_AppInit(void)
{
    ...
    pMe->m_yx_db_vocrcv_list_head = NULL;/*链表头,未做初始化前为空*/
    pMe->m_yx_db_vocrcv_list_length = 0;

    SCI_MEMSET(&pMe->m_yx_pDB_vocrcv_CurList,0,sizeof(yx_db_vocrcv_index_type));
    SCI_MEMSET(&pMe->m_yx_pDB_vocrcv_last_CurList,0,sizeof(yx_db_vocrcv_index_type));
...
int YX_Net_RestorFactory(void)
{
    MMISET_CleanUserData();
    YX_Net_FactoryHaveIP();
...
static int YX_HandleLose(DPARAM param)
{
    ...
    //YX_Net_Send_AL_40(pMe,YX_NET_ALERT_TYPE_LOSE,0);
    return 0;
}
...
static int YX_HandleBatStatus(DPARAM param)
{
    ...
    if(level < 1) //为省电改为2 -> 1
    {
        ...
        //YX_Net_Send_AL_40(pMe,YX_NET_ALERT_TYPE_LOWBAT);
    }
    return 0;
}
...
static int YX_HandleMove(DPARAM param)
{
    ...
    if(g_yx_is_stillness)
    {
        YX_Net_WL_TimerChange(1);
    }
...
static int YX_HandleStill(DPARAM param)
{
    ...
    if(g_yx_is_stillness == FALSE)
    {
        YX_Net_WL_TimerChange(10);
    }
...
{
    BOOLEAN res = TRUE;
    if(YX_DB_TIMER_IsValid_Sleep())
    {
        return FALSE;
    }
    if(g_yx_is_move == FALSE)
    {
        return FALSE;
    }
    #if 0
...
#else
            YX_Net_Send_REPORT_LOCATION_INFO(pMe,0);
...
#if 0 //defined(ZDT_GPS_SUPPORT) || defined(ZDT_WIFI_SUPPORT) 
            YX_LOCTION_Start(pMe,YX_LOCTION_TYPE_ALERT,NULL,wParam);
#else
            //YX_Net_Send_AL_40(pMe,wParam,0);
#endif
       }
            break;
