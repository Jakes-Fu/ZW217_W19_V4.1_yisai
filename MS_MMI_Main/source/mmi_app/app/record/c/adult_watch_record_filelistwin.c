...
LOCAL void WatchRec_FileList_Delete(void);
LOCAL MMI_RESULT_E WatchRec_FileList_Query_Callback(MMI_WIN_ID_T winId, MMI_MESSAGE_ID_E msgId, DPARAM param);
LOCAL void WatchRec_FileList_StartDelete(MMI_WIN_ID_T winId);
LOCAL MMI_RESULT_E HandleRecordFileListDeleteOkMsg(
    MMI_WIN_ID_T      winId,
    MMI_MESSAGE_ID_E    msgId,
    DPARAM                param
);
...
LOCAL void WatchRec_BeginFindFile(MMI_WIN_ID_T winId)
{
    ...
    MMI_MEMCPY(filter.filter_str,5,"*.wav",5,sizeof(uint8)*5);
...
LOCAL void WatchRec_FileList_OpenWindow(MMI_WIN_ID_T      winId, MMI_MESSAGE_ID_E msgId, DPARAM param)
{
    ...
    if(0 == s_list_num)
    {
        MMI_GetLabelTextByLang(TXT_RECORD_NO_FILE, &strTitle);
#ifdef ADULT_WATCH_SUPPORT
        Adult_WatchCOM_NoteWin_1Icon_1Str_Enter(MMIRECORD_WORK_QUERY_WIN_ID, &strTitle, res_aw_recorder_ic_rec, PNULL);
#else
        {
            WATCH_SOFTKEY_TEXT_ID_T softkey = {0};
            WatchCOM_NoteWin_FullScreen_Enter(MMIRECORD_WORK_QUERY_WIN_ID ,PNULL,&strTitle,res_aw_recorder_ic_rec,0,softkey,PNULL);
        }
#endif
    ...
    {
#ifdef ADULT_WATCH_SUPPORT
        AdultWatchCOM_ListItem_Draw_2Str_2line(WATCHRECORD_LIST_CTRL_ID, str1, str2);
#else
        WatchCOM_ListItem_Draw_2Str(WATCHRECORD_LIST_CTRL_ID, str1, str2);
#endif

    }
    GUIWIN_SetTitleFontColor(winId, 0x8000);
...
LOCAL void WatchRec_FileList_OpenList(void)
{
    ...
    if(0 == s_list_num)
    {
        ...
        {
            WATCH_SOFTKEY_TEXT_ID_T softkey = {0};
            WatchCOM_NoteWin_1Line_Enter(MMIRECORD_WORK_QUERY_WIN_ID, &strTitle, res_aw_recorder_ic_rec, softkey, PNULL);
        }
#endif
        MMK_CloseWin(MMIRECORD_LIST_WIN_ID);
        return;
...
WINDOW_TABLE(MMIRECORD_LIST_WIN_TAB) =
{
       WIN_HIDE_STATUS,
#ifdef ADULT_WATCH_SUPPORT
       WIN_TITLE(TXT_COMMON_RECORD_FILE_LIST),
#endif
                                                    ...
                                                    )
{
    ...
    switch (msg_id)
    {
        ...
        {
#ifndef MMIFMM_XFIND_SUPPORT //bug 2095560
...
)
{
    ...
    switch(msgId)
    {
        ...
        {
#ifdef SCREEN_SHAPE_CIRCULAR
            GUILIST_SetRect(WATCHRECORD_LIST_CTRL_ID, &list_rect);
#endif
...
{
#ifdef ADULT_WATCH_SUPPORT
    AdultWatchCOM_QueryWin_1Btn_Enter(MMIRECORD_WORK_QUERY_WIN_ID,0,res_aw_recorder_ic_delete,WatchRec_FileList_Query_Callback);
#endif
}
...
LOCAL MMI_RESULT_E WatchRec_FileList_Query_Callback(MMI_WIN_ID_T winId, MMI_MESSAGE_ID_E msgId, DPARAM param)
{
    ...
    switch(msgId)
    {
	 ...
	 //case MSG_NOTIFY_OK:
       ...
       {
           WatchRec_FileList_StartDelete(winId);
           break;
       }
       case MSG_CTL_CANCEL:
       case MSG_APP_CANCEL:
...
LOCAL void WatchRec_FileList_StartDelete(MMI_WIN_ID_T winId)
{
    ...
    if(MMIAPIFMM_DeleteFileAsyn(f_array_data_ptr.filename,MMIAPICOM_Wstrlen(f_array_data_ptr.filename),winId,MSG_VR_DELETE_BEGIN))
    {
        ...
        Adult_WatchCOM_NoteWin_1Line_Toast_Enter(
                                                        ...
                                                        &strTitle,
                                                        0,
                                                        HandleRecordFileListDeleteOkMsg
                                                     );
#endif
    }
    else
...
)
{
    ...
    {
        case MSG_CLOSE_WINDOW:
            MMK_PostMsg(MMIRECORD_LIST_WIN_ID, MSG_FULL_PAINT, PNULL, 0);
            break;
        default:
            recode = MMI_RESULT_FALSE;
            break;
