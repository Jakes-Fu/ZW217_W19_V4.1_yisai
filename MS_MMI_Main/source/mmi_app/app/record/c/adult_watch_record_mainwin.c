...
**                         MACRO DEFINITION                                                                             *
**--------------------------------------------------------------------------*/
#define RECORD_RECORD_TIME_LEFT DP2PX_VALUE(20)
#define RECORD_RECORD_TIME_TOP DP2PX_VALUE(94)
#define RECORD_RECORD_TIME_RIGHT DP2PX_VALUE(220)
#define RECORD_RECORD_TIME_BOTTOM DP2PX_VALUE(130)
#ifdef WIN32
...
WINDOW_TABLE(MMIRECORD_MAINPLAY_WIN_TAB) =
{
	...
	CREATE_BUTTON_CTRL(res_aw_recorder_ic_start, MMIRECORD_RECORD_BUTTON_CTRL_ID),
	CREATE_BUTTON_CTRL(res_aw_recorder_ic_menu_list, MMIRECORD_LIST_BUTTON_CTRL_ID),
	...
	CREATE_BUTTON_CTRL(res_aw_recorder_ic_play, MMIRECORD_RESUME_BUTTON_CTRL_ID),
    ...
    DPARAM                param)
{
    ...
    s_recorder_state = MMIVOICE_RECORD_STATE_NORMAL;
     ...
     rectTime.left = RECORD_RECORD_TIME_LEFT;
     ...
     GUILABEL_SetFont(MMIRECORD_TIME_FORM_CTRL_ID, DP_FONT_48, 0xffff);
...
LOCAL void WatchRec_MainWin_FullPaint(MMI_WIN_ID_T winId, MMI_MESSAGE_ID_E msgId, DPARAM param)
{
    ...
    if(MMIVOICE_RECORD_STATE_NORMAL == s_recorder_state)
    {
        ...
        AdultWatchCOM_Btn_SetPos_2Btn(MMIRECORD_LIST_BUTTON_CTRL_ID, MMIRECORD_RECORD_BUTTON_CTRL_ID);
#else
        WatchCOM_Btn_SetPos_2Btn(MMIRECORD_LIST_BUTTON_CTRL_ID, MMIRECORD_RECORD_BUTTON_CTRL_ID);
#endif
    ...
    {
#ifdef ADULT_WATCH_SUPPORT
        AdultWatchCOM_Btn_SetPos_2Btn(MMIRECORD_STOP_BUTTON_CTRL_ID,MMIRECORD_PAUSE_BUTTON_CTRL_ID);
#else
    ...
    {
#ifdef ADULT_WATCH_SUPPORT
        AdultWatchCOM_Btn_SetPos_2Btn(MMIRECORD_STOP_BUTTON_CTRL_ID,MMIRECORD_RESUME_BUTTON_CTRL_ID);
#else
...
LOCAL void WatchRec_MainWin_TimerCallBack(void)
{
    ...
    if(!WatchRec_MainWin_IsSpaceEnough())
    {
        ...
        MMI_GetLabelTextByLang(TXT_RECORD_SPACE_NOT_ENOUGH_SAVED, &strTitle);
#ifdef ADULT_WATCH_SUPPORT
        Adult_WatchCOM_NoteWin_1Icon_1Str_Enter(MMIRECORD_QUERY_WIN_ID, &strTitle, image_watch_warning_info, PNULL);
#else
        {
            WATCH_SOFTKEY_TEXT_ID_T softkey = {0};
            WatchCOM_NoteWin_1Line_Enter(MMIRECORD_QUERY_WIN_ID, &strTitle, 0, softkey, PNULL);
        }
#endif
        return;
    }
...
LOCAL MMI_RESULT_E HandleWebKeyMsg(MMI_HANDLE_T win_handle)
{
    ...
    if(MMIVOICE_RECORD_STATE_NORMAL == s_recorder_state)
    {
        if(!WatchRec_MainWin_IsSpaceEnough())
        {
             MMI_GetLabelTextByLang(TXT_RECORD_SPACE_NOT_ENOUGH, &strTitle);
#ifdef ADULT_WATCH_SUPPORT
             Adult_WatchCOM_NoteWin_1Icon_1Str_Enter(MMIRECORD_WORK_QUERY_WIN_ID, &strTitle, image_watch_warning_info, PNULL);
#else
             {
                WATCH_SOFTKEY_TEXT_ID_T softkey = {0};
                //WatchCOM_NoteWin_1Line_Enter(MMIRECORD_WORK_QUERY_WIN_ID, &strTitle, image_watch_warning_info, softkey, PNULL);
                WatchCOM_NoteWin_FullScreen_Enter(MMIRECORD_WORK_QUERY_WIN_ID ,PNULL,&strTitle,image_watch_warning_info,0,softkey,PNULL);
             }
#endif
...
LOCAL MMI_RESULT_E HandleOKKeyMsg(MMI_HANDLE_T win_handle)
{
    ...
    else if(MMIVOICE_RECORD_STATE_NORMAL == s_recorder_state)
    {
...
LOCAL void WatchRec_StopRecord(MMI_HANDLE_T winHandle)
{
    ...
    if(MMIRECORD_RESULT_SUCCESS == MMIRECORD_StopRecord())
    {
        SCI_TIME_T t = {0};
        SCI_DATE_T tm1 = {0};
        wchar *filename = PNULL;
        SCI_TRACE_LOW("[RECORD]WatchRec_StopRecord MMIRECORD_RESULT_SUCCESS");
        TM_GetSysDate(&tm1);
        TM_GetSysTime(&t);
        filename = MMIRECORD_GetRecordFileName();
        //录音结束时输出文件名和系统时间到日志
        SCI_TRACE_LOW("[RECORD]WatchRec_StopRecord filename:%ls",filename);
        SCI_TRACE_LOW("[RECORD]WatchRec_StopRecord date time:%d-%d-%d %d:%d:%d",tm1.year,tm1.mon,tm1.mday,t.hour,t.min,t.sec);
        s_recorder_state = MMIVOICE_RECORD_STATE_NORMAL;
        WatchRec_StopJumpText();
        WatchRec_ResetTimer();
        MMK_PostMsg(winHandle, MSG_FULL_PAINT, PNULL, PNULL);
        ...
        if(!WatchRec_MainWin_IsSpaceEnough())
        {
            SCI_TRACE_LOW("[RECORD]space is not enough");
            return;
        }
        MMI_GetLabelTextByLang(TXT_RECORDER_SAVED, &strTitle);
...
#endif
	record_win_data_ptr->record_support_type = MMIRECORD_WORK_FORMAT_WAV;
...
{
    WATCHRECORD_StopRecord(FALSE); //Bug 2130458
