...
#ifdef HTTP2_SUPPORT
    NET_PROGRESS_DISCONNECTED, //disconnectioned
#endif
...
#ifdef HTTP2_SUPPORT
    BOOLEAN             is_need_check_protocol;
#endif
}HTTP_GET_PARAM_T;
...
#ifdef HTTP2_SUPPORT
typedef struct _HTTP_VERSION_IND_SIG
{
    _SIGNAL_VARS
    HTTP_CONTEXT_ID_T   context_id;         //context id that http assigned for app
    HTTP_REQUEST_ID_T   request_id;         //the request id that http assigned for app request
    HTTP_APP_INSTANCE_T app_instance;       //the app instance
    char*               version; //http1 or http2
    char*               host_url;       //the host url
}HTTP_VERSION_IND_SIG_T;
#endif
...
PUBLIC HTTP_ERROR_E HTTP_PutRequest(
                            HTTP_CONTEXT_ID_T       context_id,
                            const HTTP_PUT_PARAM_T* put_param_ptr,  //[IN]
                            HTTP_APP_INSTANCE_T     app_instance
                            );
