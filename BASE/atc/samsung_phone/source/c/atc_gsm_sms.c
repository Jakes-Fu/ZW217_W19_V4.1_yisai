...
static const char* s_sms_mem_str[] = 
                {"BM", "SM","SR"}; 
...
AT_CMD_FUNC( ATC_ProcessCNMI )
{
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        ...
        case ATC_CMD_TYPE_READ:
        {
            sprintf((char*)g_rsp_str, "%s: %d,%d,%d,%d", g_atc_info_table[AT_CMD_CNMI].cmd_name->str_ptr,
                s_sms_config[dual_sys].mode, s_sms_config[dual_sys].mt_ind, s_sms_config[dual_sys].bm_ind,
                s_sms_config[dual_sys].ds_ind);
            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        }
...
AT_CMD_FUNC( ATC_ProcessCMGS )
{
    ...
    atc_config_ptr->atc_link_config[atc_config_ptr->current_link_id].ctrlz_end_status = ATC_CTRLZ_SMS;
#else
    // set the terminate char to receive PDU from the SIO
    SIO_ATC_SetCmdLineTerminateChar(CTRL_Z, ESC);
    SIO_ATC_WriteCmdRes(g_rsp_str, strlen((char*)g_rsp_str));
    atc_config_ptr->is_ctrlz_end = TRUE;
    atc_config_ptr->ctrlz_end_status = ATC_CTRLZ_SMS;
#endif
    return S_ATC_DEFAULT_HANDLE;
#else
    return S_ATC_FAIL;
...
AT_CMD_FUNC( ATC_ProcessCMGW )
{
    ...
    // set the terminater char to receive PDU from SIO
    SIO_ATC_SetCmdLineTerminateChar(CTRL_Z, ESC);
    SIO_ATC_WriteCmdRes(g_rsp_str, strlen((char*)g_rsp_str));
    atc_config_ptr->is_ctrlz_end = TRUE;
    atc_config_ptr->ctrlz_end_status = ATC_CTRLZ_SMS;
...
/*****************************************************************************/
//  Description : This function handles the AT+SMS command required by Sumsang
//                This command is used to store the message given in command to the specified
//                    message box in phone memory (in ME only).
//                AT+SMS=<BOX TYPE>,<Phone Number>,<Message>
//                <BOX TYPE>: INBOX, DRAFTS, OUTBOX, SENTBOX
//                <Message>: the message content need to save
//                return to AP: +SMS: Total message count in ME
//  Global resource dependence : g_rsp_str
//  Author:       Andy Huang
//  Note:
/*****************************************************************************/
AT_CMD_FUNC( ATC_ProcessSMS )
{
#ifndef _ULTRA_LOW_CODE_
    char *sms_status_str = NULL;
    char *sms_content_str = NULL;
    uint16 sms_msg_length = 0;
    MN_SMS_STATUS_E    status; 
    MN_DUAL_SYS_E dual_sys =  ATC_CURRENT_SIM_ID_MUX;
    MN_SMS_MO_SMS_T    mo_sms;
    MN_SMS_MT_SMS_T    mt_sms;
    MN_SMS_USER_DATA_HEAD_T  user_data_head_t;
    MN_SMS_USER_VALID_DATA_T user_valid_data_t;
    BOOLEAN is_msg_valid = TRUE;
    BOOLEAN is_save_sc_addr = FALSE;

    SCI_TRACE_LOW("ATC: ATC_ProcessSMS entered in!");

    SCI_MEMSET(&mo_sms, 0, sizeof(mo_sms));
    SCI_MEMSET(&mt_sms, 0, sizeof(mt_sms));
    ...
    //PARA1, the BOX TYPE
    sms_status_str = (char*)PARA1.str_ptr->str_ptr;
    if(!strcmp(sms_status_str, "INBOX"))
    {
        status = MN_SMS_MT_TO_BE_READ;
    }
    else if(!strcmp(sms_status_str, "DRAFTS"))
    {
        status = MN_SMS_MO_TO_BE_SEND;
        is_save_sc_addr = FALSE;
    }
    else if(!strcmp(sms_status_str, "OUTBOX"))
    {
        status = MN_SMS_MO_TO_BE_SEND;
        is_save_sc_addr = TRUE;
    }
    else if(!strcmp(sms_status_str, "SENTBOX"))
    {
    ...
    //PARA2, the PHONE NUMBER
    if ( !ATC_ConvertTeleStrToStruct(&s_sms_record[dual_sys].dest_num,
                                      PARA2.str_ptr->str_ptr, 
                                      PARA2.str_ptr->str_len))
    {
        SCI_TRACE_LOW("ATC: ATC_ConvertTeleStrToStruct failed!");
        return ERR_OPERATION_NOT_ALLOWED; 
    }

    //PARA3, the MESSAGE CONTENT
    sms_content_str = (char*)PARA3.str_ptr->str_ptr;
    if(NULL == sms_content_str)
    {
        return ERR_OPERATION_NOT_ALLOWED;
    }
    else
    {
        ...
        s_sms_record[dual_sys].cmd_id = AT_CMD_SMS;
        
        sms_msg_length = strlen(sms_content_str);
        
        switch (s_sms_nv_config.dcs)
        {            
            case MN_SMS_DEFAULT_ALPHABET:   // 7 bit encoded
            {
                if (ATC_SMS_7BIT_LEN < sms_msg_length)
                {
                    is_msg_valid = FALSE;
                }
                break;
            ...
            case MN_SMS_UCS2_ALPHABET:  // unicode encoded
            {
                if (ATC_SMS_UCS2_LEN < sms_msg_length)
                {
                    is_msg_valid = FALSE;
                }
                else
                {
                    ...
                    if(!ConvertAscUcs2IntoBin((char*)sms_content_str,
            ...
            case MN_SMS_8_BIT_ALPHBET:  // 8 bit encoded
            {
                if (ATC_SMS_8BIT_LEN < sms_msg_length)
                {
                    is_msg_valid = FALSE;
                }
                break;
            ...
            default:
            {
                SCI_TRACE_LOW("ATC:assert wrong dcs type: %d!", s_sms_nv_config.dcs);
                break;
            }
        }
    ...
    if(is_msg_valid)
    {
        ...
        {                
            user_valid_data_t.length = sms_msg_length;
            SCI_MEMCPY(user_valid_data_t.user_valid_data_arr, sms_content_str, sms_msg_length);
        }
        else
        {
            user_valid_data_t.length = sms_msg_length/2;               
        }
        
        //MO SMS
        if(MN_SMS_MO_TO_BE_SEND == status || MN_SMS_MO_SR_NOT_REQUEST == status)
        {
            ...
            if(is_save_sc_addr)
            {                
                SCI_MEMCPY(&mo_sms.sc_addr_t, &g_sc_addr[dual_sys],sizeof(MN_CALLED_NUMBER_T)); //save in OUTBOX
            }
            else
            {
                SCI_MEMSET(&mo_sms.sc_addr_t, 0,sizeof(MN_CALLED_NUMBER_T));    //save in DRAFTS
            }
            ...
            //Wrtie the mo sms
            if (ERR_MNSMS_NONE != MNSMS_WriteMoSmsEx(dual_sys, MN_SMS_STORAGE_ME, status, &mo_sms))
            {
                return ERR_MEMORY_FAILURE;
            }
            else
        ...
        else    //MT SMS
        {
            ...
            //Write the mt sms
            if (ERR_MNSMS_NONE != MNSMS_WriteMtSmsEx(dual_sys, MN_SMS_STORAGE_ME, status, &mt_sms))
            {
                return ERR_MEMORY_FAILURE;
            }
            else
                ...
                )
{
    ...
    switch(cmd_id)
    {
        ...
        case AT_CMD_SMS:
        {
            //If success,return total used SMS volume in ME
            if (sig_ptr->cause == MN_SMS_OPERATE_SUCCESS)
            {
                if(ERR_MNSMS_NONE != MNSMS_GetSmsNumEx(dual_sys, MN_SMS_STORAGE_ME, &max_sms_num, &used_sms_num))
                {
                    ATC_BuildResultCodeRsp(atc_config_ptr,S_ATC_RESULT_CODE_ERROR,0);
                }
                else
                {
                    sprintf((char*)g_rsp_str, "%s: %d", g_atc_info_table[cmd_id].cmd_name->str_ptr, used_sms_num);
                    ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
                }
            }
            //If not sucess,return the error.
            ...
            {
                SCI_TRACE_LOW("ATC:WRITE error, cause: %d!", sig_ptr->cause);
                ATC_BuildResultCodeRsp(atc_config_ptr,S_ATC_RESULT_CODE_ERROR,0);
            }
                ...
                )
{
    ...
    if (s_sms_config[dual_sys].ds_ind)
    {
        ...
        if (!atc_config_ptr->atc_config_nv.cmgf_flag)
        {
            ...
            MNSMS_GetScAddrEx(dual_sys, &mn_called_num);
