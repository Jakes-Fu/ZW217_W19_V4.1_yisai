...
AT_CMD_FUNC( ATC_ProcessA )
{
    ...
    if (g_calls_state[dual_sys].call_context[0].call_entity_state == ATC_CALL_INCOMING_STATE)
    {
        ...
        if(MN_RETURN_SUCCESS == MNCALL_ConnectCallEx(dual_sys, 0, PNULL))
        {
...
AT_CMD_FUNC( ATC_ProcessD )
{
    ...
    if(ATC_CMD_TYPE_EXECUTE == ATC_GET_CMD_TYPE)
    {
         ...
         #ifdef _MUX_REDUCED_MODE_   //23.101  should be emergency call when no valid ec number in sim.
                ...
                )
{
    ...
    */
    if(g_atc_call_need_disc && g_atc_call_flag)
    {
        ...
        MNCALL_DisconnectCallEx(dual_sys, (uint8)sig_ptr->call_id, PNULL);
#else
    ...
    if(ATC_16BIT1 == ((atc_config_ptr->atc_config_nv.wind_ind_level) | ATC_WIND_BIT4))
    {
        sprintf((char*)g_rsp_str, "%s: 5,%d",
            g_atc_info_table[AT_CMD_SIND].cmd_name->str_ptr,call_id + 1);
        ATC_BuildInfoRsp(atc_config_ptr, (uint8*)g_rsp_str);
    }
    return S_ATC_DEFAULT_HANDLE;
}
...
#ifdef MS_OAPI_SUPPORT
                        err_code = MNPHONE_SelectPLMNEx(dual_sys, FALSE, plmn);
#else
                ...
                case PLMN_EMERGENCY_GPRS_ONLY:
                {
                    ...
                    if(PLMN_FORMAT_NUMBERIC == s_plmn_format[dual_sys])
                    {
                        ...
                        sprintf((char *)g_rsp_str, "%s: %d,%d,\"%s\",%lu",
                                g_atc_info_table[AT_CMD_COPS].cmd_name->str_ptr,
                                g_select_mode[dual_sys], s_plmn_format[dual_sys], oper_str, 0);
                    }
                    else
                    {
                        TransformPlmnToNumeric(&plmn, oper_str);
                        sprintf((char *)g_rsp_str, "%s: %d,%d,\"%s\",%lu",
                                g_atc_info_table[AT_CMD_COPS].cmd_name->str_ptr,
                                g_select_mode[dual_sys], 2, oper_str, 0);
                    }
                    status = S_ATC_SUCCESS;
                    break;
                }
...
#ifdef  MS_OAPI_SUPPORT
            err_code = MNPHONE_ListPLMNEx(dual_sys);
#else
...
#ifdef MS_OAPI_SUPPORT
    if(ERR_MNPHONE_NO_ERR != MNPHONE_SelectPLMNEx(dual_sys, TRUE, selected_plmn))
#else
...
AT_CMD_FUNC( ATC_ProcessH )
{
    ...
    /* ATH behaves based on the AT+CVHU settings. 
    *in case of AT+CVHU=0 or 2,ATH acts automously,
    *but in caseof AT+CVHU=1, ATH is simply ignored, 
    *Although it returns OK response directly. */
    if(atc_config_ptr->voice_hangup_mode !=
                ...
                )
{
    ...
    if (1 < g_calls_state[dual_sys].call_amount)
    {
        ...
        if (atc_config_ptr->ccwa_flag && sig_ptr->calling_num_present)
        {
            ...
            {
                sprintf((char*)g_rsp_str, "%s: 5,%d",
                            g_atc_info_table[AT_CMD_SIND].cmd_name->str_ptr,sig_ptr->call_id+1);
                ATC_BuildInfoRsp(atc_config_ptr, (uint8*)g_rsp_str);
            }
...
ATC_STATUS ATC_ProcessAlertingInd(
               ...
               )
 {
    ...
    SCI_TRACE_LOW("ATC: ATC_ProcessAlertingInd");
    if(ATC_INVALID_CALLID == call_index)
    {
        SCI_TRACE_LOW("ATC: invalid call index!");
        return ERR_OPERATION_NOT_ALLOWED;
    }
    ...
    if (MN_CALL_REMOTE_USER_ALERTING == sig_ptr->alerting_type)
    {
        ...
        if(ATC_16BIT1 == ((atc_config_ptr->atc_config_nv.wind_ind_level) | ATC_WIND_BIT1))
        {
            sprintf((char*)g_rsp_str, "%s: 2",g_atc_info_table[AT_CMD_SIND].cmd_name->str_ptr);
            ATC_BuildInfoRsp(atc_config_ptr, (uint8*)g_rsp_str);
        }
    }
...
{
    SCI_ASSERT(numeric_str != PNULL);/*assert verified check NULL pointer*/
                    ...
                    )
{
    ...
    switch (atc_config_ptr->ss_operate)
    {
        case ATC_INVALID_OPERATE:
        {
            ...
            {   
                ATC_BuildResultCodeRsp(atc_config_ptr, S_ATC_RESULT_CODE_NO_CARRIER, 0);
                //Send the unsolicited +WIND: 6         
                if(ATC_16BIT1 == ((atc_config_ptr->atc_config_nv.wind_ind_level) | ATC_WIND_BIT5))
                {
                    call_id = ((APP_MN_CALL_DISCONNECTED_IND_T*)sig_ptr)->call_id;
                    sprintf((char*)g_rsp_str, "%s: 6,%d",
                        g_atc_info_table[AT_CMD_SIND].cmd_name->str_ptr,call_id+1);
                    ATC_BuildInfoRsp(atc_config_ptr, (uint8*)g_rsp_str);
                }                   
            }
...
AT_CMD_FUNC(ATC_ProcessCHUP)
{
    ...
    SCI_TRACE_LOW("ATC: ATC_ProcessCHUP\n");
    ...
    SCI_TRACE_LOW("ATC: g_calls_state.call_amout= %d", g_calls_state[dual_sys].call_amount);
    ...
    if(calls_state_ptr->call_amount >= 1)
    {
        ...
        for(i = 0; i < calls_state_ptr->call_amount; i++)
        {
        ...
        #ifdef MS_OAPI_SUPPORT
            MNCALL_DiscCallEx(dual_sys, calls_state_ptr->call_context[i].call_id, cause, PNULL);
        #else
            ...
            ATC_ADD_EXPECTED_EVENT(atc_config_ptr, APP_MN_CALL_DISCONNECTED_IND,
                                  calls_state_ptr->call_context[i].call_id, ATC_DOMAIN_CS);
        }
