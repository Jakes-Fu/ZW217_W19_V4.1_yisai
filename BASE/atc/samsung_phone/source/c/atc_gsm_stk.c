                                    ...
                                    )
{
    ...
    atc_config_ptr->atc_link_config[atc_config_ptr->current_link_id].ctrlz_end_status = ATC_CTRLZ_STK;
#else
    SIO_ATC_WriteCmdRes(g_rsp_str, strlen((char*)g_rsp_str));
    // set the terminate char to receive PDU from the SIO
    SIO_ATC_SetCmdLineTerminateChar(CTRL_Z, ESC);
    atc_config_ptr->is_ctrlz_end = TRUE;
    atc_config_ptr->ctrlz_end_status = ATC_CTRLZ_STK;
#endif
...
#endif
{
    ...
    if (ERR_MNSMS_NONE !=  MNSMS_SendSmsPDU(TRUE, &sc_addr, &tpdu, is_need_compress, FALSE))
    #else
...
AT_CMD_FUNC( ATC_ProcessPSSTKSMS)
{
    ...
    atc_config_ptr->atc_link_config[atc_config_ptr->current_link_id].is_ctrlz_end = TRUE;
    atc_config_ptr->atc_link_config[atc_config_ptr->current_link_id].ctrlz_end_status = ATC_CTRLZ_STKSMS;
#else
    SIO_ATC_WriteCmdRes(g_rsp_str, strlen((char*)g_rsp_str));
    ...
    SIO_ATC_SetCmdLineTerminateChar(CTRL_Z, ESC);
    atc_config_ptr->is_ctrlz_end = TRUE;
    atc_config_ptr->ctrlz_end_status = ATC_CTRLZ_STKSMS;
#endif
