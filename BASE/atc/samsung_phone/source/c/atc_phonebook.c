            ...
            for ( ; index1<=index2; index1++)
            {
                ...
                if (ERR_MNDATAMAG_NO_ERR == status)
                {
                    ...
                    else
                    {
                        ...
                        {
                            SCI_ASSERT(output_len <= sizeof(tel_alpha));/*assert verified check NULL pointer*/
                            SCI_ASSERT(output_len <= sizeof(output_ptr));/*assert verified check NULL pointer*/
...
AT_CMD_FUNC( ATC_ProcessCNUM )
{
    ...
    uint16  output_len=0;
    MN_DUAL_SYS_E dual_sys = ATC_CURRENT_SIM_ID_MUX;
SCI_TRACE_LOW("atc: ATC_ProcessCNUM");
    SCI_MEMSET(&entry, 0, sizeof(entry));
    SCI_MEMSET(&output_ptr[0], 0, PHONEBOOK_MAX_DN_ALPHA_IDENTIFY_LEN);
    // get the phonebook information    
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        ...
        {
            SCI_TRACE_LOW("atc: execute: record_num=%d", phonebook_info.max_record_num);
            ...
            for(i = 0;i < phonebook_info.max_record_num;i++)
            {
                ...
                {
                SCI_TRACE_LOW("atc: readphonebook no error");
                    SCI_MEMSET(tel_str,'\0', sizeof (tel_str));
                ...
                {
                SCI_TRACE_LOW("atc: read phonebook entry error");
