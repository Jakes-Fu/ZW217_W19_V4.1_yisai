...
BOOLEAN g_is_pdp_activated = FALSE;
...
static BOOLEAN s_is_pdp_active_flag = FALSE;
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            if(cur_cmd_info->param_used[0] )
            {
                ...
                if(!g_is_pdp_activated && !s_is_pdp_active_flag)
                {
                ...
                {
                    //SCI_TRACE_LOW:"atc: PDP is not activated, pls dns later"
                    SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GPRS_MODEM_369_112_2_17_22_16_58_442,(uint8*)"");
                    ATC_BuildResultCodeRsp(atc_config_ptr,S_ATC_RESULT_CODE_ERROR,ERR_GPRS_OPERATION_FAILURE);
                    return S_ATC_DEFAULT_HANDLE;
                }
            }
...
)
{
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
         ...
         //SCI_TRACE_LOW("atc: link_id=%d", atc_config_ptr->current_link_id);
        ...
        {
            //SCI_TRACE_LOW:"atc:  MUX ENABLE  link_id != ATC_DEBUG_LINK"
            SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GPRS_MODEM_485_112_2_17_22_16_58_450,(uint8*)"");
            ...
            atc_config_ptr->atc_link_config[atc_config_ptr->current_link_id].ctrlz_end_status = ATC_CTRLZ_GPRS;
        }
            ...
            atc_config_ptr->ctrlz_end_status = ATC_CTRLZ_GPRS;
#endif
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        ...
        {   
            ...
            if(0 != s_dataconf[id].socket)
            {
                ...
                if(0 != result)
                {
                    //SCI_TRACE_LOW:"atc: socket close failure ! "
                    SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GPRS_MODEM_568_112_2_17_22_16_58_455,(uint8*)"");
                    return ERR_INVALID_INDEX; 
                }
                else
                {   
                    ...
                    SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GPRS_MODEM_575_112_2_17_22_16_58_456,(uint8*)"d", id);

                    socket_num = socket_num - 1;
                    //SCI_TRACE_LOW:"atc: current socket num is %d after closed"
                    SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GPRS_MODEM_578_112_2_17_22_16_58_457,(uint8*)"d", socket_num);
                    ...
                    if(0 == socket_num)
                    {
                        ...
                        if( MNGPRS_DeactivatePdpContextEx(dual_sys, FALSE,pdp_id_arr) != ERR_MNGPRS_NO_ERR)
                        {
                            ...
                            SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GPRS_MODEM_586_112_2_17_22_16_58_458,(uint8*)"");
                            return  ERR_INVALID_INDEX;
                        }
                        ATC_ADD_EXPECTED_EVENT(atc_config_ptr,APP_MN_DEACTIVATE_PDP_CONTEXT_CNF, pdp_id_arr[0],ATC_DOMAIN_PS);

                        g_is_pdp_activated = FALSE;
                        s_is_pdp_active_flag = FALSE;
                        //SCI_TRACE_LOW:"atc: deactivate PDP success"
                        SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GPRS_MODEM_593_112_2_17_22_16_58_459,(uint8*)"");
                    }
...
{ 
    ...
    sa.family  = AF_INET;
    sa.ip_addr = htonl(ip_addr);
    sa.port    = htonl(s_dataconf[sock_index_id].fport); 
