...
/************************************************************************/
/*funciton name: SHA1_ALGO(const unsigned int *message_array,unsigned int length,unsigned char *Mesage_Digest)
/*in paramter: message_array-- need to be digest
               length --- message len
                
/*out parameter:Message_Digest--digest result
/*return:0 --success
/************************************************************************/

int SHA1_ALGO(const unsigned int *message_array,unsigned int length,unsigned char *digest_result)
{
  SHA1Context_32  sha_ctx;

  SHA1Reset_32(&sha_ctx);
  SHA1Input_32(&sha_ctx,message_array,length);
  SHA1Result_32(&sha_ctx,digest_result);
