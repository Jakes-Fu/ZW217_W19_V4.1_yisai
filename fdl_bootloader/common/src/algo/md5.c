...
static __inline unsigned int ENDIAN_CONVERT_32(unsigned int x)
{
	unsigned int i,j;
	i=x^((x<<16)|(x>>16));
	i=i&0xff00ffff;
...
void __inline __cpu_to_le32s(u32 *buf)
{
	*buf = ENDIAN_CONVERT_32(*buf);
}
void __inline __le32_to_cpus(u32 *buf)
{
	*buf = ENDIAN_CONVERT_32(*buf);
}
