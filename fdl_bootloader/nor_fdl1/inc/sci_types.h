...
#define REG_READ_ADI(reg_addr,value)		do{\
												value = ADI_REG_READ(reg_addr);\
											}while(0)
											
#define REG_WRITE_ADI(reg_addr,value)		do{\
												ADI_REG_WRITE(reg_addr,value);\
											}while(0)
											
#define REG_OR_ADI(reg_addr,or_msk)			do{\
												uint16 temp;\
												temp = ADI_REG_READ(reg_addr);\
												temp |= (or_msk);\
												ADI_REG_WRITE(reg_addr,temp);\
											}while(0)
												
#define REG_AND_ADI(reg_addr,and_msk)		do{\
												uint16 temp;\
												temp = ADI_REG_READ(reg_addr);\
												temp &= (and_msk);\
												ADI_REG_WRITE(reg_addr,temp);\
