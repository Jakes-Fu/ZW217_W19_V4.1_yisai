...
PUBLIC ERR_LCD_E LCD_Init (void)
{
    ...
    TF_UartTrace("[LCD 8910] :: LCD_Init [IN]\n");
    ...
    for (lcd_id = 0; lcd_id < LCD_SUPPORT_MAX; lcd_id++)
    {
        ...
        if (PNULL != s_lcd_spec_info_ptr[lcd_id])
        {
            ...
            TF_UartTrace("[LCD 8910] :: LCD_Init [Found - %x]\n", s_lcd_spec_info_ptr[lcd_id]);
            s_reg_flag[lcd_id] = 1;
            s_lcd_used_num++;
        }
    }
    ...
    if (s_lcd_used_num == 0)
    {
        TF_UartTrace("[LCD 8910] :: LCD_Init [Error Not Found]\n");/*assert verified*/
    ...
    hal_gouda_init();
    TF_UartTrace("[LCD 8910] :: LCD_Init [GOUDA Init]\n");
...
PUBLIC void LCD_DisplayText (uint16 x, uint16 y, char *text, uint16 len)
{
    ...
    TF_UartTrace("[LCD 8910] :: LCD_DisplayText [ IN ] ");
    LCD_GetInfo (MAIN_LCD_ID,&lcd_info);
    lcd_width = lcd_info.lcd_width;
    lcd_height = lcd_info.lcd_height;
    TF_UartTrace("[LCD 8910] :: LCD_DisplayText [ IN 1] ");
    LCD_DisableAllBlock (MAIN_LCD_ID);
    TF_UartTrace("[LCD 8910] :: LCD_DisplayText [ IN ] 2");
    ...
    TF_UartTrace("[LCD 8910] :: LCD_DisplayText [%d, %d]   lcd_buff = 0x%x\n",  x, y, dest_ptr);
    ...
    {
        ch = *text++;
        //SCI_TraceLow("[LCD 8910] :: LCD_DisplayText [%c]\n", ch);
        if ( (ch <= 0x20) || (ch & 0x80))
        {
        ...
        )
{
    ...
    TF_UartTrace("[LCD 8910] :: config block (%d:%d) -- [%d, %d, %d, %d] -%d \n", lcd_id, blk_num,
...
LOCAL void gouda_config_layers(LCD_ID_E lcd_id)
{
    ...
    for (i=0 ; i<LCDC_LAYER_NUM ; i++)
    {
        if(s_buffer_info[lcd_id][i].is_enable)
        {
            ...
            TF_UartTrace("[LCD 8910] :: configure gouda Layer [%d]   = (enable) buff = 0x%x\n",  i, over_layer_cfg.addr);
            over_layer_use_id++;
        }
        else 
        {
            TF_UartTrace("[LCD 8910] :: configure gouda Layer [%d]   = (disable)\n",  i);
        }
    }
}
        ...
        )
{
    ...
    TF_UartTrace("[LCD 8910] :: InvalidateRect (%d) [%d %d %d %d]\n", lcd_id, left, top, right, bottom);
    s_lcd_index = lcd_id;
    ...
    TF_UartTrace("[LCD 8910] :: InvalidateRect [OUT]\n");
