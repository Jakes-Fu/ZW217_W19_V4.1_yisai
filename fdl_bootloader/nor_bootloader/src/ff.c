...
static BYTE valid_vol = 0;	// fei.zhang add
...
#define TF_IsExPartition( PartitionType ) \
    ((PartitionType == 0x05) || (PartitionType == 0x0F))
...
static
BYTE check_mbr (	/* 0:not MBR, 1:valid MBR, 3:Disk error */
	BYTE *buff	/* Pointer to the data buffer to store read data */
)
{
	TF_MBR_T *mbr_ptr = (TF_MBR_T *)buff;
	uint32 i=0, sum=0, disksize=0, part_num=0;
	uint8 bootflag[4],systemid[4];
	uint32 reativesector[4],totalsectors[4];
    
	if(RES_OK != disk_ioctl(0, GET_CARD_CAPCITY, &disksize))
	{
		return 3;
	}
	...
	for(i = 0; i < 4; i++)
	{
		bootflag[i] = mbr_ptr->dpt[i].bootflag;
		systemid[i] = mbr_ptr->dpt[i].systemid;
		reativesector[i] = TF_DWORD_SWAP(mbr_ptr->dpt[i].reativesector);
		totalsectors[i] = TF_DWORD_SWAP(mbr_ptr->dpt[i].totalsectors);
	}
    
	if(0xAA55 != TF_WORD_SWAP(mbr_ptr->endFlag))
	{
		return 0;
	}
	...
	for(sum = 0,i = 0; i < 4; i++)
	{
		sum += ((TF_IsExPartition(systemid[i]))?1:0);
		if(1 < sum)
		{
			return 0;
		}
	...
	for(i = 0; i < 4; i++)
	{
		if(0x0 != systemid[i])
		{
			if((reativesector[i] > disksize) || (reativesector[i]+totalsectors[i] > disksize))
			{
				return 0;
			}
			part_num++;            
...
)
{
...
#else
		vol = valid_vol;				/* fei.zhang add: Use current vol */
#endif
	}
	...
	*rfs = fs = FatFs[vol];				/* Return pointer to the corresponding file system object */
	...
	fmt = check_fs(fs, 0);		/* Check sector 0 if it is a VBR */
	rprintf("bootloader ---check_fs (0): ret = 0x%x\n", fmt);
...
)
{
	...
	valid_vol = vol;				/* fei.zhang add: Register current vol */ 
	return FR_OK;
}
...
FRESULT f_read (
	...
	UINT *_br		/* Pointer to number of bytes read */
)
{
	FRESULT res;
	...
	*_br = 0;	/* Initialize byte counter */
	...
	for ( ;  btr;									/* Repeat until all data transferred */
		rbuff += rcnt, fp->fptr += rcnt, *_br += rcnt, btr -= rcnt) {
