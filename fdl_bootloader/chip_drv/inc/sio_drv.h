...
/* UART primitives */
#define SIO_GET_BREAK_STATUS(p)	((*(volatile unsigned  int*)((p) + ARM_UART_STS0)) & UARTSTS_BREAK_DETECT)

#define SIO_GET_RX_STATUS( p )      ( REG( ( p ) + ARM_UART_STS1 ) & 0xFF )
#define SIO_RX_READY( s )           ( 0 != ( s ) )
#define SIO_RX_READY_E( s )           ((s) & 0xFF)
...
#define SIO_TX_READY_E(s)             ((s) & 0xFF)
...
#ifdef DEBUG_TRACE
extern uint32 SIO_TraceLow(const char *x_format, ...);
#define SIO_TRACE    SIO_TraceLow
#else
#define SIO_TRACE  
#endif
