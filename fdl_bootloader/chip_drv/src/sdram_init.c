...
typedef enum MCU_CLK_TYPE_TAG
{
    ...
    ARM200_EMC200_AHB100,   //SC8800G2
    ARM150_EMC150_AHB75,   //SC8800G2
...
//     mcu_clk       arm_clk        emc_clk       ahb_clk
    ...
    {ARM_CLK_400M, ARM_CLK_200M, ARM_CLK_200M, ARM_CLK_100M},   //SC8800G2
    {ARM_CLK_300M, ARM_CLK_150M, ARM_CLK_150M, ARM_CLK_75M},   //SC8800G2
...
/*
	...
	openphone platform. Remove the selection according with GR_GEN3, and 
	config the arm clk 150Mhz.
	*/
LOCAL void __ClkConfig(uint32 *emcclk, uint32 *ahbclk)
{
    ...
    MCU_CLK_TYPE_E clk_type = ARM150_EMC150_AHB75;
...
LOCAL void SDRAM_GenMemCtlCfg (SDRAM_CFG_INFO_T_PTR sdram_cfg_ptr)
{
    ...
    REG32 (EXT_MEM_CFG0) = (BIT_0|BIT_1|BIT_6|BIT_8|BIT_9|BIT_10|BIT_11); //software address mapping
    ...
    //config channel 0-15
    if(0x68000000 == REG32 (CHIP_TYPE))
    {
        REG32 (EXT_MEM_CFG0_CH0)  = 0x0003e31c;//set clk_emc to sync mode with clk_arm
        REG32 (EXT_MEM_CFG0_CH1)  = 0x0003e31c;
    }
    else
    {
        REG32 (EXT_MEM_CFG0_CH0)  = 0x0003c31c;;
...
LOCAL void SDRAM_PinDrv_Set (void)
{
	...
	for(i = 0; i<15; i++)
	{
		REG32(PINMAP_REG_BASE + 0x019c + i*4) = ctl_drv; /*lint !e737 */
	}
	...
	for(i = 0; i<9; i++)
		REG32(PINMAP_REG_BASE + 0x0284 + i*4) = ctl_drv;/*lint !e737 */
	

	for(i = 0; i<8; i++)
	{	
		REG32(PINMAP_REG_BASE + 0x1DC + i*4) = dat_drv;/*lint !e737 */
		REG32(PINMAP_REG_BASE + 0x204 + i*4) = dat_drv;/*lint !e737 */
		REG32(PINMAP_REG_BASE + 0x22c + i*4) = dat_drv;/*lint !e737 */
		REG32(PINMAP_REG_BASE + 0x254 + i*4) = dat_drv;/*lint !e737 */
	}	
...
LOCAL uint32 Chip_ConfigClk (void)
{
    ...
    if(0x68000000 == REG32 (CHIP_TYPE))
    {
        ...
        //CHIP_REG_SET(GR_BPLL_MN, 0x001A01E1);
    ...
    //for sc6800h1 chip, set clk_arm/clk_ahb_clk_emc to 200/100/200MHz
    //for the other 68h chip, e.g. sc6800h3 chip, set clk_arm/clk_ahb_clk_emc to 180/90/180MHz
    if(0x68000000 == REG32 (CHIP_TYPE))
    {
        //set CLK_EMC to 200MHz
        i &= ~ (BIT_7 | BIT_6 | BIT_5 | BIT_4);
        i |= 0x8 << 4;//i |= 0xC << 4;               //Src_sel_emc:4'b1100 : MPLL_DIV2:
        arm_ahb_clk = ARM_CLK_100M;
    }
    else
    {
        //set CLK_EMC to 180MHz
        i |= BIT_4;               //Src_sel_emc to PLL_DIV4
        arm_ahb_clk = ARM_CLK_90M;
    }
	
    i &= ~ (BIT_9 | BIT_8);      //Div_cfg_emc:0x0
    //set CLK_ARM to 200/180MHz
    i &= ~ (BIT_2 | BIT_1);      //Div_cfg_arm:0x01
    //set CLK_AHB to 100/90 MHz
    i |= BIT_3;
    //set Src_sel_arm to  200/180Mhz
    i |= BIT_0;
...
/*****************************************************************************/
// Description :   dsp is in reset mode
// Global resource dependence :
// Author :        yuhua.shi
// Note :
/*****************************************************************************/
LOCAL void _DSP_in_Reset (void)
{
#if defined(PLATFORM_SC6800H)
    REG32 (GR_SOFT_RST) |= BIT_24;
#endif
}
...
#ifdef PLATFORM_SC8800G
    if((0 != (__GetHWRST() & 0xF)) && (0 != (__GetHWRST() & 0xF0)))
        return;
#endif
