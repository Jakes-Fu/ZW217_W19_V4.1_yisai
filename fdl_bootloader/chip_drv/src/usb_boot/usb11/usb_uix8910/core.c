...
#define URB_QUEUE_LEN   4
uint32 urb_alloc = 0;
uint32 urb_free = 0;
struct urb urb_heap[URB_QUEUE_LEN];


/* Descriptor support functions ************************************************************** */

uint32 free_urb_cnt(uint32 alloc, uint32 free, uint32 mask)
{
	if (alloc > free) {
...
struct urb *alloc_urb(void)
{
	struct urb *tmp = NULL;
	uint32 ret;

	ret = free_urb_cnt(urb_alloc, urb_free, URB_QUEUE_LEN - 1);
	if (ret >= 1) {
		tmp = &urb_heap[urb_alloc++];
		memset(tmp, 0, sizeof(struct urb));
		if (urb_alloc == URB_QUEUE_LEN)
			urb_alloc = 0;
	}
	return tmp;
}
 ...
 */
struct usb_string_descriptor *usbd_get_string (uint8 index)
{
	if (index >= maxstrings) {
		return NULL;
	}
		...
		if ((rcv_urb = endpoint->rcv_urb)) {
			...
			if (!urb_bad /*&& (len <= endpoint->rcv_packetSize)*/) {
			  /*usbdbg("updating actual_length by %d\n",len); */
				...
				rcv_urb->buffer_length = 0;
				rx_buf_actual = rcv_urb->actual_length;
			} else {
			    ...
			    struct usb_endpoint_instance *endpoint)
{
	...
	urb->device = device;
	urb->buffer = (uint8 *) urb->buffer_data;
