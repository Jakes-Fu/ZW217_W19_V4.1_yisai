			       ...
			       int index)
{
	...
	case USB_DESCRIPTOR_TYPE_CONFIGURATION:
		{
			...
			if (!(configuration_descriptor = usbd_device_configuration_descriptor (device,
								   port,
								   index))) {
				dbg_ep0 (0, "usbd_device_configuration_descriptor failed: %d",
					 index);
				return -1;
			}
	...
	case USB_DESCRIPTOR_TYPE_STRING:
		{
			...
			if (!(string_descriptor = usbd_get_string (index))) {
				rprintf("Invalid string index %d\n", index);
				return -1;
			}
	...
	case USB_DESCRIPTOR_TYPE_INTERFACE:
		rprintf("USB_DESCRIPTOR_TYPE_INTERFACE - error not implemented\n");
		return -1;
	case USB_DESCRIPTOR_TYPE_ENDPOINT:
		rprintf("USB_DESCRIPTOR_TYPE_ENDPOINT - error not implemented\n");
		return -1;
	case USB_DESCRIPTOR_TYPE_HID:
		{
			rprintf("USB_DESCRIPTOR_TYPE_HID - error not implemented\n");
	...
	case USB_DESCRIPTOR_TYPE_REPORT:
		{
			rprintf("USB_DESCRIPTOR_TYPE_REPORT - error not implemented\n");
			return -1;	/* unsupported at this time */
#if 0
			int bNumInterface =
				le16_to_cpu (urb->device_request.wIndex);
...
int ep0_recv_setup (struct urb *urb)
{
	...
	/* handle the requests that do not return data */
		...
		case USB_REQ_SET_CONFIGURATION:
			...
			/*rprintf("DEVICE_CONFIGURED.. event?\n"); */
			return 0;
		...
		case USB_REQ_SET_INTERFACE:
			...
			rprintf ("DEVICE_SET_INTERFACE.. event?\n");
			return 0;
