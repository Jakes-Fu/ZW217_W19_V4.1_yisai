...
#if 1
#define usberr(fmt,args...) rprintf("ERROR: %s(), %d: "fmt"\n",__FUNCTION__,__LINE__,##args)
#else
...
struct urb {
	...
	uint16 buffer_data[URB_BUF_SIZE];	/* data received (OUT) or being sent (IN) */
};
...
struct usb_device_instance {
	...
	uint8 address;		/* current address (zero is default) */
	uint8 configuration;	/* current show configuration (zero is default) */
	uint8 interface;		/* current interface (zero is default) */
	uint8 alternate;		/* alternate flag */
...
int usbd_setup_urb(struct urb *urb, uint8 *buf, uint32 len);
...
static inline void print_urb(struct urb *u)
{
	...
	rprintf("\tdevice %p\n", u->device);
	rprintf("\tbuffer %p\n", u->buffer);
	rprintf("\tbuffer_length %d\n", u->buffer_length);
	rprintf("\tactual_length %d\n", u->actual_length);
	rprintf("\tstatus %d\n", u->status);
	rprintf("\tdata %d\n", u->data);
}
...
{
	rprintf("usb request\n");
	rprintf("\tbmRequestType 0x%2.2x\n", r->bmRequestType);
	if ((r->bmRequestType & USB_REQ_DIRECTION_MASK) == 0)
		rprintf("\t\tDirection : To device\n");
	else
		rprintf("\t\tDirection : To host\n");
	if ((r->bmRequestType & USB_TYPE_STANDARD) == USB_TYPE_STANDARD)
		rprintf("\t\tType      : Standard\n");
	...
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
		...
		rprintf("\t\tRecipient : Device\n");
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
	    USB_REQ_RECIPIENT_INTERFACE)
		rprintf("\t\tRecipient : Interface\n");
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
	    USB_REQ_RECIPIENT_ENDPOINT)
		rprintf("\t\tRecipient : Endpoint\n");
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
	    USB_REQ_RECIPIENT_OTHER)
		rprintf("\t\tRecipient : Other\n");
	rprintf("\tbRequest      0x%2.2x\n", r->bRequest);
	if (r->bRequest == USB_REQ_GET_STATUS)
		...
		rprintf("\t\tGET_DESCRIPTOR\n");
	else if (r->bRequest == USB_REQ_SET_CONFIGURATION)
		rprintf("\t\tSET_CONFIGURATION\n");
	else if (r->bRequest == USB_REQ_SET_INTERFACE)
		rprintf("\t\tUSB_REQ_SET_INTERFACE\n");
	else
		rprintf("\tUNKNOWN %d\n", r->bRequest);
	rprintf("\twValue        0x%4.4x\n", r->wValue);
	if (r->bRequest == USB_REQ_GET_DESCRIPTOR) {
		...
		case USB_DESCRIPTOR_TYPE_DEVICE_QUALIFIER:
			rprintf("\tDEVICE_QUALIFIER\n");
			break;
		case USB_DESCRIPTOR_TYPE_OTHER_SPEED_CONFIGURATION:
			rprintf("\tOTHER_SPEED_CONFIGURATION\n");
			break;
		case USB_DESCRIPTOR_TYPE_INTERFACE_POWER:
			rprintf("\tINTERFACE_POWER\n");
			break;
		...
		case USB_DESCRIPTOR_TYPE_REPORT:
			...
			break;
		default:
			rprintf("\tUNKNOWN TYPE\n");
			break;
		}
	}
	rprintf("\twIndex        0x%4.4x\n", r->wIndex);
	rprintf("\twLength       0x%4.4x\n", r->wLength);
}
#else
/* stubs */
#define print_urb(u)
