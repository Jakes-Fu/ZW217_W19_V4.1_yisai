...
#ifdef FDL2_MODULE
extern void Dcache_InvalRegion(void *addr, unsigned int length);
#endif
...
PUBLIC void usb_enumeration_done (void)
{
...
PUBLIC int USB_EPxSendData_loop (char ep_id ,unsigned int *pBuf,int len)
{
    volatile USB_DIEPTSIZ_U *dieptsiz_ptr = (USB_DIEPTSIZ_U *) USB_DIEPTSIZ (ep_id);
    usb_start_transfer ( (USB_EP_NUM_E) ep_id, USB_EP_DIR_IN, len, TRUE, (uint32 *) pBuf);
    do
    {
        // do nothing, just only wait signal
    }
