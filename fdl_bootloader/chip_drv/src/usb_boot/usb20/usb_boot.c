...
/*--------------------------- Local Data ------------------------------------*/
__align (32) LOCAL uint8 s_usb_snd_buff[USB_BUFF_SIZE];
/*--------------------------- Global Data -----------------------------------*/
/*--------------------------- External Data ---------------------------------*/
/*----------------------------------------------------------------------------*
...
LOCAL void usb_init (unsigned long ext_clk26M);
...
//    Note:
/*****************************************************************************/
LOCAL void usb_ldo_enable (BOOLEAN is_usb_ldo_enabled)
{
...
void usb_write (unsigned char *write_buf,unsigned int write_len)
{
    ...
    if((write_len%64)==0)
    {
        USB_EPxSendData_loop (USB_EP5 , (unsigned int *) s_usb_snd_buff,32);
        split_len = 32;
    }
    USB_EPxSendData (USB_EP5 , (unsigned int *) &s_usb_snd_buff[split_len],write_len-split_len);
}
