...
#define FLASH_DELAY_US prvDelayUS
#endif
#endif
...
//#define FLASH_SUPPORT_EXT   //tao.shi for ext flash_support

#if defined(FLASH_SIZE_128MBITX32MBIT) || defined(FLASH_SIZE_128MBITX64MBIT_OLD) || defined(FLASH_SIZE_128MBITX64MBIT_NEW)
...
LOCAL BOOLEAN SPIFLASH_IsWipFinished(int cs);
...
//extern void prvDelayUS(unsigned int us);


LOCAL void prvDelayUS(unsigned int us)
{
    unsigned int  start;
	unsigned int  ticks;
	start = *((volatile unsigned int *)(0x0480601C));  //hwp_timer2->hwtimer_curval_l
    ticks = us*2;  //timer freq = 2000000HZ  0.5us + 1
    while(!(((unsigned int)(*((volatile unsigned int *)(0x0480601C)) - start)) > ticks))
    {
	}
}
...
LOCAL uint32 SPIFLASH_GetPollStatus( int cs )
{
	...
	//rprintf("[]SPIFLASH_GetPollStatus=0x%x\n",status);
...
LOCAL BOOLEAN SPIFLASH_IsWipFinished(int cs)
{
    if(SPIFLASH_GetPollStatus(cs) & (1<<nWIPPos[cs]))
        return SCI_FALSE;
...
LOCAL void SPIFLASH_WriteStatusReg(int cs, uint32 value, BOOLEAN bSet)
{
	...
	do
	{
    }while(!SPIFLASH_IsWipFinished(cs));
...
LOCAL void SPIFLASH_ResetSPIFlash(int cs)
{
    ...
    prvDelayUS(30);    /*need delay 30us after CMD_RST*/
}

PUBLIC void SPIFLASH_ResetQPIFlash(int cs)
{
	rprintf("SPIFLASH_ResetQPIFlash: CMD_RSTEN\n");
    SFCDRV_CmdNoRx(cs, CMD_RSTEN, 0, 0, 0);
  	rprintf("SPIFLASH_ResetQPIFlash: CMD_RST\n");
    SFCDRV_CmdNoRx(cs, CMD_RST, 0, 0, 0);
    rprintf("SPIFLASH_ResetQPIFlash: prvDelayUS\n");
    prvDelayUS(30);    /*need delay 30us after CMD_RST*/
	work_mode[cs] = SPI_MODE;
	rprintf("SPIFLASH_ResetQPIFlash: SPIFLASH_EnterSPIRead\n");
...
PUBLIC int SPIFLASH_Init(int cs)  /* If Init success, return true, else return false */
{
    ...
    for(i=0; i<100; i++);
	rprintf("SPIFLASH_Init: SPIFLASH_ResetSPIFlash\n");
    SPIFLASH_ResetSPIFlash(cs);
	rprintf("SPIFLASH_Init: SPIFLASH_ResetQPIFlash\n");
    SPIFLASH_ResetQPIFlash(cs);
    work_mode[cs] = SPI_MODE;

	rprintf("SPIFLASH_Init: SPIFLASH_ReadID\n");
    if (SPIFLASH_ReadID(cs, &nMID, &nDID1, &nDID2) != TRUE)
    {
        return SCI_FALSE;
    }

	rprintf("SPIFLASH_Init: SPIFLASH_SPEC_Get\n");
    ...
    if(pSpiFlashSpec->pExtcfg != SCI_NULL)
    {
        ...
        if(cfg->voltage == SPIFLASH_VOLTAGE_3V)
        {
			...
			rprintf("SPIFLASH_Init: IOMUX_PAD_SPI_FLASH_\n");
        ...
        if(cfg->desity > SPIFLASH_3_ADDR_CYCLE_MAX_REGION)
        {
			...
			rprintf("SPIFLASH_Init: SPIFLASH_Enter4ByteAddrMode\n");
    ...
    {
    	//rprintf("SPIFLASH_Init: SPIFLASH_ResetStatusReg\n");  //tao.shi test
    ...
    if(QPI_support[cs] && (!g_fdl_spiflash_disable_qpi))
    {
        ...
        else if((nMID == SPIFLASH_GD_ID) || (nMID == SPIFLASH_BG_ID))
        {
            if (0x16 == nDID2)//LQ32M
			{
				rprintf("SPIFLASH_Init: 0x16\n");
				SPIFLASH_SetReadPara(cs, GD_LQ32M_DUMMYCLK_6_FREQ_104MHZ);
           	}
			else
			{
				rprintf("SPIFLASH_Init: else 0x16\n");
...
//		rprintf("SPIFLASH_WriteBuffer addr[0x%x] size[0x%x]",addr,size);
...
uint32 FDL_FlashWriteArea (uint32 addr, char *buf, uint32 size)
{
	...
	//rprintf("enter FDL_FlashWriteArea addr[0x%x][0x%x]\n",addr,size);
    ...
    if (BLOCK_SIZE < BLOCK_SIZE_64K)
    {
            ...
            )
        {
            ...
            {
            	//rprintf("FDL_FlashErase_Block 1");
            ...
            {
            	//rprintf("FDL_FlashErase_Block 2");
                if( 0 == FDL_FlashErase_Block(addr, BLOCK_SIZE) )
                {
                    return 0;
                }
    ...
    {
    	//rprintf("SPIFLASH_IsStartAddrOfSector 2");
    	if(SPIFLASH_IsStartAddrOfSector(addr))
        {
        	for(i=0;i<size;i+=BLOCK_SIZE)
        	{
        		//rprintf("FDL_FlashErase_Block 3");
	...
	//rprintf("SPIFLASH_Write addr[0x%x] size[0x%x]",addr,size);
...
uint32 FDL_FlashErase(uint32 start_addr,  uint32 size)
{
	...
	rprintf("FDL_FlashErase sector_size[0x%x]\n",sector_size);
    ...
    if ( start_addr == FLASH_START_ADRESS )
    {
        if ( size >= flash_cs0_desity + flash_cs1_desity )
        {
				...
				//rprintf("SPIFLASH_EraseChip(0)-1\n",sector_size);
                ret = SPIFLASH_EraseChip(0);
                return ret;
            #else
				//rprintf("SPIFLASH_EraseChip(0)-2\n",sector_size);
                ret = SPIFLASH_EraseChip(0);
                if ( 0 == ret )
                {
    			return ret;
                }
				//rprintf("SPIFLASH_EraseChip(1)-1\n",sector_size);
        ...
        {
        	//rprintf("SPIFLASH_EraseChip(0)-3\n",sector_size);
            ret = SPIFLASH_EraseChip(0);
            return ret;
        }
    }
    ...
    while( (size != 0) && (0 == (size & 0x80000000))  )  // check overflow
    {
        ...
        if (addr >= latest_erase_end_addr)
        {
        	//rprintf("FDL_FlashErase_Block ADDR=0x%x,sector_size=0x%x",addr,sector_size);
        ...
        if(FDL_InEfsSpace(addr))
        {
            ...
            //FDL_SendTrace("%s %d addr=0x%x", __func__, __LINE__, addr);
			//rprintf("SPIFLASH_IsStartAddrOfSector tmp_addr=0x%x",tmp_addr);
            if (SPIFLASH_IsStartAddrOfSector(tmp_addr))
            {
            	//rprintf("SPIFLASH_WriteWord tmp_addr=0x%x",(tmp_addr + BLOCK_SIZE - 2),status);
