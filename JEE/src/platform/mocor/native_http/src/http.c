...
** 2021.11.30      xiaoju.cheng                 Create
**
******************************************************************************/
...
typedef struct
{
    ...
    _ual_cms_client_register_callback   call_back;
...
#define HTTP_ACCEPT_CHARSET_STRING    "1.0"//"UTF-8,ISO-8859-1,US-ASCII,windows-1252"//"*utf-8*"
...
#define HTTP_INIT_TIMEOUT             (5*1000)
...
LOCAL void HttpInitRequest(uint32 net_id)
{
    ...
    p_init_param->net_id = net_id;
    SCI_TRACE_LOW("[HTTP] net_id = %d", p_init_param->net_id);

    //TODO:app_param的值需要确认
    HTTP_InitRequest(HTTP_MODULE_ID, 1, p_init_param);
...
LOCAL void HttpDownloadFail(int32 task_id)
{
    ...
    {
        ual_cms_msg_t param = {0};

        param.msg_id = p_download_task->app_instance;
        param.p_body = PNULL;
        p_download_task->call_back(param);
    }
    ...
    if (HTTP_SUCCESS == signal.result)
    {
        ...
        else
        {
            ...
            if(PNULL != p_download_task->call_back)
            {
                ...
                param.msg_id = p_download_task->app_instance;
                param.p_body = (void*)p_download_task->p_receive_data;
                param.body_size = p_download_task->receive_data_len;
                p_download_task->call_back(param);
            }
...
PUBLIC http_request_result_e http_start_request(uint32 request_id, char* p_url, char* p_content, http_request_type_e request_type,_ual_cms_client_register_callback http_callback)
{
    ...
    _ual_cms_client_register_callback callback = PNULL;
    ual_cms_msg_t param = {0};
    ...
    else
    {
        ...
        {
            s_http_download_info.download_task_num++;
            SCI_TRACE_LOW("[HTTP]is_pdp_actived: download_task_num = %d", s_http_download_info.download_task_num);

            HttpDownload_SetStatus(task_id,HTTP_DOWNLOAD_DOWNLOADING);
            //TODO:后续使用get/post参数区分使用post接口还是get接口
            if(request_type == HTTP_POST_REQUEST)
            {
                HttpPostRequest(&s_http_download_info.download_task[task_id]);
            }
            else if(request_type == HTTP_GET_REQUEST)
            {
                HttpGetRequest(&s_http_download_info.download_task[task_id]);
            }
            else if(request_type == HTTP_OPTIONS_REQUEST)
            {               
...
LOCAL void HandleHttpInitTimeout(uint8 timer_id, uint32 param)
{
    SCI_TRACE_LOW("[HTTP]:timer_id is %d,s_http_init_timer_id is %d",timer_id,s_http_init_timer_id);

    if(timer_id == s_http_init_timer_id)
    {
        s_http_init_timer_id = 0;

//如果定义了networkmanager，直接获取net_id，否则需要自行激活pdp。
#ifdef UAL_NETWORKMGR_SUPPORT
        {
...
{
    SCI_TRACE_LOW("[HTTP]:enter http_request_init");

    //此处起5秒的timer，等待networkmanager pdp激活成功。
    if(0 == s_http_init_timer_id)
    {
        s_http_init_timer_id = MMK_CreateTimerCallback(HTTP_INIT_TIMEOUT,HandleHttpInitTimeout,PNULL,FALSE);
        SCI_TRACE_LOW("[HTTP]:s_http_init_timer_id is %d",s_http_init_timer_id);
    }
}
...
PUBLIC void  js_http_set_content_type(char* p_content_type)
{
    ...
    {
        SCI_TRACE_LOW("[js_http_set_content_type]:content_type is pnull.");
        return;
    }
    wf_len = SCI_STRLEN(p_content_type);
    if(0 == wf_len || wf_len > HTTP_CONTENT_TYPE_LEN)
    {
        SCI_TRACE_LOW("[js_http_set_content_type]:content_type length error");
        return;
    }
    SCI_MEMSET(s_content_type, 0, HTTP_CONTENT_TYPE_LEN + 1);
    SCI_MEMCPY(s_content_type, p_content_type, wf_len);
    SCI_TRACE_LOW("[js_http_set_content_type]:s_content_type is %s.",s_content_type);
}
