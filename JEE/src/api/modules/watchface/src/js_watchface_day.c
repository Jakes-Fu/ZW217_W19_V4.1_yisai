...
DECLARE_HANDLER(js_watchface_day_ctl_create) {
    if (JERRY_UNLIKELY(args_cnt < 6)) {
        return jerry_create_number(0);
    }

    MMI_WIN_ID_T win_id = jerry_get_number_value(args_p[0]);
    BOOLEAN digital = jerry_get_boolean_value(args_p[1]);
    double center_position_x = jerry_get_number_value(args_p[2]);
    double center_position_y = jerry_get_number_value(args_p[3]);
    ...
    ctrl_param.double_digital = digital;
    result = WATCHFACE_Day_Create(win_id, &ctrl_param);
...
void js_register_watchface_day_functions() {

    REGISTER_HANDLER(js_watchface_day_ctl_create);
    REGISTER_HANDLER(js_watchface_day_ctl_destroy);
