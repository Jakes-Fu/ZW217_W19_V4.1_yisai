 ...
 **/
 ...
 /**
  *****************************************************************************
  * @file     js_watchface_digitaltime.c
  * @brief    -
...
#include "js_watchface_digitaltime.h"
...
DECLARE_HANDLER(js_watchface_digitaltime_ctl_create) {
    if (JERRY_UNLIKELY(args_cnt < 8)) {
    ...
    int update_interval = jerry_get_number_value(args_p[3]);
    char** pStrArray = NULL;
    uint32_t length = 0;
    ...
    int type = jerry_get_number_value(args_p[7]);
    ...
    ctrl_param.update_interval = update_interval;
    SCI_MEMCPY(ctrl_param.p_image_list, pStrArray, length*sizeof(pStrArray));
    ctrl_param.type = WATCHFACE_TYPE_DIGITAL_TIME_HOURMIN+type;
    ctrl_param.p_image_connector = jerry_string_data(&img_path);

    result =  WATCHFACE_DigitalTime_Create(win_id,  &ctrl_param);
...
void js_register_watchface_digitaltime_functions() {

    REGISTER_HANDLER(js_watchface_digitaltime_ctl_create);
    REGISTER_HANDLER(js_watchface_digitaltime_ctl_destroy);
