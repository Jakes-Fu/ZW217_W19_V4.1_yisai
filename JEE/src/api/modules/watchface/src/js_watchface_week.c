...
DECLARE_HANDLER(js_watchface_week_ctl_create) {
    ...
    result = WATCHFACE_Week_Create(win_id, &ctrl_param);
    for (uint32_t i = 0; i < length; i++) {
        if(pStrArray[i] != NULL){
...
void js_register_watchface_week_functions() {

    REGISTER_HANDLER(js_watchface_week_ctl_create);
    REGISTER_HANDLER(js_watchface_week_ctl_destroy);
