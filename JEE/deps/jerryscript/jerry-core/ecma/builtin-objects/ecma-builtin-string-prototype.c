                                              ...
                                              bool charcode_mode) /**< routine mode */
{
  ...
  volatile uint32_t uit32 = ecma_number_trunc(index_num);
  if(uit32 > len)//»›¥Ì¥¶¿Ì£¨bug1970637
  {
        JERRY_ERROR_MSG(" [jerryscript] ecma_builtin_string_prototype_char_at_helper old uit32 = %d ",uit32);
        uit32 = ecma_number_trunc(index_num);
        JERRY_ERROR_MSG(" [jerryscript] ecma_builtin_string_prototype_char_at_helper new uit32 = %d ",uit32);
  }
  ecma_char_t new_ecma_char = ecma_string_get_char_at_pos (original_string_p, uit32);
  ecma_deref_ecma_string (original_string_p);
