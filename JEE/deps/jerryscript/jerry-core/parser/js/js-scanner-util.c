 ...
 * @return pointer to the literal
 */
lexer_lit_location_t * JERRY_ATTR_ALWAYS_INLINE
...
scanner_reverse_info_list (parser_context_t *context_p) /**< context */
{
  ...
  if (NULL == scanner_info_p || scanner_info_p->type == SCANNER_TYPE_END)
  {
    return;
  }
  ...
  while (NULL != scanner_info_p && scanner_info_p->type != SCANNER_TYPE_END);
