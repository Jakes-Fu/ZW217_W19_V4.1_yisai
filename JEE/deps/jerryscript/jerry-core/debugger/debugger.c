...
jerry_debugger_send_scope_variables (const uint8_t *recv_buffer_p) /**< pointer to the received data */
{
  ...
  while (prop_iter_cp != JMEM_CP_NULL)
  {
    ...
    // fit for global varibal : bin.xie
    if (!ECMA_PROPERTY_IS_PROPERTY_PAIR (prop_iter_p))
    {
      jerry_debugger_send_type (JERRY_DEBUGGER_SCOPE_VARIABLES_END);
      return;
    }
   JERRY_ASSERT (ECMA_PROPERTY_IS_PROPERTY_PAIR (prop_iter_p));
    ...
    // resove the global variable bug : bin.xie
...
void jerry_debugger_get_memstats(char* buff ,int len){
	jerry_debugger_send_memstats_t memstats;
	jerry_debugger_send_memstats_t* memstats_p = &memstats;
	...
	snprintf(buff, len, "total:%d (code:%d string:%d object:%d property:%d)  ,   peak:%d",allocated_bytes, byte_code_bytes, string_bytes, object_bytes, property_bytes, stats.peak_allocated_bytes);
...
void jerry_debugger_get_exception_info(char* buff ,int len){
	ecma_string_t* string_p = NULL;
	...
	strncpy(buff, string_data_p, string_size < len ? string_size : (len - 1));
